---
summary: "CLI reference for `crawclaw qr` (generate pairing QR + setup code)"
read_when:
  - You want to pair a client with a gateway quickly
...
```bash
crawclaw qr
crawclaw qr --setup-code-only
crawclaw qr --json
crawclaw qr --remote
crawclaw qr --url wss://gateway.example/ws
```
