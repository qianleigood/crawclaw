...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["CRAWCLAW_STATE_DIR", "CRAWCLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-subagent-"));
  ...
  it("uses isolated temp state when no explicit runtime state dir is set in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "crawclaw-test-state"));
  });
});
