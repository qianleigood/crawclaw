...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `crawclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `crawclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=crawclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${CRAWCLAW_STATE_DIR:-$HOME/.crawclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
