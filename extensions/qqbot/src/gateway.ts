...
export async function startGateway(ctx: GatewayContext): Promise<void> {
  ...
  } else if (isGlobalTTSAvailable(cfg as CrawClawConfig)) {
    const globalProvider = (cfg as CrawClawConfig).messages?.tts?.provider ?? "auto";
    log?.info(
    ...
    try {
      ...
      const handleMessage = async (event: {
        ...
        const hasTTS =
          ...
          isGlobalTTSAvailable(cfg as CrawClawConfig);
