...
CrawClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, CrawClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `crawclaw configure --section web`. It stores the key in
`~/.crawclaw/crawclaw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.crawclaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
