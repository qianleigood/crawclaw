...
vi.mock("crawclaw/plugin-sdk/fetch-runtime", () => ({
  resolveFetch: (...args: unknown[]) => resolveFetchMock(...args),
}));

vi.mock("crawclaw/plugin-sdk/core", async (importOriginal) => {
  const actual = await importOriginal<typeof import("crawclaw/plugin-sdk/core")>();
  return {
    ...actual,
...
vi.mock("crawclaw/plugin-sdk/text-runtime", () => ({
