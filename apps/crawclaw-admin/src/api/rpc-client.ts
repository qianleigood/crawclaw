import { CrawClawWebSocket } from './websocket'
import type {
  RPCResponse,
  Session,
  SessionDetail,
...
export class RPCClient {
  ...
  private normalizeSkillSource(row: Record<string, unknown>): Skill['source'] {
    ...
    const source = sourceRaw.replace(/^crawclaw-/, '')
    ...
    if (/\.crawclaw[\\/]+skills/.test(filePath)) {return 'managed'}
  ...
  private looksLikeConfigRoot(value: unknown): value is CrawClawConfig {
  ...
  private normalizeConfigPayload(payload: unknown): CrawClawConfig {
  ...
  private normalizeWorkflowInvocation(value: unknown): WorkflowInvocationHint | undefined {
    const row = this.asOptionalRecord(value)
    if (!row) {return undefined}

    const headersRow = this.asOptionalRecord(row.headers)
    const headers = headersRow
      ? Object.fromEntries(
          Object.entries(headersRow)
            .map(([key, headerValue]) => [key, this.asString(headerValue).trim()] as const)
            .filter(([, headerValue]) => !!headerValue)
        )
      : undefined
    const invocation: WorkflowInvocationHint = {
      ...(this.asOptionalString(row.method) ? { method: this.asOptionalString(row.method) } : {}),
      ...(this.asOptionalString(row.url) ? { url: this.asOptionalString(row.url) } : {}),
      ...(headers && Object.keys(headers).length > 0 ? { headers } : {}),
    }
    return Object.keys(invocation).length > 0 ? invocation : undefined
  }

  private normalizeWorkflowStatus(value: unknown): WorkflowExecutionStatus {
    switch (value) {
      case 'queued':
      case 'running':
      case 'waiting_input':
  ...
  private normalizeWorkflowSource(value: unknown): WorkflowExecutionSource {
    switch (value) {
      case 'local':
      case 'local+n8n':
      case 'n8n':
  ...
  private normalizeWorkflowRegistryEntry(value: unknown): WorkflowRegistryEntry {
    const row = this.asRecord(value)
    const workflowId = this.asString(row.workflowId || row.id).trim()
    const archivedAt = this.asOptionalNumber(row.archivedAt)
    const lastRunAt = this.asOptionalNumber(row.lastRunAt)

    return {
      workflowId,
      name: this.asString(row.name, workflowId || 'Workflow'),
      ...(this.asOptionalString(row.description) ? { description: this.asOptionalString(row.description) } : {}),
      ...(this.asOptionalString(row.ownerSessionKey) ? { ownerSessionKey: this.asOptionalString(row.ownerSessionKey) } : {}),
      ...(this.asOptionalString(row.ownerSessionId) ? { ownerSessionId: this.asOptionalString(row.ownerSessionId) } : {}),
      scope: row.scope === 'session' ? 'session' : 'workspace',
      target: 'n8n',
      enabled: this.asBoolean(row.enabled, true),
      safeForAutoRun: this.asBoolean(row.safeForAutoRun, false),
      requiresApproval: this.asBoolean(row.requiresApproval, true),
      tags: this.asStringList(row.tags),
      specVersion: this.asNumber(row.specVersion, 0),
      deploymentVersion: this.asNumber(row.deploymentVersion, 0),
      deploymentState: row.deploymentState === 'deployed' ? 'deployed' : 'draft',
      ...(this.asOptionalString(row.n8nWorkflowId) ? { n8nWorkflowId: this.asOptionalString(row.n8nWorkflowId) } : {}),
      ...(archivedAt !== undefined ? { archivedAt } : {}),
      createdAt: this.asNumber(row.createdAt, 0),
      updatedAt: this.asNumber(row.updatedAt, 0),
      ...(lastRunAt !== undefined ? { lastRunAt } : {}),
      ...(this.normalizeWorkflowInvocation(row.invocation)
        ? { invocation: this.normalizeWorkflowInvocation(row.invocation) }
        : {}),
    }
  }

  private normalizeWorkflowEvent(value: unknown): WorkflowExecutionEventRecord {
    const row = this.asRecord(value)
    const level =
      row.level === 'warn' || row.level === 'error' || row.level === 'info' ? row.level : 'info'
    return {
      at: this.asNumber(row.at, 0),
      level,
      type: this.asString(row.type),
      message: this.asString(row.message),
      ...(this.asOptionalRecord(row.details) ? { details: this.asOptionalRecord(row.details) } : {}),
    }
  }

  private normalizeWorkflowExecutionStep(value: unknown): WorkflowExecutionStepRecord {
    const row = this.asRecord(value)
    const startedAt = this.asOptionalNumber(row.startedAt)
    const updatedAt = this.asOptionalNumber(row.updatedAt)
    const endedAt = this.asOptionalNumber(row.endedAt)
    return {
      stepId: this.asString(row.stepId || row.id),
      ...(this.asOptionalString(row.title) ? { title: this.asOptionalString(row.title) } : {}),
      ...(this.asOptionalString(row.kind) ? { kind: this.asOptionalString(row.kind) as WorkflowExecutionStepRecord['kind'] } : {}),
      ...(this.asOptionalString(row.path) ? { path: this.asOptionalString(row.path) } : {}),
      ...(this.asOptionalString(row.branchGroup) ? { branchGroup: this.asOptionalString(row.branchGroup) } : {}),
      status: this.asString(row.status, 'pending'),
      ...(this.asOptionalString(row.executor) ? { executor: this.asOptionalString(row.executor) } : {}),
      ...(startedAt !== undefined ? { startedAt } : {}),
      ...(updatedAt !== undefined ? { updatedAt } : {}),
      ...(endedAt !== undefined ? { endedAt } : {}),
      ...(this.asOptionalString(row.summary) ? { summary: this.asOptionalString(row.summary) } : {}),
      ...(this.asOptionalString(row.error) ? { error: this.asOptionalString(row.error) } : {}),
      ...(this.asOptionalString(row.skippedReason) ? { skippedReason: this.asOptionalString(row.skippedReason) } : {}),
    }
  }

  private normalizeWorkflowExecution(value: unknown): WorkflowExecutionView {
    const row = this.asRecord(value)
    const waitingRow = this.asOptionalRecord(row.waiting)
    const startedAt = this.asOptionalNumber(row.startedAt)
    const updatedAt = this.asOptionalNumber(row.updatedAt)
    const endedAt = this.asOptionalNumber(row.endedAt)
    const steps = Array.isArray(row.steps)
      ? row.steps.map((item) => this.normalizeWorkflowExecutionStep(item))
      : undefined
    const events = Array.isArray(row.events)
      ? row.events.map((item) => this.normalizeWorkflowEvent(item))
      : undefined

    return {
      executionId: this.asString(row.executionId || row.id),
      ...(this.asOptionalString(row.localExecutionId) ? { localExecutionId: this.asOptionalString(row.localExecutionId) } : {}),
      ...(this.asOptionalString(row.n8nExecutionId) ? { n8nExecutionId: this.asOptionalString(row.n8nExecutionId) } : {}),
      ...(this.asOptionalString(row.workflowId) ? { workflowId: this.asOptionalString(row.workflowId) } : {}),
      ...(this.asOptionalString(row.workflowName) ? { workflowName: this.asOptionalString(row.workflowName) } : {}),
      ...(this.asOptionalString(row.n8nWorkflowId) ? { n8nWorkflowId: this.asOptionalString(row.n8nWorkflowId) } : {}),
      status: this.normalizeWorkflowStatus(row.status),
      ...(this.asOptionalString(row.currentStepId) ? { currentStepId: this.asOptionalString(row.currentStepId) } : {}),
      ...(this.asOptionalString(row.currentExecutor) ? { currentExecutor: this.asOptionalString(row.currentExecutor) } : {}),
      ...(this.asOptionalString(row.remoteStatus) ? { remoteStatus: this.asOptionalString(row.remoteStatus) } : {}),
      ...(typeof row.remoteFinished === 'boolean' ? { remoteFinished: row.remoteFinished } : {}),
      ...(startedAt !== undefined ? { startedAt } : {}),
      ...(updatedAt !== undefined ? { updatedAt } : {}),
      ...(endedAt !== undefined ? { endedAt } : {}),
      ...(steps ? { steps } : {}),
      ...
      ...(waitingRow
        ? {
            waiting: {
              kind: waitingRow.kind === 'external' ? 'external' : 'input',
              ...(this.asOptionalString(waitingRow.prompt) ? { prompt: this.asOptionalString(waitingRow.prompt) } : {}),
              ...(this.asOptionalString(waitingRow.resumeUrl) ? { resumeUrl: this.asOptionalString(waitingRow.resumeUrl) } : {}),
              canResume: this.asBoolean(waitingRow.canResume, false),
            },
          }
        : {}),
      source: this.normalizeWorkflowSource(row.source),
    }
  }

  private normalizeWorkflowField(value: unknown): WorkflowFieldSpec {
    const row = this.asRecord(value)
    return {
      name: this.asString(row.name),
      type: this.asString(row.type, 'unknown'),
      required: this.asBoolean(row.required, false),
      ...(this.asOptionalString(row.description) ? { description: this.asOptionalString(row.description) } : {}),
    }
  }

  private normalizeWorkflowStep(value: unknown): WorkflowStepSpec {
    const row = this.asRecord(value)
    return {
      id: this.asString(row.id),
      kind: this.asString(row.kind, 'native') as WorkflowStepSpec['kind'],
      ...(this.asOptionalString(row.title) ? { title: this.asOptionalString(row.title) } : {}),
      ...(this.asOptionalString(row.goal) ? { goal: this.asOptionalString(row.goal) } : {}),
      ...(this.asOptionalString(row.prompt) ? { prompt: this.asOptionalString(row.prompt) } : {}),
      ...(this.asOptionalString(row.service) ? { service: this.asOptionalString(row.service) } : {}),
      ...(this.asOptionalString(row.sourceSkill) ? { sourceSkill: this.asOptionalString(row.sourceSkill) } : {}),
      ...(this.asOptionalString(row.portability) ? { portability: this.asOptionalString(row.portability) } : {}),
      ...(Array.isArray(row.tags) ? { tags: this.asStringList(row.tags) } : {}),
      ...(this.asOptionalString(row.notes) ? { notes: this.asOptionalString(row.notes) } : {}),
      ...(this.asOptionalString(row.path) ? { path: this.asOptionalString(row.path) } : {}),
      ...(this.asOptionalString(row.branchGroup) ? { branchGroup: this.asOptionalString(row.branchGroup) } : {}),
      ...(this.asOptionalRecord(row.activation) ? { activation: this.asOptionalRecord(row.activation) } : {}),
      ...(this.asOptionalRecord(row.compensation) ? { compensation: this.asOptionalRecord(row.compensation) } : {}),
      ...(typeof row.terminalOnSuccess === 'boolean' ? { terminalOnSuccess: row.terminalOnSuccess } : {}),
      ...(this.asOptionalRecord(row.serviceRequest) ? { serviceRequest: this.asOptionalRecord(row.serviceRequest) } : {}),
      ...(this.asOptionalRecord(row.agent) ? { agent: this.asOptionalRecord(row.agent) } : {}),
      ...(this.asOptionalRecord(row.wait) ? { wait: this.asOptionalRecord(row.wait) } : {}),
      ...(Array.isArray(row.steps) ? { steps: row.steps.map((item) => this.normalizeWorkflowStep(item)) } : {}),
    }
  }

  private normalizeWorkflowSpec(value: unknown): WorkflowSpec {
    const row = this.asRecord(value)
    return {
      workflowId: this.asString(row.workflowId || row.id),
      name: this.asString(row.name, 'Workflow'),
      goal: this.asString(row.goal),
      ...(this.asOptionalString(row.topology) ? { topology: this.asOptionalString(row.topology) } : {}),
      ...(this.asOptionalString(row.description) ? { description: this.asOptionalString(row.description) } : {}),
      ...(this.asOptionalString(row.sourceSummary) ? { sourceSummary: this.asOptionalString(row.sourceSummary) } : {}),
      ...(this.asOptionalString(row.sourceWorkspaceDir) ? { sourceWorkspaceDir: this.asOptionalString(row.sourceWorkspaceDir) } : {}),
      ...(this.asOptionalString(row.sourceAgentDir) ? { sourceAgentDir: this.asOptionalString(row.sourceAgentDir) } : {}),
      tags: this.asStringList(row.tags),
      inputs: Array.isArray(row.inputs) ? row.inputs.map((item) => this.normalizeWorkflowField(item)) : [],
      outputs: Array.isArray(row.outputs) ? row.outputs.map((item) => this.normalizeWorkflowField(item)) : [],
      steps: Array.isArray(row.steps) ? row.steps.map((item) => this.normalizeWorkflowStep(item)) : [],
      ...(this.asOptionalString(row.sourceSessionKey) ? { sourceSessionKey: this.asOptionalString(row.sourceSessionKey) } : {}),
      ...(this.asOptionalString(row.sourceSessionId) ? { sourceSessionId: this.asOptionalString(row.sourceSessionId) } : {}),
      createdAt: this.asNumber(row.createdAt, 0),
      updatedAt: this.asNumber(row.updatedAt, 0),
    }
  }

  private normalizeWorkflowListEntry(value: unknown): WorkflowListEntry {
    const row = this.asRecord(value)
    return {
      ...this.normalizeWorkflowRegistryEntry(row),
      runCount: this.asNumber(row.runCount, 0),
      recentExecution: row.recentExecution ? this.normalizeWorkflowExecution(row.recentExecution) : null,
    }
  }

  private normalizeWorkflowListPayload(payload: unknown): WorkflowListResult {
    const row = this.asRecord(payload)
    const workflows = this.normalizeList<unknown>(payload, ['workflows', 'items', 'list', 'data'])
      .map((item) => this.normalizeWorkflowListEntry(item))
      .filter((item) => !!item.workflowId)
    return {
      ...(this.asOptionalString(row.agentId) ? { agentId: this.asOptionalString(row.agentId) } : {}),
      count: this.asNumber(row.count, workflows.length),
      workflows,
    }
  }

  private normalizeWorkflowGetPayload(payload: unknown): WorkflowGetResult {
    const row = this.asRecord(payload)
    return {
      ...(this.asOptionalString(row.agentId) ? { agentId: this.asOptionalString(row.agentId) } : {}),
      workflow: this.normalizeWorkflowRegistryEntry(row.workflow),
      spec: this.normalizeWorkflowSpec(row.spec),
      specPath: this.asString(row.specPath),
      storeRoot: this.asString(row.storeRoot),
      recentExecutions: this.normalizeList<unknown>(row.recentExecutions, [
        'recentExecutions',
        'executions',
        'items',
      ]).map((item) => this.normalizeWorkflowExecution(item)),
    }
  }

  private normalizeWorkflowRunsPayload(payload: unknown): WorkflowRunsResult {
    const row = this.asRecord(payload)
    const executions = this.normalizeList<unknown>(payload, ['executions', 'items', 'list', 'data'])
      .map((item) => this.normalizeWorkflowExecution(item))
      .filter((item) => !!item.executionId)
    return {
      ...(this.asOptionalString(row.agentId) ? { agentId: this.asOptionalString(row.agentId) } : {}),
      count: this.asNumber(row.count, executions.length),
      executions,
    }
  }

  private normalizeN8nWorkflowNode(value: unknown): N8nWorkflowNodeRecord {
    const row = this.asRecord(value)
    return {
      ...row,
      ...(this.asOptionalString(row.id) ? { id: this.asOptionalString(row.id) } : {}),
      ...(this.asOptionalString(row.name) ? { name: this.asOptionalString(row.name) } : {}),
      ...(this.asOptionalString(row.type) ? { type: this.asOptionalString(row.type) } : {}),
      ...(this.asOptionalNumber(row.typeVersion) ? { typeVersion: this.asOptionalNumber(row.typeVersion) } : {}),
      ...(Array.isArray(row.position) ? { position: row.position.map((item) => this.asNumber(item, 0)) } : {}),
      ...(typeof row.disabled === 'boolean' ? { disabled: row.disabled } : {}),
    }
  }

  private normalizeN8nWorkflow(value: unknown): N8nWorkflowRecord {
    const row = this.asRecord(value)
    return {
      ...row,
      id: this.asString(row.id),
      name: this.asString(row.name, 'n8n workflow'),
      ...(typeof row.active === 'boolean' ? { active: row.active } : {}),
      nodes: Array.isArray(row.nodes) ? row.nodes.map((item) => this.normalizeN8nWorkflowNode(item)) : [],
      connections: this.asRecord(row.connections),
      ...(this.asOptionalRecord(row.settings) ? { settings: this.asOptionalRecord(row.settings) } : {}),
      ...(this.asOptionalRecord(row.staticData) ? { staticData: this.asOptionalRecord(row.staticData) } : {}),
      ...(this.asOptionalRecord(row.meta) ? { meta: this.asOptionalRecord(row.meta) } : {}),
      ...(this.asOptionalString(row.createdAt) ? { createdAt: this.asOptionalString(row.createdAt) } : {}),
      ...(this.asOptionalString(row.updatedAt) ? { updatedAt: this.asOptionalString(row.updatedAt) } : {}),
    }
  }

  private normalizeN8nExecution(value: unknown): N8nExecutionRecord {
    const row = this.asRecord(value)
    return {
      ...row,
      ...(this.asOptionalString(row.id) ? { id: this.asOptionalString(row.id) } : {}),
      ...(this.asOptionalString(row.executionId) ? { executionId: this.asOptionalString(row.executionId) } : {}),
      ...(this.asOptionalString(row.workflowId) ? { workflowId: this.asOptionalString(row.workflowId) } : {}),
      ...(this.asOptionalString(row.status) ? { status: this.asOptionalString(row.status) } : {}),
      ...(typeof row.finished === 'boolean' ? { finished: row.finished } : {}),
      ...(row.stoppedAt === null ? { stoppedAt: null } : {}),
      ...(this.asOptionalString(row.stoppedAt) ? { stoppedAt: this.asOptionalString(row.stoppedAt) } : {}),
      ...(row.startedAt === null ? { startedAt: null } : {}),
      ...(this.asOptionalString(row.startedAt) ? { startedAt: this.asOptionalString(row.startedAt) } : {}),
      ...(this.asOptionalRecord(row.data) ? { data: this.asOptionalRecord(row.data) } : {}),
    }
  }

  private normalizeWorkflowN8nDetailsPayload(payload: unknown): WorkflowN8nDetailsResult {
    const row = this.asRecord(payload)
    return {
      ...(this.asOptionalString(row.agentId) ? { agentId: this.asOptionalString(row.agentId) } : {}),
      workflow: this.normalizeWorkflowRegistryEntry(row.workflow),
      remoteWorkflow: this.normalizeN8nWorkflow(row.remoteWorkflow),
      remoteExecutions: this.normalizeList<unknown>(payload, ['remoteExecutions', 'executions', 'items', 'data'])
        .map((item) => this.normalizeN8nExecution(item)),
      remoteWorkflowUrl: this.asString(row.remoteWorkflowUrl),
      remoteExecutionsUrl: this.asString(row.remoteExecutionsUrl),
    }
  }

  private normalizeWorkflowMutationPayload(payload: unknown): WorkflowMutationResult {
    const row = this.asRecord(payload)
    return {
      ...(this.asOptionalString(row.agentId) ? { agentId: this.asOptionalString(row.agentId) } : {}),
      workflow: this.normalizeWorkflowRegistryEntry(row.workflow),
      ...(row.remoteWorkflow ? { remoteWorkflow: this.normalizeN8nWorkflow(row.remoteWorkflow) } : {}),
      ...(this.asOptionalRecord(row.compiled) ? { compiled: this.asOptionalRecord(row.compiled) } : {}),
    }
  }

  private normalizeWorkflowDeletePayload(payload: unknown): WorkflowDeleteResult {
    const row = this.asRecord(payload)
    return {
      ...(this.asOptionalString(row.agentId) ? { agentId: this.asOptionalString(row.agentId) } : {}),
      deleted: this.asBoolean(row.deleted, false),
      ...(this.asOptionalString(row.workflowId) ? { workflowId: this.asOptionalString(row.workflowId) } : {}),
      ...(this.asOptionalNumber(row.removedExecutions) !== undefined
        ? { removedExecutions: this.asOptionalNumber(row.removedExecutions) }
        : {}),
    }
  }

  private normalizeWorkflowExecutionActionPayload(payload: unknown): WorkflowExecutionActionResult {
    const row = this.asRecord(payload)
    return {
      ...(this.asOptionalString(row.agentId) ? { agentId: this.asOptionalString(row.agentId) } : {}),
      ...(row.workflow ? { workflow: this.normalizeWorkflowRegistryEntry(row.workflow) } : {}),
      execution: this.normalizeWorkflowExecution(row.execution),
    }
  }
  ...
  // --- Config ---
  getConfig(): Promise<CrawClawConfig> {
  ...
  async setConfig(config: CrawClawConfig): Promise<void> {
    const snapshotPayload = await this.call<unknown>('config.get', {})
    const snapshotMeta = this.resolveConfigSnapshotMeta(snapshotPayload)
    let baseHash = snapshotMeta.hash
    if (!baseHash && snapshotMeta.raw) {
  ...
  // --- Workflows ---
  listWorkflows(params: { includeDisabled?: boolean; limit?: number } = {}): Promise<WorkflowListResult> {
    const request: Record<string, unknown> = {
      includeDisabled: params.includeDisabled ?? true,
    }
    if (typeof params.limit === 'number' && Number.isFinite(params.limit) && params.limit > 0) {
      request.limit = Math.floor(params.limit)
    }
    return this.callWithMethodAndParamsFallback<unknown>(['workflow.list'], [request]).then((payload) =>
      this.normalizeWorkflowListPayload(payload)
    )
  }

  getWorkflow(workflow: string, recentRunsLimit = 5): Promise<WorkflowGetResult> {
    return this.callWithMethodAndParamsFallback<unknown>(
      ['workflow.get'],
      [{ workflow, recentRunsLimit }],
    ).then((payload) => this.normalizeWorkflowGetPayload(payload))
  }

  listWorkflowRuns(workflow?: string, limit = 50): Promise<WorkflowRunsResult> {
    const request: Record<string, unknown> = { limit }
    if (workflow) {request.workflow = workflow}
    return this.callWithMethodAndParamsFallback<unknown>(['workflow.runs'], [request]).then((payload) =>
      this.normalizeWorkflowRunsPayload(payload)
    )
  }

  getWorkflowN8nDetails(
    workflow: string,
    options: { executionsLimit?: number } = {}
  ): Promise<WorkflowN8nDetailsResult> {
    const request: Record<string, unknown> = { workflow }
    if (
      typeof options.executionsLimit === 'number' &&
      Number.isFinite(options.executionsLimit) &&
      options.executionsLimit > 0
    ) {
      request.executionsLimit = Math.floor(options.executionsLimit)
    }
    return this.callWithMethodAndParamsFallback<unknown>(['workflow.n8n.get'], [request]).then((payload) =>
      this.normalizeWorkflowN8nDetailsPayload(payload)
    )
  }

  deployWorkflow(workflow: string): Promise<WorkflowMutationResult> {
    return this.callWithMethodAndParamsFallback<unknown>(['workflow.deploy'], [{ workflow }], 160000).then(
      (payload) => this.normalizeWorkflowMutationPayload(payload)
    )
  }

  enableWorkflow(workflow: string): Promise<WorkflowMutationResult> {
    return this.callWithMethodAndParamsFallback<unknown>(['workflow.enable'], [{ workflow }]).then((payload) =>
      this.normalizeWorkflowMutationPayload(payload)
    )
  }

  disableWorkflow(workflow: string): Promise<WorkflowMutationResult> {
    return this.callWithMethodAndParamsFallback<unknown>(['workflow.disable'], [{ workflow }]).then((payload) =>
      this.normalizeWorkflowMutationPayload(payload)
    )
  }

  archiveWorkflow(workflow: string): Promise<WorkflowMutationResult> {
    return this.callWithMethodAndParamsFallback<unknown>(['workflow.archive'], [{ workflow }]).then((payload) =>
      this.normalizeWorkflowMutationPayload(payload)
    )
  }

  unarchiveWorkflow(workflow: string): Promise<WorkflowMutationResult> {
    return this.callWithMethodAndParamsFallback<unknown>(['workflow.unarchive'], [{ workflow }]).then((payload) =>
      this.normalizeWorkflowMutationPayload(payload)
    )
  }

  deleteWorkflow(workflow: string): Promise<WorkflowDeleteResult> {
    return this.callWithMethodAndParamsFallback<unknown>(['workflow.delete'], [{ workflow }]).then((payload) =>
      this.normalizeWorkflowDeletePayload(payload)
    )
  }
  ...
  runWorkflow(
    ...
    approved = true
  ): Promise<WorkflowExecutionActionResult> {
    return this.callWithMethodAndParamsFallback<unknown>(
      ['workflow.run'],
      [{ workflow, ...(inputs ? { inputs } : {}), approved }],
      160000
    ).then((payload) => this.normalizeWorkflowExecutionActionPayload(payload))
  }

  // --- ComfyUI ---
  getComfyUiStatus(): Promise<ComfyUiStatus> {
    return this.call<ComfyUiStatus>('comfyui.status')
  }

  listComfyUiWorkflows(limit = 100): Promise<ComfyUiWorkflowListResult> {
    return this.call<ComfyUiWorkflowListResult>('comfyui.workflows.list', { limit })
  }

  getComfyUiWorkflow(workflowId: string): Promise<ComfyUiWorkflowGetResult> {
    return this.call<ComfyUiWorkflowGetResult>('comfyui.workflow.get', { workflowId })
  }

  listComfyUiRuns(workflowId?: string, limit = 50): Promise<ComfyUiRunsResult> {
    const params: Record<string, unknown> = { limit }
    if (workflowId) {params.workflowId = workflowId}
    return this.call<ComfyUiRunsResult>('comfyui.runs.list', params)
  }

  listComfyUiOutputs(workflowId?: string, limit = 50): Promise<ComfyUiOutputsResult> {
    const params: Record<string, unknown> = { limit }
    if (workflowId) {params.workflowId = workflowId}
    return this.call<ComfyUiOutputsResult>('comfyui.outputs.list', params)
  }

  validateComfyUiWorkflow(workflowId: string): Promise<ComfyUiValidationResult> {
    return this.call<ComfyUiValidationResult>(
  ...
  runComfyUiWorkflow(workflowId: string): Promise<ComfyUiRunResult> {
    return this.call<ComfyUiRunResult>(
  ...
  cancelWorkflowExecution(executionId: string): Promise<WorkflowExecutionActionResult> {
    return this.callWithMethodAndParamsFallback<unknown>(['workflow.cancel'], [{ executionId }], 160000).then(
      (payload) => this.normalizeWorkflowExecutionActionPayload(payload)
    )
  }
