...
export const A2UI_PATH = "/__crawclaw__/a2ui";

export const CANVAS_HOST_PATH = "/__crawclaw__/canvas";

export const CANVAS_WS_PATH = "/__crawclaw__/ws";
...
async function resolveA2uiRoot(): Promise<string | null> {
  ...
  for (const dir of candidates) {
    try {
      ...
      const bundlePath = path.join(dir, "generated", "a2ui.bundle.js");
...
(() => {
  ...
  // Works on:
  // - iOS: window.webkit.messageHandlers.crawclawCanvasA2UIAction.postMessage(...)
  // - Android: window.crawclawCanvasA2UIAction.postMessage(...)
  const handlerNames = ["crawclawCanvasA2UIAction"];
  ...
  globalThis.CrawClaw = globalThis.CrawClaw ?? {};
  globalThis.CrawClaw.postMessage = postToNode;
  globalThis.CrawClaw.sendUserAction = sendUserAction;
  globalThis.crawclawPostMessage = postToNode;
  globalThis.crawclawSendUserAction = sendUserAction;
