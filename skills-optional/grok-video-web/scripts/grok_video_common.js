...
function resolveProfileName(bundle, cliProfile = null) {
  return (
    slugify(cliProfile || bundle.request.profile || bundle.manifest.profile || DEFAULT_PROFILE) ||
    DEFAULT_PROFILE
  );
}
...
async function loadPlaywright() {
  throw new Error(
    "loadPlaywright() is no longer the primary path for grok-video-web run/submit. Use Puppeteer-first helpers instead.",
  );
}
...
async function openSafeEntryPage(page) {
  if (
    !/^https:\/\/grok\.com(?:[/?#]|$)/.test(page.url()) ||
    /https:\/\/grok\.com\/imagine/.test(page.url())
  ) {
...
async function resolveLoginState(page, options = {}) {
  ...
  const shouldProbeImagine =
    allowImagineFallback &&
