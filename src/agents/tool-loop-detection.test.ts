...
describe("tool-loop-detection", () => {
  ...
  describe("recordToolCall", () => {
    it("adds tool call to empty history", () => {
      ...
      expect(state.loopProgressHistory).toEqual([
    ...
    it("maintains sliding window of last N calls", () => {
      ...
      expect(state.loopProgressHistory).toHaveLength(TOOL_CALL_HISTORY_SIZE);

      const oldestCall = state.loopProgressHistory?.[0];
      expect(oldestCall?.inputFingerprint).toBe(hashToolCall("tool", { iteration: 10 }));
    });
    ...
    it("records timestamp for each call", () => {
      ...
      const timestamp = state.loopProgressHistory?.[0]?.timestamp ?? 0;
    ...
    it("respects configured historySize", () => {
      ...
      expect(state.loopProgressHistory).toHaveLength(4);
      expect(state.loopProgressHistory?.[0]?.inputFingerprint).toBe(
        hashToolCall("tool", { iteration: 6 }),
      );
    });
  });
  ...
  describe("detectToolCallLoop", () => {
    ...
    it("records fixed-size result hashes for large tool outputs", () => {
      ...
      const progressEntry = state.loopProgressHistory?.find(
      ...
      expect(progressEntry?.outputFingerprint?.length).toBe(64);
    });
    ...
    it("can detect loops from progress envelopes without legacy history", () => {
      const state = createState();
      const inputFingerprint = hashToolCall("read", { path: "/same.txt" });
      state.loopProgressHistory = Array.from({ length: WARNING_THRESHOLD }, (_, index) => ({
        toolName: "read",
