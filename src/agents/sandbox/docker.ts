...
import { markCrawClawExecEnv } from "../../infra/crawclaw-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "crawclaw.sandbox=1");
  args.push("--label", `crawclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `crawclaw.createdAtMs=${createdAtMs}`);
  args.push("--label", `crawclaw.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `crawclaw.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markCrawClawExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "crawclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`crawclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`crawclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("crawclaw sandbox recreate --all");
}
