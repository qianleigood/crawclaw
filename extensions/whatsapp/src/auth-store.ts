...
import { info, success } from "crawclaw/plugin-sdk/runtime-env";
import { getChildLogger } from "crawclaw/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "crawclaw/plugin-sdk/runtime-env";
import { resolveOAuthDir } from "crawclaw/plugin-sdk/config-runtime";
import type { WebChannel } from "crawclaw/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("crawclaw channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
