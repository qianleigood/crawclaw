...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing crawclaw.extensions; update the plugin package to include crawclaw.extensions (for example ["./dist/index.js"]). See https://docs.crawclaw.ai/help/troubleshooting#plugin-install-fails-with-missing-crawclaw-extensions';
...
function ensureCrawClawExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_CRAWCLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json crawclaw.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_CRAWCLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    return {
      ok: false,
      error: `Bundle "${pluginId}" installation blocked: code safety scan failed (${String(err)}). Run "crawclaw security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureCrawClawExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureCrawClawExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from crawclaw.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-crawclaw"), the plugin registry
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json crawclaw.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires CrawClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set CRAWCLAW_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires CrawClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade CrawClaw and retry.`,
  ...
  try {
    ...
    return {
      ok: false,
      error: `Plugin "${pluginId}" installation blocked: code safety scan failed (${String(err)}). Run "crawclaw security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
...
export async function installPluginFromFile(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `Plugin file "${pluginId}" installation blocked: code safety scan failed (${String(err)}). Run "crawclaw security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
...
export async function installPluginFromNpmSpec(
  ...
  const flowResult = await runtime.installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "crawclaw-npm-pack-",
    spec,
    timeoutMs,
