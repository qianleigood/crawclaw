...
read_when:
  - 更新 CrawClaw
  - 更新后出现问题
summary: 安全更新 CrawClaw（全局安装或源码），以及回滚策略
title: 更新
x-i18n:
...
CrawClaw 发展迅速（尚未到"1.0"）。将更新视为发布基础设施：更新 → 运行检查 → 重启（或使用会重启的 `crawclaw update`）→ 验证。

## 推荐：重新运行网站安装程序（原地升级）

**首选**的更新路径是重新运行网站上的安装程序。它会检测现有安装、原地升级，并在需要时运行 `crawclaw doctor`。
...
- 快照你的定制内容：
  - 配置：`~/.crawclaw/crawclaw.json`
  - 凭证：`~/.crawclaw/credentials/`
  - 工作区：`~/.crawclaw/workspace`
...
- 如果你的 Gateway 网关作为服务运行，`crawclaw gateway restart` 优于杀死 PID。
...
- 当你运行打包的 `crawclaw` 二进制文件（[`crawclaw.mjs`](https://github.com/qianleigood/crawclaw/blob/main/crawclaw.mjs)）或使用 Node 运行 `dist/` 时，`pnpm build` 很重要。
- 如果你从仓库 checkout 运行而没有全局安装，CLI 命令使用 `pnpm crawclaw ...`。
- 如果你直接从 TypeScript 运行（`pnpm crawclaw ...`），通常不需要重新构建，但**配置迁移仍然适用** → 运行 doctor。
- 在全局和 git 安装之间切换很容易：安装另一种方式，然后运行 `crawclaw doctor` 以便将 Gateway 网关服务入口点重写为当前安装。

## 始终运行：`crawclaw doctor`

Doctor 是"安全更新"命令。它故意很无聊：修复 + 迁移 + 警告。

注意：如果你是**源码安装**（git checkout），`crawclaw doctor` 会提供先运行 `crawclaw update`。
...
- 检测并将旧版 Gateway 网关服务（launchd/systemd；旧版 schtasks）迁移到当前 CrawClaw 服务。
...
```bash
pnpm add -g crawclaw@<version>
```

提示：要查看当前发布的版本，运行 `npm view crawclaw version`。
