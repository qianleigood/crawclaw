...
`crawclaw.invoke --each`) without adding new CrawClaw code per workflow.
...
This extension depends on CrawClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** CrawClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.crawclaw/extensions` as a standalone plugin directory.
