...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevCrawClawHome = process.env.CRAWCLAW_HOME;
    const prevHome = process.env.HOME;
    process.env.CRAWCLAW_HOME = "/srv/crawclaw-home";
    process.env.HOME = "/home/other";
    const crawClawHome = path.join(path.resolve("/srv/crawclaw-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", crawClawHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevCrawClawHome === undefined) {
        delete process.env.CRAWCLAW_HOME;
      } else {
        process.env.CRAWCLAW_HOME = prevCrawClawHome;
      }
