...
- Provider notes:
  ...
  - Moonshot (`moonshot/*`) maps `/think off` to `thinking: { type: "disabled" }` and any non-`off` level to `thinking: { type: "enabled" }`. When thinking is enabled, Moonshot only accepts `tool_choice` `auto|none`; CrawClaw normalizes incompatible values to `auto`.
...
- CrawClaw resolves fast mode in this order:
...
- For `openai-codex/*`, fast mode sends the same `service_tier=priority` flag on Codex Responses. CrawClaw keeps one shared `/fast` toggle across both auth paths.
...
- Explicit Anthropic `serviceTier` / `service_tier` model params override the fast-mode default when both are set. CrawClaw still skips Anthropic service-tier injection for non-Anthropic proxy base URLs.
...
- The config fallback default is `off`, but onboarding presets may write a different explicit
  `agents.defaults.verboseDefault` into `crawclaw.json`. For example, the default `balanced`
  presentation preset writes `on`.
- Send `/verbose` (or `/verbose:`) with no argument to see the current verbose level.
- When verbose is on, Rust agent runs that emit structured tool results send each tool call back as its own metadata-only message, prefixed with `<emoji> <tool-name>: <arg>` when available (path/command). These tool summaries are sent as soon as each tool starts (separate bubbles), not as streaming deltas.
...
- `stream` (Feishu only): streams reasoning into the Feishu draft bubble while the reply is generating, then sends the final answer without reasoning.
- Alias: `/reason`.
...
Legacy heartbeat-style runs can still use the configured heartbeat prompt and
reasoning delivery options for compatibility. New scheduled automation should
use [Scheduled Tasks](/automation/cron-jobs) instead of heartbeat prompts.
