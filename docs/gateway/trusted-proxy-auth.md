...
read_when:
  - Running CrawClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of CrawClaw
...
- You run CrawClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. CrawClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
...
- CrawClaw can stay on loopback HTTP behind the proxy.
...
If CrawClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
CrawClaw rejects ambiguous configurations where both a `gateway.auth.token` (or `CRAWCLAW_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
