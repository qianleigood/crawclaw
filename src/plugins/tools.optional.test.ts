...
const loadCrawClawPluginsMock = vi.fn();
...
function setRegistry(entries: MockRegistryToolEntry[]) {
  ...
  loadCrawClawPluginsMock.mockReturnValue(registry);
  return registry;
}
...
function expectLoaderCall(overrides: Record<string, unknown>) {
  expect(loadCrawClawPluginsMock).toHaveBeenCalledWith(expect.objectContaining(overrides));
}
  ...
  it.each([
    {
      ...
      expectedLoaderCall: {
        env: { CRAWCLAW_HOME: "/srv/crawclaw-home" },
      },
    },
    {
