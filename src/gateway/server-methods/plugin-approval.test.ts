...
describe("createPluginApprovalHandlers", () => {
      ...
      expect(events).toContainEqual(
        expect.objectContaining({
          ...
          data: expect.objectContaining({
            actionId: expect.stringMatching(/^approval:/),
            kind: "approval",
            status: "blocked",
  ...
  describe("plugin.approval.resolve", () => {
    it("emits an approval action when a plugin approval is granted", async () => {
      const handlers = createPluginApprovalHandlers(manager);
      const events: Array<{ stream: string; data: Record<string, unknown> }> = [];
      const stop = onAgentEvent((event) => {
        events.push({ stream: event.stream, data: event.data });
      ...
      try {
        ...
        const acceptedCall = requestRespond.mock.calls.find(
          (call) => (call[1] as Record<string, unknown>)?.status === "accepted",
        );
        const approvalId = (acceptedCall?.[1] as Record<string, unknown>)?.id as string;
