...
const clearPlannerShardEnv = (env) => {
  const nextEnv = { ...env };
  delete nextEnv.CRAWCLAW_TEST_SHARDS;
  delete nextEnv.CRAWCLAW_TEST_SHARD_INDEX;
  delete nextEnv.CRAWCLAW_TEST_FORCE_THREADS;
  delete nextEnv.CRAWCLAW_TEST_FORCE_FORKS;
  delete nextEnv.CRAWCLAW_TEST_DISABLE_THREAD_EXPANSION;
  delete nextEnv.CRAWCLAW_TEST_SHOW_POOL_DECISION;
  delete nextEnv.CRAWCLAW_TEST_PROFILE;
  delete nextEnv.CRAWCLAW_TEST_WORKERS;
  delete nextEnv.CRAWCLAW_TEST_SKIP_DEFAULT;
  delete nextEnv.CRAWCLAW_TEST_INCLUDE_EXTENSIONS;
  delete nextEnv.CRAWCLAW_TEST_INCLUDE_CHANNELS;
  delete nextEnv.CRAWCLAW_TEST_INCLUDE_GATEWAY;
  delete nextEnv.CRAWCLAW_TEST_UNIT_FAST_BATCH_TARGET_MS;
  return nextEnv;
};
...
const HIGH_MEMORY_LOCAL_PLANNER_ENV = {
  RUNNER_OS: "macOS",
  CRAWCLAW_TEST_HOST_CPU_COUNT: "12",
  CRAWCLAW_TEST_HOST_MEMORY_GIB: "128",
} as const;
...
function runManifestOutputWriter(workflow: string, envOverrides: NodeJS.ProcessEnv = {}): string {
  const outputPath = path.join(os.tmpdir(), `crawclaw-${workflow}-output-${Date.now()}.txt`);
  try {
...
describe("scripts/test-parallel memory trace parsing", () => {
  ...
  it("parses memory trace summary lines and hotspot deltas", () => {
    const summaries = parseMemoryTraceSummaryLines(
      [
        "2026-03-20T04:32:18.7721466Z [test-parallel][mem] summary unit-fast files=360 peak=13.22GiB totalDelta=6.69GiB peakAt=poll top=src/config/config.secrets-schema.test.ts:1.06GiB, src/infra/update-runner.test.ts:+463.6MiB",
      ].join("\n"),
    );
    ...
    expect(summaries[0]).toEqual({
      ...
      top: [
        {
          file: "src/config/config.secrets-schema.test.ts",
...
describe("scripts/test-parallel lane planning", () => {
  ...
  it("recycles default local unit-fast runs into bounded batches", () => {
    const output = runPlannerPlan(["--plan"], {
      CI: "",
      CRAWCLAW_TEST_UNIT_FAST_LANES: "1",
      CRAWCLAW_TEST_UNIT_FAST_BATCH_TARGET_MS: "1",
    });
  ...
  it("reports capability-derived output for mid-memory local macOS hosts", () => {
    const output = runPlannerPlan(
      ...
      createLocalPlannerEnv({
        RUNNER_OS: "macOS",
        CRAWCLAW_TEST_HOST_CPU_COUNT: "10",
        CRAWCLAW_TEST_HOST_MEMORY_GIB: "64",
      }),
    );
  ...
  it("prints the planner-backed CI manifest as JSON", () => {
    const output = runPlannerPlan(["--ci-manifest"], {
      ...
      CRAWCLAW_CI_DOCS_CHANGED: "false",
      CRAWCLAW_CI_RUN_NODE: "true",
      CRAWCLAW_CI_RUN_WINDOWS: "true",
      CRAWCLAW_CI_RUN_SKILLS_PYTHON: "false",
      CRAWCLAW_CI_HAS_CHANGED_EXTENSIONS: "false",
      CRAWCLAW_CI_CHANGED_EXTENSIONS_MATRIX: '{"include":[]}',
    });
  ...
  it("rejects explicit existing files that are not known test files", () => {
    const tempFilePath = path.join(os.tmpdir(), `crawclaw-non-test-${Date.now()}.ts`);
