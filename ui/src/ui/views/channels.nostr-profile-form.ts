...
export function renderNostrProfileForm(params: {
  ...
  const renderPicturePreview = () => {
    ...
    return html`
      <div style="margin-bottom: 12px;">
        <img
          src=${picture}
          alt=${uiLiteral("Profile picture preview")}
  ...
  return html`
    ...
    >
      ...
      >
        <div style="font-weight: 600; font-size: 16px;">${uiLiteral("Edit Profile")}</div>
        <div style="font-size: 12px; color: var(--text-muted);">${uiLiteral("Account")}: ${accountId}</div>
      </div>
      ...
      ${renderField("name", uiLiteral("Username"), {
        placeholder: uiLiteral("satoshi"),
        maxLength: 256,
        help: uiLiteral("Short username (e.g., satoshi)"),
      })}
      ${renderField("displayName", uiLiteral("Display Name"), {
        placeholder: uiLiteral("Satoshi Nakamoto"),
        maxLength: 256,
        help: uiLiteral("Your full display name"),
      })}
      ${renderField("about", uiLiteral("Bio"), {
        type: "textarea",
        placeholder: uiLiteral("Tell people about yourself..."),
        maxLength: 2000,
        help: uiLiteral("A brief bio or description"),
      })}
      ${renderField("picture", uiLiteral("Avatar URL"), {
        type: "url",
        placeholder: "https://example.com/avatar.jpg",
        help: uiLiteral("HTTPS URL to your profile picture"),
      })}
      ${state.showAdvanced
        ? html`
            <div
            ...
            >
              ...
              ${renderField("banner", uiLiteral("Banner URL"), {
                type: "url",
                placeholder: "https://example.com/banner.jpg",
                help: uiLiteral("HTTPS URL to a banner image"),
              })}
              ${renderField("website", uiLiteral("Website"), {
                type: "url",
                placeholder: "https://example.com",
                help: uiLiteral("Your personal website"),
              })}
              ${renderField("nip05", uiLiteral("NIP-05 Identifier"), {
                placeholder: uiLiteral("you@example.com"),
                help: uiLiteral("Verifiable identifier (e.g., you@domain.com)"),
              })}
              ${renderField("lud16", uiLiteral("Lightning Address"), {
                placeholder: uiLiteral("you@getalby.com"),
                help: uiLiteral("Lightning address for tips (LUD-16)"),
              })}
            </div>
          `
        : nothing}
      ...
      <div style="display: flex; gap: 8px; margin-top: 16px; flex-wrap: wrap;">
        ...
        >
          ${state.saving ? uiLiteral("Saving...") : uiLiteral("Save & Publish")}
        </button>
        ...
        >
          ${state.importing ? uiLiteral("Importing...") : uiLiteral("Import from Relays")}
        </button>

        <button class="btn" @click=${callbacks.onToggleAdvanced}>
          ${state.showAdvanced ? uiLiteral("Hide Advanced") : uiLiteral("Show Advanced")}
        </button>

        <button class="btn" @click=${callbacks.onCancel} ?disabled=${state.saving}>${uiLiteral("Cancel")}</button>
      </div>

      ${isDirty
        ? html`
            <div style="font-size: 12px; color: var(--warning-color); margin-top: 8px">
              ${uiLiteral("You have unsaved changes")}
            </div>
          `
        : nothing}
    </div>
