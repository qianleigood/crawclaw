...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-dotenv-test-"));
...
describe("loadDotEnv", () => {
  it("loads ~/.crawclaw/.env as fallback without overriding CWD .env", async () => {
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "CRAWCLAW_STATE_DIR=./evil-state",
            "CRAWCLAW_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.CRAWCLAW_STATE_DIR).toBe(stateDir);
        expect(process.env.CRAWCLAW_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "CRAWCLAW_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "CRAWCLAW_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "CRAWCLAW_LIVE_GEMINI_KEY=sk-gemini-live",
            "CRAWCLAW_LIVE_OPENAI_KEY=sk-openai-live",
            "CRAWCLAW_GATEWAY_TOKEN=attacker-token",
            "CRAWCLAW_GATEWAY_PASSWORD=attacker-password",
            "CRAWCLAW_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
        ...
        delete process.env.CRAWCLAW_LIVE_ANTHROPIC_KEY;
        delete process.env.CRAWCLAW_LIVE_ANTHROPIC_KEYS;
        delete process.env.CRAWCLAW_LIVE_GEMINI_KEY;
        delete process.env.CRAWCLAW_LIVE_OPENAI_KEY;
        delete process.env.CRAWCLAW_GATEWAY_TOKEN;
        delete process.env.CRAWCLAW_GATEWAY_PASSWORD;
        delete process.env.CRAWCLAW_GATEWAY_SECRET;
        ...
        expect(process.env.CRAWCLAW_LIVE_ANTHROPIC_KEY).toBeUndefined();
        expect(process.env.CRAWCLAW_LIVE_ANTHROPIC_KEYS).toBeUndefined();
        expect(process.env.CRAWCLAW_LIVE_GEMINI_KEY).toBeUndefined();
        expect(process.env.CRAWCLAW_LIVE_OPENAI_KEY).toBeUndefined();
        expect(process.env.CRAWCLAW_GATEWAY_TOKEN).toBeUndefined();
        expect(process.env.CRAWCLAW_GATEWAY_PASSWORD).toBeUndefined();
        expect(process.env.CRAWCLAW_GATEWAY_SECRET).toBeUndefined();
      });
    });
  });

  it("blocks CRAWCLAW_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
        ...
        expect(process.env.CRAWCLAW_STATE_DIR).toBeUndefined();
        expect(process.env.CRAWCLAW_CONFIG_PATH).toBeUndefined();
      });
    });
  });

  it("blocks CrawClaw and legacy path-override vars from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "CRAWCLAW_AGENT_DIR=./evil-agent",
            `CRAWCLAW_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "CRAWCLAW_AGENT_DIR=./evil-agent",
            `CRAWCLAW_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "PI_CODING_AGENT_DIR=./evil-coding",
            "CRAWCLAW_OAUTH_DIR=./evil-oauth",
          ].join("\n"),
        );
        ...
        expect(process.env.CRAWCLAW_AGENT_DIR).toBeUndefined();
        expect(process.env.CRAWCLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.CRAWCLAW_AGENT_DIR).toBeUndefined();
        expect(process.env.CRAWCLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.CRAWCLAW_OAUTH_DIR).toBeUndefined();
      });
    });
  });

  it("blocks CRAWCLAW_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "CRAWCLAW_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.CRAWCLAW_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.CRAWCLAW_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "CRAWCLAW_PINNED_PYTHON=./attacker-python",
            "CRAWCLAW_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.CRAWCLAW_PINNED_PYTHON;
        delete process.env.CRAWCLAW_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.CRAWCLAW_PINNED_PYTHON).toBeUndefined();
        expect(process.env.CRAWCLAW_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "CRAWCLAW_PINNED_PYTHON=/trusted/python",
            "CRAWCLAW_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.CRAWCLAW_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.CRAWCLAW_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "CRAWCLAW_LIVE_GEMINI_KEY=sk-gemini-live",
            "CRAWCLAW_LIVE_OPENAI_KEY=sk-openai-live",
            "CRAWCLAW_GATEWAY_TOKEN=trusted-token",
            "CRAWCLAW_GATEWAY_PASSWORD=trusted-password",
            "CRAWCLAW_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_API_KEY;
        ...
        expect(process.env.CRAWCLAW_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.CRAWCLAW_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.CRAWCLAW_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.CRAWCLAW_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.CRAWCLAW_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.CRAWCLAW_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.CRAWCLAW_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "CRAWCLAW_STATE_DIR=./evil-state\n");
