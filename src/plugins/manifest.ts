...
function parsePluginKind(raw: unknown): PluginKind | PluginKind[] | undefined {
  ...
  if (
    Array.isArray(raw) &&
    raw.length > 0 &&
    raw.every((k) => typeof k === "string" && k === "memory")
  ) {
    return "memory";
  }
  return undefined;
...
export function loadPluginManifest(
  ...
  if (raw.kind !== undefined && kind === undefined) {
    return {
      ok: false,
      error: 'plugin manifest kind must be "memory" when provided',
      manifestPath,
    };
  }
...
export type PackageManifest = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, CrawClawPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): CrawClawPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
