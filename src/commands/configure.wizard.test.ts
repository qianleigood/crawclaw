...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "~/.crawclaw/crawclaw.json",
...
vi.mock("./onboard-helpers.js", () => ({
  ...
  applyWizardMetadata: (cfg: CrawClawConfig) => cfg,
...
function setupBaseWizardState(config: CrawClawConfig = {}) {
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.setupSearch.mockImplementation(async (cfg: CrawClawConfig) => cfg);
  });
  ...
  it("persists provider-owned web search config changes returned by setupSearch", async () => {
    setupBaseWizardState();
    mocks.setupSearch.mockImplementation(async (cfg: CrawClawConfig) =>
      createEnabledWebSearchConfig("brave", {
        enabled: true,
        config: { webSearch: { apiKey: "BSA-entered-key" } },
      })(cfg),
    );
    queueWizardPrompts({
      select: ["local"],
          ...
          entries: expect.objectContaining({
            brave: expect.objectContaining({
              ...
              config: expect.objectContaining({
                webSearch: expect.objectContaining({ apiKey: "BSA-entered-key" }),
              }),
            }),
          }),
        }),
