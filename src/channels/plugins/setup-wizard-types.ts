...
export type ChannelOnboardingPostWriteHook = {
  channel: ChannelId;
  accountId: string;
  run: (ctx: { cfg: CrawClawConfig; runtime: RuntimeEnv }) => Promise<void> | void;
};
...
export type ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: CrawClawConfig, accountId?: string) => DmPolicy;
  setPolicy: (cfg: CrawClawConfig, policy: DmPolicy, accountId?: string) => CrawClawConfig;
