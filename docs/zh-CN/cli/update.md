...
read_when:
  - 你想安全地更新源码检出
  - 你需要了解 `--update` 简写行为
summary: "`crawclaw update` 的 CLI 参考（相对安全的源码更新 + Gateway 网关自动重启）"
title: update
x-i18n:
...
安全更新 CrawClaw 并在 stable/beta/dev 渠道之间切换。
...
当你显式切换渠道（`--channel ...`）时，CrawClaw 也会保持安装方式一致：

- `dev` → 确保存在 git 检出（默认路径是 `~/crawclaw`，可通过 `CRAWCLAW_GIT_DIR` 覆盖；兼容旧版 `CRAWCLAW_GIT_DIR`），更新它，并从该检出安装全局 CLI。
...
`crawclaw --update` 会重写为 `crawclaw update`（便于 shell 和启动脚本使用）。

## 另请参阅

- `crawclaw doctor`（在 git 检出上会提供先运行更新的选项）
