...
- Off by default.
- Android node-client settings use a selector: Off / While Using.
...
- Android node clients currently support foreground location only.
...
Optional. macOS node reports `location` via the permissions map; other node runtimes may omit it.
...
- Android node clients deny `location.get` while backgrounded.
- Keep CrawClaw open when requesting location on Android.
- Other node platforms may differ.

## Model/tooling integration

- Tool surface: `nodes` tool adds `location_get` action (node required).
- CLI: `crawclaw nodes location get --node <id>`.
- Agent guidelines: only call when user enabled location and understands the scope.

## UX copy (suggested)

- Off: “Location sharing is disabled.”
- While Using: “Only when CrawClaw is open.”
