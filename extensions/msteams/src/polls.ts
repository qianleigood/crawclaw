...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["crawclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["crawclaw", "pollId"]) ??
    readNestedString(value, ["crawclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "crawclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "crawclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          crawclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "crawclaw poll vote",
          displayText: "Vote recorded",
          value: { crawclawPollId: pollId, pollId },
        },
      },
    ],
  };
