...
export const resolveCrawClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
beforeEach(() => {
  ...
  resolveCrawClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  originalUpdateInProgress = process.env.CRAWCLAW_UPDATE_IN_PROGRESS;
  process.env.CRAWCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-doctor-state-"));
