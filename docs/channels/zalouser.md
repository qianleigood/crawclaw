...
read_when:
  - Setting up Zalo Personal for CrawClaw
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside CrawClaw.
...
- Install via CLI: `crawclaw plugins install @crawclaw/zalouser`
- Or from a source checkout: `crawclaw plugins install ./path/to/local/zalouser-plugin`
...
- `crawclaw pairing list zalouser`
- `crawclaw pairing approve zalouser <code>`
...
- On startup, CrawClaw resolves group/user names in allowlists to IDs and logs the mapping.
...
- Sender checks apply to both normal group messages and control commands (for example `/new`).
...
- When a group message is skipped because mention is required, CrawClaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in CrawClaw state. Example:
...
- CrawClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, CrawClaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `crawclaw channels logout --channel zalouser && crawclaw channels login --channel zalouser`
...
- The channel now runs fully in CrawClaw without external CLI binaries.
