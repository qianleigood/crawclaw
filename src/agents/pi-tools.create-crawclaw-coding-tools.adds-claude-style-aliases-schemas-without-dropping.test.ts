...
import { __testing, createCrawClawCodingTools } from "./pi-tools.js";
import { createCrawClawReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
import { createHostSandboxFsBridge } from "./test-helpers/host-sandbox-fs-bridge.js";

const defaultTools = createCrawClawCodingTools();
...
describe("createCrawClawCodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createCrawClawCodingTools({ senderIsOwner: true }).find(
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-depth-policy-"));
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createCrawClawCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createCrawClawCodingTools({
      config: {
        plugins: {
          enabled: true,
    ...
    // Provider-specific web_search presence is covered by
    // pi-tools.model-provider-collision.test.ts. This test focuses on schema
    // cleanup across whatever direct Grok tool surface is currently available.
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "crawclaw-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createCrawClawReadTool(
      baseRead as unknown as Parameters<typeof createCrawClawReadTool>[0],
    );
