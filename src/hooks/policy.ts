import type { CrawClawConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  ...
  "crawclaw-workspace": {
    ...
    canOverride: ["crawclaw-workspace"],
    canBeOverriddenBy: ["crawclaw-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "crawclaw-plugin") {
    return { enabled: true };
  }
