...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      CRAWCLAW_GATEWAY_PORT: String(params.port),
      ...(params.token ? { CRAWCLAW_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.CRAWCLAW_STATE_DIR = "/tmp/crawclaw-cli-state";
    process.env.CRAWCLAW_CONFIG_PATH = "/tmp/crawclaw-cli-state/crawclaw.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        CRAWCLAW_STATE_DIR: "/tmp/crawclaw-daemon-state",
        CRAWCLAW_CONFIG_PATH: "/tmp/crawclaw-daemon-state/crawclaw.json",
        CRAWCLAW_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.crawclaw.gateway.plist",
    });
