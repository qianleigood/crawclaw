...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/crawclaw-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/crawclaw-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/crawclaw-install.sh
  guest_exec /usr/bin/env CRAWCLAW_NO_ONBOARD=1 bash /tmp/crawclaw-install.sh "${version_args[@]}" --no-onboard
  guest_exec crawclaw --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec crawclaw --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" crawclaw onboard \
    --non-interactive \
    --mode local \
...
start_gateway_background() {
  ...
  cmd="$(cat <<EOF
pkill -f "crawclaw gateway run" >/dev/null 2>&1 || true
rm -f /tmp/crawclaw-parallels-linux-gateway.log
setsid sh -lc 'exec env CRAWCLAW_HOME=/root CRAWCLAW_STATE_DIR=/root/.crawclaw CRAWCLAW_CONFIG_PATH=/root/.crawclaw/crawclaw.json ${API_KEY_ENV}=${api_key_value_q} crawclaw gateway run --bind loopback --port 18789 --force >/tmp/crawclaw-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
...
show_gateway_status_compat() {
  if guest_exec crawclaw gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_exec crawclaw gateway status --deep --require-rpc
    return
  fi
  guest_exec crawclaw gateway status --deep
}

verify_local_turn() {
  guest_exec crawclaw models set "$MODEL_ID"
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" crawclaw agent \
    --local \
    --agent main \
    --message ping \
    --json
