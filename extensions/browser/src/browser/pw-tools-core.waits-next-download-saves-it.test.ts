...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredCrawClawTmpDir: vi.fn(() => "/tmp/crawclaw"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-crawclaw-dir.js");

describe("pw-tools-core", () => {
  beforeAll(async () => {
    vi.doMock("./pw-session.js", () => sessionMocks);
    vi.doMock("./chrome.js", () => chromeMocks);
    tmpDirModule = await import("../infra/tmp-crawclaw-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredCrawClawTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredCrawClawTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredCrawClawTmpDir.mockReturnValue("/tmp/crawclaw");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".crawclaw-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredCrawClawTmpDir.mockReturnValue("/tmp/crawclaw-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "crawclaw-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "crawclaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredCrawClawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "crawclaw-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "crawclaw-preferred", "downloads")}${path.sep}`),
    );
  });
