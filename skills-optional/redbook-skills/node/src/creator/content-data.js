...
function formatPostTime(postTimeMs) {
  if (typeof postTimeMs !== "number" || Number.isNaN(postTimeMs)) {
    return "-";
  }
  try {
...
async function fetchContentDataViaPage(page, { pageNum = 1, pageSize = 10, noteType = 0 } = {}) {
  const result = await page.evaluate(
    async ({ inputPageNum, inputPageSize, inputNoteType, apiPath }) => {
    ...
    },
    {
      ...
      apiPath: XHS_CONTENT_DATA_API_PATH,
    },
  );
...
async function extractContentDataFromDom(page, { pageNum = 1, pageSize = 10, noteType = 0 } = {}) {
  const result = await page.evaluate((inputPageNum) => {
    ...
    const table = Array.from(document.querySelectorAll("table")).find((tbl) => {
      ...
      return (
        text.includes("笔记基础信息") &&
        text.includes("封面点击率") &&
        text.includes("人均观看时长")
      );
    });
    if (!table) {
      return null;
...
async function clickPaginationPage(page, pageNum) {
  const clicked = await page.evaluate((targetPageNum) => {
    ...
    const clickable =
      textNode.closest('.d-pagination-page, .d-clickable, button, a, [role="button"]') ||
  ...
  if (!clicked) {
    throw new XhsCliError(
      `Could not find pagination page ${pageNum}. Please click it manually in the browser.`,
      {
