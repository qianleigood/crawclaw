...
describe("shared/entry-metadata", () => {
  it("prefers metadata emoji and homepage when present", () => {
    expect(
      resolveEmojiAndHomepage({
        metadata: { emoji: "🦀", homepage: " https://docs.crawclaw.ai " },
  ...
  it("falls back through frontmatter homepage aliases and drops blanks", () => {
    expect(
      resolveEmojiAndHomepage({
        frontmatter: { emoji: "🙂", website: " https://docs.crawclaw.ai " },
      }),
    ).toEqual({
      emoji: "🙂",
      homepage: "https://docs.crawclaw.ai",
    ...
    expect(
      resolveEmojiAndHomepage({
        frontmatter: { url: " https://docs.crawclaw.ai/install " },
      }),
    ).toEqual({
      homepage: "https://docs.crawclaw.ai/install",
    });
  });
    ...
    expect(
      resolveEmojiAndHomepage({
        frontmatter: {
          ...
          url: "https://docs.crawclaw.ai/install",
        },
      }),
    ).toEqual({});
  });
