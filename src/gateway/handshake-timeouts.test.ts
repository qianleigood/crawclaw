...
describe("gateway handshake timeouts", () => {
  ...
  test("prefers CRAWCLAW_HANDSHAKE_TIMEOUT_MS and falls back on the test-only env", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        CRAWCLAW_HANDSHAKE_TIMEOUT_MS: "75",
        CRAWCLAW_TEST_HANDSHAKE_TIMEOUT_MS: "20",
      }),
    ).toBe(75);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        CRAWCLAW_HANDSHAKE_TIMEOUT_MS: "",
        CRAWCLAW_TEST_HANDSHAKE_TIMEOUT_MS: "20",
        VITEST: "1",
      }),
    ).toBe(20);
