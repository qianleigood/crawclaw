...
read_when:
  ...
  - 你想部署到云平台
  - 你需要更新、迁移或卸载
summary: 安装 CrawClaw —— 安装脚本、npm/pnpm、从源码、Docker 等
title: 安装
x-i18n:
...
<Tip>
**安装脚本** 是安装 CrawClaw 的推荐方式。它会一步完成 Node 检测、安装和新手引导。
</Tip>

<Warning>
对于 VPS/云主机，尽量避免使用第三方“一键式”市场镜像。优先选择干净的基础 OS 镜像（例如 Ubuntu LTS），然后使用安装脚本自行安装 CrawClaw。
</Warning>
  ...
  <Accordion title="npm / pnpm" icon="package">
    ...
    <Tabs>
      ...
      <Tab title="pnpm">
        ...
        pnpm approve-builds -g        # 批准 crawclaw、node-llama-cpp、sharp 等
        crawclaw onboard --install-daemon
        ```
    ...
    ```bash
    pnpm add -g github:crawclaw/crawclaw#main
    ```
  ...
  <Accordion title="从源码" icon="github">
    ...
    <Steps>
      ...
      <Step title="链接 CLI">
        ...
        或者，你也可以跳过链接，直接在仓库内通过 `pnpm crawclaw ...` 运行命令。
      </Step>
      <Step title="运行新手引导">
        ```bash
        crawclaw onboard --install-daemon
...
```bash
crawclaw doctor         # 检查配置问题
crawclaw status         # Gateway 网关状态
crawclaw dashboard      # 打开浏览器 UI
```

如果你需要自定义运行时路径，请使用：

- `CRAWCLAW_HOME` 用于基于主目录的内部路径
- `CRAWCLAW_STATE_DIR` 用于可变状态位置
- `CRAWCLAW_CONFIG_PATH` 用于配置文件位置
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不在**你的 `$PATH` 中，那么你的 shell 就找不到全局 npm 二进制文件（包括 `crawclaw`）。
