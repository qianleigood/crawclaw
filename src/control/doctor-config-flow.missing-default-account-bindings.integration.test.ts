...
describe("doctor missing default account binding warning", () => {
  ...
  it("emits a doctor warning when named accounts have no valid account-scoped bindings", async () => {
    await withEnvAsync(
      {
        FEISHU_BOT_TOKEN: undefined,
        FEISHU_BOT_TOKEN_FILE: undefined,
      },
      async () => {
        await runDoctorConfigWithInput({
          config: {
            ...
            bindings: [{ agentId: "ops", match: { channel: "feishu" } }],
          },
    ...
    expect(noteSpy).toHaveBeenCalledWith(
      expect.stringContaining("channels.feishu: accounts.default is missing"),
      "Doctor warnings",
    );
  });
  ...
  it("emits a warning when multiple accounts have no explicit default", async () => {
    ...
    expect(noteSpy).toHaveBeenCalledWith(
      expect.stringContaining(
        "channels.feishu: multiple accounts are configured but no explicit default is set",
      ),
      "Doctor warnings",
    );
  });
  ...
  it("emits a warning when defaultAccount does not match configured accounts", async () => {
    ...
    expect(noteSpy).toHaveBeenCalledWith(
      expect.stringContaining(
        'channels.feishu: defaultAccount is set to "missing" but does not match configured accounts',
      ),
      "Doctor warnings",
    );
  });
