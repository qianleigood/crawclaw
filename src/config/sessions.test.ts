...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ CRAWCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("derives session transcripts dir from CRAWCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { CRAWCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when CRAWCLAW_STATE_DIR differs from stored paths", () => {
