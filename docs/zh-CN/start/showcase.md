---
description: Real-world CrawClaw projects from the community
summary: 社区构建的基于 CrawClaw 的项目和集成
title: 案例展示
x-i18n:
...
向"Robby"（@crawclaw）请求一个本地酒窖 skill。它请求一个示例 CSV 导出 + 存储位置，然后快速构建/测试该 skill（示例中有 962 瓶酒）。
...
跨 Agents、Claude、Codex 和 CrawClaw 管理 skills/命令的桌面应用。
...
上传到 Cloudflare R2/S3 并生成安全的预签名下载链接。非常适合远程 CrawClaw 实例。
</Card>

<Card title="通过 Telegram 构建移动端" icon="mobile" href="#">
  **@coard** • `ios` `xcode` `testflight`

构建了一个完整的带有地图和语音录制功能的移动客户端，完全通过 Telegram 聊天部署到 TestFlight。

  <img src="/assets/showcase/ios-testflight.jpg" alt="移动客户端 TestFlight 截图" />
</Card>
...
与智能体工作流（Claude Code、CrawClaw）集成的 Linear CLI。从终端管理问题、项目和工作流。首个外部 PR 已合并！
</Card>
...
Claude Code 发现并确认了净化器控制，然后 CrawClaw 接管来管理房间空气质量。
...
<Card title="xuezh 中文学习" icon="language" href="https://github.com/joshp123/xuezh">
  **@joshp123** • `learning` `voice` `skill`
  ...
  通过 CrawClaw 实现带有发音反馈和学习流程的中文学习引擎。
...
<Card title="Clawdia 电话桥接" icon="phone" href="https://github.com/alejandroOPI/clawdia-bridge">
  **@alejandroOPI** • `voice` `vapi` `bridge`
  ...
  Vapi 语音助手 ↔ CrawClaw HTTP 桥接。与你的智能体进行近实时电话通话。
</Card>
...
<Card title="Home Assistant 插件" icon="home" href="https://github.com/ngutman/crawclaw-ha-addon">
  **@ngutman** • `homeassistant` `docker` `raspberry-pi`
  ...
  在 Home Assistant OS 上运行的 CrawClaw Gateway 网关，支持 SSH 隧道和持久状态。
</Card>
...
<Card title="Nix 打包" icon="snowflake" href="https://github.com/crawclaw/nix-crawclaw">
  **@crawclaw** • `nix` `packaging` `deployment`
  ...
  开箱即用的 nixified CrawClaw 配置，用于可复现的部署。
</Card>
