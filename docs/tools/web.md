...
read_when:
  ...
  - You want to understand the bundled SearXNG path
---

# Web Search

The `web_search` tool searches the web through the bundled SearXNG provider
and returns structured results. Results are cached by query for 15 minutes by
default.
...
<Steps>
  <Step title="Enable web_search">
    `searxng` is the only bundled managed `web_search` provider. In most
...
`web_search` routes through CrawClaw-managed SearXNG:

- no API key is required for `web_search`
- CrawClaw can auto-start and reuse the local loopback sidecar
- the model-visible schema supports `query`, `count`, `engines`,
...
SearXNG-specific config lives under
`plugins.entries.searxng.config.webSearch.*`. `baseUrl` is an advanced
override for an explicitly managed SearXNG endpoint; plain `http://` endpoints
must be loopback, while remote endpoints must use `https://`.
...
| `web_search` | `engines`        | Optional SearXNG engine ids         |
| `web_search` | `categories`     | Optional SearXNG categories         |
| `web_search` | `language`       | Optional result language            |
| `web_search` | `safeSearch`     | `off`, `moderate`, or `strict`      |
| `web_search` | `timeRange`      | `day`, `week`, `month`, or `year`   |
| `web_search` | `timeoutSeconds` | Request timeout                     |
