...
describe("qqbot local media path remapping", () => {
  ...
  it("remaps missing workspace media paths to the real media directory", () => {
    ...
    const crawclawDir = path.join(actualHome, ".crawclaw");
    fs.mkdirSync(crawclawDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(crawclawDir, "qqbot-platform-test-"));
