---
summary: "CLI reference for `crawclaw daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `crawclaw daemon ...` in scripts
...
`crawclaw daemon ...` maps to the same service control surface as `crawclaw gateway ...` service commands.

## Usage

```bash
crawclaw daemon status
crawclaw daemon install
crawclaw daemon start
crawclaw daemon stop
crawclaw daemon restart
crawclaw daemon uninstall
```
...
Use [`crawclaw gateway`](/cli/gateway) for current docs and examples.
