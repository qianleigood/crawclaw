...
export class GatewayClient {
  ...
  start() {
    ...
    const allowPrivateWs = process.env.CRAWCLAW_ALLOW_INSECURE_PRIVATE_WS === "1";
    ...
    if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
      ...
      const error = new Error(
        `SECURITY ERROR: Cannot connect to "${displayHost}" over plaintext ws://. ` +
          ...
          (allowPrivateWs
            ? ""
            : "Break-glass (trusted private networks only): set CRAWCLAW_ALLOW_INSECURE_PRIVATE_WS=1. ") +
          "Run `crawclaw doctor --fix` for guidance.",
      );
  ...
  private sendConnect() {
    ...
    const { authToken, authPassword } = this.selectConnectAuth();
    const auth =
      authToken || authPassword ? { token: authToken, password: authPassword } : undefined;
  ...
  private selectConnectAuth(): SelectedConnectAuth {
