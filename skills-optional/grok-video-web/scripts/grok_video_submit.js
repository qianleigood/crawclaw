...
function textTokens(values) {
  return Array.from(
    new Set((values || []).map((value) => normText(value).toLowerCase()).filter(Boolean)),
  );
}
...
async function markVisiblePromptTarget(page) {
  return page
    .evaluate(() => {
      ...
      document
        .querySelectorAll("[data-crawclaw-prompt-target]")
        .forEach((node) => node.removeAttribute("data-crawclaw-prompt-target"));
...
async function clickVisibleByTexts(page, texts, options = {}) {
  ...
  if (!desired.length) {
    return { ok: false, selector: "", label: "", score: 0 };
  }
  ...
  const roles = Array.isArray(options.roles)
    ? options.roles.map((item) => String(item || "").toLowerCase())
    : [];

  const candidate = await page
    .evaluate(
      ({ desired, attr, exactOnly, roles }) => {
        const visible = (node) => {
          const style = window.getComputedStyle(node);
          const rect = node.getBoundingClientRect();
          return (
        ...
        const nodes = Array.from(
          document.querySelectorAll(
            'button, a, [role="button"], [role="tab"], [role="radio"], [role="option"], input[type="button"], input[type="submit"]',
          ),
        );
        const scored = [];
        for (const node of nodes) {
          ...
          for (const token of desired) {
            ...
            } else if (!exactOnly && text.includes(token)) {
          ...
          if (/(primary|accent|selected|active|checked)/i.test(String(node.className || ""))) {
            score += 3;
          }
          if (
  ...
  const probe = await page
    .evaluate(() => {
      const texts = Array.from(
        document.querySelectorAll('button, [role="button"], [role="tab"], [role="radio"]'),
      ).map((node) => ({
      ...
      const sideSignals = /480p|720p|6s|10s|宽高比|aspect ratio/i.test(
        document.body?.innerText || "",
      );
      return {
        found: Boolean(hit),
        selected:
          Boolean(hit && /true|selected|active|checked|on/.test(hit.selected)) || sideSignals,
        text: hit ? hit.text : "",
      };
    })
  ...
  try {
    ...
    const writeResult = await page.evaluate(
      ({ selector, value }) => {
        ...
        if (!node) {
          return { ok: false, probe: "", note: "node_missing" };
        }
        node.focus?.();
        if ("value" in node) {
          node.value = value;
...
async function uploadViaPuppeteerInput(page, filePath, inputSelector) {
  ...
  if (!input) {
    throw new Error(`file_input_not_found:${inputSelector}`);
  }
    ...
    .evaluate(() => {
      ...
      const norm = (value) =>
        String(value || "")
          ...
          .trim();
      document
        .querySelectorAll("[data-crawclaw-grok-submit]")
        .forEach((node) => node.removeAttribute("data-crawclaw-grok-submit"));
      const nodes = Array.from(
        document.querySelectorAll(
      ...
      for (const node of nodes) {
        ...
        if (String(node.className || "").match(/primary|accent|submit|confirm|generate|create/i)) {
          score += 3;
        }
        const disabled =
  ...
  return page
    .evaluate(() => {
      ...
      const hrefs = Array.from(document.querySelectorAll("a[href], [data-href], [data-url]"))
        .map(
          (node) =>
            node.href || node.getAttribute("data-href") || node.getAttribute("data-url") || "",
        )
        .filter(Boolean)
        .join("\n");
      const combined = `${text}\n${html}\n${hrefs}`;
...
async function watchManualSubmitHandoff({
  page,
  job,
  profile,
  checkedAt,
...
function recordBlockedSubmit(job, checkedAt, payload) {
  ...
  if (payload.handoff) {
    writeJson(
      job.files.submitHandoffPath || path.join(job.stateDir, "submit-handoff.json"),
      payload.handoff,
    );
  }
  updateWorkflowStatus(job, {
  ...
  try {
    ...
    if (!loginGate.ok) {
      const blocker = {
        ...
        matchedSignals:
          loginGate.state === "not_logged_in"
            ? loginGate.signals.loggedOut
            : loginGate.signals.cloudflare,
        note:
          loginGate.state === "not_logged_in"
