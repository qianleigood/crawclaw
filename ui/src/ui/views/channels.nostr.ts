...
export function renderNostrCard(params: {
    ...
    return html`
      <div class="account-card">
        ...
        <div class="status-list account-card-status">
          ...
          <div>
            <span class="label">${uiLiteral("Public Key")}</span>
  ...
  const renderProfileSection = () => {
    ...
    if (showingForm && profileFormCallbacks) {
      return renderNostrProfileForm({
        ...
        accountId: nostrAccounts[0]?.accountId ?? uiLiteral("default"),
      });
    }
    ...
    return html`
      ...
      >
        ...
        >
          <div style="font-weight: 500;">${uiLiteral("Profile")}</div>
          ${summaryConfigured
            ? html`
                <button
                  class="btn btn--sm"
        ...
        ${hasAnyProfileData
          ? html`
              <div class="status-list">
                ${picture
                  ...
                  : nothing}
                ${name
                  ? html`<div><span class="label">${uiLiteral("Name")}</span><span>${name}</span></div>`
                  : nothing}
                ${displayName
                  ? html`<div>
                      <span class="label">${uiLiteral("Display Name")}</span><span>${displayName}</span>
                    </div>`
                  : nothing}
                ${about
                  ? html`<div>
                      <span class="label">${uiLiteral("About")}</span
          ...
          : html`
              <div style="color: var(--text-muted); font-size: 13px">
                ${uiLiteral('No profile set. Click "Edit Profile" to add your name, bio, and avatar.')}
              </div>
            `}
      </div>
    `;
  ...
  return html`
    <div class="card">
      <div class="card-title">${uiLiteral("Nostr")}</div>
      <div class="card-sub">${uiLiteral("Decentralized DMs via Nostr relays (NIP-04).")}</div>
      ${accountCountLabel}
      ${hasMultipleAccounts
        ? html`
        ...
        : html`
            <div class="status-list" style="margin-top: 16px;">
              <div>
                <span class="label">${uiLiteral("Configured")}</span>
                <span>${summaryConfigured ? uiLiteral("Yes") : uiLiteral("No")}</span>
              </div>
              <div>
                <span class="label">${uiLiteral("Running")}</span>
                <span>${summaryRunning ? uiLiteral("Yes") : uiLiteral("No")}</span>
              </div>
              <div>
                <span class="label">${uiLiteral("Public Key")}</span>
              ...
              <div>
                <span class="label">${uiLiteral("Last start")}</span>
                <span
                  >${summaryLastStartAt ? formatRelativeTimestamp(summaryLastStartAt) : uiLiteral("n/a")}</span
                >
              </div>
            </div>
          `}
      ...
      <div class="row" style="margin-top: 12px;">
        <button class="btn" @click=${() => props.onRefresh(false)}>${uiLiteral("Refresh")}</button>
      </div>
    </div>
  `;
}
