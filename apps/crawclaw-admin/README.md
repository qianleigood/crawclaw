...
CrawClaw Desktop 是该控制台的本机桌面产品形态，复用这里的 Vue 前端和本地 Node 后端，并通过 Electron host 在 macOS、Windows 和 Linux 上打包发布。桌面版内置 CrawClaw runtime，首次启动会初始化 `~/.crawclaw`、安装并启动本机 Gateway 服务；远程 Gateway、VPS 和 headless 部署继续由 CLI/docs 支持。更多信息见 [Desktop install guide](https://docs.crawclaw.ai/install/desktop)。

### 版本兼容性

| CrawClaw Admin | CrawClaw Gateway | Hermes Agent | 状态         |
| -------------- | ---------------- | ------------ | ------------ |
| 1.0.2          | 2026.5.3         | 随部署而定   | 当前仓库基线 |

Hermes Agent 是外部集成，兼容性需要按实际部署版本验证；不要把旧的固定版本号当成当前仓库基线。
...
- 必须部署在网关所在的节点上，.env填写地址时仅支持127.0.0.1或localhost
...
HERMES_API_KEY=1212                     # Hermes API 密钥（可选）
