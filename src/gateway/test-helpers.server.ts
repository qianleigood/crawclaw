...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "CRAWCLAW_SKIP_PROVIDERS",
  "CRAWCLAW_SKIP_CRON",
  "CRAWCLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.CRAWCLAW_CONFIG_PATH) {
    configPaths.add(process.env.CRAWCLAW_CONFIG_PATH);
  }
  if (process.env.CRAWCLAW_STATE_DIR) {
    configPaths.add(path.join(process.env.CRAWCLAW_STATE_DIR, "crawclaw.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.CRAWCLAW_SKIP_PROVIDERS = "1";
  process.env.CRAWCLAW_SKIP_CRON = "1";
  process.env.CRAWCLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.CRAWCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "crawclaw-test-no-bundled-extensions")
    : "crawclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".crawclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".crawclaw-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".crawclaw-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>crawclaw-test-control-ui</title>\n",
    "utf-8",
  );
...
const CONNECT_CHALLENGE_NONCE_KEY = "__crawclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__crawclawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["CRAWCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.CRAWCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.CRAWCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.CRAWCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
