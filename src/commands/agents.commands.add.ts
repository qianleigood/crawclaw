...
export async function agentsAddCommand(
  ...
  const nonInteractive = opts.nonInteractive || hasFlags;
  ...
  try {
    await prompter.intro("Add CrawClaw agent");
    const name =
      nameInput ??
      (await prompter.text({
        message: "Agent name",
    ...
    const workspaceDir = resolveUserPath((workspaceInput ?? "").trim() || workspaceDefault);
      ...
      if (wantsBindings) {
        ...
        await prompter.note(
          [
            ...
            "Docs: https://docs.crawclaw.ai/concepts/multi-agent",
          ].join("\n"),
          "Routing",
        );
      }
