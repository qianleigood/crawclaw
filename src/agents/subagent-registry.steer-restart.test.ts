...
const noop = () => {};
const { emitAgentEventMock, getAgentRunContextMock } = vi.hoisted(() => ({
  emitAgentEventMock: vi.fn((_event?: unknown) => undefined),
  getAgentRunContextMock: vi.fn(
...
describe("subagent registry steer restarts", () => {
  ...
  it("completes persistent session-mode runs without deleting the run", async () => {
