...
import type { CrawClawConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: CrawClawConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: CrawClawConfig): string[] {
  ...
  const defaultRequestedSecuritySource = "CrawClaw default (full)";
  const defaultRequestedAskSource = "CrawClaw default (off)";
...
function collectDurableExecApprovalWarnings(cfg: CrawClawConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: CrawClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("crawclaw security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.crawclaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("crawclaw approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("crawclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("crawclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("crawclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("crawclaw config set gateway.bind loopback")}`,
