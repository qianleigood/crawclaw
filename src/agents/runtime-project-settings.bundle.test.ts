...
const { loadEnabledBundleRuntimeSettingsSnapshot } = await import("./runtime-project-settings.js");
...
async function createWorkspaceBundle(params: {
  ...
  const pluginRoot = path.join(params.workspaceDir, ".crawclaw", "extensions", pluginId);
...
describe("loadEnabledBundleRuntimeSettingsSnapshot", () => {
  it("loads sanitized settings from enabled bundle plugins", async () => {
    const workspaceDir = await tempDirs.make("crawclaw-workspace-");
    ...
    const snapshot = loadEnabledBundleRuntimeSettingsSnapshot({
      cwd: workspaceDir,
      cfg: {
        plugins: {
          entries: {
  ...
  it("loads enabled bundle MCP servers into the runtime settings snapshot", async () => {
    const workspaceDir = await tempDirs.make("crawclaw-workspace-");
    const pluginRoot = await createWorkspaceBundle({ workspaceDir });
