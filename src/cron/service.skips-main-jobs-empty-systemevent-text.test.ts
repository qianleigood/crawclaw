...
describe("CronService", () => {
  ...
  it("skips main jobs with empty systemEvent text", async () => {
    await withCronService(true, async ({ cron, enqueueSystemEvent, requestMainSessionWake }) => {
  ...
  it("does not schedule timers when cron is disabled", async () => {
    await withCronService(false, async ({ cron, enqueueSystemEvent, requestMainSessionWake }) => {
      const atMs = Date.parse("2025-12-13T00:00:01.000Z");
      await cron.add({
