...
async function checkFeedPageAccessible(page) {
  ...
  if (typeof issue === "string" && issue.trim()) {
    throw new XhsCliError(`Feed page is not accessible: ${issue.trim()}`, {
      code: "FEED_INACCESSIBLE",
    });
  }
}
...
async function focusCommentInput(page, interactionMode = "normal") {
  ...
  for (const selector of selectors) {
    ...
    const visible = await handle
      .evaluate((el) => el instanceof HTMLElement && el.offsetParent !== null)
      .catch(() => false);
    if (!visible) {
      continue;
    }
...
async function fillCommentContent(page, content, interactionMode = "normal") {
  ...
  throw new XhsCliError("Failed to fill comment content: comment_input_not_found", {
...
async function clickCommentSubmitButton(page, interactionMode = "normal") {
  ...
  if (!element) {
    throw new XhsCliError("Could not find visible comment submit button.", {
...
async function postCommentToFeed(page, { feedId, xsecToken, content, interactionMode = "normal" }) {
  ...
  if (!normalizedContent) {
    throw new XhsCliError("content cannot be empty.", { code: "EMPTY_CONTENT" });
  }

  await gotoAndSettled(page, makeFeedDetailUrl(normalizedFeedId, normalizedToken), {
    settleMs: 1800,
    timeout: 30000,
  });
...
async function replyToCommentViaFeedDetail(
  page,
  {
    feedId,
    xsecToken,
  ...
  const normalizedToken = String(xsecToken || "").trim();
  if (!normalizedFeedId) {
    throw new XhsCliError("feed_id cannot be empty for detail-page fallback.", {
      code: "EMPTY_FEED_ID",
    });
  }
  if (!normalizedToken) {
    throw new XhsCliError("xsec_token cannot be empty for detail-page fallback.", {
      code: "EMPTY_XSEC_TOKEN",
    });
  }
  ...
  const matchResult = await page.evaluate(
    ({ author, text }) => {
      ...
      const buttons = Array.from(document.querySelectorAll("button, span, div, a")).filter((node) =>
        isVisible(node),
      );
      for (const node of buttons) {
        const label = normalize(node.innerText || node.textContent || "");
        if (!replyTexts.has(label)) {
          continue;
        }
        ...
        while (container && depth < 8) {
          ...
          if (authorOk && textOk) {
            ...
            if (rect.width >= 4 && rect.height >= 4) {
              return {
                ok: true,
                containerText: containerText.slice(0, 400),
                x: rect.x,
                y: rect.y,
                width: rect.width,
                height: rect.height,
  ...
  if (!matchResult || !matchResult.ok) {
    throw new XhsCliError(
      `Failed to locate target comment reply button: ${matchResult && matchResult.reason ? matchResult.reason : "unknown"}`,
      {
  ...
  await page.mouse.move(
    Number(matchResult.x) + Number(matchResult.width) / 2,
    Number(matchResult.y) + Number(matchResult.height) / 2,
  );
...
async function replyToCommentInFeed(
  page,
  {
    feedId = "",
    xsecToken = "",
  ...
  if (!normalizedAuthor && !normalizedTargetText) {
    throw new XhsCliError("At least one of target_author or target_text must be provided.", {
      code: "EMPTY_REPLY_TARGET",
    });
  }

  await gotoAndSettled(page, "https://www.xiaohongshu.com/notification", {
    settleMs: 2000,
    timeout: 30000,
  });
  ...
  const matchResult = await page.evaluate(
    ({ author, text }) => {
      ...
      const isVisible = (node) => node instanceof HTMLElement && node.offsetParent !== null;
      const replyTexts = new Set(["回复"]);
      const candidates = Array.from(document.querySelectorAll("button, span, div, a")).filter(
        (node) => isVisible(node),
      );
      const matches = [];
      ...
      if (!matches.length) {
        return { ok: false, reason: "notification_reply_button_not_found" };
      }
  ...
  throw new XhsCliError(
    `Failed to locate reply target on notification page: ${matchResult && matchResult.reason ? matchResult.reason : "unknown"}`,
    {
      code: "REPLY_TARGET_NOT_FOUND",
    },
  );
