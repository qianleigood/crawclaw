import { afterAll, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";

const chatHistoryMock = vi.fn<(sessionKey: string) => Promise<{ messages?: Array<unknown> }>>(
  async (_sessionKey: string) => ({ messages: [] }),
);
const loadConfigMock = vi.fn(() => ({}));
const loadSessionStoreMock = vi.fn<(storePath: string) => Record<string, unknown>>(
  (_storePath: string) => ({}),
);
const resolveStorePathMock = vi.fn(
  (_store: unknown, opts?: { agentId?: string }) => `/tmp/${opts?.agentId ?? "main"}-sessions.json`,
);

vi.mock("../gateway/call.js", () => ({
  callGateway: vi.fn(async (request: unknown) => {
    const typed = request as { method?: string; params?: { sessionKey?: string } };
    if (typed.method === "chat.history") {
      return await chatHistoryMock(typed.params?.sessionKey ?? "");
    }
    return {};
  }),
}));
vi.mock("../config/config.js", () => ({
  loadConfig: () => loadConfigMock(),
}));
vi.mock("../config/sessions.js", async () => {
  const actual =
    await vi.importActual<typeof import("../config/sessions.js")>("../config/sessions.js");
  return {
    ...actual,
    loadSessionStore: (storePath: string) => loadSessionStoreMock(storePath),
    resolveStorePath: (store: unknown, opts?: { agentId?: string }) =>
      resolveStorePathMock(store, opts),
  };
});

describe("captureSubagentCompletionReply", () => {
  let previousFastTestEnv: string | undefined;
  let captureSubagentCompletionReply: (typeof import("./subagent-announce.js"))["captureSubagentCompletionReply"];

  async function loadFreshSubagentAnnounceModuleForTest() {
    vi.resetModules();
    ({ captureSubagentCompletionReply } = await import("./subagent-announce.js"));
  }

  beforeAll(async () => {
    previousFastTestEnv = process.env.CRAWCLAW_TEST_FAST;
    process.env.CRAWCLAW_TEST_FAST = "1";
  });

  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.CRAWCLAW_TEST_FAST;
      return;
    }
    process.env.CRAWCLAW_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(async () => {
    await loadFreshSubagentAnnounceModuleForTest();
    chatHistoryMock.mockReset().mockResolvedValue({ messages: [] });
    loadConfigMock.mockReset().mockReturnValue({});
    loadSessionStoreMock.mockReset().mockReturnValue({});
    resolveStorePathMock.mockClear();
  });

  it("returns immediate assistant output from history without polling", async () => {
    chatHistoryMock.mockResolvedValueOnce({
      messages: [
        {
          role: "assistant",
          content: [{ type: "text", text: "Immediate assistant completion" }],
        },
      ],
    });

    const result = await captureSubagentCompletionReply("agent:main:subagent:child");

    expect(result).toBe("Immediate assistant completion");
    expect(chatHistoryMock).toHaveBeenCalledTimes(1);
  });

  it("polls briefly and returns late tool output once available", async () => {
    vi.useFakeTimers();
    chatHistoryMock
      .mockResolvedValueOnce({ messages: [] })
      .mockResolvedValueOnce({ messages: [] })
      .mockResolvedValueOnce({
        messages: [
          {
            role: "toolResult",
            content: [
              {
                type: "text",
                text: "Late tool result completion",
              },
            ],
          },
        ],
      });

    const pending = captureSubagentCompletionReply("agent:main:subagent:child");
    await vi.runAllTimersAsync();
    const result = await pending;

    expect(result).toBe("Late tool result completion");
    expect(chatHistoryMock).toHaveBeenCalledTimes(3);
    vi.useRealTimers();
  });

  it("returns undefined when no completion output arrives before retry window closes", async () => {
    vi.useFakeTimers();
    chatHistoryMock.mockResolvedValue({ messages: [] });

    const pending = captureSubagentCompletionReply("agent:main:subagent:child");
    await vi.runAllTimersAsync();
    const result = await pending;

    expect(result).toBeUndefined();
    expect(chatHistoryMock).toHaveBeenCalled();
    vi.useRealTimers();
  });

  it("returns partial assistant progress when the latest assistant turn is tool-only", async () => {
    chatHistoryMock.mockResolvedValueOnce({
      messages: [
        {
          role: "assistant",
          content: [
            { type: "text", text: "Mapped the modules." },
            { type: "toolCall", id: "call-1", name: "read", arguments: {} },
          ],
        },
        {
          role: "assistant",
          content: [{ type: "toolCall", id: "call-2", name: "exec", arguments: {} }],
        },
      ],
    });

    const result = await captureSubagentCompletionReply("agent:main:subagent:child");

    expect(result).toBe("Mapped the modules.");
  });

  it("falls back to the ACP backend session when the wrapper session has no transcript output", async () => {
    loadSessionStoreMock.mockReturnValue({
      "agent:crawclaw:acp:child": {
        sessionId: "wrapper-session-id",
        updatedAt: Date.now(),
        spawnedBy: "agent:main:main",
        acp: {
          backend: "acpx",
          agent: "crawclaw",
          runtimeSessionName: "runtime-crawclaw",
          mode: "oneshot",
          state: "idle",
          lastActivityAt: Date.now(),
          identity: {
            state: "resolved",
            acpxSessionId: "backend-session-id",
            source: "status",
            lastUpdatedAt: Date.now(),
          },
        },
      },
    });
    chatHistoryMock.mockImplementation(async (sessionKey: string) => {
      if (sessionKey === "agent:main:backend-session-id") {
        return {
          messages: [
            {
              role: "assistant",
              content: [{ type: "text", text: "ACP_LIVE_OK" }],
            },
          ],
        };
      }
      return { messages: [] };
    });

    const result = await captureSubagentCompletionReply("agent:crawclaw:acp:child");

    expect(result).toBe("ACP_LIVE_OK");
    expect(resolveStorePathMock).toHaveBeenCalledWith(undefined, { agentId: "crawclaw" });
    expect(chatHistoryMock).toHaveBeenCalledWith("agent:crawclaw:acp:child");
    expect(chatHistoryMock).toHaveBeenCalledWith("agent:main:backend-session-id");
  });
});
