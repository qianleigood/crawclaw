...
describe("fetchBrowserJson loopback auth", () => {
  ...
  beforeEach(() => {
    vi.restoreAllMocks();
    vi.stubEnv("CRAWCLAW_GATEWAY_TOKEN", "loopback-token");
  ...
  it("preserves dispatcher error context while keeping no-retry hint", async () => {
    await expectThrownBrowserFetchError(() => fetchBrowserJson<{ ok: boolean }>("/tabs"), {
      contains: [
        "Relative browser dispatcher paths are no longer supported",
        "Do NOT retry the browser tool",
      ],
      omits: ["Can't reach the CrawClaw browser control service"],
    });
  });
  ...
  it("treats legacy dispatcher-relative URLs as unsupported", async () => {
    await expectThrownBrowserFetchError(() => fetchBrowserJson<{ ok: boolean }>("/tabs"), {
      contains: [
        "Relative browser dispatcher paths are no longer supported",
        "Do NOT retry the browser tool",
      ],
      omits: ["Browser service rate limit reached"],
    });
  });
  ...
  it("keeps absolute URL failures wrapped as reachability errors", async () => {
    ...
    await expectThrownBrowserFetchError(
      ...
      {
        contains: [
          "Can't reach the CrawClaw browser control service",
          "Do NOT retry the browser tool",
        ],
      },
    );
