...
export function resolveExecApprovalGuardUnavailable(params: {
  runId?: string;
  agentId?: string;
  sessionKey?: string;
  sessionId?: string;
  ...
  if (guard.interactiveApprovalAvailable || !guard.interactiveApprovalBlocker) {
    return null;
  }
  return {
    warningText: formatAgentInteractiveApprovalUnavailableReason({
      blocker: guard.interactiveApprovalBlocker,
      subject: "exec",
    }),
    unavailableReason: guard.interactiveApprovalBlocker,
  };
}

export function mergeExecApprovalWarningText(
...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    "Stricter values from tools.exec and ~/.crawclaw/exec-approvals.json both apply.",
    "Fix one of these:",
