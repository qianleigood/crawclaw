...
CrawClaw supports Grok as a `web_search` provider, using xAI web-grounded
...
`plugins.entries.xai.config.webSearch.apiKey`, CrawClaw now reuses it as a
...
CrawClaw can show a separate follow-up step to enable `x_search` with the same
...
For a gateway install, put it in `~/.crawclaw/.env`.
