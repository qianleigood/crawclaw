...
function createCrawClawRoot(params: {
  prefix: string;
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "crawclaw" }, null, 2)}\n`,
    "utf8",
  );
  return repoRoot;
}
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.CRAWCLAW_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    ...
    const repoRoot = createCrawClawRoot(layout);
    expectResolvedBundledDirFromRoot({
      repoRoot,
  ...
  it.each([
    {
      ...
      createScenario: () => {
        const installedRoot = createCrawClawRoot({
          prefix: "crawclaw-bundled-dir-installed-",
          hasDistExtensions: true,
        });
        const cwdRepoRoot = createCrawClawRoot({
          prefix: "crawclaw-bundled-dir-cwd-",
          hasExtensions: true,
          hasSrc: true,
          hasGitCheckout: true,
        });
    ...
    },
    {
      ...
      createScenario: () => {
        const installedRoot = createCrawClawRoot({
          prefix: "crawclaw-bundled-dir-override-",
          hasDistExtensions: true,
        });
        return {
          installedRoot,
