...
export function resolveGitInstallDir(): string {
  const override = process.env.CRAWCLAW_GIT_DIR?.trim();
  if (override) {
...
function resolveDefaultGitDir(): string {
  ...
  if (home.startsWith("/")) {
    return path.posix.join(home, "crawclaw");
  }
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveCrawClawPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", CRAWCLAW_REPO_URL, params.dir],
      env: gitEnv,
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `CRAWCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set CRAWCLAW_GIT_DIR to an empty folder or a crawclaw checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`CRAWCLAW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "crawclaw.mjs");
