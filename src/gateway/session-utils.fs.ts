...
export function attachCrawClawTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__crawclaw && typeof record.__crawclaw === "object" && !Array.isArray(record.__crawclaw)
      ? (record.__crawclaw as Record<string, unknown>)
      : {};
  return {
    ...record,
    __crawclaw: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachCrawClawTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "crawclaw" && model === "delivery-mirror";
