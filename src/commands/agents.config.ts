...
function resolveAgentName(cfg: CrawClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: CrawClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: CrawClawConfig): AgentSummary[] {
