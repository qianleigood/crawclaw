...
async function maybePersistAutoGeneratedGatewayInstallToken(params: {
  ...
  try {
    ...
    const baseConfig = snapshot.exists ? (snapshot.sourceConfig ?? snapshot.runtimeConfig) : {};
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("crawclaw config set gateway.auth.mode token")} or ${formatCliCommand("crawclaw config set gateway.auth.mode password")}.`,
  ].join(" ");
}
