...
export async function dispatchReplyFromConfig(params: {
  ...
  try {
    ...
    // Feishu groups also support incremental card/thread updates well enough to
    // surface tool/skill progress inline, so keep summaries enabled there too.
    const shouldSendToolSummaries =
      (ctx.ChatType !== "group" ||
        ctx.IsForum === true ||
        normalizeMessageChannel(ctx.Surface ?? ctx.Provider) === "feishu") &&
