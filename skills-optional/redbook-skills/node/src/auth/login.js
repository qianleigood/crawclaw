...
async function creatorLoggedInUiPresent(page) {
  return Boolean(
    await page.evaluate(() => {
      ...
      if (!href.includes("creator.xiaohongshu.com")) {
        return false;
      }
      ...
      const visibleNodes = Array.from(
        document.querySelectorAll("a, button, div, span, li, h1, h2, h3"),
      ).filter((node) => {
        if (!(node instanceof HTMLElement)) {
          return false;
        }
      ...
      const hasCreatorKeyword = visibleText.some((text) =>
        creatorKeywords.some((item) => text.includes(item)),
      );
...
async function homeLoginPromptVisible(page, keyword = XHS_HOME_LOGIN_MODAL_KEYWORD) {
  return Boolean(
    await page.evaluate((inputKeyword) => {
      const normalize = (text) => (text || "").replace(/\s+/g, " ").trim();
...
async function homeLoggedInUiPresent(page) {
  return Boolean(
    await page.evaluate(() => {
      ...
      const hasVisibleLoginAction = Array.from(
      ...
      for (const selector of profileLinkSelectors) {
        for (const node of document.querySelectorAll(selector)) {
          ...
          if (!href || href === "/" || href.startsWith("/explore")) {
            continue;
          }
          const rect = node.getBoundingClientRect();
      ...
      const meCandidates = Array.from(document.querySelectorAll("a, button, div, span")).filter(
        (node) => {
          if (!(node instanceof HTMLElement)) {
            return false;
          }
...
async function checkCreatorLogin(context) {
  ...
  const loggedIn =
    (await creatorLoggedInUiPresent(page)) && !(await creatorLoginPromptVisible(page));
  if (loggedIn) {
  ...
  while (Date.now() < deadline) {
    ...
    if ((payload && payload.found && href) || strategy === "exact-me-tab") {
      ...
      return {
        loggedIn: true,
        scope,
        source: strategy || "profile-discovery",
        currentUrl: page.url(),
        profile: payload,
      };
    }
...
async function expandLoginQrPanel(page) {
  const result = await page.evaluate(() => {
    ...
    const candidates = Array.from(
      document.querySelectorAll('img, button, [role="button"], div, span'),
    );
    for (const el of candidates) {
      if (!isVisible(el)) {
        continue;
      ...
      const hit = keywords.some(
        (k) => text.includes(k) || cls.includes(k.toLowerCase()) || src.includes(k.toLowerCase()),
      );
      if (hit) {
...
function sendLoginQrToFeishu(outputPath, receiveId, receiveIdType) {
  if (!fs.existsSync(FEISHU_FILE_SENDER_SCRIPT)) {
    throw new XhsCliError(`Feishu helper script not found: ${FEISHU_FILE_SENDER_SCRIPT}`, {
      code: "FEISHU_HELPER_NOT_FOUND",
    });
  }
  const cmd = [
    "python3",
...
async function waitForLoginSuccess(
  context,
  { timeoutSeconds = 180, pollSeconds = 2, scope = "creator" } = {},
) {
  const { page, host, port, accountName } = context;
