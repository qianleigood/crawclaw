...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers CRAWCLAW_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for CRAWCLAW_HOME",
      env: { CRAWCLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when CRAWCLAW_HOME is tilde-only and no fallback home exists",
      env: { CRAWCLAW_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores CRAWCLAW_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          CRAWCLAW_HOME: "/srv/crawclaw-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { CRAWCLAW_HOME: "/srv/crawclaw-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/crawclaw-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/crawclaw-home " },
      expected: "/srv/crawclaw-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/crawclaw-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of CRAWCLAW_HOME", () => {
    expect(
