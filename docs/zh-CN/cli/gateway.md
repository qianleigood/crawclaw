...
summary: CrawClaw Gateway 网关 CLI（`crawclaw gateway`）— 运行、查询和发现 Gateway 网关
title: gateway
x-i18n:
...
Gateway 网关是 CrawClaw 的 WebSocket 服务器（渠道、节点、会话、hooks）。

本页中的子命令位于 `crawclaw gateway …` 下。
...
- 默认情况下，除非在 `~/.crawclaw/crawclaw.json` 中设置了 `gateway.mode=local`，否则 Gateway 网关将拒绝启动。使用 `--allow-unconfigured` 进行临时/开发运行。
- 在没有认证的情况下绑定到 loopback 之外的地址会被阻止（安全护栏）。
- `SIGUSR1` 在授权时触发进程内重启（`commands.restart` 默认启用；设置 `commands.restart: false` 可禁用手动重启，但 gateway 工具/config apply/update 仍然可用）。
...
- `--token <token>`：令牌覆盖（同时为进程设置 `CRAWCLAW_GATEWAY_TOKEN`；迁移期间仍兼容 `CRAWCLAW_GATEWAY_TOKEN`）。
- `--password <password>`：密码覆盖（同时为进程设置 `CRAWCLAW_GATEWAY_PASSWORD`；迁移期间仍兼容旧前缀）。
...
节点主机的"通过 SSH 远程"模式使用本地端口转发，因此远程 Gateway 网关（可能仅绑定到 loopback）变得可以通过 `ws://127.0.0.1:<port>` 访问。
...
`gateway discover` 扫描 Gateway 网关信标（`_crawclaw-gw._tcp`）。

- 组播 DNS-SD：`local.`
- 单播 DNS-SD（广域 Bonjour）：选择一个域（示例：`crawclaw.internal.`）并设置分割 DNS + DNS 服务器；参见 [/gateway/bonjour](/gateway/bonjour)
