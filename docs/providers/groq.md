...
read_when:
  - You want to use Groq with CrawClaw
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. CrawClaw connects
...
available to that process (for example, in `~/.crawclaw/.env` or via
`env.shellEnv`).
