...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("crawclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("crawclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("crawclaw-sbx-");
