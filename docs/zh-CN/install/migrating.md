...
本指南将 CrawClaw Gateway 网关从一台机器迁移到另一台，**无需重新进行新手引导**。

迁移在概念上很简单：

- 复制**状态目录**（`$CRAWCLAW_STATE_DIR`，默认：`~/.crawclaw/`）— 这包括配置、认证、会话和渠道状态。
- 复制你的**工作区**（默认 `~/.crawclaw/workspace/`）— 这包括你的智能体文件（记忆、提示等）。
...
- `--profile <name>`（通常变成 `~/.crawclaw-<profile>/`）
- `CRAWCLAW_STATE_DIR=/some/path`
...
tar -czf crawclaw-workspace.tgz .crawclaw/workspace
```

如果你有多个配置文件/状态目录（例如 `~/.crawclaw-main`、`~/.crawclaw-work`），分别归档每个。
...
在这个阶段，如果新手引导创建了一个新的 `~/.crawclaw/` 也没关系 — 你将在下一步覆盖它。
...
如果你在旧 Gateway 网关上使用了配置文件（或 `CRAWCLAW_STATE_DIR`），而新 Gateway 网关使用了不同的配置，你会看到如下症状：
...
`crawclaw.json` 是不够的。许多提供商在以下位置存储状态：

- `$CRAWCLAW_STATE_DIR/credentials/`
- `$CRAWCLAW_STATE_DIR/agents/<agentId>/...`

始终迁移整个 `$CRAWCLAW_STATE_DIR` 文件夹。
...
- [CrawClaw 在哪里存储数据？](/help/faq#where-does-crawclaw-store-its-data)
