...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.CRAWCLAW_CONFIG_PATH = path.join(stateDir, "crawclaw.json");
    return stateDir;
  };
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("crawclaw-onboard-");
  ...
  it("writes gateway token auth into config", async () => {
    await withStateDir("state-noninteractive-", async (stateDir) => {
      ...
      const workspace = path.join(stateDir, "crawclaw");
  ...
  it("uses CRAWCLAW_GATEWAY_TOKEN when --gateway-token is omitted", async () => {
    await withStateDir("state-env-token-", async (stateDir) => {
      ...
      process.env.CRAWCLAW_GATEWAY_TOKEN = envToken;
        ...
        await runNonInteractiveSetup(
          {
            ...
            gatewayBind: "loopback",
            gatewayAuth: "token",
            gatewayTokenRefEnv: "CRAWCLAW_GATEWAY_TOKEN",
          },
          runtime,
        );
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `crawclaw gateway status --deep` for more detail.");
