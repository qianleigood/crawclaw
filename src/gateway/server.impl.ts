...
import { pinActivePluginHttpRouteRegistry, setActivePluginRegistry } from "../plugins/runtime.js";
...
const rustOnlyCoreGatewayHandlers: GatewayRequestHandlers = {};

const handleRustOnlyGatewayRequest: GatewayRequestDispatcher = async (opts) => {
  const handler = opts.extraHandlers?.[opts.req.method];
  if (!handler) {
    opts.respond(
      false,
      undefined,
      errorShape(ErrorCodes.INVALID_REQUEST, `unknown method: ${opts.req.method}`),
    );
    return;
  }
  const invokeHandler = () =>
...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("crawclaw doctor")}" to repair, then retry.`
    : "";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.CRAWCLAW_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.CRAWCLAW_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "CRAWCLAW_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "CRAWCLAW_RAW_STREAM_PATH",
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `crawclaw config set gateway.auth.mode token` and `crawclaw config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  if (!minimalTestGateway) {
    ...
    ({ pluginRegistry, gatewayMethods: baseGatewayMethods } = loadGatewayStartupPlugins({
      ...
      workspaceDir: defaultWorkspaceDir,
      log,
      coreGatewayHandlers: rustOnlyCoreGatewayHandlers,
      baseMethods,
  ...
  try {
    ...
    transcriptUnsub = minimalTestGateway
      ...
      : onSessionTranscriptUpdate((update) => {
          ...
          const sessionSnapshot = sessionRow
            ...
            : {};
          const message = attachCrawClawTranscriptMeta(update.message, {
    ...
    const gatewayRequestContext: import("./request-types.js").GatewayRequestContext = {
      ...
      disconnectClientsForDevice: (deviceId: string, opts?: { role?: string }) => {
        for (const gatewayClient of clients) {
          if (gatewayClient.connect.client.instanceId !== deviceId) {
            continue;
          }
    ...
    attachGatewayWsHandlers({
      ...
      handleGatewayRequest: handleRustOnlyGatewayRequest,
    });
    logGatewayStartup({
      cfg: cfgAtStart,
      bindHost,
