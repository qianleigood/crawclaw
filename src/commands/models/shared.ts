...
export async function loadValidConfigOrThrow(): Promise<CrawClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: CrawClawConfig) => CrawClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: CrawClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: CrawClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("crawclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
