...
describe("sanitizeUserFacingText", () => {
  ...
  it.each([
    "Context overflow: prompt too large for the model. Try /new to start a fresh session, or use a larger-context model.",
  ...
  it.each([
    "Changelog note: we fixed false positives for `Context overflow: prompt too large for the model. Try /new to start a fresh session, or use a larger-context model.` in 2026.2.9",
  ...
  it("does not strip ordinary text that merely mentions internal marker strings", () => {
    const input = [
      "The literal header `CrawClaw runtime context (internal):` appears in this note.",
  ...
  it("does not strip text that starts with the legacy header phrase but is not the canonical block", () => {
    const input =
      "CrawClaw runtime context (internal): is the label used by the old runtime block formatter.";
