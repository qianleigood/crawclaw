...
import {
  resetActionFeed as resetActionFeedInternal,
...
import {
  syncInspectPolling as syncInspectPollingInternal,
  syncWorkflowPolling as syncWorkflowPollingInternal,
} from "./app-polling.ts";
...
import {
  loadAgentInspection as loadAgentInspectionInternal,
  loadToolsEffective as loadToolsEffectiveInternal,
  refreshVisibleAgentInspectionForCurrentRun as refreshVisibleAgentInspectionForCurrentRunInternal,
  refreshVisibleToolsEffectiveForCurrentSession as refreshVisibleToolsEffectiveForCurrentSessionInternal,
  resetAgentInspectionState as resetAgentInspectionStateInternal,
...
function resolveStoredOnboardingMode(gatewayUrl: string): boolean {
  const progress = loadOnboardingProgress(gatewayUrl);
  return progress.mode === "guided";
}
...
export class CrawClawApp extends LitElement {
  ...
  private onboardingRequestedByUrl = resolveOnboardingMode();
  constructor() {
    super();
  ...
  @state() tab: Tab =
    this.onboardingRequestedByUrl || resolveStoredOnboardingMode(this.settings.gatewayUrl)
      ? "overview"
      : "chat";
  @state() onboarding =
    this.onboardingRequestedByUrl || resolveStoredOnboardingMode(this.settings.gatewayUrl);
  @state() onboardingProgress: OnboardingProgress = loadOnboardingProgress(
  ...
  private actionFeedSyncTimer: number | null = null;
  ...
  @state() chatActionFeed: ActionFeedEntry[] = [];
  ...
  actionFeedById = new Map<string, ActionFeedEntry>();
  ...
  @state() feishuCliStatus: FeishuCliStatusSnapshot | null = null;
  @state() feishuCliError: string | null = null;
  @state() feishuCliLastSuccess: number | null = null;
  @state() feishuCliSupported: boolean | null = null;
  ...
  @state() agentInspectionError: string | null = null;
  @state() agentInspectionSnapshot: import("./types.js").AgentInspectionSnapshot | null = null;
  @state() agentInspectionRunId: string | null = null;
  @state() agentInspectionTaskId: string | null = null;
  @state() agentInspectionTimelineFilter: "all" | "errors" | "decisions" = "all";
  ...
  @state() workflowsList: WorkflowListEntry[] = [];
  @state() workflowSelectedId: string | null = null;
  @state() workflowDetailLoading = false;
  @state() workflowDetailError: string | null = null;
  @state() workflowDetail: WorkflowDetailSnapshot | null = null;
  @state() workflowRunsLoading = false;
  @state() workflowRunsError: string | null = null;
  @state() workflowRuns: import("../../../src/workflows/types.js").WorkflowExecutionView[] = [];
  @state() workflowVersionsLoading = false;
  @state() workflowVersionsError: string | null = null;
  @state() workflowVersions: WorkflowVersionsSnapshot | null = null;
  @state() workflowDiffLoading = false;
  @state() workflowDiffError: string | null = null;
  @state() workflowDiff: WorkflowDiffSnapshot | null = null;
  @state() workflowEditorDraft: WorkflowEditorDraft | null = null;
  @state() workflowSelectedExecutionId: string | null = null;
  @state() workflowSelectedExecution:
    | import("../../../src/workflows/types.js").WorkflowExecutionView
    | null = null;
  @state() workflowStatusLoading = false;
  @state() workflowStatusError: string | null = null;
  @state() workflowActionBusyKey: string | null = null;
  @state() workflowFilterQuery = "";
  @state() workflowFilterState: "all" | "enabled" | "disabled" | "deployed" | "approval" = "all";
  ...
  private inspectPollInterval: number | null = null;
  private workflowPollInterval: number | null = null;
  ...
  connectedCallback() {
    ...
    this.onSlashAction = (action: string) => {
      switch (action) {
        ...
        case "refresh-agent-inspect": {
          void refreshVisibleAgentInspectionForCurrentRunInternal(this);
          break;
        }
      }
    };
  ...
  protected updated(changed: Map<PropertyKey, unknown>) {
    ...
    if (changed.has("settings")) {
      const loadedProgress = loadOnboardingProgress(this.settings.gatewayUrl);
      if (!onboardingProgressEquals(loadedProgress, this.onboardingProgress)) {
        this.onboardingProgress = loadedProgress;
      }
      const shouldOnboard = loadedProgress.mode === "guided";
      if (!this.onboardingRequestedByUrl && this.onboarding !== shouldOnboard) {
        this.onboarding = shouldOnboard;
      }
    }
    if (this.onboarding && this.onboardingProgress.mode !== "guided") {
      const nextProgress = setOnboardingMode(this.onboardingProgress, "guided");
      this.onboardingProgress = nextProgress;
      saveOnboardingProgress(this.settings.gatewayUrl, nextProgress);
    }
    if (
      changed.has("connected") ||
      changed.has("assistantName") ||
      changed.has("channelsSnapshot") ||
      changed.has("skillsReport") ||
      changed.has("sessionsResult") ||
      changed.has("settings")
    ) {
      const nextProgress = mergeOnboardingProgress(this.onboardingProgress, {
        connected: this.connected,
        assistantName: this.assistantName,
        channelsSnapshot: this.channelsSnapshot,
        skillsReport: this.skillsReport,
        sessionsCount: this.sessionsResult?.count ?? 0,
      });
      if (!onboardingProgressEquals(nextProgress, this.onboardingProgress)) {
        this.onboardingProgress = nextProgress;
        saveOnboardingProgress(this.settings.gatewayUrl, nextProgress);
      }
    }
    if (
      ...
      changed.has("agentsSelectedId") ||
      changed.has("sessionKey") ||
      changed.has("chatRunId") ||
      changed.has("agentInspectionLoading")
    ) {
      syncInspectPollingInternal(
        this as unknown as Parameters<typeof syncInspectPollingInternal>[0],
      );
    }
    if (
      changed.has("tab") ||
      changed.has("connected") ||
      changed.has("workflowSelectedExecutionId") ||
      changed.has("workflowSelectedExecution") ||
      changed.has("workflowStatusLoading")
    ) {
      syncWorkflowPollingInternal(
        this as unknown as Parameters<typeof syncWorkflowPollingInternal>[0],
      );
    }
    if (this.agentsPanel === "inspect" && (changed.has("sessionKey") || changed.has("chatRunId"))) {
      void refreshVisibleAgentInspectionForCurrentRunInternal(this);
    }
  ...
  async loadAgentInspection(params: { runId?: string | null; taskId?: string | null }) {
    await loadAgentInspectionInternal(this, params);
  }

  resetAgentInspectionState() {
    resetAgentInspectionStateInternal(this);
  }
  ...
  setOnboardingMode(
    mode: OnboardingMode,
    options?: { active?: boolean; tab?: Tab; reset?: boolean },
  ) {
    const base = options?.reset ? createEmptyOnboardingProgress() : this.onboardingProgress;
    const nextProgress = setOnboardingMode(base, mode);
    this.onboardingProgress = nextProgress;
    this.onboarding = options?.active ?? mode === "guided";
    if (options?.tab) {
      this.tab = options.tab;
    } else if (this.onboarding && this.tab !== "overview") {
      this.tab = "overview";
    }
    saveOnboardingProgress(this.settings.gatewayUrl, nextProgress);
  }
  ...
  pauseOnboarding() {
    this.setOnboardingMode("paused", { active: false });
  }

  resumeOnboarding() {
    this.setOnboardingMode("guided", { active: true, tab: "overview" });
  }

  completeOnboarding() {
    this.setOnboardingMode("completed", { active: false });
  }

  restartOnboarding() {
    this.setOnboardingMode("guided", {
      active: true,
      tab: "overview",
      reset: !isOnboardingFinished(this.onboardingProgress),
    });
  }
  ...
  resetToolStream() {
    ...
    resetActionFeedInternal(this as unknown as Parameters<typeof resetActionFeedInternal>[0]);
  }
