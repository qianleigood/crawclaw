...
def get_debug_port(account_name: Optional[str] = None) -> int:
    """Get the preferred CDP debug port for the given account."""
    data = _load_accounts()
    if account_name is None:
        account_name = data.get("default_account", DEFAULT_PROFILE_NAME)
    info = data.get("accounts", {}).get(account_name)
    if not isinstance(info, dict):
        return DEFAULT_DEBUG_PORT
    port = info.get("debug_port", DEFAULT_DEBUG_PORT)
    try:
        return int(port)
...
def set_debug_port(account_name: str, port: int) -> bool:
    """Set preferred CDP debug port for one account."""
    data = _load_accounts()
    if account_name not in data["accounts"]:
        return False
    data["accounts"][account_name]["debug_port"] = int(port)
    _save_accounts(data)
    return True


def resolve_debug_port(account_name: Optional[str], explicit_port: Optional[int]) -> int:
    """Resolve explicit port or fall back to the account's preferred port."""
    if explicit_port is not None:
        return int(explicit_port)
    return get_debug_port(account_name)
...
def list_accounts() -> list[dict]:
    ...
    for name, info in data["accounts"].items():
        result.append({
            ...
            "debug_port": info.get("debug_port", DEFAULT_DEBUG_PORT),
            "is_default": name == default,
        })
    return result


def add_account(name: str, alias: Optional[str] = None, debug_port: Optional[int] = None) -> bool:
    """
    Add a new account.
    ...
    data["accounts"][name] = {
        ...
        "debug_port": int(debug_port) if debug_port is not None else DEFAULT_DEBUG_PORT,
    }
    _save_accounts(data)
    return True
...
def main():
    ...
    p_add.add_argument("--port", type=int, help="Preferred CDP debug port for this account")
    ...
    # set-port
    p_port = sub.add_parser("set-port", help="Set preferred CDP debug port for an account")
    p_port.add_argument("name", help="Account name")
    p_port.add_argument("port", type=int, help="Preferred CDP debug port")
    ...
    if args.command == "list":
        ...
        if not accounts:
            ...
            return
        print(f"{'Name':<20} {'Alias':<20} {'Port':<8} {'Default':<10}")
        print("-" * 60)
        for acc in accounts:
            default_mark = "*" if acc["is_default"] else ""
            print(f"{acc['name']:<20} {acc['alias']:<20} {str(acc['debug_port']):<8} {default_mark:<10}")

    elif args.command == "add":
        if add_account(args.name, args.alias, args.port):
            print(f"Account '{args.name}' added.")
            print(f"Profile dir: {get_profile_dir(args.name)}")
            print(f"Preferred port: {get_debug_port(args.name)}")
        ...
        if info:
            ...
            print(f"Preferred port: {info.get('debug_port', DEFAULT_DEBUG_PORT)}")
        else:
        ...
        if set_debug_port(args.name, args.port):
            print(f"Preferred port for '{args.name}' set to {args.port}.")
        else:
            print(f"Error: Account '{args.name}' not found.", file=sys.stderr)
            sys.exit(1)
