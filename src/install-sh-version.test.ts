...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-install-sh-"));
  const cliPath = path.join(root, "crawclaw");
...
CRAWCLAW_BIN="$FAKE_CRAWCLAW_BIN"
resolve_crawclaw_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_CRAWCLAW_BIN: cliPath,
        CRAWCLAW_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("CrawClaw 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("CrawClaw dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("CrawClaw dev's build");
    },
  );
