...
export async function dashboardCommand(
  ...
  const cfg = snapshot.valid ? (snapshot.sourceConfig ?? snapshot.runtimeConfig) : {};
  ...
  if (resolvedToken.unresolvedRefReason) {
    ...
    runtime.log(
      "Set CRAWCLAW_GATEWAY_TOKEN in this shell or resolve your secret provider, then rerun `crawclaw dashboard`.",
    );
  }
  ...
  if (opened) {
    runtime.log("Opened in your browser. Keep that tab to control CrawClaw.");
  } else if (hint) {
    runtime.log(hint);
  }
}
