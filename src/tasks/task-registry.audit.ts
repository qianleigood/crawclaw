...
function findInvalidAgentMetadata(task: TaskRecord): TaskAuditFinding | null {
...
export function listTaskAuditFindings(options: TaskAuditOptions = {}): TaskAuditFinding[] {
  ...
  for (const task of tasks) {
    ...
    const invalidAgentMetadata = findInvalidAgentMetadata(task);
    if (invalidAgentMetadata) {
      findings.push(invalidAgentMetadata);
    }
  }
