...
This document explains how CrawClaw manages sessions end-to-end:
...
- UIs (Control UI, TUI, and other clients) should query the Gateway for session lists and token counts.
...
CrawClaw persists sessions in two layers:
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.crawclaw/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
- **Reset** (`/new`) creates a new `sessionId` for that `sessionKey`.
...
- If it’s already higher, CrawClaw leaves it alone.
...
CrawClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- CrawClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, CrawClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
