...
describe("ports-format", () => {
  it.each([
    ...
    [{ command: "ssh" }, "ssh"],
    [{ commandLine: "/Users/me/Projects/crawclaw/dist/native/crawclaw-gateway" }, "gateway"],
  ...
  it("builds ordered hints for mixed listener kinds and multiplicity", () => {
    expect(
      buildPortHints(
        [
          { commandLine: "/tmp/crawclaw/dist/native/crawclaw-gateway" },
  ...
  it("treats single-process loopback dual-stack gateway listeners as benign", () => {
    const listeners = [
      { pid: 4242, commandLine: "crawclaw-gateway", address: "127.0.0.1:18789" },
      { pid: 4242, commandLine: "crawclaw-gateway", address: "[::1]:18789" },
    ];
