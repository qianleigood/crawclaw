...
describe("createOllamaStreamFn streaming events", () => {
  ...
  it("emits text_end as soon as Ollama switches from text to tool calls", async () => {
    ...
    try {
      ...
      const terminalEvent = await nextEventWithin(iterator);
      expect(terminalEvent).not.toBe("timeout");
      if (terminalEvent !== "timeout" && !terminalEvent.done) {
        ...
        expect(terminalEvent).toMatchObject({ value: undefined, done: true });
      }
