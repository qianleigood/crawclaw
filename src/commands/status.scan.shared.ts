...
export function hasExplicitMemorySearchConfig(cfg: CrawClawConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: CrawClawConfig, agentId: string) => { store: { path: string } } | null;
