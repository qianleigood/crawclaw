...
const GATEWAY_DEVICE_ID_HEADER = "x-crawclaw-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-crawclaw-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-crawclaw-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.CRAWCLAW_APNS_RELAY_BASE_URL);
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.CRAWCLAW_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require CRAWCLAW_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.CRAWCLAW_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
