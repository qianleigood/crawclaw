...
describe("sessions.usage", () => {
  ...
  it("resolves store entries by sessionId when queried via discovered agent-prefixed key", async () => {
    const storeKey = "agent:opus:ddingtalk:dm:u123";
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-usage-test-"));

    try {
      await withEnvAsync({ CRAWCLAW_STATE_DIR: stateDir }, async () => {
