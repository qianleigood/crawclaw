...
CrawClaw 围绕一个拥有会话状态的单一 **Gateway 网关进程**设计。

- 客户端（TUI、自动化、浏览器来源客户端）应该向 Gateway 网关查询会话列表和 token 计数。
...
CrawClaw 有意**不**"修复"记录；Gateway 网关使用 `SessionManager` 来读/写它们。
...
2. **会话存储计数器**：写入 `sessions.json` 的滚动统计（用于 `/status` 和客户端视图）
...
CrawClaw 支持用于后台任务的"静默"回合，用户不应该看到中间输出。
...
从 `2026.1.10` 开始，当部分块以 `NO_REPLY` 开头时，CrawClaw 还会抑制**草稿/打字流式输出**，因此静默操作不会在回合中途泄漏部分输出。
