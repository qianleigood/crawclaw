...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    note.mockClear();
    pluginRegistry.list = [];
    prevToken = process.env.CRAWCLAW_GATEWAY_TOKEN;
    prevPassword = process.env.CRAWCLAW_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    if (prevToken === undefined) {
      ...
      process.env.CRAWCLAW_GATEWAY_TOKEN = prevToken;
    }
    if (prevPassword === undefined) {
      delete process.env.CRAWCLAW_GATEWAY_PASSWORD;
    } else {
      process.env.CRAWCLAW_GATEWAY_PASSWORD = prevPassword;
    }
  ...
  async function withExecApprovalsFile(
    ...
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-doctor-security-"));
    process.env.HOME = home;
    await fs.mkdir(path.join(home, ".crawclaw"), { recursive: true });
    await fs.writeFile(
      path.join(home, ".crawclaw", "exec-approvals.json"),
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as CrawClawConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.CRAWCLAW_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as CrawClawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats SecretRef token config as authenticated for exposure warning level", async () => {
    const cfg = {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: { source: "env", provider: "default", id: "CRAWCLAW_GATEWAY_TOKEN" },
        },
      },
    } as CrawClawConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as CrawClawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    ...
    pluginRegistry.list = [
      {
        ...
        security: {
          resolveDmPolicy: () => ({
            policy: "allowlist",
            allowFrom: ["alice", "bob"],
            allowFromPath: "channels.weixin.",
            approveHint: "approve",
          }),
        },
      },
    ];
    const cfg = { session: { dmScope: "main" } } as CrawClawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("crawclaw approvals get --gateway");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    await noteSecurityWarnings({} as CrawClawConfig);
    const message = lastMessage();
    expect(message).toContain("[secrets]");
    expect(message).toContain("failed to resolve account");
    expect(message).toContain("Run: crawclaw security audit --deep");
  });
