...
import type { CrawClawConfig, MemorySearchConfig } from "../config/config.js";
...
} from "../memory/search/multimodal.js";
...
type MemoryProviderSpec = {
  defaultModel: string;
  transport: "local" | "remote";
  supportsMultimodalEmbeddings?: (params: { model: string }) => boolean;
};

const MEMORY_PROVIDER_SPECS: Record<string, MemoryProviderSpec> = {
  openai: {
...
function getMemoryProviderSpec(id: string | undefined): MemoryProviderSpec | undefined {
  if (!id || id === "auto") {
    return undefined;
  }
  return MEMORY_PROVIDER_SPECS[id];
}
...
function mergeConfig(
  ...
  const primaryAdapter = getMemoryProviderSpec(provider);
...
export function resolveMemorySearchConfig(
  ...
  const multimodalProvider = getMemoryProviderSpec(resolved.provider);
  if (
    multimodalActive &&
