...
- `node.pair.*`（CLI：`crawclaw nodes pending/approve/reject`）是一个单独的 Gateway 网关拥有的
...
- **批准**：通过 `~/.crawclaw/exec-approvals.json` 在节点主机上执行。
...
- 令牌是 Gateway 网关配置中的 `gateway.auth.token`（Gateway 网关主机上的 `~/.crawclaw/crawclaw.json`）。
- `crawclaw node run` 读取 `CRAWCLAW_GATEWAY_TOKEN` 进行认证。
...
- 在 `crawclaw node run` / `crawclaw node install` 上使用 `--display-name`（持久化在节点上的 `~/.crawclaw/node.json` 中）。
- `crawclaw nodes rename --node <id|name|ip> --name "Build Node"`（Gateway 网关覆盖）。
...
批准存储在节点主机的 `~/.crawclaw/exec-approvals.json` 中。
...
crawclaw nodes camera snap --node <idOrNameOrIp>            # 默认：两个朝向（2 个 MEDIA 行）
...
旧版移动端节点曾暴露 `sms.send`。对应源码已从本仓库移除；当前发布版本不再把短信能力作为现役节点功能面来文档化。
...
CrawClaw 可以运行**无头节点主机**（无 UI），它连接到 Gateway 网关
