...
const GENERATED_VIDEO_URLS_PATH = path.join(
  WORKSPACE_ROOT,
  "runtime",
  "browser-jobs",
  "grok-video-web",
  "generated-video-urls.jsonl",
);
...
function resolveJob(args = {}) {
  ...
  const downloadsDir =
    pickString(args["downloads-dir"], manifest.downloadsDir) || path.join(jobDir, "downloads");
  const exportsDir =
    pickString(args["exports-dir"], manifest.exportsDir) || path.join(jobDir, "exports");
...
function normalizeActionType(value) {
  const raw = String(value || "")
    .trim()
    .toLowerCase();
  if (!raw || raw === "generate_video" || raw === "submit") {
    return "generate";
  }
  if (raw === "redo" || raw === "redo-video") {
    return "redo_video";
  }
  if (raw === "extend" || raw === "extend-video") {
    return "extend_video";
  }
  return raw;
}
...
function resolveLineagePatch(job, patch = {}) {
  ...
  const actionType = resolveActionType(
    job,
    patch.actionType || current.actionType || existing.actionType,
  );
  const lineageCompatible =
    !current.actionType || resolveActionType(job, current.actionType) === actionType;
  const inheritedCurrent = lineageCompatible ? current : {};
  const inheritedExisting = lineageCompatible ? existing : {};
  const sourceResultUrl = maybeResultUrl(
    pickLineageValue(
  ...
  const newResultUrl = maybeResultUrl(
    pickLineageValue(
      patch.newResultUrl,
      patch.resultUrl,
  ...
  return {
    ...
    checkedAt,
    lastObservedUrl:
      patch.lastObservedUrl ||
      current.lastObservedUrl ||
      existing.lastObservedUrl ||
      newResultUrl ||
      sourceResultUrl ||
      "",
  };
...
function recordLineage(job, patch = {}) {
  const existing = readJson(job.files.lineagePath, {
    ...
    current: null,
    history: [],
  }) || { version: 1, actionType: "", current: null, history: [] };
...
function appendGeneratedVideoUrl(job, entry = {}) {
  const payload = {
    ...
    postId: pickLineageValue(
      entry.postId,
      extractPostIdFromUrl(entry.url || entry.resultUrl || entry.newResultUrl || ""),
    ),
    sourcePostId: pickLineageValue(
      entry.sourcePostId,
      extractPostIdFromUrl(entry.sourceResultUrl || ""),
    ),
  ...
  if (!payload.url) {
    return { written: false, entry: payload, reason: "missing_url" };
  }

  let existing = [];
  if (fileExists(job.files.generatedVideoUrlsPath)) {
    existing = fs
      .readFileSync(job.files.generatedVideoUrlsPath, "utf8")
      .split(/\r?\n/)
      .map((line) => line.trim())
      .filter(Boolean)
      .map((line) => {
...
async function confirmLoggedInAtSafeEntry({
  page,
  job = null,
  logger = null,
  action = "account_gate",
...
function resolveResultUrlConsistencyMode(job = null, explicitMode = "") {
  ...
  const normalized = String(raw || "")
    .trim()
    .toLowerCase();
  if (["ignore", "off", "disabled"].includes(normalized)) {
    return "ignore";
  }
  if (["block", "strict", "fail"].includes(normalized)) {
    return "block";
  }
  return "warn";
}

function assessResultUrlConsistency({
  expectedResultUrl = "",
  observedUrl = "",
  observedPostId = "",
  job = null,
  ...
  const finalObservedPostId = pickString(
    observedPostId,
    extractPostIdFromUrl(observedComparableUrl),
  );
  const policy = resolveResultUrlConsistencyMode(job, mode);
  const urlMatched =
    Boolean(expectedUrl) && Boolean(observedComparableUrl) && expectedUrl === observedComparableUrl;
  const postIdMatched =
    Boolean(expectedPostId) &&
    Boolean(finalObservedPostId) &&
    expectedPostId === finalObservedPostId;
    ...
    .evaluate(() => {
      ...
      const nodes = Array.from(
        document.querySelectorAll(
          'button, a, [role="button"], [role="menuitem"], [role="tab"], [role="radio"]',
        ),
      );
      const labels = nodes
        .filter(visible)
...
async function detectCompletion(page) {
  ...
  if (onResultPage) {
    completionSignals.push("/imagine/post/<id>");
  }
  if (hasDownload) {
    completionSignals.push("下载");
  }
  if (hasShare) {
    completionSignals.push("创建共享链接");
  }
  if (hasFollowupGenerate) {
    completionSignals.push("生成视频");
  }
  if (playableVideo && onResultPage) {
    completionSignals.push("video-ready");
  } else if (hasVideo && onResultPage) {
    progressSignals.push("video-element-present");
  }
  ...
  if (
    !onResultPage &&
    /\/imagine(?:$|[?#])/i.test(url) &&
    /登录|注册|sign in|log in/i.test(bodyText)
  ) {
    blockerSignals.push("redirected-away-from-result");
  }
  if (
    /套餐|订阅|升级|limit|quota|subscribe|subscription/i.test(bodyText) &&
...
async function settleAfterNavigation(page, timeoutMs = 60000) {
  ...
  if (page && typeof page.waitForNetworkIdle === "function") {
    await page
      .waitForNetworkIdle({ idleTime: 800, timeout: Math.min(timeoutMs, 10000) })
      .catch(() => {});
    return;
  }
  await sleep(1200);
...
async function waitForCompletion({
  ...
  if (logger) {
    logger.info("wait.page_opened", {
      resultUrl,
      currentUrl: openedUrl,
      refresh,
      timeoutMs,
      intervalMs,
      resultUrlConsistencyMode: consistencyMode,
    });
  }
  ...
  while (Date.now() - startedAt <= timeoutMs) {
    ...
    const redirectedToImagineRoot =
      expectedPostId &&
      !currentPostId &&
      /\/imagine(?:$|[?#])/i.test(last.url || "") &&
      !/\/imagine\/post\//i.test(last.url || "");
    const looksLikeAuthOrLoggedOut = /(sign in|log in|sign up|登录|注册)/i.test(
      last.bodyExcerpt || "",
    );
    ...
    if (redirectedToImagineRoot && (looksLikeAuthOrLoggedOut || looksLikeTemplateLanding)) {
      last = {
        ...
        blockerSignals: looksLikeAuthOrLoggedOut
          ...
          : (last.blockerSignals || []).filter(
              (signal) =>
                signal !== "redirected-away-from-result" && signal !== "result-page-not-opened",
            ),
        ...
        completionSignals: (last.completionSignals || []).filter(
          (signal) => signal !== "video-element" && signal !== "video-ready",
        ),
      };
      ...
      last = {
        ...last,
        status: "generating",
        blockerSignals: (last.blockerSignals || []).filter(
          (signal) =>
            signal !== "redirected-away-from-result" && signal !== "result-page-not-opened",
        ),
        progressSignals: Array.from(
          new Set([
            ...(last.progressSignals || []),
            "result-page-not-opened",
            "redirected-away-from-result",
    ...
    if (
      retryableRedirectToImagineRoot &&
      resultUrlConsistency.mismatch &&
      resultUrlConsistency.action === "block"
    ) {
    ...
    if (logger) {
      ...
      if (record.resultUrlConsistency && record.resultUrlConsistency.mismatch) {
        ...
        if (consistencyKey !== previousConsistencyKey) {
          logger[record.resultUrlConsistency.action === "block" ? "warn" : "info"](
            "wait.result_url_mismatch",
            {
...
function selectionFromSnapshot(snapshot = {}) {
  ...
  const visibleRegion = regions.find(
    (item) => Number.isFinite(item?.startPct) && Number.isFinite(item?.endPct),
  );
  if (visibleRegion) {
...
function inferTimelineBoundaryRole(item = {}, index = 0, total = 0) {
  const tokens = [item.ariaLabel, item.testId, item.className, item.text, item.kind, item.source]
    ...
    .join(" ");
  if (/trim_start|start|left|in\b|起点|开始/.test(tokens)) {
    return "start";
  }
  if (/trim_end|end|right|out\b|终点|结束/.test(tokens)) {
    return "end";
  }
  if (total >= 2) {
    if (index === 0) {
...
function buildTimelineAutomationModel(
  snapshot = {},
  ...
  const container =
    dominantContainer &&
    dominantContainer.rect &&
    Number.isFinite(dominantContainer.rect.width) &&
    dominantContainer.rect.width > 0
      ? {
  ...
  const rangeInputs = (snapshot.rangeInputs || [])
    .map((item) => {
      ...
      const normalizedValue =
        Number.isFinite(value) && Number.isFinite(min) && Number.isFinite(max) && max > min
  ...
  const endHandle =
    handles.find((item) => item.role === "end") ||
    (handles.length >= 2 ? handles[handles.length - 1] : null);
  const startRange = rangeInputs.find((item) => item.role === "start") || rangeInputs[0] || null;
  const endRange =
    rangeInputs.find((item) => item.role === "end") ||
    (rangeInputs.length >= 2 ? rangeInputs[rangeInputs.length - 1] : null);
  const trimStart = trimControls.find((item) => item.role === "start") || null;
  const trimEnd = trimControls.find((item) => item.role === "end") || null;
  const fixedWindowDurationPct =
    currentSelection &&
    currentSelection.detected &&
    Number.isFinite(currentSelection.startPct) &&
    Number.isFinite(currentSelection.endPct)
      ? roundNumber(currentSelection.endPct - currentSelection.startPct)
      : null;

  const readyReasons = [];
  if (container) {
    readyReasons.push("timeline_container_locked");
  }
  if (currentSelection && currentSelection.detected) {
    readyReasons.push(`selection:${currentSelection.source || "unknown"}`);
  }
  if (startHandle && endHandle) {
    readyReasons.push("handles:start_end");
  }
  if (startRange && endRange) {
    readyReasons.push("range_inputs:start_end");
  }
  if (trimEnd && Number.isFinite(fixedWindowDurationPct)) {
    readyReasons.push("single_end_trim_handle_fixed_window");
  }
  if (endHandle && Number.isFinite(fixedWindowDurationPct)) {
    readyReasons.push("single_end_handle_fixed_window");
  }
  ...
  return {
    ...
    selection: {
      ...
      startPct:
        currentSelection && Number.isFinite(currentSelection.startPct)
          ? currentSelection.startPct
          : null,
      endPct:
        currentSelection && Number.isFinite(currentSelection.endPct)
          ? currentSelection.endPct
          : null,
      startPx:
        currentSelection && Number.isFinite(currentSelection.startPx)
          ? currentSelection.startPx
          : null,
      endPx:
        currentSelection && Number.isFinite(currentSelection.endPx) ? currentSelection.endPx : null,
    },
    handles: {
      all: handles,
      start: startHandle,
    ...
    adjustmentMode:
      (trimEnd && Number.isFinite(fixedWindowDurationPct)) ||
      (endHandle && Number.isFinite(fixedWindowDurationPct))
        ? "single_handle_fixed_window"
        : (startHandle && endHandle) || (startRange && endRange)
          ? "dual_boundary"
          : "",
    canResolveTarget: Boolean(
      ...
      ((startHandle && endHandle) ||
        (startRange && endRange) ||
        ((trimEnd || endHandle) && Number.isFinite(fixedWindowDurationPct))),
    ),
    readyReasons,
  };
}
...
function classifyTimelineSnapshot(snapshot = {}) {
  ...
  const trimKinds = Array.from(
    new Set((snapshot.trimSignals || []).map((item) => item.kind).filter(Boolean)),
  );
  const rangeKinds = Array.from(
    new Set(
      [counts.rangeInput ? "range_input" : "", counts.slider ? "slider" : ""].filter(Boolean),
    ),
  );
  const automationModel = buildTimelineAutomationModel(
    snapshot,
    dominantContainer,
    currentSelection,
  );
  ...
  if (dominantContainer) {
    const name =
      dominantContainer.ariaLabel ||
      dominantContainer.testId ||
      dominantContainer.className ||
      dominantContainer.tag ||
      "timeline";
    signals.push(`container:${String(name).slice(0, 80)}`);
  }
  if (counts.handle) {
    signals.push(`handles:${counts.handle}`);
  }
  if (counts.rangeInput) {
    signals.push(`range-inputs:${counts.rangeInput}`);
  }
  if (counts.slider) {
    signals.push(`sliders:${counts.slider}`);
  }
  if (counts.trim) {
    signals.push(`trim:${trimKinds.join(",") || counts.trim}`);
  }
  if (currentSelection.detected) {
    signals.push(
      `selection:${currentSelection.source}:${currentSelection.startPct}-${currentSelection.endPct}`,
    );
  }
  ...
  if (timelineMode !== "not_detected" && !dominantContainer) {
    unknowns.push("timeline_container_not_isolated");
  }
  if (counts.container > 1) {
    unknowns.push("multiple_timeline_candidates");
  }
  if (
    (counts.handle > 0 || counts.slider > 0 || counts.rangeInput > 0 || counts.selection > 0) &&
    !currentSelection.detected
  ) {
    unknowns.push("selection_start_end_unresolved");
  }
  if (!counts.trim) {
    unknowns.push("trim_signal_not_observed");
  }
  if (!counts.handle && !counts.slider && !counts.rangeInput) {
    unknowns.push("no_explicit_handles_or_range_inputs");
  }
  ...
  if (timelineMode === "manual_handoff") {
    boundaryClass = automationModel.canResolveTarget
      ? "interactive_auto_adjust_ready"
      : "interactive_manual_handoff";
    ...
    if (counts.handle) {
      boundaryReasons.push("interactive_handles_detected");
    }
    if (counts.slider) {
      boundaryReasons.push("slider_controls_detected");
    }
    if (counts.rangeInput) {
      boundaryReasons.push("range_inputs_detected");
    }
    if (currentSelection.detected) {
      boundaryReasons.push("selection_window_detected");
    }
    boundaryReasons.push(
      automationModel.canResolveTarget
        ? "safe_drag_target_committed"
        : "safe_drag_target_not_committed",
    );
  } else if (timelineMode === "timeline_detected") {
    boundaryClass = "probe_only_manual_handoff";
    manualHandoffRequired = true;
    boundarySummary =
      "Timeline-like UI signals were detected, but the runner cannot safely resolve drag targets; stop at probe + handoff.";
    if (counts.container || counts.track) {
      boundaryReasons.push("timeline_container_detected");
    }
    if (counts.trim) {
      boundaryReasons.push("trim_signal_detected");
    }
    if (counts.selection) {
      boundaryReasons.push("selection_signal_detected");
    }
  ...
  if (counts.container > 1) {
    boundaryReasons.push("multiple_timeline_candidates");
  }
  if (unknowns.includes("selection_start_end_unresolved")) {
    boundaryReasons.push("selection_start_end_unresolved");
  }
  ...
  const snapshot = await page
    .evaluate(
      ({
        ...
        knownTimelineDragHandleXpaths,
        knownTimelineRangeLabelXpaths,
      }) => {
        const norm = (value) =>
          String(value || "")
            .replace(/\s+/g, " ")
        ...
        const containerScore = (node) => {
          const tokens = [
            ...
            .toLowerCase();
          let score = 0;
          if (/timeline|time line|时间轴/.test(tokens)) {
            score += 10;
          }
          if (/track|轨道|scrubber|seek/.test(tokens)) {
            score += 6;
          }
          if (/trim|裁剪|clip|range|selection|segment|window/.test(tokens)) {
            score += 4;
          }
          const rect = node.getBoundingClientRect();
          if (rect.width > 140) {
        ...
        const parseTimelineRangeText = (text, totalSeconds, endPctHint = null) => {
          ...
          const numbers =
            mmssValues.length >= 2
              ...
              : secValues.length >= 2
                ? secValues
                : Array.from(normalized.matchAll(/\d+(?:\.\d+)?/g))
          ...
          if (!Number.isFinite(durationSec) || durationSec <= 0 || durationSec < endSec) {
            ...
            durationSec =
              Number.isFinite(endPct) && endPct > 0 && endPct <= 100
                ? endSec / (endPct / 100)
                : endSec;
          }
          if (
        ...
        const fallbackContainerNodes =
          scopeRoot instanceof Element && isVisible(scopeRoot) ? [scopeRoot] : [];
        const containers = uniq([...containerNodes, ...fallbackContainerNodes], (node) => node)
          ...
          .toSorted(
            (a, b) =>
              b.score - a.score ||
              b.node.getBoundingClientRect().width - a.node.getBoundingClientRect().width,
          )
          .slice(0, 4)
          .map(({ node, score }, index) =>
            serialize(node, {
              score,
        ...
        const handles = uniq(
          [...Array.from(queryAll(handleSelectors)).filter(isVisible), ...knownHandleNodes],
          (node) => node,
        )
          .slice(0, 8)
          .map((node, index) => {
            ...
            const info = serialize(node, {
              ...
              source: isKnownHandle
                ? "known_timeline_drag_handle_xpath"
                : node.matches('[role="slider"]')
                  ? "slider"
                  : "handle",
        ...
        const rangeInputs = Array.from(queryAll('input[type="range"]'))
          .filter(isVisible)
          .slice(0, 8)
          .map((node, index) => {
            const info = serialize(node, {
        ...
        const trimSignals = uniq(
          ...
          .map((node, index) => {
            ...
            if (/start|left|in|起点|开始/.test(text)) {
              kind = "trim_start";
            } else if (/end|right|out|终点|结束/.test(text)) {
              kind = "trim_end";
            }
        ...
        const selectionRegions = uniq(
          Array.from(queryAll(selectionSelectors)).filter(isVisible),
          (node) => node,
        )
          .slice(0, 8)
          .map((node, index) => {
        ...
        const knownRangeLabelNodes = knownTimelineRangeLabelXpaths
          .map(firstXPath)
          .filter((node) => node instanceof Element && isVisible(node));
        const primaryHandlePctHint =
          handles.find((item) => Number.isFinite(item.positionPct))?.positionPct || null;
        knownRangeLabelNodes.forEach((node, index) => {
          const info = serialize(node, {
            selector: attachSelector(
              node,
              "data-crawclaw-timeline-selection",
              selectionRegions.length + index,
            ),
            source: "known_timeline_range_label_xpath",
          });
          const parsed = parseTimelineRangeText(
            info.text || info.ariaLabel || "",
            videoDuration,
            primaryHandlePctHint,
          );
          if (parsed) {
            selectionRegions.push({
              ...
              startPx:
                baseContainerRect && baseContainerRect.width
                  ? (parsed.startPct / 100) * baseContainerRect.width
                  : null,
              endPx:
                baseContainerRect && baseContainerRect.width
                  ? (parsed.endPct / 100) * baseContainerRect.width
                  : null,
        ...
        const draggableCount = Array.from(document.querySelectorAll('[draggable="true"]')).filter(
          isVisible,
        ).length;
