...
} from "./pi-embedded-subscribe.lifecycle.compaction.js";
import { resetRunLoopLifecycleHandlersForTests } from "./runtime/lifecycle/bus.js";
import { __testing as lifecycleSubscriberTesting } from "./runtime/lifecycle/shared-subscribers.js";

beforeEach(() => {
  resetAgentEventsForTest();
  resetRunLoopLifecycleHandlersForTests();
  lifecycleSubscriberTesting.resetSharedRunLoopLifecycleSubscribers();
});
...
describe("reconcileSessionStoreCompactionCountAfterSuccess", () => {
  it("raises the stored compaction count to the observed value", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-compaction-reconcile-"));
  ...
  it("does not double count when the store is already at or above the observed value", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-compaction-idempotent-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
...
describe("handleAutoCompactionEnd", () => {
  it("reconciles the session store after a successful compaction end event", async () => {
    resetAgentEventsForTest();
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-compaction-handler-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
  ...
  it("emits normalized action events for compaction progress", async () => {
    resetAgentEventsForTest();
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-compaction-actions-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
