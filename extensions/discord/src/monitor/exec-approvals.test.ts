...
const { STORE_PATH, mockSessionStoreEntries } = vi.hoisted(() => ({
  STORE_PATH: "/tmp/crawclaw-exec-approvals-test.json",
...
beforeEach(() => {
  ...
  mockResolveGatewayConnectionAuth.mockReset().mockImplementation(
    async (params: {
      ...
      const envToken = params.env.CRAWCLAW_GATEWAY_TOKEN;
      const envPassword = params.env.CRAWCLAW_GATEWAY_PASSWORD;
...
vi.mock("../../../../src/gateway/operator-approvals-client.js", () => ({
  createOperatorApprovalsGatewayClient: async (params: {
    ...
    const envUrl = process.env.CRAWCLAW_GATEWAY_URL?.trim();
...
vi.mock("crawclaw/plugin-sdk/text-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("crawclaw/plugin-sdk/text-runtime")>();
  return {
    ...actual,
    logDebug: vi.fn(),
    logError: vi.fn(),
...
describe("DiscordExecApprovalHandler gateway auth", () => {
  ...
  it("prefers CRAWCLAW_GATEWAY_TOKEN when config token is missing", async () => {
    vi.stubEnv("CRAWCLAW_GATEWAY_TOKEN", "env-gateway-token");
    const handler = new DiscordExecApprovalHandler({
      token: "discord-bot-token",
      accountId: "default",
      config: { enabled: true, approvers: ["123"] },
...
describe("DiscordExecApprovalHandler timeout cleanup", () => {
  ...
  it("deletes the pending approval message on timeout when cleanup is enabled", async () => {
    const handler = createHandler({
      enabled: true,
      approvers: ["123"],
    ...
    await handler.handleApprovalTimeout(request.id);

    expect(mockRestDelete).toHaveBeenCalledTimes(1);
    expect(mockRestDelete).toHaveBeenCalledWith(Routes.channelMessage("dm-1", "msg-1"));
    expect(mockRestPatch).not.toHaveBeenCalled();
  });

  it("falls back to updating the expired approval message when delete fails", async () => {
    const handler = createHandler({
      enabled: true,
      approvers: ["123"],
      cleanupAfterResolve: true,
    ...
    mockRestPost.mockImplementation(async (route: string) => {
      if (route === Routes.channelMessages("999888777")) {
        return { id: "msg-1", channel_id: "999888777" };
      }
      throw new Error(`unexpected route: ${route}`);
    });
    mockRestDelete.mockRejectedValue(new Error("delete failed"));
    ...
    expect(mockRestDelete).toHaveBeenCalledTimes(1);
    expect(mockRestDelete).toHaveBeenCalledWith(Routes.channelMessage("999888777", "msg-1"));
    expect(mockRestPatch).toHaveBeenCalledTimes(1);
    expect(mockRestPatch).toHaveBeenCalledWith(
      Routes.channelMessage("999888777", "msg-1"),
      expect.objectContaining({
...
describe("DiscordExecApprovalHandler gateway auth resolution", () => {
  ...
  it("passes env URL overrides to shared gateway auth resolver", async () => {
    const previousGatewayUrl = process.env.CRAWCLAW_GATEWAY_URL;
    try {
      process.env.CRAWCLAW_GATEWAY_URL = "wss://gateway-from-env.example/ws";
      const handler = new DiscordExecApprovalHandler({
        token: "test-token",
        accountId: "default",
        config: { enabled: true, approvers: ["123"] },
      ...
      if (typeof previousGatewayUrl === "string") {
        process.env.CRAWCLAW_GATEWAY_URL = previousGatewayUrl;
      } else {
