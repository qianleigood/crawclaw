...
async function withModeExecProviderFixture(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), `crawclaw-tui-mode-${label}-`));
...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    ...
    resolveStateDir.mockImplementation(
      (env: NodeJS.ProcessEnv) => env.CRAWCLAW_STATE_DIR ?? "/tmp/crawclaw",
    );
    resolveConfigPath.mockImplementation(
      (env: NodeJS.ProcessEnv, stateDir: string) =>
        env.CRAWCLAW_CONFIG_PATH ?? `${stateDir}/crawclaw.json`,
    );
  ...
  it("uses config auth token for local mode when both config and CRAWCLAW env tokens are set", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local", auth: { token: "config-token" } } });

    await withEnvAsync({ CRAWCLAW_GATEWAY_TOKEN: "env-token" }, async () => {
      const result = await resolveGatewayConnection({});
      expect(result.token).toBe("config-token");
    });
  });

  it("falls back to CRAWCLAW_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("prefers CRAWCLAW_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "CRAWCLAW_GATEWAY_PASSWORD"; // pragma: allowlist secret
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-tui-file-secret-"));
...
describe("GatewayChatClient", () => {
  it("identifies the TUI as a tui client and skips device identity on insecure local ui paths", () => {
    ...
    expect(
      (client as unknown as { client: { opts: { clientDisplayName?: string } } }).client.opts
        .clientDisplayName,
    ).toBe("crawclaw-tui");
    expect(
      (client as unknown as { client: { opts: { clientName?: string; mode?: string } } }).client
