...
import { createCrawClawTools } from "../agents/crawclaw-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-crawclaw-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-crawclaw-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-crawclaw-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-crawclaw-thread-id")?.trim() || undefined;
  ...
  const allTools = createCrawClawTools({
