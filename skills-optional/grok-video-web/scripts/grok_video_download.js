  ...
  return page
    .evaluate((maxItems) => {
      ...
      const nodes = Array.from(
        document.querySelectorAll('button, a, [role="button"], [role="menuitem"], [role="menu"]'),
      );
      const labels = [];
    ...
    .evaluate((maxItems) => {
      ...
      const nodes = Array.from(
        document.querySelectorAll(
          '[role="menuitem"], [role="menu"] button, [role="menu"] a, [role="menu"] [role="button"]',
        ),
      );
      const labels = [];
      for (const node of nodes) {
...
async function hoverVideoSurface(page) {
  ...
  await page.mouse
    .move(box.x + box.width / 2, box.y + Math.max(12, box.height - 24))
    .catch(() => {});
  await sleep(500);
  return true;
}
...
async function markVisibleClickableByText(page, texts, attrName) {
  ...
  if (!desired.length) {
    return { found: false, selector: "", label: "" };
  }
  return page
    .evaluate(
      ({ desired, attrName }) => {
    ...
    .catch(() => ({ found: false, selector: "", label: "" }));
}
...
async function waitForDownloadArtifact(downloadsDir, before, timeoutMs) {
  ...
  if (
    lastCandidate &&
    fs.existsSync(lastCandidate.path) &&
    !lastCandidate.name.endsWith(".crdownload")
  ) {
    return lastCandidate;
  }
  return null;
...
async function openMenuAndClickDownload(page, timeoutMs, downloadsDir) {
  ...
  for (const candidate of menuButtons) {
    const trigger = await markVisibleClickableByText(
      page,
      candidate.texts,
      "data-crawclaw-menu-target",
    );
    if (!trigger.found) {
      continue;
    }
    ...
    if (menuItems.length) {
      menuEvidence.push(`${candidate.source}: ${menuItems.join(" | ")}`);
    }
    const downloadTarget = await markVisibleClickableByText(
      page,
      ["下载", "download"],
      "data-crawclaw-download-target",
    );
    if (downloadTarget.found) {
      const hit = await clickAndWaitForDownload(
        page,
        downloadTarget.selector,
        timeoutMs,
        downloadsDir,
        `${candidate.source}:${downloadTarget.label || "download-item"}`,
      );
      if (hit) {
        return { ...hit, menuEvidence };
      }
    }
...
async function clickDownloadButton(page, timeoutMs, downloadsDir) {
  ...
  const directTarget = await markVisibleClickableByText(
    page,
    ["下载", "download"],
    "data-crawclaw-download-target",
  );
  if (directTarget.found) {
    const directHit = await clickAndWaitForDownload(
      page,
      directTarget.selector,
      timeoutMs,
      downloadsDir,
      `direct:${directTarget.label || "download"}`,
    );
    if (directHit) {
      return directHit;
    }
...
async function readChromeDownloads(browser, timeoutMs, { sinceMs = 0, resultUrl = "" } = {}) {
  ...
  const expectedPostId =
    (String(resultUrl || "").match(/\/imagine\/post\/([^/?#]+)/i) || [])[1] || "";
  let downloadsPage = null;
  try {
    downloadsPage = await browser.newPage();
    await downloadsPage
      .goto("chrome://downloads/", { waitUntil: "domcontentloaded", timeout: 30000 })
      .catch(() => {});
    while (Date.now() <= deadline) {
      const items = await downloadsPage
        .evaluate(() => {
          const manager = document.querySelector("downloads-manager");
          const list = manager?.shadowRoot?.querySelector("#downloadsList");
          const data = Array.isArray(list?.items)
            ? list.items
            : Array.isArray(manager?.items_)
              ? manager.items_
              : [];
      ...
      const complete = items
        ...
        .filter(
          (item) =>
            /complete|2/i.test(String(item.state || "")) || item.resolvedPath || item.fileUrl,
        )
        .filter((item) => {
          if (!expectedPostId) {
            return true;
        ...
        .map((item) => {
          ...
          try {
            if (item.resolvedPath && fs.existsSync(item.resolvedPath)) {
              statTime = fs.statSync(item.resolvedPath).mtimeMs;
            }
          } catch {}
    ...
    if (downloadsPage) {
      await downloadsPage.close().catch(() => {});
    }
  }
}
...
async function main() {
  ...
  updateWorkflowStatus(job, {
    status: shouldWait ? "running" : "queued",
    blocked: false,
    phase: "download_started",
    currentUrl: resultUrl,
    resultUrl,
  ...
  try {
    ...
    if (shouldWait) {
      ...
      if (waitResult.status === "blocked") {
        throw new Error(
          `result page blocked: ${(waitResult.blockerSignals || []).join(", ") || "unknown blocker"}`,
        );
      }
    ...
    const resultUrlConsistency =
      waitResult.resultUrlConsistency ||
      assessResultUrlConsistency({
    ...
    if (resultUrlConsistency.mismatch) {
      logger[resultUrlConsistency.action === "block" ? "warn" : "info"](
        "download.result_url_mismatch",
        {
      ...
      if (resultUrlConsistency.action === "block") {
        throw new Error(
          `result URL mismatch before download: expected ${resultUrlConsistency.expectedPostId || "<none>"}, observed ${resultUrlConsistency.observedPostId || "<none>"} (${resultUrlConsistency.observedUrl || "no observed url"})`,
        );
      }
    }
    ...
    if (!downloadAttempt || !downloadAttempt.file) {
      const uiEvidence =
        Array.isArray(downloadAttempt?.visibleControls) && downloadAttempt.visibleControls.length
          ? downloadAttempt.visibleControls.join(" | ")
          : "none";
      const menuEvidence =
        Array.isArray(downloadAttempt?.menuEvidence) && downloadAttempt.menuEvidence.length
      ...
      if (!chromeItem && headless) {
        ...
        if (retryAttempt && retryAttempt.file) {
          downloaded = await saveBrowserDownload(
            retryAttempt.file,
            job,
            resultUrl,
            "page-download",
          ...
          if (chromeItem) {
            ...
            logger.warn("download.button_unavailable", {
              ...
              resultUrl,
              visibleControls:
                retryAttempt?.visibleControls || downloadAttempt?.visibleControls || [],
              menuEvidence: retryAttempt?.menuEvidence || downloadAttempt?.menuEvidence || [],
              nativeDownloadsDir,
            });
            throw new Error(
              `download click did not materialize into browser downloads; visible controls: ${uiEvidence}; menus: ${menuEvidence}`,
            );
          }
        }
      } else if (chromeItem) {
    ...
    const payload = {
      ...
      observedPostId:
        resultUrlConsistency.observedPostId || waitResult.observedPostId || waitResult.postId || "",
      observedUrl:
        resultUrlConsistency.observedUrl ||
        pageStateBeforeDownload.url ||
        waitResult.url ||
        resultUrl,
      rawPath: downloaded.rawPath,
      exportPath: downloaded.exportPath,
