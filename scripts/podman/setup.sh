#!/usr/bin/env bash
# One-time host setup for rootless CrawClaw in Podman. Uses the current
# non-root user throughout, builds or pulls the image into that user's Podman
# store, writes config under ~/.crawclaw by default, and uses the repo-local
# launch script at ./scripts/run-crawclaw-podman.sh.
#
...
#   Or set CRAWCLAW_PODMAN_QUADLET=1 (or 0) to choose without a flag.
#
# After this, start the gateway manually:
#   ./scripts/run-crawclaw-podman.sh launch
#   ./scripts/run-crawclaw-podman.sh launch setup
# Or, if you used --quadlet:
#   systemctl --user start crawclaw.service
set -euo pipefail

REPO_PATH="${CRAWCLAW_REPO_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
RUN_SCRIPT_SRC="$REPO_PATH/scripts/run-crawclaw-podman.sh"
QUADLET_TEMPLATE="$REPO_PATH/scripts/podman/crawclaw.container.in"
CRAWCLAW_USER="$(id -un)"
CRAWCLAW_HOME="${HOME:-}"
CRAWCLAW_CONFIG_DIR="${CRAWCLAW_CONFIG_DIR:-}"
CRAWCLAW_WORKSPACE_DIR="${CRAWCLAW_WORKSPACE_DIR:-}"
CRAWCLAW_IMAGE="${CRAWCLAW_PODMAN_IMAGE:-${CRAWCLAW_IMAGE:-crawclaw:local}}"
CRAWCLAW_CONTAINER_NAME="${CRAWCLAW_PODMAN_CONTAINER:-crawclaw}"
PLATFORM_NAME="$(uname -s 2>/dev/null || echo unknown)"
HOST_GATEWAY_PORT="${CRAWCLAW_PODMAN_GATEWAY_HOST_PORT:-${CRAWCLAW_GATEWAY_PORT:-18789}}"
...
PY
  ...
  echo "Missing dependency: need openssl or python3 (or od) to generate CRAWCLAW_GATEWAY_TOKEN." >&2
  exit 1
}
...
if [[ -n "${CRAWCLAW_PODMAN_QUADLET:-}" ]]; then
  case "${CRAWCLAW_PODMAN_QUADLET,,}" in
...
if [[ "$CRAWCLAW_IMAGE" == "crawclaw:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
  echo "Dockerfile not found at $REPO_PATH. Set CRAWCLAW_REPO_PATH to the repo root." >&2
  exit 1
fi
...
if [[ -z "$CRAWCLAW_HOME" ]]; then
  CRAWCLAW_HOME="$(resolve_user_home "$CRAWCLAW_USER")"
fi
if [[ -z "$CRAWCLAW_HOME" ]]; then
  echo "Unable to resolve HOME for user $CRAWCLAW_USER." >&2
  exit 1
fi
if [[ -z "$CRAWCLAW_CONFIG_DIR" ]]; then
  CRAWCLAW_CONFIG_DIR="$CRAWCLAW_HOME/.crawclaw"
fi
if [[ -z "$CRAWCLAW_WORKSPACE_DIR" ]]; then
  CRAWCLAW_WORKSPACE_DIR="$CRAWCLAW_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$CRAWCLAW_HOME"
validate_mount_source_path "config directory" "$CRAWCLAW_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$CRAWCLAW_WORKSPACE_DIR"
validate_container_name "$CRAWCLAW_CONTAINER_NAME"
validate_image_name "$CRAWCLAW_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$CRAWCLAW_CONFIG_DIR" "$CRAWCLAW_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$CRAWCLAW_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$CRAWCLAW_WORKSPACE_DIR"

BUILD_ARGS=()
if [[ -n "${CRAWCLAW_DOCKER_APT_PACKAGES:-}" ]]; then
  BUILD_ARGS+=(--build-arg "CRAWCLAW_DOCKER_APT_PACKAGES=${CRAWCLAW_DOCKER_APT_PACKAGES}")
fi
if [[ -n "${CRAWCLAW_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "CRAWCLAW_EXTENSIONS=${CRAWCLAW_EXTENSIONS}")
fi

if [[ "$CRAWCLAW_IMAGE" == "crawclaw:local" ]]; then
  echo "Building image $CRAWCLAW_IMAGE ..."
  podman build -t "$CRAWCLAW_IMAGE" -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"}" "$REPO_PATH"
else
  if podman image exists "$CRAWCLAW_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $CRAWCLAW_IMAGE"
  else
    echo "Pulling image $CRAWCLAW_IMAGE ..."
    podman pull "$CRAWCLAW_IMAGE"
  fi
fi

ENV_FILE="$CRAWCLAW_CONFIG_DIR/.env"
if [[ ! -f "$ENV_FILE" ]]; then
...
EOF
  ...
  echo "Generated CRAWCLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
upsert_env_var "$ENV_FILE" "CRAWCLAW_PODMAN_CONTAINER" "$CRAWCLAW_CONTAINER_NAME"
upsert_env_var "$ENV_FILE" "CRAWCLAW_PODMAN_IMAGE" "$CRAWCLAW_IMAGE"

CONFIG_JSON="$CRAWCLAW_CONFIG_DIR/crawclaw.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  ...
  QUADLET_DST="$QUADLET_DIR/crawclaw.container"
  echo "Installing Quadlet to $QUADLET_DST ..."
  mkdir -p "$QUADLET_DIR"
  ensure_safe_existing_dir "quadlet directory" "$QUADLET_DIR"
  CRAWCLAW_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$CRAWCLAW_HOME")"
  CRAWCLAW_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$CRAWCLAW_CONFIG_DIR")"
  CRAWCLAW_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$CRAWCLAW_WORKSPACE_DIR")"
  CRAWCLAW_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$CRAWCLAW_IMAGE")"
  CRAWCLAW_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$CRAWCLAW_CONTAINER_NAME")"
  sed \
    -e "s|{{CRAWCLAW_HOME}}|$CRAWCLAW_HOME_ESCAPED|g" \
    -e "s|{{CRAWCLAW_CONFIG_DIR}}|$CRAWCLAW_CONFIG_ESCAPED|g" \
    -e "s|{{CRAWCLAW_WORKSPACE_DIR}}|$CRAWCLAW_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$CRAWCLAW_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$CRAWCLAW_CONTAINER_ESCAPED|g" \
    "$QUADLET_TEMPLATE" | write_file_atomically "$QUADLET_DST" 644

  if command -v systemctl >/dev/null 2>&1; then
    echo "Reloading and starting user service..."
    if systemctl --user daemon-reload && systemctl --user start crawclaw.service; then
      echo "Quadlet installed and service started."
    else
      echo "Quadlet installed, but automatic start failed." >&2
      echo "Try: systemctl --user daemon-reload && systemctl --user start crawclaw.service" >&2
