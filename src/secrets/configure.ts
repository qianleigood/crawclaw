...
function getSecretProviders(config: CrawClawConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: CrawClawConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function toSourceChoices(config: CrawClawConfig): Array<{ value: SecretRefSource; label: string }> {
...
async function promptOptionalPositiveInt(params: {
  ...
  const parsed = parseOptionalPositiveInt(raw ?? "", params.max);
  return parsed;
}

function configureCandidateKey(candidate: {
  configFile: "crawclaw.json" | "auth-profiles.json";
  path: string;
  agentId?: string;
}): string {
  if (candidate.configFile === "auth-profiles.json") {
    return `auth-profiles:${(candidate.agentId ?? "").trim()}:${candidate.path}`;
  }
  return `crawclaw:${candidate.path}`;
}
...
function resolveConfigureAgentId(config: CrawClawConfig, explicitAgentId?: string): string {
...
async function configureProvidersInteractive(config: CrawClawConfig): Promise<void> {
  while (true) {
...
export async function runSecretsConfigureInteractive(
  ...
  const stagedConfig = structuredClone(snapshot.runtimeConfig);
  ...
  const providerChanges = collectConfigureProviderChanges({
    original: snapshot.runtimeConfig,
    next: stagedConfig,
  });

  const selectedByPath = new Map<string, ConfigureCandidate & { ref: SecretRef }>();
  if (!params.providersOnly) {
    const configureAgentId = resolveConfigureAgentId(snapshot.runtimeConfig, params.agentId);
    ...
    const candidates = buildConfigureCandidatesForScope({
      config: stagedConfig,
      authoredCrawClawConfig: snapshot.resolved,
      authProfiles: {
        agentId: configureAgentId,
        store: authStore,
      },
    ...
    while (true) {
      ...
      const options = visibleCandidates.map((candidate) => ({
        ...
        hint: [
          candidate.configFile === "auth-profiles.json" ? "auth-profiles.json" : "crawclaw.json",
      ...
      const providerAlias = provider.trim();
