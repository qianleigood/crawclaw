  ...
  it.each([
    {
      ...
      setup: () => {
        const dir = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-catalog-"));
        const catalogPath = path.join(dir, "catalog.json");
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@crawclaw/demo-channel",
    ...
    },
    {
      ...
      setup: () => {
        const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-channel-catalog-state-"));
    ...
    },
    {
      ...
      setup: () => {
        const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-catalog-state-"));
  ...
  it.each([
    {
      ...
      setup: () => {
        const home = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-catalog-home-"));
        const catalogPath = path.join(home, "catalog.json");
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@crawclaw/env-demo-channel",
        ...
        return {
          env: {
            ...process.env,
            CRAWCLAW_PLUGIN_CATALOG_PATHS: "~/catalog.json",
            CRAWCLAW_HOME: home,
            HOME: home,
          },
    ...
    },
    {
      ...
      setup: () => {
        ...
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@crawclaw/default-env-demo",
