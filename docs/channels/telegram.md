...
```
    ...
    Telegram does **not** use `crawclaw channels login telegram`; configure token in config/env, then start gateway.
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `crawclaw doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `crawclaw doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `crawclaw logs --follow`
...
- DM messages can carry `message_thread_id`; CrawClaw routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    CrawClaw can stream partial replies in real time:
    ...
    - DM: CrawClaw keeps the same preview message and performs a final edit in place (no second message)
    - group/topic: CrawClaw keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), CrawClaw falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, CrawClaw skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, CrawClaw automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, CrawClaw retries as plain text.
...
```
    ...
    2. paste the code into the paired client
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - CrawClaw pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    - `~/.crawclaw/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, CrawClaw enqueues system events like:
...
```bash
crawclaw message send --channel telegram --target 123456789 --message "hi"
crawclaw message send --channel telegram --target @name --message "hi"
```

    Telegram polls use `crawclaw message poll` and support forum topics:

```bash
crawclaw message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
crawclaw message poll --channel telegram --target -1001234567890:topic:42 \
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat, so only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, CrawClaw preserves the topic for both the approval prompt and the post-approval follow-up. Exec approvals expire after 30 minutes by default.
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `crawclaw channels status` warns when config expects unmentioned group messages.
    - `crawclaw channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `crawclaw logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - Modern Node runtimes + custom fetch/proxy can trigger immediate abort behavior if AbortSignal types mismatch.
    - Some hosts resolve `api.telegram.org` to IPv6 first; broken IPv6 egress can cause intermittent Telegram API failures.
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, CrawClaw now retries these as recoverable network errors.
...
```
    ...
    - Current supported Node runtimes default to `autoSelectFamily=true` and `dnsResultOrder=ipv4first`.
    - If your host works better with IPv4-only behavior, force family selection:
...
```
    ...
    - Environment overrides (temporary):
      ...
      - `CRAWCLAW_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `crawclaw doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `crawclaw doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
- Multi-account precedence:
  ...
  - If neither is set, CrawClaw falls back to the first normalized account ID and `crawclaw doctor` warns.
...
- `channels.telegram.network.autoSelectFamily`: override Node autoSelectFamily (true=enable, false=disable). Defaults to enabled on the current supported Node runtime.
- `channels.telegram.network.dnsResultOrder`: override DNS result order (`ipv4first` or `verbatim`). Defaults to `ipv4first` on the current supported Node runtime.
