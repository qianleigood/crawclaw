...
describe("bundle plugin hooks", () => {
  ...
  function createConfig(enabled: boolean): CrawClawConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("crawclaw-plugin");
  ...
  it("does not treat Claude hooks.json bundles as CrawClaw hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".crawclaw", "extensions", "claude-bundle");
