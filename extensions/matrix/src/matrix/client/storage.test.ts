...
const createBackupArchiveMock = vi.hoisted(() =>
  vi.fn(async (_params: unknown) => ({
    ...
    archiveRoot: "2026-03-17-crawclaw-backup",
...
describe("matrix client storage paths", () => {
  ...
  function setupStateDir(
    ...
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-matrix-storage-"));
    const stateDir = path.join(homeDir, ".crawclaw");
  ...
  function createMigrationEnv(stateDir: string): NodeJS.ProcessEnv {
    return {
      ...
      CRAWCLAW_HOME: path.dirname(stateDir),
      CRAWCLAW_STATE_DIR: stateDir,
      CRAWCLAW_TEST_FAST: "1",
