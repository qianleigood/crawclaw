...
// Keep this mirror aligned with crawclaw/acpx src/agent-registry.ts built-ins.
const ACPX_STATIC_BUILTIN_AGENT_COMMANDS: Record<string, string> = {
...
function resolveNearestCrawClawEntrypoint(moduleUrl: string = import.meta.url): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (;;) {
    const candidate = path.join(cursor, "crawclaw.mjs");
...
const COLOCATED_CRAWCLAW_ENTRYPOINT = resolveNearestCrawClawEntrypoint();
...
export const __testing = {
  quoteCommandPart,
  resolveNearestCrawClawEntrypoint,
};
...
function resolveBuiltInAgentCommand(agent: string): string | null {
  if (agent === "crawclaw") {
    if (COLOCATED_CRAWCLAW_ENTRYPOINT) {
      return toCommandLine([process.execPath, COLOCATED_CRAWCLAW_ENTRYPOINT, "acp"]);
    }
    return "crawclaw acp";
  }
  return ACPX_STATIC_BUILTIN_AGENT_COMMANDS[agent] ?? null;
}
...
export async function resolveAcpxAgentCommand(params: {
  ...
  return overrides[normalizedAgent] ?? resolveBuiltInAgentCommand(normalizedAgent);
}
