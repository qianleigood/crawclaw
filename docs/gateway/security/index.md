...
CrawClaw is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
**On this page:** [Trust model](#scope-first-personal-assistant-security-model) | [Quick audit](#quick-check-crawclaw-security-audit) | [Hardened baseline](#hardened-baseline-in-60-seconds) | [DM access model](#dm-access-model-pairing--allowlist--open--disabled) | [Configuration hardening](#configuration-hardening-examples) | [Incident response](#incident-response)

## Scope first: personal assistant security model

CrawClaw security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
CrawClaw is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
CrawClaw assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.crawclaw`, including `crawclaw.json`), treat them as a trusted operator.
...
4. Claim is not listed in [Out of Scope](https://github.com/qianleigood/crawclaw/blob/main/SECURITY.md#out-of-scope).
...
CrawClaw separates two concepts:
...
| ------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                           | critical      | Other users/processes can modify full CrawClaw state                                 | filesystem perms on `~/.crawclaw`                                                                    | yes      |
| `fs.config.perms_writable`                                    | critical      | Others can change auth/tool policy/config                                            | filesystem perms on `~/.crawclaw/crawclaw.json`                                                      | yes      |
...
CrawClaw stores session transcripts on disk under `~/.crawclaw/agents/<agentId>/sessions/*.jsonl`.
...
By default, CrawClaw routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
- “Paste the full contents of ~/.crawclaw or your logs.”

## Unsafe external content bypass flags

CrawClaw includes explicit bypass flags that disable external-content safety wrapping:
...
paths, set `CRAWCLAW_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `CRAWCLAW_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `CRAWCLAW_GATEWAY_PASSWORD`).
2. Restart the Gateway (or restart any local supervisor that manages it).
3. Update any remote clients (`gateway.remote.token` / `.password` on machines that call into the Gateway).
4. Verify you can no longer connect with the old credentials.

### 0.6) Tailscale Serve identity headers

When `gateway.auth.allowTailscale` is `true` (default for Serve), CrawClaw
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. CrawClaw verifies the identity by resolving the
...
- On the OpenAI-compatible HTTP surface, shared-secret bearer auth restores the full default operator scopes and owner semantics for agent turns; narrower `x-crawclaw-scopes` values do not reduce that shared-secret path.
...
CrawClaw’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default CrawClaw limits these tools
...
- Timestamp, gateway host OS + CrawClaw version
...
Found a vulnerability in CrawClaw? Please report responsibly:

1. Email: [security@crawclaw.ai](mailto:security@crawclaw.ai)
