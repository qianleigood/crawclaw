/**
 * Standalone MCP server that exposes CrawClaw plugin-registered tools
 * (for example provider-owned recall/store/forget tools)
...
function resolveTools(config: CrawClawConfig): AnyAgentTool[] {
...
export function createPluginToolsMcpServer(
  ...
  const server = new Server(
    { name: "crawclaw-plugin-tools", version: VERSION },
    { capabilities: { tools: {} } },
  );
