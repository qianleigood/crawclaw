...
const FALLBACK_GATEWAY_CONTEXT_STATE_KEY: unique symbol = Symbol.for(
  "crawclaw.fallbackGatewayContextState",
);
...
const PLUGIN_SUBAGENT_POLICY_STATE_KEY: unique symbol = Symbol.for(
  "crawclaw.pluginSubagentOverridePolicyState",
);
  ...
  if (!policy?.allowModelOverride) {
    return {
      ...
      reason:
        ...
        "See https://docs.crawclaw.ai/tools/plugin#runtime-helpers and search for: " +
    ...
    async run(params) {
      ...
      const payload = await dispatchGatewayMethod<{ runId?: string }>(
        "agent",
        {
          ...
          ...(params.toolsAllow?.length ? { toolsAllow: params.toolsAllow } : {}),
          ...(params.skillsAllow?.length ? { skillsAllow: params.skillsAllow } : {}),
