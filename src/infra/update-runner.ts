...
const DEFAULT_PACKAGE_NAME = "crawclaw";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, [DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-update-preflight-"));
