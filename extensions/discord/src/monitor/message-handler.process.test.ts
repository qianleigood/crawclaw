import { DEFAULT_EMOJIS } from "crawclaw/plugin-sdk/channel-feedback";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/crawclaw-discord-process-test-sessions.json",
  ),
}));
...
const replyRuntimeModule = await import("crawclaw/plugin-sdk/reply-runtime");
...
const conversationRuntimeModule = await import("crawclaw/plugin-sdk/conversation-runtime");
...
const configRuntimeModule = await import("crawclaw/plugin-sdk/config-runtime");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/crawclaw-discord-process-test-sessions.json");
