...
CrawClaw defaults to **host-local time for transport timestamps** and **user timezone only in the system prompt**.
...
When `timeFormat: "auto"`, CrawClaw inspects the OS preference (macOS/Windows)
...
- Feishu: epoch-like strings from the API
- community chat: UTC ISO timestamps
- Feishu/Weixin: provider-specific numeric/ISO timestamps
