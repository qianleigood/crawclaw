...
summary: CrawClaw 在线状态条目如何生成、合并和显示
title: 在线状态
x-i18n:
...
CrawClaw"在线状态"是以下内容的轻量级、尽力而为的视图：

- **Gateway 网关**本身，以及
- **连接到 Gateway 网关的客户端**（CLI、自动化、浏览器来源客户端等）

在线状态主要用于为运维人员提供快速可见性。
...
客户端可以渲染 `system-presence` 的输出，并根据最后更新的时间应用状态指示器（活跃/空闲/过期）。
