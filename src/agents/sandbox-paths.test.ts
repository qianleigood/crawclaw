...
async function withOutsideHardlinkInCrawClawTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside crawclaw tmp root",
      media: path.join(os.tmpdir(), "outside-crawclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked CrawClaw tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked CrawClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInCrawClawTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked CrawClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInCrawClawTmp(
