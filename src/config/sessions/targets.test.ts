...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): CrawClawConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: CrawClawConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        CRAWCLAW_STATE_DIR: envStateDir,
      };
      const cfg: CrawClawConfig = {};
