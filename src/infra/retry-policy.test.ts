...
import { createMessageRetryRunner } from "./retry-policy.js";

const ZERO_DELAY_RETRY = { attempts: 3, minDelayMs: 0, maxDelayMs: 0, jitter: 0 };

async function runRetryCase(params: {
  runnerOptions: Parameters<typeof createMessageRetryRunner>[0];
  ...
  vi.useFakeTimers();
  const runner = createMessageRetryRunner(params.runnerOptions);
  const fn = vi.fn();
  const allRejects =
...
describe("createMessageRetryRunner", () => {
  afterEach(() => {
    vi.useRealTimers();
    vi.restoreAllMocks();
  });
  ...
  describe("strictShouldRetry", () => {
    it.each([
      ...
      {
        name: "retries wrapped network errors via .cause traversal",
        runnerOptions: {
          retry: { ...ZERO_DELAY_RETRY, attempts: 2 },
        },
        fnSteps: [
          {
            type: "reject" as const,
            value: Object.assign(new Error("Network request failed"), {
  ...
  it("honors nested retry_after hints before retrying", async () => {
    ...
    const runner = createMessageRetryRunner({
