...
export function resolveCompactionReserveTokensFloor(cfg?: CrawClawConfig): number {
...
export function shouldDisablePiAutoCompaction(params: {
  memoryRuntimeInfo?: MemoryRuntimeInfo;
}): boolean {
  return params.memoryRuntimeInfo?.ownsCompaction === true;
}

/** Disable Pi auto-compaction via settings when a memory runtime owns compaction. */
export function applyPiAutoCompactionGuard(params: {
  ...
  const disable = shouldDisablePiAutoCompaction({
    memoryRuntimeInfo: params.memoryRuntimeInfo,
  });
