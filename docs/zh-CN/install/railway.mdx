...
通过一键模板在 Railway 上部署 CrawClaw，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/crawclaw` — 控制面板 UI

## 你将获得

- 托管的 CrawClaw Gateway网关 + 控制面板 UI
- `/setup` 网页设置向导（无需终端命令）
...
- `CRAWCLAW_GATEWAY_TOKEN`（推荐；请视为管理员密钥）

## 设置流程

1. 访问 `https://<your-railway-domain>/setup` 并输入你的 `SETUP_PASSWORD`。
2. 选择模型/认证提供商并粘贴你的密钥。
3. （可选）添加 Feishu/QQBot/DingTalk 令牌。
4. 点击 **Run setup**。

如果 Feishu 私信设置为配对模式，设置向导可以批准配对码。

## 获取聊天令牌

### Feishu 机器人令牌

1. 在 Feishu 中给 `@BotFather` 发消息
2. 执行 `/newbot`
...
1. 前往 https://qqbot.com/developers/applications
...
这会导出你的 CrawClaw 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
