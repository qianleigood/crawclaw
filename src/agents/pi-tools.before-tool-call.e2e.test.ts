...
describe("before_tool_call loop detection behavior", () => {
  ...
  function expectCriticalLoopEvent(
    ...
    expect(loopEvent?.action).toBe(params.action);
  ...
  it("bridges loop tracking through runtime state when only runId identifies the session", async () => {
    registerAgentRuntimeRun({
      runId: "run-runtime-1",
    ...
    const runtimeAwareTool = createWrappedTool("read", execute, {
      agentId: "stale-agent",
      runId: "run-runtime-1",
      loopDetection: { enabled: true },
    });
    ...
    await runtimeAwareTool.execute("read-runtime-1", params, undefined, undefined);

    const sessionState = getDiagnosticSessionState({
      sessionKey: "agent-main",
      sessionId: "ephemeral-main",
    });
    expect(sessionState.sessionKey).toBe("agent-main");
    expect(sessionState.sessionId).toBe("ephemeral-main");
    expect(sessionState.state).toBe("processing");
    expect(sessionState.loopProgressHistory).toHaveLength(1);
    expect(sessionState.loopProgressHistory?.[0]?.toolName).toBe("read");
  });
  ...
  it("emits structured warning diagnostic events for ping-pong loops", async () => {
    await withToolLoopEvents(async (emitted) => {
      ...
      expect(loopEvent?.action).toBe("nudge");
...
describe("before_tool_call requireApproval handling", () => {
  ...
  it("blocks interactive approval for background runs without contacting gateway", async () => {
    hookRunner.runBeforeToolCall.mockResolvedValue({
      requireApproval: {
        title: "Background approval",
        description: "Should not prompt from background run",
      },
    });
    registerAgentRuntimeRun({
    ...
    const result = await runBeforeToolCallHook({
      toolName: "bash",
      params: {},
      ctx: { agentId: "worker", sessionKey: "agent:bg", runId: "background-run" },
    });

    expect(result.blocked).toBe(true);
    expect(result).toHaveProperty(
      "reason",
      "Plugin approval required, but interactive approvals are unavailable for background agent runs.",
    );
    expect(mockCallGateway).not.toHaveBeenCalled();
  });
  ...
  it("blocks interactive approval when browser client visibility is disabled", async () => {
    ...
    const result = await runBeforeToolCallHook({
      toolName: "bash",
      params: {},
      ctx: { agentId: "main", sessionKey: "agent:hidden", runId: "hidden-ui-run" },
    });
