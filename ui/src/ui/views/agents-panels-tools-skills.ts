...
function renderToolBadges(section: AgentToolSection, tool: AgentToolEntry) {
  ...
  if (source === "plugin" && pluginId) {
    ...
    badges.push(uiLiteral("core"));
  }
  if (tool.optional) {
    badges.push(uiLiteral("optional"));
  }
  if (badges.length === 0) {
    return nothing;
  }
...
function renderEffectiveToolBadge(tool: {
  ...
  if (tool.source === "plugin") {
    return tool.pluginId ? `${uiLiteral("Connected")}: ${tool.pluginId}` : uiLiteral("Connected");
  }
  if (tool.source === "channel") {
    return tool.channelId ? `${uiLiteral("Channel")}: ${tool.channelId}` : uiLiteral("Channel");
  }
  return uiLiteral("Built-in");
}
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; flex-wrap: wrap;">
        <div style="min-width: 0;">
          <div class="card-title">${uiLiteral("Tool Access")}</div>
          <div class="card-sub">
            Profile + per-tool overrides for this agent.
            <span class="mono">${enabledCount}/${toolIds.length}</span> ${uiLiteral("enabled")}.
          </div>
        </div>
        <div class="row" style="gap: 8px; flex-wrap: wrap;">
          <button class="btn btn--sm" ?disabled=${!editable} @click=${() => updateAll(true)}>
            ${uiLiteral("Enable All")}
          </button>
          <button class="btn btn--sm" ?disabled=${!editable} @click=${() => updateAll(false)}>
            ${uiLiteral("Disable All")}
          </button>
          <button
            class="btn btn--sm"
          ...
          >
            ${params.configSaving ? uiLiteral("Saving…") : uiLiteral("Save")}
          </button>
        </div>
      </div>
      ...
      ${!params.configForm
        ? html`
            <div class="callout info" style="margin-top: 12px">
              ${uiLiteral("Load the gateway config to adjust tool profiles.")}
            </div>
          `
        : nothing}
      ${hasAgentAllow
      ...
      ${params.toolsCatalogLoading && !params.toolsCatalogResult && !params.toolsCatalogError
        ? html`
            <div class="callout info" style="margin-top: 12px">${uiLiteral("Loading runtime tool catalog…")}</div>
          `
        : nothing}
      ${params.toolsCatalogError
        ? html`
            <div class="callout info" style="margin-top: 12px">
              ${uiLiteral("Could not load runtime tool catalog. Showing built-in fallback list instead.")}
            </div>
          `
        : nothing}
      ...
      <div class="agent-tools-meta" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">${uiLiteral("Profile")}</div>
          <div class="mono">${profile}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${uiLiteral("Source")}</div>
          <div>${profileSource}</div>
        </div>
        ${params.configDirty
          ? html`
              <div class="agent-kv">
                <div class="label">${uiLiteral("Status")}</div>
                <div class="mono">${uiLiteral("unsaved")}</div>
              </div>
            `
          : nothing}
      </div>
      ...
      <div style="margin-top: 18px;">
        <div class="label">${uiLiteral("Available Right Now")}</div>
        <div class="card-sub">
