const SETTINGS_KEY_PREFIX = "crawclaw.control.settings.v1:";
const ONBOARDING_PROGRESS_KEY_PREFIX = "crawclaw.control.onboarding.v1:";
const SETTINGS_KEY = "crawclaw.control.settings.v1";
const TOKEN_SESSION_KEY_PREFIX = "crawclaw.control.token.v1:";
...
function onboardingProgressKeyForGateway(gatewayUrl: string): string {
  return `${ONBOARDING_PROGRESS_KEY_PREFIX}${normalizeGatewayTokenScope(gatewayUrl)}`;
}
...
import { inferBasePathFromPathname, normalizeBasePath, type NavigationMode } from "./navigation.ts";
import {
...
export function loadSettings(): UiSettings {
  ...
  try {
    ...
    const settings = {
      ...
      uiMode: parsed.uiMode === "advanced" ? "advanced" : defaults.uiMode,
      theme,
      themeMode: mode,
      chatFocusMode:
...
export function loadOnboardingProgress(gatewayUrl: string): OnboardingProgress {
  const storage = getSafeLocalStorage();
  try {
    const raw = storage?.getItem(onboardingProgressKeyForGateway(gatewayUrl));
    if (!raw) {
    ...
    const completedAt = Object.fromEntries(
      ONBOARDING_STEP_ORDER.flatMap((step) =>
        typeof parsed?.completedAt?.[step] === "number" ? [[step, parsed.completedAt[step]]] : [],
      ),
    ) as OnboardingProgress["completedAt"];
    return {
      mode:
        parsed?.mode === "guided" || parsed?.mode === "completed" || parsed?.mode === "paused"
          ? parsed.mode
          : "paused",
      completedAt,
    };
...
export function saveOnboardingProgress(gatewayUrl: string, progress: OnboardingProgress) {
  const storage = getSafeLocalStorage();
  try {
    storage?.setItem(onboardingProgressKeyForGateway(gatewayUrl), JSON.stringify(progress));
  } catch {
    // best-effort
  }
}
...
function persistSettings(next: UiSettings) {
  ...
  const persisted: PersistedUiSettings = {
    ...
    uiMode: next.uiMode === "advanced" ? "advanced" : "simple",
  ...
  try {
    ...
    storage?.setItem(SETTINGS_KEY, serialized);
  } catch {
