---
summary: "How CrawClaw manages conversation sessions"
read_when:
...
CrawClaw organizes conversations into **sessions**. Each message is routed to a
...
- **Manual reset** -- type `/new` in chat. `/new <model>` also
  switches the model.
...
CrawClaw automatically bounds session storage over time. By default, it runs
