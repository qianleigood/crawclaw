...
function createTelegramCfg(botToken: string, enabled?: boolean): CrawClawConfig {
  return {
    channels: {
      telegram: {
        botToken,
...
function createMSTeamsCatalogEntry(): ChannelPluginCatalogEntry {
  return {
    id: "msteams",
    pluginId: "@crawclaw/msteams-plugin",
    meta: {
      id: "msteams",
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveConfigured: ({ cfg }: { cfg: CrawClawConfig }) =>
            ...
            credentials: [
              {
                ...
                inspect: ({ cfg }: { cfg: CrawClawConfig }) => ({
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveSelectionHint: async ({ cfg }: { cfg: CrawClawConfig }) =>
            ...
            textInputs: [
              {
                ...
                required: true,
                applySet: ({ cfg, value }: { cfg: CrawClawConfig; value: string }) =>
                  ({
                    ...cfg,
                    channels: {
                      ...cfg.channels,
...
function patchTelegramAdapter(overrides: ChannelSetupWizardAdapterPatch) {
  ...
  const patch = {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: CrawClawConfig }) => ({
        channel: "telegram",
...
async function expectQuickstartPickerSkipsWithoutRuntime() {
  ...
  await expect(
    runSetupChannels({} as CrawClawConfig, prompter, {
      quickstartDefaults: true,
    }),
  ).resolves.toEqual({} as CrawClawConfig);
...
vi.mock("./channel-setup/plugin-install.js", async (importOriginal) => {
  ...
  return {
    ...
    ensureChannelSetupPluginInstalled: vi.fn(async ({ cfg }: { cfg: CrawClawConfig }) => ({
      cfg,
      installed: true,
    })),
...
describe("setupChannels", () => {
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@crawclaw/msteams-plugin": { enabled: true },
          },
        },
      } as CrawClawConfig,
      prompter,
          ...
          registry.channels.push({
            ...
            plugin: {
              ...
              config: {
                ...
                resolveAccount: (cfg: CrawClawConfig, accountId: string) =>
                  (
                    cfg.channels?.msteams as
                      | {
                          accounts?: Record<string, Record<string, unknown>>;
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: CrawClawConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: CrawClawConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
