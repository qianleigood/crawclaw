...
describe("noteWorkspaceStatus", () => {
  it("does not emit compatibility notes unless warnings are supplied", async () => {
  ...
  it("includes imported plugin counts in the plugins note", async () => {
    ...
    try {
      const pluginCalls = noteSpy.mock.calls.filter(([, title]) => title === "Plugins");
      expect(pluginCalls).toHaveLength(1);
      expect(pluginCalls[0]?.[0] ?? "").toContain("Imported: 1");
    } finally {
      noteSpy.mockRestore();
    }
  });
  ...
  it("passes the shared status report into compatibility warnings", async () => {
    ...
    try {
      ...
      expect(compatibilityCalls[0]?.[0] ?? "").toContain("legacy-plugin is hook-only");
    } finally {
      noteSpy.mockRestore();
    }
  });
  ...
  it("adds TaskFlow recovery hints for broken blocked flows", async () => {
    ...
    try {
      ...
      expect(recoveryCalls[0]?.[0] ?? "").toContain("crawclaw tasks flow show <flow-id>");
    } finally {
      noteSpy.mockRestore();
    }
  });
