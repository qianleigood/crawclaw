...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: CrawClawConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<CrawClawConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<CrawClawConfig["agents"]>["list"]>[number]["tools"];
  }): CrawClawConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createCrawClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createCrawClawCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createCrawClawCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createCrawClawCodingTools({
      config: cfg,
    ...
    const bobTools = createCrawClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createCrawClawCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createCrawClawCodingTools({
      config: cfg,
