...
function createBundledBraveEntry(): PluginWebSearchProviderEntry {
  return {
    id: "brave",
    pluginId: "brave",
    label: "Brave Search",
...
describe("onboard-search provider resolution", () => {
  ...
  it("does not treat hard-disabled bundled providers as selectable credentials", async () => {
    const braveEntry = createBundledBraveEntry();
    mocks.resolvePluginWebSearchProviders.mockReturnValue([]);
    mocks.listBundledWebSearchProviders.mockReturnValue([braveEntry]);
    mocks.resolveBundledWebSearchPluginId.mockReturnValue("brave");
    ...
    expect(mod.hasExistingKey(cfg, "brave")).toBe(false);
    expect(mod.resolveExistingKey(cfg, "brave")).toBeUndefined();
    expect(mod.applySearchProviderSelection(cfg, "brave")).toBe(cfg);
  });
  ...
  it("defaults to a keyless provider when no search credentials exist", async () => {
    ...
    const result = await mod.setupSearch({} as CrawClawConfig, {} as never, prompter as never);
  ...
  it("keeps the legacy default onboarding search surface when no config is present", async () => {
    ...
    mocks.listBundledWebSearchProviders.mockReturnValue([customEntry, duckduckgoEntry, braveEntry]);
    mocks.resolvePluginWebSearchProviders.mockReturnValue([customEntry]);

    const options = mod.resolveSearchProviderOptions();

    expect(options.map((entry) => entry.id)).toEqual(["brave"]);
  });
});
