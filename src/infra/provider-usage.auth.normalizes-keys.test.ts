...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-provider-auth-suite-"));
  });
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".crawclaw");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".crawclaw", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      CRAWCLAW_STATE_DIR: path.join(home, ".crawclaw"),
      ...env,
    };
