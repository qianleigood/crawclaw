...
当你使用 `--channel` **显式**切换渠道时，CrawClaw 还会对齐安装方式：

- `dev` 确保有一个 git checkout（默认 `~/crawclaw`，可通过 `CRAWCLAW_GIT_DIR` 覆盖），
...
当你使用 `crawclaw update` 切换渠道时，CrawClaw 还会同步插件来源：
...
Beta 和 dev 构建可能**不**包含 节点主机发布。这没问题：
