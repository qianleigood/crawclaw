...
describe("loadAgentInspection", () => {
  it("loads inspection snapshot for a run id", async () => {
    const { state, request } = createState();
    const payload = { lookup: { runId: "run-1" }, runId: "run-1", refs: {}, warnings: [] };
    request.mockResolvedValue(payload);

    await loadAgentInspection(state, { runId: "run-1" });

    expect(request).toHaveBeenCalledWith("agent.inspect", { runId: "run-1" });
    expect(state.agentInspectionSnapshot).toEqual(payload);
    expect(state.agentInspectionRunId).toBe("run-1");
    expect(state.agentInspectionError).toBeNull();
    expect(state.agentInspectionLoading).toBe(false);
  });

  it("captures inspection request errors", async () => {
    const { state, request } = createState();
    request.mockRejectedValue(new Error("inspect unavailable"));

    await loadAgentInspection(state, { runId: "run-1" });

    expect(state.agentInspectionSnapshot).toBeNull();
    expect(state.agentInspectionError).toContain("inspect unavailable");
    expect(state.agentInspectionLoading).toBe(false);
  });
});

describe("resetAgentInspectionState", () => {
  it("clears cached inspection state", () => {
    const { state } = createState();
    state.agentInspectionLoading = true;
    state.agentInspectionError = "boom";
    state.agentInspectionRunId = "run-1";
    state.agentInspectionTaskId = "task-1";
    state.agentInspectionSnapshot = { lookup: {}, refs: {}, warnings: [] } as never;

    resetAgentInspectionState(state);

    expect(state.agentInspectionLoading).toBe(false);
    expect(state.agentInspectionError).toBeNull();
    expect(state.agentInspectionRunId).toBeNull();
    expect(state.agentInspectionTaskId).toBeNull();
    expect(state.agentInspectionSnapshot).toBeNull();
  });
});
