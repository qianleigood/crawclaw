...
/**
 * Return a path under `~/.crawclaw/qqbot`, creating it on demand.
 */
export function getQQBotDataDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".crawclaw", "qqbot", ...subPaths);
...
/**
 * Return a path under `~/.crawclaw/media/qqbot`, creating it on demand.
 *
 * Unlike `getQQBotDataDir`, this lives under CrawClaw's core media allowlist so
...
export function getQQBotMediaDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".crawclaw", "media", "qqbot", ...subPaths);
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }
  return dir;
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".crawclaw", "workspace", "qqbot");
