...
const createCrawClawToolsMock = vi.fn();
...
async function loadFreshInlineActionsModuleForTest() {
  ...
  vi.doMock("../../agents/crawclaw-tools.runtime.js", () => ({
    createCrawClawTools: (...args: unknown[]) => createCrawClawToolsMock(...args),
  }));
...
describe("handleInlineActions", () => {
  beforeEach(async () => {
    ...
    createCrawClawToolsMock.mockReset();
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(createCrawClawToolsMock).toHaveBeenCalledWith(
      expect.objectContaining({
