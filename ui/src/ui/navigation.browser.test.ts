...
function expectConfirmedGatewayChange(app: ReturnType<typeof mountApp>) {
  expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/crawclaw");
...
describe("control UI routing", () => {
  ...
  it("hydrates workflow management from the location", async () => {
    const app = mountApp("/workflows");
    await app.updateComplete;

    expect(app.tab).toBe("workflows");
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/crawclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/crawclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/crawclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__CRAWCLAW_CONTROL_UI_BASE_PATH__ = "/crawclaw";
    const app = mountApp("/crawclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/crawclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/crawclaw/sessions");
  });
  ...
  it("keeps the topbar and sidebar visible during guided onboarding", async () => {
    ...
    expect(shell?.classList.contains("shell--onboarding")).toBe(true);
    ...
    expect(getComputedStyle(topbar).display).not.toBe("none");
  ...
  it("hydrates token from query params and strips them", async () => {
    ...
    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("crawclaw.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "crawclaw.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/crawclaw" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("crawclaw.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/crawclaw",
    });
    expect(JSON.parse(localStorage.getItem("crawclaw.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/crawclaw";
  ...
  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/crawclaw#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/crawclaw");
    expect(app.settings.token).toBe("");
