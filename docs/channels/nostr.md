...
read_when:
  - You want CrawClaw to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables CrawClaw to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`crawclaw onboard`) and `crawclaw channels add` list optional channel plugins.
...
```bash
crawclaw plugins install @crawclaw/nostr
```

Use a local checkout (dev workflows):

```bash
crawclaw plugins install --link <path-to-local-nostr-plugin>
```
...
crawclaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
Profile data is published as a NIP-01 `kind:0` event. Manage it directly in config or through any client surface that exposes Nostr profile settings.
...
```json5
{
  channels: {
    nostr: {
      ...
      profile: {
        name: "crawclaw",
        displayName: "CrawClaw",
        about: "Personal assistant DM bot",
        picture: "https://example.com/avatar.png",
        banner: "https://example.com/banner.png",
        website: "https://example.com",
        nip05: "crawclaw@example.com",
        lud16: "crawclaw@example.com",
      },
    },
  },
}
