...
describe("applyExtraParamsToAgent", () => {
  function createOptionsCaptureAgent() {
    const calls: Array<
      (SimpleStreamOptions & { openaiWsWarmup?: boolean; toolChoice?: unknown }) | undefined
    > = [];
    const baseStreamFn: StreamFn = (_model, _context, options) => {
      calls.push(
        options as
          | (SimpleStreamOptions & { openaiWsWarmup?: boolean; toolChoice?: unknown })
          | undefined,
      );
  ...
  function runToolChoicePayloadMutationCase(params: {
    applyProvider: string;
    applyModelId: string;
    model: Model<"anthropic-messages"> | Model<"openai-completions"> | Model<"openai-responses">;
    extraParamsOverride?: Record<string, unknown>;
    payload?: Record<string, unknown>;
  }) {
    const payload = params.payload ?? {};
  ...
  it("passes configured toolChoice through stream options", () => {
    ...
    applyExtraParamsToAgent(agent, undefined, "anthropic", "claude-sonnet-4-6", {
      toolChoice: { type: "tool", name: "memory_note_write" },
    });
  ...
  it("injects configured toolChoice into anthropic-family payloads", () => {
    const payload = runToolChoicePayloadMutationCase({
      applyProvider: "anthropic",
      applyModelId: "claude-sonnet-4-6",
      model: {
        api: "anthropic-messages",
  ...
  it("injects prompt cache fields from stream-param style extra params for direct OpenAI responses", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "openai",
      applyModelId: "gpt-5",
      model: {
  ...
  it("suppresses prompt-cache writes while preserving inherited cache keys for direct OpenAI responses", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "openai",
      applyModelId: "gpt-5",
      model: {
  ...
  it("forces cacheRetention=none when skipCacheWrite is enabled", () => {
    ...
    applyExtraParamsToAgent(
      ...
      undefined,
      {
        ...
        provider: "anthropic",
        id: "claude-opus-4-6",
      } as unknown as import("../plugins/types.js").ProviderRuntimeModel,
    );
    ...
    expect(capturedOptions[0]?.cacheRetention).toBe("none");
  });
