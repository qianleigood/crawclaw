...
const TALK_CONFIG_DEVICE_PATH = path.join(
  os.tmpdir(),
  `crawclaw-talk-config-device-${process.pid}.json`,
);
...
async function withSpeechProviders<T>(
  providers: NonNullable<ReturnType<typeof createEmptyPluginRegistry>["speechProviders"]>,
  run: () => Promise<T>,
): Promise<T> {
...
describe("gateway talk.config", () => {
    ...
    try {
      await withServer(async (ws) => {
        await withSpeechProviders(
          [{ pluginId: "openai", source: "test", provider: buildOpenAISpeechProvider() }],
          async () => {
  ...
  it("resolves talk voice aliases case-insensitively and forwards output format", async () => {
    ...
    try {
      await withServer(async (ws) => {
        await withSpeechProviders(
          [{ pluginId: "elevenlabs", source: "test", provider: buildElevenLabsSpeechProvider() }],
          async () => {
            await connectOperator(ws, ["operator.read", "operator.write"]);
            const res = await fetchTalkSpeak(ws, {
              text: "Hello from talk mode.",
