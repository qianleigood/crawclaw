...
function pickPriority(item: UnifiedRankedItem): { source: DurableMemorySelectionItem["prioritySource"]; value: number } {
  ...
  if (typeof item.importance === "number" && Number.isFinite(item.importance) && item.importance > 0) {
    return { source: "importance", value: item.importance };
  }
  return { source: "score", value: item.score };
}

function compareSelection(left: DurableMemorySelectionItem, right: DurableMemorySelectionItem): number {
...
export function selectDurableMemories(input: DurableMemorySelectionInput): DurableMemorySelectionResult {
  const limit = clampLimit(input.limit);
  const items = input.items
    .map((item) => {
      const classification = classifyClaudeMemoryItem(item);
      const priority = pickPriority(item);
      if (classification.bucket !== "durable" || !classification.durableType) {return null;}
      const reasons = [
