...
This document describes how the CrawClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`crawclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
CrawClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing CrawClaw Gateway
...
- ACP client spawns `crawclaw acp` and speaks ACP messages over stdio.
...
`crawclaw acp` resolves the Gateway URL and auth from CLI flags or config:
