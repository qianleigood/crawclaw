...
#
# Suggested cron: */30 * * * * /home/admin/crawclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send CrawClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., crawclaw-alerts)
...
STATE_FILE="$HOME/.crawclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via CrawClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use crawclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via CrawClaw to $NOTIFY_PHONE..."
            crawclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! CrawClaw cannot refresh provider access until you run: claude setup-token" "urgent"
    exit 1
