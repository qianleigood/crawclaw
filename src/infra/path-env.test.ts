...
import { ensureCrawClawCliOnPath } from "./path-env.js";
...
describe("ensureCrawClawCliOnPath", () => {
  const envKeys = [
    "PATH",
    "CRAWCLAW_PATH_BOOTSTRAPPED",
    "CRAWCLAW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/crawclaw-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "crawclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureCrawClawCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  function resetBootstrapEnv(pathValue = "/usr/bin") {
    process.env.PATH = pathValue;
    delete process.env.CRAWCLAW_PATH_BOOTSTRAPPED;
    delete process.env.CRAWCLAW_ALLOW_PROJECT_LOCAL_BIN;
  ...
  it("prepends the bundled app bin dir when a sibling crawclaw exists", () => {
  ...
  it("is idempotent", () => {
    ...
    process.env.CRAWCLAW_PATH_BOOTSTRAPPED = "1";
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "crawclaw");
      ...
      if (envValue === undefined) {
        ...
        process.env.CRAWCLAW_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        const tmp = abs("/tmp/crawclaw-path/case-linuxbrew");
