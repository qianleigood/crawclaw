...
describe("loadVitestExperimentalConfig", () => {
  ...
  it("passes through the filesystem module cache path when provided", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          CRAWCLAW_VITEST_FS_MODULE_CACHE_PATH: "/tmp/crawclaw-vitest-cache",
        },
        "linux",
      ),
    ).toEqual({
      experimental: {
        fsModuleCache: true,
        fsModuleCachePath: "/tmp/crawclaw-vitest-cache",
      },
    });
  });
  ...
  it("enables import timing output and import breakdown reporting", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          CRAWCLAW_VITEST_IMPORT_DURATIONS: "true",
          CRAWCLAW_VITEST_PRINT_IMPORT_BREAKDOWN: "1",
        },
        "linux",
      ),
    ).toEqual({
