...
export const DEFAULT_NOTEBOOKLM_CLI: NotebookLmCliConfig = {
  ...
  notebookId: "",
  queryInstruction: [
    "你是 CrawClaw experience recall 的唯一语义排序层。",
    "请只基于当前 NotebookLM 笔记本中与当前问题直接相关的内容回答。",
    "使用简体中文。",
    "按当前问题的适用性从高到低排序；不要为了覆盖面返回泛相关经验。",
    "如果没有直接可用的经验，请返回空 JSON 数组或明确表示无相关经验。",
    "优先提炼成简短、可复用、可直接注入系统上下文的经验卡片内容。",
