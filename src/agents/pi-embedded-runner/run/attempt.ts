...
export async function runEmbeddedAttempt(
  ...
  return await withObservationContext(runObservation, async () => {
    ...
    try {
      ...
      const parentPromptToolNames = extractParentPromptToolNames(
        params.specialParentPromptEnvelope,
      );
      ...
      if (promptCapabilities.length > 0) {
        ...
        const seenCapabilities = new Set(
          runtimeCapabilities.map((cap) => cap.trim().toLowerCase()),
        );
      ...
      const effectivePromptMode = useMinimalPromptForAllowedTools
        ? ("minimal" as const)
        : promptMode;
