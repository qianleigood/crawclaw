...
function installDiscoveryHooks(state: DiscoveryState) {
  beforeEach(async () => {
    vi.resetModules();
    vi.doMock("crawclaw/plugin-sdk/agent-runtime", async () => {
    ...
    vi.doMock("crawclaw/plugin-sdk/provider-auth", async () => {
      const actual = await vi.importActual<object>("crawclaw/plugin-sdk/provider-auth");
      return {
        ...actual,
        ensureAuthProfileStore: ensureAuthProfileStoreMock,
        listProfilesForProvider: listProfilesForProviderMock,
