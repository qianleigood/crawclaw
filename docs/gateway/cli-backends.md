...
```bash
crawclaw agent --message "hi" --model claude-cli/opus-4.6
```

Codex CLI also works out of the box (via the bundled OpenAI plugin):

```bash
crawclaw agent --message "hi" --model codex-cli/gpt-5.4
```
...
- **No CrawClaw tools** (the CLI backend never receives tool calls). Some CLIs
