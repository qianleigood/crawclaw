...
import { planCrawClawModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  ...
  sourceConfigForSecrets: CrawClawConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: CrawClawConfig): {
  config: CrawClawConfig;
  sourceConfigForSecrets: CrawClawConfig;
} {
...
export async function ensureCrawClawModelsJson(
  config?: CrawClawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveCrawClawAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planCrawClawModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
