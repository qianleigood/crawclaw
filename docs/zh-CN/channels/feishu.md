...
飞书（Lark）是企业用于消息沟通与协作的团队聊天平台。此插件通过平台的 WebSocket 事件订阅将 CrawClaw 连接到飞书/Lark 机器人，因此无需暴露公共 webhook URL 即可接收消息。

---

## 捆绑插件

飞书随当前的 CrawClaw 版本一同捆绑提供，因此无需单独安装插件。
...
CrawClaw 现在支持两条并存的飞书执行面：

- `extensions/feishu` = 机器人 / 应用身份，用于渠道回复、卡片、@ 提及和流式发送
- `extensions/feishu-cli` = 通过官方 `lark-cli` 使用用户身份，适合个人日历、任务和消息搜索

这两条链不是互相替代的：

- 当 CrawClaw 需要作为飞书机器人出现时，用 bot 插件
- 当 CrawClaw 需要“以登录用户本人身份”执行飞书操作时，用 Feishu CLI

Feishu CLI 用户层的配置路径是：
...
1. 你已经为飞书运行过 `crawclaw channels add`
2. Gateway 网关正在运行（`crawclaw gateway status`）
...
除了允许群组本身外，该群组中的**所有消息**还会按发送者 `open_id` 进行限制：只有列在 `groups.<chat_id>.allowFrom` 中的用户，其消息才会被处理；其他成员发送的消息会被忽略（这是完整的发送者级限制，而不只是对 `/new` 等控制命令生效）。
...
2. 运行 `crawclaw logs --follow` 并查找 `chat_id`
...
2. 运行 `crawclaw logs --follow` 并查找 `open_id`
...
| Command                    | Description                |
| -------------------------- | -------------------------- |
| `crawclaw gateway status`  | 显示 Gateway 网关状态      |
| `crawclaw gateway install` | 安装/启动 Gateway 网关服务 |
| `crawclaw gateway stop`    | 停止 Gateway 网关服务      |
| `crawclaw gateway restart` | 重启 Gateway 网关服务      |
| `crawclaw logs --follow`   | 跟踪 Gateway 网关日志      |
...
3. 确保已启用**长连接**
4. 确保应用权限完整
5. 确保 Gateway 网关正在运行：`crawclaw gateway status`
6. 检查日志：`crawclaw logs --follow`
...
对支持卡片的回复，流式模式默认开启；但在以下场景，CrawClaw 会自动降级为非流式发送：

- `renderMode: "raw"` 强制使用纯文本输出
- 回复已经处于飞书话题/线程上下文中，此时直接回复元数据比流式卡片更可靠
