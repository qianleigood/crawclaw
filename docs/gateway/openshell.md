---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for CrawClaw agents"
read_when:
...
OpenShell is a managed sandbox backend for CrawClaw. Instead of running Docker
containers locally, CrawClaw delegates sandbox lifecycle to the `openshell` CLI,
...
- CrawClaw Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, CrawClaw creates an OpenShell
...
- You edit files locally outside CrawClaw and want those changes visible in the
...
- When the sandbox is first created, CrawClaw seeds the remote workspace from
...
- CrawClaw does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside CrawClaw after the initial seed,
the remote sandbox does **not** see those changes. Use
`crawclaw sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"crawclaw"`  | Sandbox source for first-time create                  |
...
1. CrawClaw calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. CrawClaw calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `crawclaw sandbox` commands
