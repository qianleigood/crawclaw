...
export async function getReplyFromConfig(
  ...
  const cfg =
    configOverride == null
      ? loadConfig()
      : (applyMergePatch(loadConfig(), configOverride) as CrawClawConfig);
  ...
  const maybeEmitMissingResetHooks = async () => {
    ...
    const resetMatch = command.commandBodyNormalized.match(/^\/new(?:\s|$)/);
    if (!resetMatch) {
      return;
    }
    const { emitResetCommandHooks } = await import("./commands-core.runtime.js");
    const action: ResetCommandAction = "new";
