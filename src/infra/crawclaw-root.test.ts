...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__crawclaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "crawclaw-root");
...
function setPackageRoot(root: string, name = "crawclaw") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./crawclaw-root.js").resolveCrawClawPackageRootSync,
  asyncResolver: typeof import("./crawclaw-root.js").resolveCrawClawPackageRoot,
  opts: Parameters<typeof import("./crawclaw-root.js").resolveCrawClawPackageRootSync>[0],
...
describe("resolveCrawClawPackageRoot", () => {
  let resolveCrawClawPackageRoot: typeof import("./crawclaw-root.js").resolveCrawClawPackageRoot;
  let resolveCrawClawPackageRootSync: typeof import("./crawclaw-root.js").resolveCrawClawPackageRootSync;

  beforeEach(async () => {
    vi.resetModules();
    ({ resolveCrawClawPackageRoot, resolveCrawClawPackageRootSync } =
      await import("./crawclaw-root.js"));
    state.entries.clear();
  ...
  it.each([
    ...
    {
      name: "falls through from a non-crawclaw moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-crawclaw");
    ...
    {
      name: "returns null for non-crawclaw package roots",
      setup: () => {
        const pkgRoot = fx("not-crawclaw");
        setPackageRoot(pkgRoot, "not-crawclaw");
