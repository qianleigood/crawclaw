...
export function parseCrawClawVersion(raw: string | null | undefined): CrawClawVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeCrawClawVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseCrawClawVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameCrawClawStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseCrawClawVersion(a);
  const parsedB = parseCrawClawVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareCrawClawVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseCrawClawVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseCrawClawVersion(current);
  const parsedTouched = parseCrawClawVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameCrawClawStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareCrawClawVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: CrawClawVersion): number {
