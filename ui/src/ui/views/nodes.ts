  ...
  return html`
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${uiLiteral("Nodes")}</div>
          <div class="card-sub">${uiLiteral("Paired devices and live links.")}</div>
        </div>
      ...
      <div class="list" style="margin-top: 16px;">
        ${props.nodes.length === 0
          ? html` <div class="muted">${uiLiteral("No nodes found.")}</div> `
...
function renderDevices(props: NodesProps) {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${uiLiteral("Devices")}</div>
          <div class="card-sub">${uiLiteral("Pairing requests + role tokens.")}</div>
        </div>
        <button class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
          ${props.devicesLoading ? uiLiteral("Loading…") : uiLiteral("Refresh")}
        </button>
      </div>
      ${props.devicesError
      ...
      <div class="list" style="margin-top: 16px;">
        ${pending.length > 0
          ? html`
              <div class="muted" style="margin-bottom: 8px;">${uiLiteral("Pending")}</div>
              ${pending.map((req) => renderPendingDevice(req, props))}
            `
          : nothing}
        ${paired.length > 0
          ? html`
              <div class="muted" style="margin-top: 12px; margin-bottom: 8px;">${uiLiteral("Paired")}</div>
              ${paired.map((device) => renderPairedDevice(device, props))}
            `
          : nothing}
        ${pending.length === 0 && paired.length === 0
          ? html` <div class="muted">${uiLiteral("No paired devices.")}</div> `
          : nothing}
      </div>
    </section>
  `;
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const age = typeof req.ts === "number" ? formatRelativeTimestamp(req.ts) : uiLiteral("n/a");
  const roleValue = req.role?.trim() || formatList(req.roles);
  const scopesValue = formatList(req.scopes);
  const repair = req.isRepair ? ` · ${uiLiteral("repair")}` : "";
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="muted" style="margin-top: 6px;">
          ${uiLiteral("role")}: ${roleValue} · ${uiLiteral("scopes")}: ${scopesValue} · ${uiLiteral("requested")} ${age}${repair}
        </div>
      </div>
      <div class="list-meta">
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        ${tokens.length === 0
          ? html` <div class="muted" style="margin-top: 6px">${uiLiteral("Tokens")}: ${uiLiteral("none")}</div> `
          : html`
              <div class="muted" style="margin-top: 10px;">${uiLiteral("Tokens")}</div>
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? uiLiteral("revoked") : uiLiteral("active");
  const scopes = `${uiLiteral("scopes")}: ${formatList(token.scopes)}`;
