...
Read-only diff viewer plugin for **CrawClaw** agents.
...
Set plugin-wide defaults in `~/.crawclaw/crawclaw.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/crawclaw",
        },
      },
    },
  },
...
Before:
CrawClaw supports plugins.

After:
CrawClaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/crawclaw-diffs`).
