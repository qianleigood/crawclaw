...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["crawclaw_service_marker"];
  const kindKeys = ["crawclaw_service_kind"];
...
function isCanonicalGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.crawclaw.");
}

function isCanonicalGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("crawclaw-gateway")) {
    return false;
  }
...
function isCanonicalGatewayTaskName(name: string): boolean {
  ...
  return normalized === defaultName || normalized.startsWith("crawclaw gateway");
}
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (marker === "crawclaw" && isCanonicalGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isCanonicalGatewayTaskName(name)) {
        continue;
      }
