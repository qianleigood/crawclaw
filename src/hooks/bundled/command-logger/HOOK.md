...
2. **Appends to log file** - Writes a JSON line to `~/.crawclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.crawclaw/logs/commands.log ~/.crawclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/crawclaw`:
   ```
   /home/username/.crawclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
