...
async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-pdf-"));
  try {
...
function withDefaultModel(primary: string): CrawClawConfig {
  return {
    agents: { defaults: { model: { primary } } },
  } as CrawClawConfig;
}

function withPdfModel(primary: string): CrawClawConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  vi.spyOn(modelsConfig, "ensureCrawClawModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });
...
describe("parsePageRange", () => {
  it.each([
    ["parses a single page number", "3", 20, [3]],
    ["parses a page range", "1-5", 20, [1, 2, 3, 4, 5]],
    ["parses comma-separated pages and ranges", "1,3,5-7", 20, [1, 3, 5, 6, 7]],
    ["clamps to maxPages", "1-100", 5, [1, 2, 3, 4, 5]],
    ["deduplicates and sorts", "5,3,1,3,5", 20, [1, 3, 5]],
    ["handles empty parts gracefully", "1,,3", 20, [1, 3]],
  ])("%s", (_name, input, maxPages, expected) => {
    expect(parsePageRange(input, maxPages)).toEqual(expected);
  });

  it.each([
    ["throws on invalid page number", "abc", "Invalid page number"],
    ["throws on invalid range (start > end)", "5-3", "Invalid page range"],
    ["throws on zero page number", "0", "Invalid page number"],
    ["throws on negative page number", "-1", "Invalid page number"],
  ])("%s", (_name, input, expectedError) => {
    expect(() => parsePageRange(input, 20)).toThrow(expectedError);
  });
});
...
describe("providerSupportsNativePdf", () => {
  it.each([
    ...
    ["Anthropic", true],
    ["GOOGLE", true],
  ])("returns %s support as %s", (provider, expected) => {
    expect(providerSupportsNativePdf(provider)).toBe(expected);
  });
});
...
describe("resolvePdfModelConfigForTool", () => {
  ...
  it.each([
    {
      name: "returns null without any auth",
      cfg: { agents: { defaults: { model: { primary: "openai/gpt-5.4" } } } } as CrawClawConfig,
      expected: null,
    },
    {
      name: "prefers explicit pdfModel config",
      cfg: {
        agents: {
          defaults: {
      ...
      expected: { primary: "anthropic/claude-opus-4-6" },
    },
    {
      name: "falls back to imageModel config when no pdfModel set",
      cfg: {
        agents: {
          defaults: {
            model: { primary: "openai/gpt-5.4" },
      ...
      expected: { primary: "openai/gpt-5-mini" },
    },
  ])("$name", async ({ cfg, expected }) => {
    await withTempAgentDir(async (agentDir) => {
      expect(resolvePdfModelConfigForTool({ cfg, agentDir })).toEqual(expected);
    });
  });
...
describe("createPdfTool", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempAgentDir(async (agentDir) => {
      vi.stubEnv("ANTHROPIC_API_KEY", "anthropic-test");
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-pdf-ws-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-pdf-out-"));
      try {
        const cfg = withDefaultModel(ANTHROPIC_PDF_MODEL);
