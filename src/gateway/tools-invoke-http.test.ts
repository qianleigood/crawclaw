...
let lastCreateCrawClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../config/sessions.js", () => ({
  resolveMainSessionKey: (params?: {
    ...
    const agentId = (rawDefault ?? "main").trim().toLowerCase() || "main";
...
vi.mock("../agents/crawclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateCrawClawToolsContext?.agentTo,
          agentThreadId: lastCreateCrawClawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createCrawClawTools: (ctx: Record<string, unknown>) => {
      lastCreateCrawClawToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateCrawClawToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-crawclaw-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-crawclaw-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateCrawClawToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-crawclaw-message-to": "channel:24514",
        "x-crawclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
