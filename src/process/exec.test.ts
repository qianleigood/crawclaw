...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        CRAWCLAW_BASE_ENV: "base",
        CRAWCLAW_TO_REMOVE: undefined,
      },
      env: {
        CRAWCLAW_TEST_ENV: "ok",
      },
    });

    expect(resolved.CRAWCLAW_BASE_ENV).toBe("base");
    expect(resolved.CRAWCLAW_TEST_ENV).toBe("ok");
    expect(resolved.CRAWCLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.CRAWCLAW_CLI).toBe(CRAWCLAW_CLI_ENV_VALUE);
  });
