...
describe("createCrawClawCodingTools", () => {
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-read-"));
    try {
      ...
      const contents = "Hello from crawclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "crawclaw-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "crawclaw-workspace"),
    ...
    const tools = createCrawClawCodingTools({ sandbox });
