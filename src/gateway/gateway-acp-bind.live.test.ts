...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.CRAWCLAW_LIVE_ACP_BIND);
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.CRAWCLAW_GATEWAY_PORT,
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "crawclaw.json");
      ...
      try {
        ...
        if (previous.port === undefined) {
          ...
          process.env.CRAWCLAW_GATEWAY_PORT = previous.port;
        }
