...
export const POSIX_CRAWCLAW_TMP_DIR = "/tmp/crawclaw";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredCrawClawTmpDirOptions = {
...
export function resolvePreferredCrawClawTmpDir(
  options: ResolvePreferredCrawClawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "crawclaw" : `crawclaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[crawclaw] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback CrawClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback CrawClaw temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_CRAWCLAW_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_CRAWCLAW_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_CRAWCLAW_TMP_DIR)) {
      return POSIX_CRAWCLAW_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_CRAWCLAW_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_CRAWCLAW_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_CRAWCLAW_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_CRAWCLAW_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_CRAWCLAW_TMP_DIR;
