...
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://crawclaw.ai/install.sh"
HOST_PORT="18425"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/crawclaw-parallels-smoke.XXXXXX)"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://crawclaw.ai/install.sh
...
spawn {*}$cmd
send -- "printf '__CRAWCLAW_READY__\\n'\r"
expect "__CRAWCLAW_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/crawclaw-prl.sh <<'__CRAWCLAW_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__CRAWCLAW_SCRIPT__\r"
send -- "/bin/bash /tmp/crawclaw-prl.sh; rc=\$?; rm -f /tmp/crawclaw-prl.sh; printf '__CRAWCLAW_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
expect {
  -re {__CRAWCLAW_RC__:(-?[0-9]+)} {
...
export CRAWCLAW_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/crawclaw-install.sh
bash /tmp/crawclaw-install.sh${version_arg_q}
$GUEST_CRAWCLAW_BIN --version
EOF
)"
}
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_CRAWCLAW_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
start_server() {
  ...
  ) >/tmp/crawclaw-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_gateway() {
  guest_current_user_cli "$GUEST_CRAWCLAW_BIN" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_cli "$GUEST_CRAWCLAW_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_cli "$GUEST_CRAWCLAW_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_cli "$GUEST_CRAWCLAW_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_cli "$GUEST_CRAWCLAW_BIN" models set "$MODEL_ID"
  guest_current_user_cli \
    /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" \
    "$GUEST_CRAWCLAW_BIN" agent \
    --agent main \
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/crawclaw-discord-token <<'__CRAWCLAW_TOKEN__'
$DISCORD_TOKEN_VALUE
__CRAWCLAW_TOKEN__
cat >/tmp/crawclaw-discord-guilds.json <<'__CRAWCLAW_GUILDS__'
$guilds_json
__CRAWCLAW_GUILDS__
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "crawclaw-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "crawclaw-main-upgrade.tgz"
