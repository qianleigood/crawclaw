...
import { runAcpRuntimeAdapterContract } from "crawclaw/plugin-sdk/testing";
import { afterAll, beforeAll, describe, expect, it, vi } from "vitest";
import { resolveAcpxPluginConfig } from "./config.js";
import { __testing as mcpAgentCommandTesting } from "./runtime-internals/mcp-agent-command.js";
...
describe("AcpxRuntime", () => {
  ...
  it("ensures sessions and streams prompt events", async () => {
    ...
    expect(prompt?.crawclawShell).toBe("acp");
  ...
  it("completes the turn after the first done event even if acpx keeps the prompt process open", async () => {
    const previousHangAfterDone = process.env.MOCK_ACPX_PROMPT_HANG_AFTER_DONE;
    process.env.MOCK_ACPX_PROMPT_HANG_AFTER_DONE = "1";

    try {
      const { runtime } = await createMockRuntimeFixture();
      const handle = await runtime.ensureSession({
        sessionKey: "agent:codex:acp:hang-after-done",
        agent: "codex",
        mode: "persistent",
      });
      ...
      for await (const event of runtime.runTurn({
        handle,
        text: "hang-after-done",
        mode: "prompt",
        requestId: "req-hang-after-done",
      })) {
        events.push(event);
      }
      ...
      if (previousHangAfterDone === undefined) {
        delete process.env.MOCK_ACPX_PROMPT_HANG_AFTER_DONE;
      } else {
        process.env.MOCK_ACPX_PROMPT_HANG_AFTER_DONE = previousHangAfterDone;
      }
    }
  });
  ...
  it("passes the resolved built-in crawclaw ACP command through --agent without MCP servers", async () => {
    const { runtime, logPath } = await createMockRuntimeFixture();

    await runtime.ensureSession({
      sessionKey: "agent:crawclaw:acp:resolved-built-in",
      agent: "crawclaw",
      mode: "persistent",
    });
    ...
    const entrypoint = mcpAgentCommandTesting.resolveNearestCrawClawEntrypoint();
    const expectedCommand = entrypoint
      ? [
          mcpAgentCommandTesting.quoteCommandPart(process.execPath),
          mcpAgentCommandTesting.quoteCommandPart(entrypoint),
          "acp",
        ].join(" ")
      : "crawclaw acp";

    expect(rawAgentCommand).toBe(expectedCommand);
    expect(ensureArgs).not.toContain("crawclaw");
  });
  ...
  it("routes ACPX commands through the built-in plugin-tools bridge only when explicitly enabled", async () => {
    ...
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-acpx-plugin-tools-"));
    ...
    try {
      ...
      fs.writeFileSync(path.join(pluginRoot, "crawclaw.plugin.json"), "{}\n", "utf8");
  ...
  it("does not mark backend unhealthy when a per-session cwd is missing", async () => {
    const { runtime } = await createMockRuntimeFixture();
    const missingCwd = path.join(os.tmpdir(), "crawclaw-acpx-runtime-test-missing-cwd");
