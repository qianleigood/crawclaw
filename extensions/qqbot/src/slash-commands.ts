...
import { resolveRuntimeServiceVersion } from "crawclaw/plugin-sdk/cli-runtime";
...
/**
 * /bot-ping — test current network latency between CrawClaw and QQ.
 */
registerCommand({
  name: "bot-ping",
  description: "测试 CrawClaw 与 QQ 之间的网络延迟",
  usage: [
    `/bot-ping`,
    ``,
    `测试当前 CrawClaw 宿主机与 QQ 服务器之间的网络延迟。`,
    `返回网络传输耗时和插件处理耗时。`,
  ].join("\n"),
  handler: (ctx) => {
    const now = Date.now();
...
/**
 * /bot-version — show the CrawClaw framework version.
 */
registerCommand({
  name: "bot-version",
  description: "查看 CrawClaw 框架版本",
...
function getConfiguredLogFiles(): string[] {
  ...
  const files: string[] = [];
  for (const cli of ["crawclaw", "clawdbot", "moltbot"]) {
    try {
...
function collectCandidateLogDirs(): string[] {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (!value) continue;
    if (/STATE_DIR$/i.test(key) && /(CRAWCLAW|CLAWDBOT|MOLTBOT)/i.test(key)) {
      pushStateDir(value);
    }
  }

  for (const name of [".crawclaw", ".clawdbot", ".moltbot", "crawclaw", "clawdbot", "moltbot"]) {
  ...
  for (const root of searchRoots) {
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(crawclaw|clawdbot|moltbot)/i.test(entry.name)) continue;
  ...
  if (!isWindows()) {
    for (const name of ["crawclaw", "clawdbot", "moltbot"]) {
  ...
  for (const dir of logDirs) {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(gateway|crawclaw|clawdbot|moltbot)/i.test(entry.name)) continue;
