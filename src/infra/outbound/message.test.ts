...
vi.mock("../../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: () => "/tmp/crawclaw-test-workspace",
}));
...
describe("sendMessage", () => {
  ...
  it("uses requester session and account for outbound delivery policy context", async () => {
    await sendMessage({
      cfg: {},
      channel: "telegram",
      to: "123456",
      content: "hi",
      requesterSessionKey: "agent:main:telegram:group:ops",
    ...
    expect(mocks.deliverOutboundPayloads).toHaveBeenCalledWith(
      expect.objectContaining({
        session: expect.objectContaining({
          key: "agent:main:telegram:group:ops",
          policyKey: "agent:main:telegram:group:ops",
          requesterAccountId: "work",
          requesterSenderId: "attacker",
        }),
