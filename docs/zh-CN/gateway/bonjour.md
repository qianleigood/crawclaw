---
read_when:
  - 在 macOS 或节点客户端上调试 Bonjour 设备发现问题时
...
CrawClaw 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现
...
2. 在专用区域下发布 `_crawclaw-gw._tcp` 的 DNS‑SD 记录
   （示例：`crawclaw.internal.`）。
3. 配置 Tailscale **分割 DNS**，使你选择的域名通过该
   DNS 服务器为客户端解析。

CrawClaw 支持任何发现域名；`crawclaw.internal.` 只是一个示例。
当前节点客户端同时浏览 `local.` 和你配置的广域域名。
...
- 从 `~/.crawclaw/dns/<domain>.db` 提供你选择的域名服务（示例：`crawclaw.internal.`）
...
一旦客户端接受 Tailnet DNS，节点客户端就可以在
你的发现域名中浏览 `_crawclaw-gw._tcp`，无需多播。
...
只有 Gateway 网关广播 `_crawclaw-gw._tcp`。

## 服务类型

- `_crawclaw-gw._tcp` — Gateway 网关传输信标（被 macOS 和节点客户端使用）。
...
- `transport=gateway`
- `cliPath=<path>`（可选；可运行的 `crawclaw` 入口点的绝对路径）
...
当前 Apple 平台节点客户端使用 `NWBrowser` 来发现 `_crawclaw-gw._tcp`。
...
- `CRAWCLAW_DISABLE_BONJOUR=1` 禁用广播（旧版：`CRAWCLAW_DISABLE_BONJOUR`）。
- `~/.crawclaw/crawclaw.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
- `CRAWCLAW_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（旧版：`CRAWCLAW_SSH_PORT`）。
- `CRAWCLAW_TAILNET_DNS` 在 TXT 中发布 MagicDNS 提示（旧版：`CRAWCLAW_TAILNET_DNS`）。
- `CRAWCLAW_CLI_PATH` 覆盖广播的 CLI 路径（旧版：`CRAWCLAW_CLI_PATH`）。
