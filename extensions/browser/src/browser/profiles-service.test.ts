...
vi.mock("./chrome.js", () => ({
  resolveCrawClawUserDataDir: vi.fn(() => "/tmp/crawclaw-test/crawclaw/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("allocates next local port for new profiles", async () => {
    ...
    expect(result.transport).toBe("pinchtab");
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "crawclaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveCrawClawUserDataDir).mockReturnValue(userDataDir);
