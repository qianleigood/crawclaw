...
function emitToolLifecycleEvent(params: {
  phase: "tool_call_start" | "tool_call_stop" | "tool_call_error";
  ...
  const sessionId =
    typeof params.ctx.params.sessionId === "string" && params.ctx.params.sessionId.trim()
      ? params.ctx.params.sessionId.trim()
      : params.ctx.params.runId;
  const decisionCode =
    params.phase === "tool_call_start"
      ? "tool_call_started"
      : params.phase === "tool_call_stop"
        ? "tool_call_succeeded"
        : params.timedOut
          ? "tool_call_timed_out"
          : "tool_call_failed";
  ...
  void emitRunLoopLifecycleEvent({
    ...
    decision: {
      ...
      details: {
        ...
        ...(params.isToolError !== undefined ? { isToolError: params.isToolError } : {}),
    ...
    refs: {
      ...
      toolCallId: params.toolCallId,
      isTopLevel: true,
      ...(params.meta ? { toolMeta: params.meta } : {}),
    },
  }).catch((err) => {
  ...
  const continueToolExecutionStart = async () => {
    ...
    if (isMessagingTool(toolName)) {
      ...
      if (isMessagingSend) {
        ...
        // Field names vary by tool: channel adapters use "content", sessions_send uses "message"
...
export async function handleToolExecutionEnd(
  ...
  const loadedSkillName = !isToolError
    ? inferLoadedSkillNameFromToolCall({
        toolName,
  ...
  const timedOut = isToolResultTimedOut(sanitizedResult);
  emitToolLifecycleEvent({
    phase: isToolError ? "tool_call_error" : "tool_call_stop",
    ctx,
    toolName,
    toolCallId,
    meta,
