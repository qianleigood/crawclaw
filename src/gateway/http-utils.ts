...
export const CRAWCLAW_MODEL_ID = "crawclaw";
export const CRAWCLAW_DEFAULT_MODEL_ID = "crawclaw/default";
...
export function resolveTrustedHttpOperatorScopes(
  ...
  const headerValue = getHeader(req, "x-crawclaw-scopes");
...
export function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    getHeader(req, "x-crawclaw-agent-id")?.trim() ||
    getHeader(req, "x-crawclaw-agent")?.trim() ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === CRAWCLAW_MODEL_ID || lowered === CRAWCLAW_DEFAULT_MODEL_ID) {
    return resolveDefaultAgentId(cfg);
  }

  const m =
    raw.match(/^crawclaw[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `crawclaw` or `crawclaw/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-crawclaw-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-crawclaw-model`." };
  }
...
export function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-crawclaw-session-key")?.trim();
  if (explicit) {
    return explicit;
  }
