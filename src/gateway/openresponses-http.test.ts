...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("CRAWCLAW_CONFIG_PATH is required for gateway config tests");
  }
...
async function postResponses(port: number, body: unknown, headers?: Record<string, string>) {
  const res = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
    ...
    headers: {
      ...
      "x-crawclaw-scopes": "operator.write",
      ...headers,
    },
...
describe("OpenResponses HTTP API (e2e)", () => {
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingScopes = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        method: "POST",
        headers: { "content-type": "application/json", "x-crawclaw-scopes": "" },
        body: JSON.stringify({ model: "crawclaw", input: "hi" }),
      });
      expect(resMissingScopes.status).toBe(403);
      await ensureResponseConsumed(resMissingScopes);
      ...
      const resHeader = await postResponses(
        port,
        { model: "crawclaw", input: "hi" },
        { "x-crawclaw-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "crawclaw/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "crawclaw/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "crawclaw", input: "hi" },
        { "x-crawclaw-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-crawclaw-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("treats HTTP callers as non-owner regardless of requested scopes", async () => {
    ...
    const adminScopeResponse = await postResponses(
      port,
      { model: "crawclaw", input: "hi" },
      { "x-crawclaw-scopes": "operator.admin, operator.write" },
    );
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "crawclaw", input: "hi" },
      { "x-crawclaw-scopes": "operator.admin, operator.write" },
    );
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        headers: {
          ...
          "content-type": "application/json",
          "x-crawclaw-scopes": "operator.approvals",
        },
        body: JSON.stringify({
          model: "crawclaw",
          input: "hi",
