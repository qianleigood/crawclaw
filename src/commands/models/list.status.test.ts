...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveCrawClawAgentDir: vi.fn().mockReturnValue("/tmp/crawclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/crawclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/crawclaw-agent/auth-profiles.json"),
    ...
    shouldDeferShellEnvFallback: vi.fn().mockReturnValue(false),
    resolveShellEnvFallbackTimeoutMs: vi.fn().mockReturnValue(15_000),
    loadShellEnvFallback: vi.fn().mockReturnValue({
      ok: true,
      applied: [],
      skippedReason: "already-has-keys",
    }),
    loadModelsConfig: vi.fn(async () => loadConfig()),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveCrawClawAgentDir: mocks.resolveCrawClawAgentDir,
  }));
  ...
  vi.doMock("../../infra/shell-env.js", async (importOriginal) => {
    ...
    return {
      ...
      shouldDeferShellEnvFallback: mocks.shouldDeferShellEnvFallback,
      resolveShellEnvFallbackTimeoutMs: mocks.resolveShellEnvFallbackTimeoutMs,
      loadShellEnvFallback: mocks.loadShellEnvFallback,
    };
  });
  ...
  vi.doMock("./load-config.js", async (importOriginal) => {
    const actual = await importOriginal<typeof import("./load-config.js")>();
    return {
      ...actual,
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/crawclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveCrawClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/crawclaw-dev/crawclaw.json");
    expect(payload.auth.storePath).toBe("/tmp/crawclaw-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/crawclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/crawclaw-agent-custom");
