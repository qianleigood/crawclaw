...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-crawclaw-native-e2e-2026-03-12"
MODE="both"
PROVIDER="openai"
API_KEY_ENV=""
AUTH_CHOICE=""
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://crawclaw.ai/install.ps1"
HOST_PORT="18426"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/crawclaw-parallels-windows.XXXXXX)"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-crawclaw-native-e2e-2026-03-12"
  ...
  --install-url <url>        Installer URL for latest release. Default: https://crawclaw.ai/install.ps1
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: crawclaw@2026.3.13-beta.1
...
# crawclaw.cmd preserves multi-word --message args reliably here; Start-Process
# against the shim can re-split argv and make Commander reject the turn.
\$output = & \$crawclaw @args 2>&1
if (\$null -ne \$output) {
...
matches = re.findall(r"(?:CrawClaw|CrawClaw) [^\r\n]+ \([0-9a-f]{7,}\)", text)
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "crawclaw-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_exec cmd.exe /d /s /c "if exist \"%LOCALAPPDATA%\\CrawClaw\\deps\\portable-git\" rmdir /s /q \"%LOCALAPPDATA%\\CrawClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "if not exist \"%LOCALAPPDATA%\\CrawClaw\\deps\" mkdir \"%LOCALAPPDATA%\\CrawClaw\\deps\""
  guest_exec cmd.exe /d /s /c "mkdir \"%LOCALAPPDATA%\\CrawClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "curl.exe -fsSL \"$mingit_url\" -o \"%TEMP%\\$MINGIT_ZIP_NAME\""
  guest_exec cmd.exe /d /s /c "tar.exe -xf \"%TEMP%\\$MINGIT_ZIP_NAME\" -C \"%LOCALAPPDATA%\\CrawClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "del /q \"%TEMP%\\$MINGIT_ZIP_NAME\" & set \"PATH=%LOCALAPPDATA%\\CrawClaw\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\CrawClaw\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\CrawClaw\\deps\\portable-git\\usr\\bin;%PATH%\" && git.exe --version"
}
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/crawclaw-main-$short_head.tgz"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/crawclaw-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_crawclaw "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
write_onboard_runner_script() {
  WINDOWS_ONBOARD_SCRIPT_PATH="$MAIN_TGZ_DIR/crawclaw-onboard-$PROVIDER.ps1"
...
run_ref_onboard() {
  ...
  runner_name="crawclaw-onboard-$RANDOM-$RANDOM.ps1"
  log_name="crawclaw-onboard-$RANDOM-$RANDOM.log"
  done_name="crawclaw-onboard-$RANDOM-$RANDOM.done"
...
verify_gateway() {
  guest_run_crawclaw "" "" gateway status --deep --require-rpc
}
...
show_gateway_status_compat() {
  if guest_run_crawclaw "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_crawclaw "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_crawclaw "" "" gateway status --deep
}

verify_turn() {
  guest_run_crawclaw "" "" models set "$MODEL_ID"
  guest_run_crawclaw "$API_KEY_ENV" "$API_KEY_VALUE" \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "crawclaw-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "crawclaw-main-upgrade.tgz" || return $?
