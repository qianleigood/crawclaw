...
describe("task-registry audit", () => {
  ...
  it("summarizes findings by severity and code", () => {
    const summary = summarizeTaskAuditFindings([
      ...
      },
      {
        ...
        task: createTask({ taskId: "c", agentMetadata: { parentAgentId: "parent-agent" } }),
        detail: "agentMetadata is present without agentId",
      },
    ]);
  ...
  it("flags agent metadata without a backing agentId", () => {
    const now = Date.parse("2026-03-30T00:00:00.000Z");
    const findings = listTaskAuditFindings({
      now,
      tasks: [
        createTask({
          taskId: "agent-metadata-without-agent",
          agentMetadata: {
    ...
    expect(findings.map((finding) => [finding.code, finding.task.taskId])).toEqual([
      ["invalid_agent_metadata", "agent-metadata-without-agent"],
    ]);
  });
});
