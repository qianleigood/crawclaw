...
const { buildExpectedNames, runSelfTest } = require("./grok_reference_upload");
const {
  nowIso,
  parseArgs,
  readJson,
...
async function main() {
  ...
  logger.info("reference.scan_completed", {
    ...
    message: references.length
      ...
      : "No staged references found.",
  });
  const previousStatus = readJson(job.files.statusStatePath, {}) || {};
  const blocked = Boolean(
    previousStatus.blocked || String(previousStatus.status || "").startsWith("blocked_"),
  );
