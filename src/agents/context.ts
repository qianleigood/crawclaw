...
let configuredConfig: CrawClawConfig | undefined;
...
function isLikelyCrawClawCliProcess(argv: string[] = process.argv): boolean {
...
function primeConfiguredContextWindows(): CrawClawConfig | undefined {
  if (configuredConfig) {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureCrawClawModelsJson(cfg);
    } catch {
