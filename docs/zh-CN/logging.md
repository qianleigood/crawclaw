...
所有日志配置都在 `~/.crawclaw/crawclaw.json` 的 `logging` 下。
...
- `channel.streaming.decision`：按渠道记录流式输出启用/降级决策，并带
  `surface` 与 `reason` 元数据。
...
- 渠道流式输出决策会作为指标 `crawclaw.channel.streaming.decision`
  导出，并携带 `crawclaw.channel`、`crawclaw.streaming.surface`、
  `crawclaw.streaming.reason`、`crawclaw.streaming.enabled` 等属性。
- 启用追踪时，CrawClaw 还会导出
  `crawclaw.channel.streaming.decision` span，便于把渠道侧的流式/降级行为
  与整条消息流追踪串起来。
...
- `crawclaw.channel.streaming.decision`
  ...
  - 可选：`crawclaw.accountId`、`crawclaw.sessionKey`、
...
- **Gateway 网关无法访问？** 先运行 `crawclaw doctor`。
