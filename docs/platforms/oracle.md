---
summary: "CrawClaw on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up CrawClaw on Oracle Cloud
  - Looking for low-cost VPS hosting for CrawClaw
  - Want 24/7 CrawClaw on a small server
title: "Oracle Cloud (Platform)"
---

# CrawClaw on Oracle Cloud (OCI)

## Goal

Run a persistent CrawClaw Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for CrawClaw (especially if you already have an OCI account), but it comes with tradeoffs:
...
This enables Tailscale SSH, so you can connect via `ssh crawclaw` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@crawclaw` (or use the Tailscale IP).
...
- `~/.crawclaw/` — config, credentials, session data
- `~/.crawclaw/workspace/` — workspace (SOUL.md, memory, artifacts)
