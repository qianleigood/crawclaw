...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      const memoryComplete = createEmbeddedMemoryCompleteFn({
      ...
      try {
        const emitOwnedCompactionLifecycle = async (event: {
          ...
          const postCompactArtifactSummary = summarizeCompactPostArtifacts(
            event.compactResult?.result?.postCompactArtifacts,
          );
          await emitRunLoopLifecycleEvent({
            ...
            messageCount: -1,
            ...(typeof event.compactResult?.result?.tokensAfter === "number"
              ? { tokenCount: event.compactResult.result.tokensAfter }
              : {}),
            metadata: {
              ...
              config: params.config,
              ...(event.phase === "post_compact"
                ? {
                    compactedCount: -1,
                    tokensBefore: event.compactResult?.result?.tokensBefore,
                    firstKeptEntryId: event.compactResult?.result?.firstKeptEntryId,
                    summaryLength:
                      typeof event.compactResult?.result?.summary === "string"
                        ? event.compactResult.result.summary.length
                        : undefined,
                    postCompactSummaryMessages: postCompactArtifactSummary.summaryMessageCount,
                    postCompactKeptMessages: postCompactArtifactSummary.keptMessageCount,
                    postCompactAttachments: postCompactArtifactSummary.attachmentCount,
                    postCompactDiscoveredTools: postCompactArtifactSummary.discoveredToolsCount,
                    postCompactHasPreservedSegment: postCompactArtifactSummary.hasPreservedSegment,
                  }
                : {}),
            },
          });
        ...
        while (true) {
          ...
          await emitRunLoopLifecycleEvent({
            ...
            isTopLevel: !normalizedSessionKey || !isSubagentSessionKey(normalizedSessionKey),
            sessionFile: params.sessionFile,
            turnIndex: runLoopIterations,
            metadata: {
          ...
          if (timedOut && !timedOutDuringCompaction) {
            ...
            if (timeoutCompactionAttempts >= MAX_TIMEOUT_COMPACTION_ATTEMPTS) {
              ...
              try {
                ...
                timeoutCompactResult = await memoryRuntime.compact({
                  sessionId: params.sessionId,
                  sessionKey: params.sessionKey,
                  sessionFile: params.sessionFile,
                ...
                log.warn(
                  `[timeout-compaction] memoryRuntime.compact() threw during timeout recovery for ${provider}/${modelId}: ${String(compactErr)}`,
                );
              ...
              if (timeoutCompactResult.ok && timeoutCompactResult.compacted) {
                await emitOwnedCompactionLifecycle({
                  phase: "post_compact",
                  trigger: "timeout_recovery",
                  compactResult: timeoutCompactResult,
                });
              }
          ...
          if (contextOverflowError) {
            ...
            if (
              ...
              try {
                ...
                compactResult = await memoryRuntime.compact({
                  sessionId: params.sessionId,
                  sessionKey: params.sessionKey,
                  sessionFile: params.sessionFile,
                  tokenBudget: ctxInfo.tokens,
                ...
                log.warn(
                  `memoryRuntime.compact() threw during overflow recovery for ${provider}/${modelId}: ${String(compactErr)}`,
                );
                compactResult = {
                  ok: false,
                  compacted: false,
          ...
          if (timedOut && !timedOutDuringCompaction && payloads.length === 0) {
            return {
              ...
              meta: {
                ...
                aborted,
                systemPromptReport: attempt.systemPromptReport,
                skillExposureState: attempt.skillExposureState,
              },
              didSendViaMessagingTool: attempt.didSendViaMessagingTool,
              didSendDeterministicApprovalPrompt: attempt.didSendDeterministicApprovalPrompt,
