...
const EXPLICIT_PLAN_SURFACES = new Set(["unit", "extensions", "contracts", "gateway"]);
...
const validateExplicitSurfaces = (surfaces) => {
  ...
  if (invalidSurfaces.length > 0) {
    throw new Error(
      `Unsupported --surface value(s): ${invalidSurfaces.join(", ")}. Supported surfaces: unit, extensions, contracts, gateway.`,
    );
  }
};
...
const buildRequestedSurfaces = (request, env) => {
  ...
  const surfaces = [];
  const skipDefaultRuns = env.CRAWCLAW_TEST_SKIP_DEFAULT === "1";
  if (!skipDefaultRuns) {
    surfaces.push("unit");
  }
  if (env.CRAWCLAW_TEST_INCLUDE_EXTENSIONS === "1") {
    surfaces.push("extensions");
  }
  if (env.CRAWCLAW_TEST_INCLUDE_CONTRACTS === "1") {
    surfaces.push("contracts");
  }
  if (env.CRAWCLAW_TEST_INCLUDE_GATEWAY === "1") {
...
const resolveCIManifestScope = (scope = {}, env = process.env) => ({
  ...
  docsOnly: parseBooleanLike(scope.docsOnly ?? env.CRAWCLAW_CI_DOCS_ONLY, false),
  docsChanged: parseBooleanLike(scope.docsChanged ?? env.CRAWCLAW_CI_DOCS_CHANGED, false),
  runNode: parseBooleanLike(scope.runNode ?? env.CRAWCLAW_CI_RUN_NODE, true),
  runWindows: parseBooleanLike(scope.runWindows ?? env.CRAWCLAW_CI_RUN_WINDOWS, true),
  runSkillsPython: parseBooleanLike(
    scope.runSkillsPython ?? env.CRAWCLAW_CI_RUN_SKILLS_PYTHON,
    true,
  ),
  hasChangedExtensions: parseBooleanLike(
    scope.hasChangedExtensions ?? env.CRAWCLAW_CI_HAS_CHANGED_EXTENSIONS,
    false,
  ),
  changedExtensionsMatrix: parseChangedExtensionsMatrix(
    scope.changedExtensionsMatrix ?? env.CRAWCLAW_CI_CHANGED_EXTENSIONS_MATRIX,
  ),
  runChangedSmoke: parseBooleanLike(
    scope.runChangedSmoke ?? env.CRAWCLAW_CI_RUN_CHANGED_SMOKE,
    true,
  ),
});
...
const resolveMaxWorkersForUnit = (unit, context) => {
  const overrideWorkers = Number.parseInt(context.env.CRAWCLAW_TEST_WORKERS ?? "", 10);
...
const withIncludeFileEnv = (context, unitId, files) => ({
  CRAWCLAW_VITEST_INCLUDE_FILE: context.writeTempJsonArtifact(unitId, files),
});
...
const resolveUnitHeavyFileGroups = (context) => {
  ...
  const heavyUnitFileLimit = parseEnvNumber(
    env,
    "CRAWCLAW_TEST_HEAVY_UNIT_FILE_LIMIT",
...
const resolveExtensionHeavyFileGroups = (context) => {
  ...
  const memoryHeavyFiles = selectMemoryHeavyFiles({
    ...
    limit: parseEnvNumber(
      env,
      "CRAWCLAW_TEST_MEMORY_HEAVY_EXTENSION_FILE_LIMIT",
      runtime.isCI ? 12 : 6,
    ),
    minDeltaKb: parseEnvNumber(
      env,
      "CRAWCLAW_TEST_MEMORY_HEAVY_EXTENSION_MIN_KB",
  ...
  const timedHeavyFiles = selectTimedHeavyFiles({
    candidates: extensionCandidates,
    limit: parseEnvNumber(env, "CRAWCLAW_TEST_HEAVY_EXTENSION_FILE_LIMIT", runtime.isCI ? 16 : 8),
    minDurationMs: parseEnvNumber(
      env,
      "CRAWCLAW_TEST_HEAVY_EXTENSION_MIN_MS",
...
const buildDefaultUnits = (context, request) => {
  const { env, executionBudget, catalog, unitTimingManifest, extensionTimingManifest } = context;
  ...
  const extensionsBatchTargetMs = parseEnvNumber(
    env,
    "CRAWCLAW_TEST_EXTENSIONS_BATCH_TARGET_MS",
  ...
  const unitFastLaneCount = Math.max(
    1,
    parseEnvNumber(env, "CRAWCLAW_TEST_UNIT_FAST_LANES", defaultUnitFastLaneCount),
  );
  ...
  const unitFastBatchTargetMs = parseEnvNumber(
    env,
    "CRAWCLAW_TEST_UNIT_FAST_BATCH_TARGET_MS",
...
const buildTargetedUnits = (context, request) => {
  ...
  const targetedUnitBatchTargetMs = parseEnvNumber(
    context.env,
    "CRAWCLAW_TEST_TARGETED_UNIT_BATCH_TARGET_MS",
  ...
  const nextUnit = createExecutionUnit(context, {
    ...
    env:
      Array.isArray(unit.includeFiles) && unit.includeFiles.length > 0
        ? {
            ...unit.env,
            CRAWCLAW_VITEST_INCLUDE_FILE: context.writeTempJsonArtifact(
...
export function explainExecutionTarget(request, options = {}) {
  ...
  context.noIsolateArgs =
    context.env.CRAWCLAW_TEST_ISOLATE === "1" || context.env.CRAWCLAW_TEST_ISOLATE === "true"
      ? []
      : context.env.CRAWCLAW_TEST_NO_ISOLATE !== "0" &&
          context.env.CRAWCLAW_TEST_NO_ISOLATE !== "false"
        ? ["--isolate=false"]
        : [];
...
export function buildExecutionPlan(request, options = {}) {
  ...
  context.noIsolateArgs =
    env.CRAWCLAW_TEST_ISOLATE === "1" || env.CRAWCLAW_TEST_ISOLATE === "true"
      ? []
      : env.CRAWCLAW_TEST_NO_ISOLATE !== "0" && env.CRAWCLAW_TEST_NO_ISOLATE !== "false"
        ? ["--isolate=false"]
        : [];
  ...
  const shardOverride = Number.parseInt(env.CRAWCLAW_TEST_SHARDS ?? "", 10);
  ...
  const shardIndexOverride = Number.parseInt(env.CRAWCLAW_TEST_SHARD_INDEX ?? "", 10);
  context.shardIndexOverride =
    Number.isFinite(shardIndexOverride) && shardIndexOverride > 0 ? shardIndexOverride : null;

  if (context.shardIndexOverride !== null && context.shardCount <= 1) {
    throw new Error(
      `CRAWCLAW_TEST_SHARD_INDEX=${String(context.shardIndexOverride)} requires CRAWCLAW_TEST_SHARDS>1.`,
    );
  }
  if (context.shardIndexOverride !== null && context.shardIndexOverride > context.shardCount) {
    throw new Error(
      `CRAWCLAW_TEST_SHARD_INDEX=${String(context.shardIndexOverride)} exceeds CRAWCLAW_TEST_SHARDS=${String(context.shardCount)}.`,
    );
  }
  ...
  const topLevelParallelLimit = Math.max(
    1,
    parseEnvNumber(env, "CRAWCLAW_TEST_TOP_LEVEL_CONCURRENCY", defaultTopLevelParallelLimit),
  );
