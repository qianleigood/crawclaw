...
const loadConfig = vi.fn<() => CrawClawConfig>(() => ({
  gateway: {
    auth: {
      token: "config-token",
    },
...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { CRAWCLAW_GATEWAY_TOKEN: "service-token" },
    });
    stubEmptyGatewayEnv();
  });
  ...
  it("compares restart drift against config token even when caller env is set", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { CRAWCLAW_GATEWAY_TOKEN: "env-token" },
    });
    vi.stubEnv("CRAWCLAW_GATEWAY_TOKEN", "env-token");
  ...
  it("resolves config token SecretRefs using service command env before drift checks", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: {
        CRAWCLAW_GATEWAY_TOKEN: "service-token",
  ...
  it("falls back to not-loaded hints when start finds no install artifacts", async () => {
    ...
    expect(payload.hints).toEqual(expect.arrayContaining(["crawclaw gateway install"]));
    expect(payload.hintItems).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "install",
          text: "crawclaw gateway install",
        }),
      ]),
    );
    expect(service.restart).not.toHaveBeenCalled();
