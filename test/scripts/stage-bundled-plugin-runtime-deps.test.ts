...
describe("stageBundledPluginRuntimeDeps", () => {
  function createBundledPluginFixture(params: {
    ...
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-runtime-deps-"));
  ...
  it("skips restaging when runtime deps stamp matches the sanitized manifest", () => {
    const { pluginDir, repoRoot } = createBundledPluginFixture({
      packageJson: {
        name: "@crawclaw/fixture-plugin",
        version: "1.0.0",
        dependencies: { "left-pad": "1.3.0" },
        peerDependencies: { crawclaw: "^1.0.0" },
        peerDependenciesMeta: { crawclaw: { optional: true } },
        devDependencies: { crawclaw: "^1.0.0" },
        crawclaw: { bundle: { stageRuntimeDependencies: true } },
      },
    });
    ...
    stageBundledPluginRuntimeDeps({
      ...
      installPluginRuntimeDepsImpl: ({ fingerprint }: { fingerprint: string }) => {
        installCount += 1;
        fs.writeFileSync(
          path.join(pluginDir, ".crawclaw-runtime-deps-stamp.json"),
