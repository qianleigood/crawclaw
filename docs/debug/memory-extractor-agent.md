...
read_when:
  ...
  - You want a practical mapping from Claude-style memory extraction to CrawClaw's runtime architecture
...
**move CrawClaw durable memory auto-write from the former `afterTurn -> structured extraction worker` flow into a constrained background `memory_extractor` agent.**

The goal is not to copy Claude's implementation literally. It is to borrow two things that Claude does better:
...
1. Trigger point is too early  
   It currently hangs off `afterTurn`, which is closer to "new messages arrived, maybe extract" than "the turn has stabilized, now consolidate memory".
...
3. `feedback` semantics are incomplete  
   Right now feedback behaves more like corrective memory. Claude's rule is stronger:
   - record corrections
...
- Claude-style prompt workflow: manifest-first, candidate review first, then tightly batched durable writes within the 5-turn budget
- Claude-style scoped memory file tools now back the extractor:
...
- **borrow Claude's execution pattern**
...
- Claude-style scoped memory file tools replace the earlier high-level durable write tools inside `memory_extractor`
...
**upgrade durable auto-write into a Claude-style rule set running on an CrawClaw-style background special agent.**

The key architectural choice is:

- adopt Claude's rules
