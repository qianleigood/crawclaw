    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL) => {
        ...
        return {
          ...
          json: async () => ({
            version,
            engines: version != null ? { node: ">=24.0.0 <26" } : undefined,
          }),
        } as Response;
      }),
    );
...
describe("checkDepsStatus", () => {
  it("reports unknown, missing, stale, and ok states from lockfile markers", async () => {
    const base = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-update-check-"));
...
describe("checkUpdateStatus", () => {
  ...
  it("detects package installs for non-git roots", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-update-check-"));
    await fs.writeFile(
