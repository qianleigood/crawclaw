...
const mocks = vi.hoisted(() => ({
  ...
  emitAgentEvent: vi.fn((_event?: unknown) => undefined),
  getAgentRunContext: vi.fn(
    (_runId: string): { sessionId?: string; agentId?: string } | undefined => undefined,
  ),
  loadConfig: vi.fn(() => ({
...
vi.mock("./runtime/lifecycle/bus.js", () => ({
  emitRunLoopLifecycleEvent: mocks.emitRunLoopLifecycleEvent,
}));

vi.mock("./runtime/lifecycle/shared-subscribers.js", () => ({
  ensureSharedRunLoopLifecycleSubscribers: mocks.ensureSharedRunLoopLifecycleSubscribers,
}));
...
describe("subagent registry seam flow", () => {
  ...
  beforeEach(async () => {
    ...
    mocks.onAgentEvent.mockImplementation((handler: unknown) => {
      lifecycleHandler = handler as typeof lifecycleHandler;
      return noop;
    });
  ...
  it("emits subagent_start from tracked child lifecycle start events", async () => {
  ...
  it("removes attachments for killed delete-mode runs", async () => {
    const attachmentsRootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "crawclaw-kill-attachments-"),
    );
  ...
  it("removes attachments for released delete-mode runs", async () => {
    const attachmentsRootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "crawclaw-release-attachments-"),
    );
    const attachmentsDir = path.join(attachmentsRootDir, "child");
    await fs.mkdir(attachmentsDir, { recursive: true });
    await fs.writeFile(path.join(attachmentsDir, "artifact.txt"), "artifact");
