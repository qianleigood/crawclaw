...
function printHelp() {
  process.stdout.write(
    `Usage: node scripts/start_login.js [options]\n\n` +
      `Options:\n` +
...
async function main() {
  ...
  const { resolveAccountName, resolveDebugPort } = requireFrom(
    xhsRoot,
    "node/src/core/accounts.js",
  );
  const {
    checkCreatorLogin,
    checkHomeLogin,
  ...
  if (args.launchBrowser) {
    ...
    const launchResult = runCommand(
      runPython,
      [launcherScript, "--account", accountName, "--port", String(port)],
      {
        cwd: xhsRoot,
      },
    );
  ...
  const payload = await withBrowserSession(
    ...
    async ({ page, browser, reusedExistingTab }) => {
      ...
      if (!args.force && mode !== "re-login") {
        const existing =
          scope === "home" ? await checkHomeLogin(ctx) : await checkCreatorLogin(ctx);
