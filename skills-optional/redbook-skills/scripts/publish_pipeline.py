...
def _resolve_debug_port(account_name: str | None, explicit_port: int | None) -> int:
    """Resolve explicit port or fall back to the account preferred port."""
    if explicit_port is not None:
        return int(explicit_port)
    try:
        from account_manager import resolve_debug_port
        return int(resolve_debug_port(_resolve_account_name(account_name), None))
    except Exception:
        return 9222


def _resolve_account_port(account_name: str | None, explicit_port: int | None) -> int:
...
def main():
    ...
    host = args.host
    port = _resolve_account_port(args.account, args.port)
