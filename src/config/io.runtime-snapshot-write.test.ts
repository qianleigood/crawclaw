...
function createSourceConfig(): CrawClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createRuntimeConfig(): CrawClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("runtime config snapshot writes", () => {
  ...
  it("returns the source snapshot when runtime snapshot is active", async () => {
    await withTempHome("crawclaw-config-runtime-source-", async () => {
  ...
  it("skips source projection for non-runtime-derived configs", async () => {
    await withTempHome("crawclaw-config-runtime-projection-shape-", async () => {
      ...
      const independentConfig: CrawClawConfig = {
        models: {
          providers: {
            openai: {
              baseUrl: "https://api.openai.com/v1",
  ...
  it("preserves source secret refs when writeConfigFile receives runtime-resolved config", async () => {
    await withTempHome("crawclaw-config-runtime-write-", async (home) => {
  ...
  it("refreshes the runtime snapshot after writes so follow-up reads see persisted changes", async () => {
    await withTempHome("crawclaw-config-runtime-write-refresh-", async (home) => {
      const configPath = path.join(home, ".crawclaw", "crawclaw.json");
      const sourceConfig: CrawClawConfig = {
        models: {
          providers: {
      ...
      const nextRuntimeConfig: CrawClawConfig = {
        ...runtimeConfig,
  ...
  it("keeps the last-known-good runtime snapshot active while a specialized refresh is pending", async () => {
    await withTempHome("crawclaw-config-runtime-refresh-pending-", async (home) => {
      const configPath = path.join(home, ".crawclaw", "crawclaw.json");
      const sourceConfig = createSourceConfig();
      const runtimeConfig = createRuntimeConfig();
      const nextRuntimeConfig: CrawClawConfig = {
  ...
  it("notifies in-process write listeners with the refreshed runtime snapshot", async () => {
    await withTempHome("crawclaw-config-runtime-write-listener-", async (home) => {
      const configPath = path.join(home, ".crawclaw", "crawclaw.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, `${JSON.stringify({ gateway: { port: 18789 } }, null, 2)}\n`);

      const seen: Array<{ configPath: string; runtimeConfig: CrawClawConfig }> = [];
