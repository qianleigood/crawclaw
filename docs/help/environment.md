---
summary: "Where CrawClaw loads environment variables and the precedence order"
read_when:
...
CrawClaw pulls environment variables from multiple sources. The rule is **never override existing values**.

## Precedence (highest → lowest)

1. **Process environment** (what the Gateway process already has from the parent shell/daemon).
2. **`.env` in the current working directory** (dotenv default; does not override).
3. **Global `.env`** at `~/.crawclaw/.env` (aka `$CRAWCLAW_STATE_DIR/.env`, legacy: `$CRAWCLAW_STATE_DIR/.env`; does not override).
4. **Config `env` block** in `~/.crawclaw/crawclaw.json` (legacy: `~/.crawclaw/crawclaw.json`; applied only if missing).
5. **Optional login-shell import** (`env.shellEnv.enabled` or `CRAWCLAW_LOAD_SHELL_ENV=1`, legacy: `CRAWCLAW_LOAD_SHELL_ENV=1`), applied only for missing expected keys.
...
- `CRAWCLAW_LOAD_SHELL_ENV=1` (legacy: `CRAWCLAW_LOAD_SHELL_ENV=1`)
- `CRAWCLAW_SHELL_ENV_TIMEOUT_MS=15000` (legacy: `CRAWCLAW_SHELL_ENV_TIMEOUT_MS=15000`)

## Runtime-injected env vars

CrawClaw also injects context markers into spawned child processes:
...
CrawClaw supports two env-driven patterns:
...
When set, `CRAWCLAW_HOME` replaces the system home directory (`$HOME` / `os.homedir()`) for all internal path resolution. Legacy `CRAWCLAW_HOME` is still accepted. This enables full filesystem isolation for headless service accounts.

**Precedence:** `CRAWCLAW_HOME` > `CRAWCLAW_HOME` > `$HOME` > `USERPROFILE` > `os.homedir()`
...
`CRAWCLAW_HOME` can also be set to a tilde path (e.g. `~/svc`), which gets expanded using `$HOME` before use.
...
Export the variable before starting CrawClaw:
