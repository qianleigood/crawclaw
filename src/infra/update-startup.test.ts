...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["CRAWCLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.CRAWCLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveCrawClawPackageRoot } = await import("./crawclaw-root.js"));
    ...
    vi.mocked(resolveCrawClawPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveCrawClawPackageRoot).mockResolvedValue("/opt/crawclaw");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/crawclaw/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
