...
describe("createSubsystemLogger().isEnabled", () => {
  ...
  it("renders operator trace fields in console output", () => {
    setLoggerOverride({ level: "silent", consoleLevel: "info" });
    const info = vi.fn();
    loggingState.rawConsole = {
      log: info,
    ...
    log.info("operator decision", {
      runId: "run-123",
      sessionId: "sess-123",
      agentId: "main",
      phase: "provider_request_start",
      decision: "provider_model_selected",
      status: "ok",
      traceId: "run-loop:run-123",
      spanId: "provider:req-1",
    });

    expect(info).toHaveBeenCalledTimes(1);
    expect(String(info.mock.calls[0]?.[0] ?? "")).toContain(
      "[run=run-123 session=sess-123 agent=main phase=provider_request_start decision=provider_model_selected status=ok trace=run-loop:run-123 span=provider:req-1]",
    );
  });

  it("binds default operator context with withContext", () => {
    setLoggerOverride({ level: "silent", consoleLevel: "warn" });
    const warn = installConsoleMethodSpy("warn");
    const log = createSubsystemLogger("agent/embedded").withContext({
      runId: "run-ctx",
      sessionId: "sess-ctx",
      traceId: "run-loop:run-ctx",
    });

    log.warn("bound context warning", { phase: "tool_call_error", decision: "tool_call_failed" });
