...
import { buildPluginConfigSchema } from "crawclaw/plugin-sdk/core";
import { z } from "crawclaw/plugin-sdk/zod";
import type { CrawClawPluginConfigSchema } from "../runtime-api.js";
...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "crawclaw-plugin-tools";
const ACPX_BIN_NAME = process.platform === "win32" ? "acpx.cmd" : "acpx";

function isAcpxPluginRoot(dir: string): boolean {
  return (
    (fs.existsSync(path.join(dir, "crawclaw.plugin.json")) ||
      fs.existsSync(path.join(dir, "crawclaw.plugin.json"))) &&
...
function resolveCrawClawRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const openClawRoot = resolveCrawClawRoot(pluginRoot);
...
export function createAcpxPluginConfigSchema(): CrawClawPluginConfigSchema {
