...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".crawclaw", "extensions", params.id);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "crawclaw");
      await fs.mkdir(workspaceDir, { recursive: true });

      const configDir = path.join(tempHome, ".crawclaw");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-gw-http-tools-home-"));
      process.env.HOME = tempHome;
      process.env.CRAWCLAW_STATE_DIR = path.join(tempHome, ".crawclaw");
      delete process.env.CRAWCLAW_CONFIG_PATH;
      process.env.CRAWCLAW_SKIP_CHANNELS = "1";
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-wizard-home-"));
      process.env.HOME = tempHome;
