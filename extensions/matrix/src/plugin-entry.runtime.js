...
const CRAWCLAW_PLUGIN_SDK_PREFIX = ["crawclaw", "plugin-sdk"].join("/");
...
function findCrawClawPackageRoot(startDir) {
...
function buildPluginSdkAliasMap(moduleUrl) {
  const location = findCrawClawPackageRoot(path.dirname(fileURLToPath(moduleUrl)));
  if (!location) {
    return {};
  }
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findCrawClawPackageRoot(moduleDir);
  if (location) {
