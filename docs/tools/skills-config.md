...
All skills-related configuration lives under `skills` in `~/.crawclaw/crawclaw.json`.
...
`skills.entries.*` is only for custom or third-party skill workflows.

## Fields

- Built-in skill roots always include `~/.crawclaw/skills`, `~/.agents/skills`,
...
- Keys under `entries` map to the skill name by default. If a skill defines
  `metadata.crawclaw.skillKey`, use that key instead.
- Load precedence is `<workspace>/skills` → `<workspace>/.agents/skills` →
  `~/.agents/skills` → `~/.crawclaw/skills` → bundled skills →
  `skills.load.extraDirs`.
- Changes to skills are picked up on the next agent turn when the watcher is enabled.
- Bundled core skill helper dependencies are owned by the native runtime and
