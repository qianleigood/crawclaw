...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<CrawClawConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<CrawClawConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: CrawClawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: CrawClawConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: CrawClawConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: CrawClawConfig, result: CrawClawConfig): CrawClawConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as CrawClawConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with crawclaw configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "CrawClaw will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.crawclaw.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — CrawClaw will store a reference instead of the API key.",
