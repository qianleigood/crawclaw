...
const SEARCH_TOOL = "https://docs.crawclaw.ai/mcp.SearchCrawClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.crawclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('crawclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.crawclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('crawclaw docs "your query"')}`);
    }
    return;
  }
