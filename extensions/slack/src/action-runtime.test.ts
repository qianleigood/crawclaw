...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): CrawClawConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as CrawClawConfig;
  ...
  async function resolveReadToken(cfg: CrawClawConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: CrawClawConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
