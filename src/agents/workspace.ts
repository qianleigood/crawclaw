...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = (env.CRAWCLAW_PROFILE ?? env.CRAWCLAW_PROFILE)?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".crawclaw", `workspace-${profile}`);
  }
  return path.join(home, ".crawclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".crawclaw";
