...
export function diffConfigPaths(prev: unknown, next: unknown, prefix = ""): string[] {
  ...
  if (Array.isArray(prev) && Array.isArray(next)) {
    // Arrays can contain object entries (for example session.sendPolicy.rules);
...
export function resolveGatewayReloadSettings(cfg: CrawClawConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: CrawClawConfig;
  initialInternalWriteHash?: string | null;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: CrawClawConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: CrawClawConfig) => void | Promise<void>;
  ...
  let pendingInProcessConfig: CrawClawConfig | null = null;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: CrawClawConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: CrawClawConfig) => {
