...
function makeTempDir() {
  return makeTrackedTempDir("crawclaw-doctor-plugin-manifests", tempDirs);
}
...
describe("doctor plugin manifest legacy contract repair", () => {
  ...
  it("collects legacy top-level capability keys for migration", () => {
    ...
    const migrations = collectLegacyPluginManifestContractMigrations({
      env: {
        ...process.env,
        CRAWCLAW_BUNDLED_PLUGINS_DIR: pluginsRoot,
      },
    });
  ...
  it("rewrites legacy top-level capability keys into contracts", async () => {
    ...
    const next = JSON.parse(fs.readFileSync(path.join(root, "crawclaw.plugin.json"), "utf-8")) as {
