...
describe("canvas host state dir defaults", () => {
  ...
  it("uses CRAWCLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("crawclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("CrawClaw Canvas");
      } finally {
        await handler.close();
      }
    });
