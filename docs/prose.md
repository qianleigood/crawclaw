---
summary: "OpenProse: .prose workflows, slash commands, and state in CrawClaw"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In CrawClaw it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses CrawClaw tools under the hood.
...
## CrawClaw runtime mapping

OpenProse programs map to CrawClaw primitives:

| OpenProse concept         | CrawClaw tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use CrawClaw tool allowlists and approval gates to control side effects.
