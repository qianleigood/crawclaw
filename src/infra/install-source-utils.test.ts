...
const TEMP_DIR_PREFIX = "crawclaw-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: {
  ...
  const cwd = await createTempDir("crawclaw-install-source-utils-");
  const archivePath = path.join(cwd, "crawclaw-plugin-1.2.3.tgz");
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("crawclaw-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
  ...
  it.each([
    {
      ...
      path: async () => "/tmp/does-not-exist-crawclaw-archive.tgz",
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "crawclaw-plugin@1.2.3",
          name: "crawclaw-plugin",
          version: "1.2.3",
          filename: "crawclaw-plugin-1.2.3.tgz",
    ...
    const result = await runPack("crawclaw-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "crawclaw-plugin",
        version: "1.2.3",
        resolvedSpec: "crawclaw-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "crawclaw-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "crawclaw-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\ncrawclaw-plugin-1.2.3.tgz\n",
    });
  ...
  it.each([
    ...
    },
    {
      ...
      stdout:
        'npm notice using cache\n[{"id":"crawclaw-plugin@1.2.3","name":"crawclaw-plugin","version":"1.2.3","integrity":"sha512-test-integrity","shasum":"abc123"}]\n',
      expectedMetadata: {
        name: "crawclaw-plugin",
        version: "1.2.3",
        resolvedSpec: "crawclaw-plugin@1.2.3",
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("crawclaw-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "crawclaw-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@crawclaw/plugin-demo@2.0.0",
            filename: "crawclaw-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@crawclaw/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "crawclaw-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@crawclaw/plugin-demo@2.0.0",
      },
    });
  });
