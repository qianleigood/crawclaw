---
summary: "Run CrawClaw Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want CrawClaw running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on CrawClaw Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

# CrawClaw on Azure Linux VM

This guide sets up an Azure Linux VM with the Azure CLI, applies Network Security Group (NSG) hardening, configures Azure Bastion for SSH access, and installs CrawClaw.
...
- **Deallocate the VM** when not in use (stops compute billing; disk charges remain). The CrawClaw Gateway will not be reachable while the VM is deallocated — restart it when you need it live again:
