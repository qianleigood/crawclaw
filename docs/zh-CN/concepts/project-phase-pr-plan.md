...
| `PR-06` | Phase 6    | `已完成` | channel runtime 收口已完成：workflow/outbound projection、interactive controls、inbound normalization、threading/binding/typing、Feishu/Feishu/LINE/DingTalk channel transform 已统一收进 `src/channels`，`auto-reply` / `workflows` 只保留语义层。                                                                                 |
...
- 已新增共享 channels seam：
  ...
  - `src/channels/index-directives.ts`
  - `src/channels/index-directives.test.ts`
  - `src/channels/index-context.ts`
  - `src/channels/index-context.test.ts`
  - `src/channels/index-context.ts`
  - `src/channels/index-context.test.ts`
  - `src/channels/index-command-replies.ts`
  - `src/channels/index-command-replies.test.ts`
  - `src/channels/index-pagination.ts`
  - `src/channels/index-pagination.test.ts`
  - `src/channels/index-model-picker.ts`
  - `src/channels/index-model-picker.test.ts`
...
- `src/workflows/channel-forwarder.ts` 不再直接内联组装 DingTalk / LINE / Feishu / QQBot 的 workflow payload，而是委托给 `src/channels/workflow-projection.ts`。
- `src/auto-reply/reply/commands-workflow.ts` 与 `src/workflows/channel-forwarder.ts` 已开始复用 `src/channels/workflow-controls.ts`，不再各自维护 Feishu / QQBot 的 workflow 控件形态。
...
- `src/auto-reply/reply/commands-models.ts` 与 `src/auto-reply/reply/directive-handling.model.ts` 已改为复用 `src/channels/index-model-picker.ts`，删除 `auto-reply/reply` 下的纯 Feishu picker 实现。
- `src/auto-reply/reply/commands-info.ts` 与 `src/plugin-sdk/command-auth.ts` 已改为复用 `src/channels/index-pagination.ts`，删除 `auto-reply/reply` 下的 Feishu 分页键盘实现。
...
- `src/channels/acp-delivery-visibility.ts` 已新增，承接 ACP delivery 的渠道可见性判定；`dispatch-acp-delivery.ts` 不再内联 Feishu block visibility 规则。
...
- `src/channels/index-command-replies.ts` 已新增，承接 `/commands` 与 `/models` 的 Feishu reply payload 成形；`commands-info.ts` 与 `commands-models.ts` 不再直接拼 `channelData.feishu.buttons`。
...
- `src/channels/typing-mode.ts` 已新增，承接渠道侧 typing mode / feishuer 语义；`agent-runner.ts`、`followup-runner.ts` 与 `get-reply-run.ts` 不再从 `auto-reply/reply` 读取 typing mode helper。
...
- `src/channels/index-context.ts` 与 `src/channels/index-context.ts` 已新增，承接 Feishu / Feishu conversationId 解析；`commands-session.ts` 与 `commands-subagents/*` 不再从 `auto-reply/reply` 读取这些渠道上下文 helper。
- `src/channels/line-directives.ts` 与 `src/channels/index-directives.ts` 已新增，承接 LINE / DingTalk reply directive transform；`normalize-reply.ts` 不再从 `auto-reply/reply` 读取这些渠道特化 helper。
- workflow 层现在主要保留 `/workflow status|cancel|resume` 命令语义和 QQBot resume callback 语义；渠道层开始接管 payload projection 与控件成形本身。
- `normalize-reply.ts` 已不再保留 LINE / DingTalk 渠道 transform；相关 reply directive transform 已统一收进 `src/channels`。
...
- 原来的 `src/auto-reply/reply/typing-mode.ts` 已删除，typing mode / feishuer 语义已转为 channels seam。
...
- 原来的 `src/auto-reply/reply/feishu-context.ts` 与 `src/auto-reply/reply/matrix-context.ts` 已删除，相关 conversation helper 已转为 channels seam。
- 原来的 `src/auto-reply/reply/line-directives.ts` 与 `src/auto-reply/reply/ddingtalk-directives.ts` 已删除，相关 reply directive transform 已转为 channels seam。
- 已把 `deliverable-target.ts`、`acp-delivery-visibility.ts`、`inbound-context.ts`、`inbound-dedupe.ts`、`reply-to-mode.ts`、`reply-threading.ts`、`session-delivery-route.ts`、`command-surface-context.ts`、`conversation-binding-input.ts`、`typing-mode.ts`、`typing-policy.ts`、`feishu-context.ts`、`matrix-context.ts`、`line-directives.ts`、`ddingtalk-directives.ts`、`feishu-command-replies.ts`、`feishu-pagination.ts`、`feishu-model-picker.ts`、`workflow-projection.ts` 与 `workflow-controls.ts` 补进 `src/channels/README.md` 的 maintainer 入口。
...
- `vitest run src/channels/index-model-picker.test.ts src/auto-reply/reply/directive-handling.model.test.ts src/auto-reply/reply/commands.test.ts -t "/models command|lists providers on feishu|model directive info"`
- `vitest run src/channels/index-pagination.test.ts src/auto-reply/reply/commands.test.ts -t "buildCommandsPaginationKeyboard|/commands"`
...
- `vitest run src/channels/typing-mode.test.ts src/channels/typing-policy.test.ts src/auto-reply/reply/get-reply-run.media-only.test.ts src/auto-reply/reply/reply-utils.test.ts -t "resolveTypingMode|createTypingFeishuer|suppressTyping"`
...
- `vitest run src/channels/index-command-replies.test.ts src/auto-reply/reply/commands.test.ts -t "buildCommandsPaginationKeyboard|/commands|/models command" src/auto-reply/reply/directive-handling.model.test.ts -t "model directive info"`
- `vitest run src/channels/index-context.test.ts src/channels/index-context.test.ts src/auto-reply/reply/commands-session-lifecycle.test.ts src/auto-reply/reply/commands-subagents-focus.test.ts`
...
- `pnpm lint src/channels/index-model-picker.ts src/channels/index-model-picker.test.ts src/auto-reply/reply/commands-models.ts src/auto-reply/reply/directive-handling.model.ts src/channels/README.md`
- `pnpm lint src/channels/index-pagination.ts src/channels/index-pagination.test.ts src/auto-reply/reply/commands-info.ts src/auto-reply/reply/commands.test.ts src/channels/README.md`
...
- `pnpm lint src/channels/line-directives.ts src/channels/line-directives.test.ts src/channels/index-directives.ts src/channels/index-directives.test.ts src/auto-reply/reply/normalize-reply.ts src/auto-reply/reply/reply-flow.test.ts src/channels/README.md`
- `pnpm lint src/channels/index-command-replies.ts src/channels/index-command-replies.test.ts src/auto-reply/reply/commands-info.ts src/auto-reply/reply/commands-models.ts src/channels/README.md`
- `pnpm lint src/channels/index-context.ts src/channels/index-context.test.ts src/channels/index-context.ts src/channels/index-context.test.ts src/auto-reply/reply/commands-session.ts src/auto-reply/reply/commands-subagents/action-focus.ts src/auto-reply/reply/commands-subagents/action-unfocus.ts src/auto-reply/reply/commands-subagents/shared.ts src/channels/README.md`
- `pnpm check`
