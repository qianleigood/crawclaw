...
const PLUGIN_SDK_SUBPATH_PATTERN = /crawclaw\/plugin-sdk\/([a-z0-9][a-z0-9-]*)\b/g;
...
function packCrawClawToTempDir(packDir: string): string {
...
async function readPackedRootPackageJson(archivePath: string): Promise<{
  ...
  const extractDir = mkdtempSync(join(os.tmpdir(), "crawclaw-packed-root-package-json-"));
  try {
    await tar.x({
      file: archivePath,
      cwd: extractDir,
...
function buildLegacyPluginSourceAlias(): string {
  return ["crawclaw", ["plugin", "source"].join("-")].join("/") + "/";
}
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it("keeps curated public plugin-sdk references on exported built subpaths", () => {
    ...
    for (const reference of collectPluginSdkSubpathReferences()) {
      ...
      failures.push(
        `${reference.file} references crawclaw/plugin-sdk/${reference.subpath}, but ${reference.subpath} is missing from ${missingFrom.join(" and ")}`,
      );
    }
  ...
  it("mirrors Bedrock runtime deps needed by the bundled host graph", () => {
    ...
    const allowlist = bedrockPackageJson.crawclaw?.releaseChecks?.rootDependencyMirrorAllowlist;

    expect(Array.isArray(allowlist)).toBe(true);
    expect(bedrockPackageJson.crawclaw?.bundle?.stageRuntimeDependencies).not.toBe(true);
