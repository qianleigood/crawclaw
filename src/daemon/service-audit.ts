...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds CRAWCLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `crawclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service CRAWCLAW_GATEWAY_TOKEN does not match gateway.auth.token in crawclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.CRAWCLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.CRAWCLAW_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `crawclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
