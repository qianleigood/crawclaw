...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "CrawClaw", ".crawclaw");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/crawclaw-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".crawclaw");
  ...
  it("anchors cloud detection to OS homedir when CRAWCLAW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".crawclaw");
    const originalCrawClawHome = process.env.CRAWCLAW_HOME;
    process.env.CRAWCLAW_HOME = "/tmp/crawclaw-home-override";
