...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-crawclaw",
    targetDir: params?.targetDir ?? "/tmp/opik-crawclaw",
...
function createCodexAppServerInstallConfig(params: {
  ...
  return {
    plugins: {
      installs: {
        "crawclaw-codex-app-server": {
          source: "npm" as const,
          spec: params.spec,
          installPath: "/tmp/crawclaw-codex-app-server",
          ...(params.resolvedName ? { resolvedName: params.resolvedName } : {}),
          ...(params.resolvedSpec ? { resolvedSpec: params.resolvedSpec } : {}),
        },
      },
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@crawclaw/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  expect(params.result.config.plugins?.installs?.["crawclaw-codex-app-server"]).toMatchObject({
    source: "npm",
    spec: params.spec,
    installPath: "/tmp/crawclaw-codex-app-server",
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-crawclaw",
        spec: "@opik/opik-crawclaw",
        integrity: "sha512-old",
        installPath: "/tmp/opik-crawclaw",
      }),
      pluginIds: ["opik-crawclaw"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-crawclaw",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "crawclaw-codex-app-server",
        spec: "crawclaw-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/crawclaw-codex-app-server",
      }),
      pluginIds: ["crawclaw-codex-app-server"],
      specOverrides: {
        "crawclaw-codex-app-server": "crawclaw-codex-app-server@0.2.0-beta.4",
      },
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "crawclaw-codex-app-server",
        targetDir: "/tmp/crawclaw-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "crawclaw-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @crawclaw/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@crawclaw/missing",
        installPath: "/tmp/missing",
      }),
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @crawclaw/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        spec: "crawclaw-codex-app-server@beta",
        resolvedName: "crawclaw-codex-app-server",
        resolvedSpec: "crawclaw-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "crawclaw-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "crawclaw-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "crawclaw-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
        spec: "crawclaw-codex-app-server",
      }),
      specOverrides: {
        "crawclaw-codex-app-server": "crawclaw-codex-app-server@beta",
      },
      expectedSpec: "crawclaw-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "crawclaw-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "crawclaw-codex-app-server",
      });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@crawclaw/voice-call",
      targetDir: "/tmp/crawclaw-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@crawclaw/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@crawclaw/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@crawclaw/voice-call");
    expect(result.config.plugins?.entries?.["@crawclaw/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@crawclaw/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@crawclaw/voice-call",
      installPath: "/tmp/crawclaw-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/crawclaw-home";
    mockBundledSources(
      createBundledSource({
