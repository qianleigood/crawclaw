...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  const override = env.CRAWCLAW_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${resolveGatewaySystemdServiceName(env.CRAWCLAW_PROFILE)}.service`;
}

function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = env.CRAWCLAW_LAUNCHD_LABEL?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.CRAWCLAW_PROFILE);
}

function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  const override = env.CRAWCLAW_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
...
export async function prepareRestartScript(
  ...
  try {
    if (platform === "linux") {
      ...
      filename = `crawclaw-restart-${timestamp}.sh`;
...
`;
      ...
      filename = `crawclaw-restart-${timestamp}.bat`;
