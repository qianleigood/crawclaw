---
summary: "CLI reference for `crawclaw message` (send + channel actions)"
read_when:
...
- On cache miss, CrawClaw will attempt a live directory lookup when the provider supports it.
...
- `crawclaw message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, CrawClaw does not force a `default` account SecretRef scope
...
```
crawclaw message send --channel discord \
...
```
crawclaw message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
crawclaw message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
crawclaw message poll --channel msteams \
...
```
crawclaw message react --channel slack \
...
```
crawclaw message react --channel signal \
...
```
crawclaw message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
crawclaw message send --channel telegram --target @mychat \
