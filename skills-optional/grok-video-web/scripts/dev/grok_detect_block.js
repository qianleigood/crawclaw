...
function usage() {
  process.stderr.write(
    `Usage: grok_detect_block.js [options]\n\n` +
...
function recommendedAction(primary) {
  ...
  return {
    ...
    summary:
      "Stop automation and wait for a human to clear the verification challenge in the same browser profile/tab, then resume from the latest checkpoint.",
    noBruteRetry: true,
    resumeCommandHint:
      "node skills/grok-video-web/scripts/dev/grok_video_resume.js plan --state-dir <state-dir>",
  };
}
...
function main() {
  ...
  const logger =
    args.writeState || args.stateDir || args.jobDir || args.manifestPath
