...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `crawclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      CRAWCLAW_LAUNCHD_LABEL: `ai.crawclaw.launchd-int-${testId}`,
      CRAWCLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
