import type { AnyAgentTool, CrawClawPluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: CrawClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<CrawClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
