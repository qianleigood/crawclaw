import { describe, expect, it } from "vitest";
import type { CrawClawConfig } from "../config/config.js";
import { getHistoryLimitFromSessionKey } from "./pi-embedded-runner.js";

describe("getHistoryLimitFromSessionKey", () => {
  it("falls back to provider default when per-DM not set", () => {
    const config = {
      channels: {
        feishu: {
          dmHistoryLimit: 15,
          dms: { "456": { historyLimit: 5 } },
        },
      },
    } as CrawClawConfig;
    expect(getHistoryLimitFromSessionKey("feishu:dm:123", config)).toBe(15);
  });
  it("returns per-DM override for agent-prefixed keys", () => {
    const config = {
      channels: {
        feishu: {
          dmHistoryLimit: 20,
          dms: { "789": { historyLimit: 3 } },
        },
      },
    } as CrawClawConfig;
    expect(getHistoryLimitFromSessionKey("agent:main:feishu:dm:789", config)).toBe(3);
  });
  it("handles userId with colons (e.g., email)", () => {
    const config = {
      channels: {
        qqbot: {
          dmHistoryLimit: 10,
          dms: { "user@example.com": { historyLimit: 7 } },
        },
      },
    } as CrawClawConfig;
    expect(getHistoryLimitFromSessionKey("qqbot:dm:user@example.com", config)).toBe(7);
  });
  it("returns undefined when per-DM historyLimit is not set", () => {
    const config = {
      channels: {
        feishu: {
          dms: { "123": {} },
        },
      },
    } as CrawClawConfig;
    expect(getHistoryLimitFromSessionKey("feishu:dm:123", config)).toBeUndefined();
  });
  it("returns 0 when per-DM historyLimit is explicitly 0 (unlimited)", () => {
    const config = {
      channels: {
        feishu: {
          dmHistoryLimit: 15,
          dms: { "123": { historyLimit: 0 } },
        },
      },
    } as CrawClawConfig;
    expect(getHistoryLimitFromSessionKey("feishu:dm:123", config)).toBe(0);
  });
});
