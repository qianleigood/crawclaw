...
describe("resolveBootstrapFilesForRun", () => {
  ...
  it("applies bootstrap hook overrides", async () => {
    ...
    const workspaceDir = await makeTempWorkspace("crawclaw-bootstrap-");
...
describe("resolveBootstrapContextForRun", () => {
  ...
  it("keeps default session bootstrap scoped to core files", async () => {
    const workspaceDir = await makeTempWorkspace("crawclaw-bootstrap-");
    await fs.writeFile(path.join(workspaceDir, "SOUL.md"), "persona", "utf8");
    await fs.writeFile(path.join(workspaceDir, "HEARTBEAT.md"), "heartbeat", "utf8");
    await fs.writeFile(path.join(workspaceDir, "BOOTSTRAP.md"), "bootstrap", "utf8");
