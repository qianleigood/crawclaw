...
export const agentHandlers: GatewayRequestHandlers = {
  agent: async ({ params, respond, context, client, isWebchatConnect }) => {
    ...
    const newCommandMatch = message.match(NEW_COMMAND_RE);
    if (newCommandMatch && requestedSessionKey) {
      if (!canResetSession) {
        respond(
          false,
          undefined,
      ...
      const postResetMessage = newCommandMatch[1]?.trim() ?? "";
      if (postResetMessage) {
        message = postResetMessage;
      } else {
        // Keep bare /new behavior aligned with chat.send:
    ...
    // See: https://github.com/crawclaw/crawclaw/issues/3658
    ...
    dispatchAgentRunFromGateway({
      ingressOpts: {
        ...
        ...(typeof request.maxTurns === "number"
          ? { maxTurns: String(Math.max(1, Math.floor(request.maxTurns))) }
          : {}),
        bestEffortDeliver,
        ...(request.toolsAllow?.length ? { toolsAllow: request.toolsAllow } : {}),
        ...(request.skillsAllow?.length ? { skillsAllow: request.skillsAllow } : {}),
        messageChannel: originMessageChannel,
        runId,
        lane: request.lane,
  ...
  "agent.inspect": ({ params, respond }) => {
    ...
    if (!inspection) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "agent inspection target not found"),
      );
      return;
    }
