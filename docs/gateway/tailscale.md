...
- `funnel`: Public HTTPS via `tailscale funnel`. CrawClaw requires a shared password.
...
- `token` (default when `CRAWCLAW_GATEWAY_TOKEN` is set)
...
CrawClaw only treats a request as Serve when it arrives from loopback with
...
- Set `gateway.tailscale.resetOnExit` if you want CrawClaw to undo `tailscale serve`
