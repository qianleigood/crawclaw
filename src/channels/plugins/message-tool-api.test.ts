...
const { tryLoadActivatedBundledPluginPublicSurfaceModuleSyncMock } = vi.hoisted(() => ({
  tryLoadActivatedBundledPluginPublicSurfaceModuleSyncMock: vi.fn(
    ({ artifactBasename, dirName }: { artifactBasename: string; dirName: string }) => {
      ...
      if (dirName === "disabled" && artifactBasename === "message-tool-api.js") {
        return null;
      }
      throw new Error(
...
describe("bundled channel message tool fast path", () => {
  beforeEach(() => {
    ...
    tryLoadActivatedBundledPluginPublicSurfaceModuleSyncMock.mockClear();
  });
  ...
  it("treats disabled artifacts as absent discovery", () => {
    expect(resolveBundledChannelMessageToolDiscoveryAdapter("disabled")).toBeUndefined();
  });
