...
elif [[ -n "${CRAWCLAW_LIVE_PROVIDERS:-}" && -n "${CRAWCLAW_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      crawclaw_live_collect_auth_dirs_from_csv "${CRAWCLAW_LIVE_PROVIDERS:-}"
      crawclaw_live_collect_auth_dirs_from_csv "${CRAWCLAW_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    ...
    {
      crawclaw_live_collect_auth_files_from_csv "${CRAWCLAW_LIVE_PROVIDERS:-}"
      crawclaw_live_collect_auth_files_from_csv "${CRAWCLAW_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
docker run --rm -t \
  ...
  -e CRAWCLAW_LIVE_MODELS="${CRAWCLAW_LIVE_MODELS:-modern}" \
  -e CRAWCLAW_LIVE_PROVIDERS="${CRAWCLAW_LIVE_PROVIDERS:-}" \
  -e CRAWCLAW_LIVE_MAX_MODELS="${CRAWCLAW_LIVE_MAX_MODELS:-12}" \
  -e CRAWCLAW_LIVE_MODEL_TIMEOUT_MS="${CRAWCLAW_LIVE_MODEL_TIMEOUT_MS:-}" \
  -e CRAWCLAW_LIVE_REQUIRE_PROFILE_KEYS="${CRAWCLAW_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e CRAWCLAW_LIVE_GATEWAY_MODELS="${CRAWCLAW_LIVE_GATEWAY_MODELS:-}" \
  -e CRAWCLAW_LIVE_GATEWAY_PROVIDERS="${CRAWCLAW_LIVE_GATEWAY_PROVIDERS:-}" \
  -e CRAWCLAW_LIVE_GATEWAY_MAX_MODELS="${CRAWCLAW_LIVE_GATEWAY_MAX_MODELS:-}" \
  -v "$ROOT_DIR":/src:ro \
