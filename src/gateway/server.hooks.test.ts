...
describe("gateway server hooks", () => {
  test("handles auth, wake, and agent flows", async () => {
    ...
    await withGatewayServer(async ({ port }) => {
      ...
      const resWake = await postHook(port, "/hooks/wake", { text: "Ping", mode: "now" });
      ...
      const resHeader = await postHook(
        ...
        "/hooks/wake",
        { text: "Header auth" },
        { token: null, headers: { "x-crawclaw-token": HOOK_TOKEN } },
      );
  ...
  test("preserves target-agent prefixes before isolated dispatch", async () => {
    ...
    await withGatewayServer(async ({ port }) => {
      ...
      const resAgent = await postHook(port, "/hooks/agent", {
        ...
        name: "Email",
        agentId: "hooks",
        sessionKey: "agent:hooks:ddingtalk:channel:c123",
      });
      expect(resAgent.status).toBe(200);
      await waitForSystemEvent();
      ...
      const routedCall = (cronIsolatedRun.mock.calls[0] as unknown[] | undefined)?.[0] as
        ...
        | undefined;
      expect(routedCall?.job?.agentId).toBe("hooks");
      expect(routedCall?.sessionKey).toBe("agent:hooks:ddingtalk:channel:c123");
      drainSystemEvents(resolveMainKey());
    });
  });
