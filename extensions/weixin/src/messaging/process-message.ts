...
function createLocalTypingCallbacks(params: {
  start: () => Promise<void>;
  stop: () => Promise<void>;
  ...
  const stopOnce = () => {
    ...
    stopIssued = true;
    void params.stop().catch((err) => params.onStopError?.(err));
  };
...
export type ProcessMessageDeps = {
  accountId: string;
  config: import("crawclaw/plugin-sdk/core").CrawClawConfig;
...
export async function processOneMessage(
  ...
  const typingCallbacks = createLocalTypingCallbacks({
