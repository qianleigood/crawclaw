...
async function loadFreshCrawClawToolsModuleForTest() {
  vi.resetModules();
...
describe("gateway tool", () => {
  beforeEach(async () => {
    await loadFreshCrawClawToolsModuleForTest();
  });
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-test-"));

    try {
      await withEnvAsync(
        { CRAWCLAW_STATE_DIR: stateDir, CRAWCLAW_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: crawclaw --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.patch when a legacy tools.bash alias no longer maps to protected exec security", async () => {
    const { callGatewayTool } = await import("./tools/gateway.js");
  ...
  it("rejects config.apply when protected exec settings are omitted", async () => {
    ...
    await expect(
      tool.execute("call-missing-protected", {
        action: "config.apply",
        raw: '{ agents: { defaults: { workspace: "~/crawclaw" } } }',
      }),
    ).rejects.toThrow(
