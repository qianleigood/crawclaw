...
async function main() {
  ...
  if (!logger) {
    logger = createLogger(resolveJob(Object.fromEntries([[selector[0].slice(2), selector[1]]])), {
      script: "grok_video_orchestrate",
    });
  }
  ...
  logger.info("orchestrate.step_started", {
    phase: "run",
    step: "run",
    message: "Running prepare/login check.",
  });
  const runArgs = selector.slice();
  if (args.headful) {
    runArgs.push("--headed");
  }
  if (args["manual-handoff-wait-sec"]) {
    runArgs.push("--manual-handoff-wait-sec", args["manual-handoff-wait-sec"]);
  }
  ...
  logger.info("orchestrate.step_started", {
    phase: "reference",
    step: "reference",
    message: "Inspecting staged references.",
  });
      ...
      JSON.stringify(
        {
          ...
          reason:
            "run step is blocked; refusing to call submit without an explicit safe bypass such as a known result URL",
          steps,
        },
        null,
        2,
  ...
  if (args["result-url"]) {
    submitArgs.push("--result-url", args["result-url"]);
  }
  if (args["submit-timeout-sec"]) {
    submitArgs.push("--submit-timeout-sec", args["submit-timeout-sec"]);
  }
  if (args["manual-handoff-wait-sec"]) {
    submitArgs.push("--manual-handoff-wait-sec", args["manual-handoff-wait-sec"]);
  }
  if (args.headful) {
    submitArgs.push("--headful");
  }
  logger.info("orchestrate.step_started", {
    phase: "submit",
    step: "submit",
    message: "Recording submit stage.",
  });
      ...
      JSON.stringify(
        {
          ...
          stoppedAfter: "submit",
          reason:
            submitJson.status === "awaiting_result_url"
              ? "submit placeholder needs a result URL before wait/download"
              : "--no-wait requested",
          steps,
        },
        null,
  ...
  if (args["result-url"]) {
    waitArgs.push("--result-url", args["result-url"]);
  }
  if (args["timeout-sec"]) {
    waitArgs.push("--timeout-sec", args["timeout-sec"]);
  }
  if (args["interval-sec"]) {
    waitArgs.push("--interval-sec", args["interval-sec"]);
  }
  if (args.headful) {
    waitArgs.push("--headful");
  }
  logger.info("orchestrate.step_started", {
    phase: "wait",
    step: "wait",
    message: "Waiting for result completion.",
  });
  ...
  if (boolFlag(args, "no-download")) {
    console.log(
      JSON.stringify(
        { ok: true, stoppedAfter: "wait", reason: "--no-download requested", steps },
        null,
        2,
      ),
    );
  ...
  if (args["result-url"]) {
    downloadArgs.push("--result-url", args["result-url"]);
  }
  if (args["timeout-sec"]) {
    downloadArgs.push("--timeout-sec", args["timeout-sec"]);
  }
  if (args["interval-sec"]) {
    downloadArgs.push("--interval-sec", args["interval-sec"]);
  }
  if (args["download-timeout-sec"]) {
    downloadArgs.push("--download-timeout-sec", args["download-timeout-sec"]);
  }
  if (args.headful) {
    downloadArgs.push("--headful");
  }
  logger.info("orchestrate.step_started", {
    phase: "download",
    step: "download",
    message: "Downloading final artifact.",
  });
