...
import { __testing as openClawToolsTesting, createCrawClawTools } from "./crawclaw-tools.js";
...
describe("sessions tools", () => {
  ...
  it("sessions_list filters kinds and includes messages", async () => {
    ...
    const tool = createCrawClawTools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createCrawClawTools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("sessions_send supports fire-and-forget and wait", async () => {
    ...
    await waitForCalls(() => calls.filter((call) => call.method === "chat.history").length, 9);
