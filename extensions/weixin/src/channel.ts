...
import { weixinSetupWizard } from "./setup-surface.js";
...
function resolveLocalPath(mediaUrl: string): string {
  ...
  // Resolve any relative path (./foo, ../foo, .crawclaw/foo, foo/bar) against cwd
 ...
 */
function resolveOutboundAccountId(cfg: CrawClawConfig, to: string): string {
  const allIds = listWeixinAccountIds(cfg);

  if (allIds.length === 0) {
    throw new Error(
      `weixin: no accounts registered — run \`crawclaw channels login --channel weixin\``,
    );
  }
...
async function sendWeixinOutbound(params: {
  ...
  if (!account.configured) {
    ...
    throw new Error("weixin not configured: please run `crawclaw channels login --channel weixin`");
  }
  ...
  if (sendingResult.cancelled) {
    ...
    return { channel: "weixin", messageId: "" };
  }
  ...
  try {
    ...
    return { channel: "weixin", messageId: result.messageId };
  } catch (err) {
    emitWeixinMessageSent({
      to: params.to,
      content: filteredText,
  ...
  outbound: {
    ...
    sendMedia: async (ctx) => {
      ...
      if (!account.configured) {
        ...
        throw new Error(
          "weixin not configured: please run `crawclaw channels login --channel weixin`",
        );
      }
  ...
  auth: {
    login: async ({ cfg, accountId, verbose, runtime }) => {
      ...
      if (waitResult.connected && waitResult.botToken && waitResult.accountId) {
        try {
          const localAccountId = account.accountId;
          saveWeixinAccount(localAccountId, {
            token: waitResult.botToken,
            baseUrl: waitResult.baseUrl,
            userId: waitResult.userId,
          });
          registerWeixinAccountId(localAccountId);
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      if (!account.configured) {
        ...
        ctx.log?.error?.(
          `[${account.accountId}] weixin not logged in — run: crawclaw channels login --channel weixin`,
        );
    ...
    loginWithQrWait: async (params) => {
      const localAccountId = normalizeAccountId(params.accountId) || DEFAULT_ACCOUNT_ID;
      ...
      if (result.connected && result.botToken && result.accountId) {
        try {
          ...
          triggerWeixinChannelReload();
          logger.info(`loginWithQrWait: saved account data for accountId=${localAccountId}`);
        } catch (err) {
