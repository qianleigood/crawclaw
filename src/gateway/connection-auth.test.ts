...
function cfg(input: Partial<CrawClawConfig>): CrawClawConfig {
  return input as CrawClawConfig;
}
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when CRAWCLAW env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when CRAWCLAW env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when CRAWCLAW env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
