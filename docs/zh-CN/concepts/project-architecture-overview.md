...
CrawClaw 是一个以 Gateway 为控制平面、以 Agent Kernel 为执行内核、以 Plugin Platform 为扩展底座、以 Channel Runtime 为渠道运行层、并同时承载 Memory、Workflow、ACP 和 Browser client 的统一 AI 运行时平台。

## 当前仓库的一级结构

- `src/`: 主运行时与控制平面
- `ui/`: Browser client 独立前端包
- `extensions/`: 官方扩展生态
- `packages/`: 辅助包与兼容包
