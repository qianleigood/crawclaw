...
export function resolveCrawClawUserDataDir(profileName = DEFAULT_CRAWCLAW_BROWSER_PROFILE_NAME) {
...
export function buildCrawClawChromeLaunchArgs(params: {
...
export async function launchCrawClawChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveCrawClawUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_CRAWCLAW_BROWSER_COLOR).toUpperCase(),
  );

  // First launch to create preference files if missing, then decorate and relaunch.
  const spawnOnce = () => {
    const args = buildCrawClawChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
    });
  ...
  if (needsDecorate) {
    try {
      decorateCrawClawProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦀 crawclaw browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`crawclaw browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`crawclaw browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦀 crawclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopCrawClawChrome(
