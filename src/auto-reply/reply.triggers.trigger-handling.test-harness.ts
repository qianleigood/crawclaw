...
const piEmbeddedMocks = getSharedMocks("crawclaw.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("crawclaw.trigger-handling.model-catalog-mocks", () => ({
...
const webSessionMocks = getSharedMocks("crawclaw.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    crawclawHome: process.env.CRAWCLAW_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("CRAWCLAW_HOME", snapshot.crawclawHome);
  restoreKey("CRAWCLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.CRAWCLAW_STATE_DIR = join(home, ".crawclaw");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "crawclaw-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".crawclaw", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): CrawClawConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "crawclaw"),
...
export async function runGreetingPromptForBareNew(params: {
  home: string;
  body: "/new";
  getReplyFromConfig: typeof import("./reply.js").getReplyFromConfig;
}) {
