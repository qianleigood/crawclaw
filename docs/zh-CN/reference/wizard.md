...
这是 `crawclaw onboard` CLI 向导的完整参考。
...
<Steps>
  ...
  <Step title="模型/认证">
    ...
    - OAuth 凭证保存在 `~/.crawclaw/credentials/oauth.json`；auth-profile 保存在 `~/.crawclaw/agents/<agentId>/agent/auth-profiles.json`（API key + OAuth）。
    - 更多细节： [/concepts/oauth](/concepts/oauth)
    <Note>
    无头/服务器提示：在有浏览器的机器上完成 OAuth，然后将
    `~/.crawclaw/credentials/oauth.json`（或 `$CRAWCLAW_STATE_DIR/credentials/oauth.json`）复制到
    Gateway 网关主机上。
    </Note>
  </Step>
  <Step title="工作区">
    - 默认为 `~/.crawclaw/workspace`（可配置）。
    - 为工作区植入智能体引导仪式所需的文件。
  ...
  <Step title="Web 搜索">
    ...
    - 使用 `--skip-search` 跳过。
    - 之后再配置：`crawclaw configure --section web`。
  </Step>
  <Step title="守护进程安装">
    - macOS：LaunchAgent
  ...
  <Step title="健康检查">
    - 启动 Gateway 网关（如果需要）并运行 `crawclaw health`。
    - 提示：`crawclaw status --deep` 会在状态输出中添加 Gateway 网关健康探测（需要能访问到 Gateway 网关）。
  </Step>
