  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${uiLiteral("Logs")}</div>
          <div class="card-sub">${uiLiteral("Gateway file logs (JSONL).")}</div>
        </div>
        <div class="row" style="gap: 8px;">
          ...
          >
            ${uiLiteral("Export")} ${uiLiteral(exportLabel)}
          </button>
        </div>
      </div>
      ...
      <div class="filters" style="margin-top: 14px;">
        <label class="field" style="min-width: 220px;">
          <span>${uiLiteral("Filter")}</span>
          <input
            .value=${props.filterText}
            @input=${(e: Event) => props.onFilterTextChange((e.target as HTMLInputElement).value)}
            placeholder=${uiLiteral("Search logs")}
          />
        </label>
        <label class="field checkbox">
          <span>${uiLiteral("Auto-follow")}</span>
          <input
            type="checkbox"
      ...
      ${props.file
        ? html`<div class="muted" style="margin-top: 10px;">${uiLiteral("File")}: ${props.file}</div>`
        : nothing}
      ${props.truncated
        ? html`
            <div class="callout" style="margin-top: 10px">
              ${uiLiteral("Log output truncated; showing latest chunk.")}
            </div>
          `
        : nothing}
      ${props.error
      ...
      <div class="log-stream" style="margin-top: 12px;" @scroll=${props.onScroll}>
        ${filtered.length === 0
          ? html` <div class="muted" style="padding: 12px">${uiLiteral("No log entries.")}</div> `
          : filtered.map(
              (entry) => html`
                <div class="log-row">
