...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevConfiguredHandshakeTimeout = process.env.CRAWCLAW_HANDSHAKE_TIMEOUT_MS;
      const prevHandshakeTimeout = process.env.CRAWCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      delete process.env.CRAWCLAW_HANDSHAKE_TIMEOUT_MS;
      process.env.CRAWCLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        const ws = await openWs(port);
        ...
        if (prevConfiguredHandshakeTimeout === undefined) {
          delete process.env.CRAWCLAW_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CRAWCLAW_HANDSHAKE_TIMEOUT_MS = prevConfiguredHandshakeTimeout;
        }
        if (prevHandshakeTimeout === undefined) {
          delete process.env.CRAWCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CRAWCLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers CRAWCLAW_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.CRAWCLAW_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.CRAWCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.CRAWCLAW_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.CRAWCLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(75);
        process.env.CRAWCLAW_HANDSHAKE_TIMEOUT_MS = "";
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.CRAWCLAW_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CRAWCLAW_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.CRAWCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CRAWCLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `crawclaw-test-device-${randomUUID()}.json`),
        nonce,
      });
