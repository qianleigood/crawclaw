...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleCrawClawDevices.length > 0) {
      params.logger.warn(
        `matrix: stale CrawClaw devices detected for ${params.auth.userId}: ${deviceHealth.staleCrawClawDevices.map((device) => device.deviceId).join(", ")}. Run 'crawclaw matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
