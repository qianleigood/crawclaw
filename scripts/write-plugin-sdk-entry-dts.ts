...
function rewriteFacadeTypeMapSpecifier(specifier: string): string {
  if (!specifier.startsWith("@crawclaw/")) {
    return specifier;
  }
  return `${GENERATED_FACADE_TYPE_MAP_DIST_PREFIX}${specifier.slice("@crawclaw/".length)}`;
}
...
function rewriteGeneratedFacadeTypeMapDts(): void {
  ...
  const rewritten = source.replace(/@crawclaw\/([^")\s]+)/g, (_match, suffix: string) =>
    rewriteFacadeTypeMapSpecifier(`@crawclaw/${suffix}`),
  );
  if (rewritten !== source) {
