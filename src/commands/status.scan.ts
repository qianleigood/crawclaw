...
export async function scanStatus(
  ...
  return await withProgress(
    ...
    async (progress) => {
      const feishuCliTimeoutMs = Math.min(8000, opts.timeoutMs ?? 10_000);
      const coldStart = isMissingConfigColdStart();
      ...
      const channels = await buildChannelsTable(cfg, {
        ...
        // Set `CRAWCLAW_SHOW_SECRETS=0` to force redaction.
        showSecrets: process.env.CRAWCLAW_SHOW_SECRETS?.trim() !== "0",
        sourceConfig: loadedRaw,
      });
      progress.tick();
