...
describe("resolveAcpxAgentCommand", () => {
  const expectedCrawClawCommand = (() => {
    const entrypoint = __testing.resolveNearestCrawClawEntrypoint();
    return entrypoint
      ? [
          __testing.quoteCommandPart(process.execPath),
          __testing.quoteCommandPart(entrypoint),
          "acp",
        ].join(" ")
      : "crawclaw acp";
  })();
  ...
  it.each([
    ...
    ["gemini", "gemini --acp"],
    ["crawclaw", expectedCrawClawCommand],
