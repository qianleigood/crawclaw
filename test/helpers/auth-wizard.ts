...
export function requireCrawClawAgentDir(): string {
  const agentDir = process.env.CRAWCLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("CRAWCLAW_AGENT_DIR not set");
  }
  return agentDir;
}
