...
export const loadConfig = vi.fn<() => CrawClawConfig>(() => ({}) as CrawClawConfig);
export const readConfigFileSnapshot = vi.fn();
export const writeConfigFile = vi.fn<(config: CrawClawConfig) => Promise<void>>(
  async () => undefined,
);
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: (config: CrawClawConfig) => writeConfigFile(config),
  replaceConfigFile: (params: { nextConfig: CrawClawConfig }) => replaceConfigFile(params),
}));
...
export function resetPluginsCliTestState() {
  ...
  loadConfig.mockReturnValue({} as CrawClawConfig);
  readConfigFileSnapshot.mockImplementation(async () => {
    const config = loadConfig();
    return {
      path: "/tmp/crawclaw-config.json5",
      exists: true,
      raw: "{}",
      parsed: config,
      resolved: config,
  ...
  applyExclusiveSlotSelection.mockImplementation(({ config }: { config: CrawClawConfig }) => ({
    config,
    warnings: [],
  }));
  ...
  recordHookInstall.mockImplementation((cfg: CrawClawConfig) => cfg);
}
