---
summary: "CrawClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up CrawClaw on a Raspberry Pi
  - Running CrawClaw on ARM devices
...
Run a persistent, always-on CrawClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
- `CRAWCLAW_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running CrawClaw, add a service drop-in to reduce restart
...
If possible, keep CrawClaw state/cache on SSD-backed storage to avoid SD-card
...
Most CrawClaw features work on ARM64, but some external binaries may need ARM builds:
...
# Common fix: rebuild
cd ~/crawclaw  # if using hackable install
npm run build
sudo systemctl restart crawclaw
```
