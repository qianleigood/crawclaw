...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.CRAWCLAW_GATEWAY_PORT).toBe("18789");
    expect(env.CRAWCLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(env.CRAWCLAW_SERVICE_MARKER).toBe("crawclaw");
    expect(env.CRAWCLAW_SERVICE_KIND).toBe("gateway");
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", CRAWCLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.CRAWCLAW_SYSTEMD_UNIT).toBe("crawclaw-gateway-work.service");
...
describe("resolveGatewayStateDir", () => {
  ...
  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", CRAWCLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".crawclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", CRAWCLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".crawclaw"));
  });

  it("uses CRAWCLAW_STATE_DIR when provided", () => {
  ...
  it("expands ~ in CRAWCLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", CRAWCLAW_STATE_DIR: "~/crawclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/crawclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { CRAWCLAW_STATE_DIR: "C:\\State\\crawclaw" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\crawclaw");
  });
