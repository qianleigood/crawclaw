---
summary: "How CrawClaw builds prompt context and reports token usage + costs"
read_when:
...
CrawClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

CrawClaw assembles its own system prompt on every run. It includes:
...
- Workspace bootstrap files. Default runtime injection is intentionally narrow:
  ...
  Large injected files are truncated by `agents.defaults.bootstrapMaxChars`
...
For images, CrawClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from CrawClaw session logs.

Other surfaces:

- **Web clients:** `/status` + `/usage` are supported.
- **Desktop and Gateway API:** show provider quota windows (not per-response costs).
...
`cacheWrite`. If pricing is missing, CrawClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. CrawClaw can
...
Anthropic's 1M context window is currently beta-gated. CrawClaw can inject the
...
CrawClaw skips the `context-1m-*` beta header because Anthropic currently
