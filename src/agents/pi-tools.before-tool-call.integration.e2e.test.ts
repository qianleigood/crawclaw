...
describe("before_tool_call hook integration", () => {
  ...
  it("blocks non-allowlisted special-agent tools before plugin hooks run", async () => {
    const execute = vi.fn().mockResolvedValue({ content: [], details: { ok: true } });
    // oxlint-disable-next-line typescript/no-explicit-any
    const tool = wrapToolWithBeforeToolCallHook({ name: "write", execute } as any, {
      specialToolAllowlist: ["read"],
    });
    const extensionContext = {} as Parameters<typeof tool.execute>[3];
    ...
    await expect(
      tool.execute(
        "call-special-deny",
        { file_path: "/tmp/blocked.txt", content: "blocked" },
        undefined,
        extensionContext,
      ),
  ...
  it("prefers runtime-derived session context over stale hook context", async () => {
    registerAgentRuntimeRun({
      runId: "run-runtime-hook",
      sessionKey: "session-from-runtime",
      sessionId: "ephemeral-runtime",
    ...
    const tool = wrapToolWithBeforeToolCallHook({ name: "read", execute } as any, {
      ...
      sessionId: "stale-ephemeral",
      runId: "run-runtime-hook",
    });

    await tool.execute("call-runtime-hook", { path: "/tmp/file" }, undefined, undefined);
  ...
  it("exposes runtime-derived guard context to hooks for hidden-control-ui runs", async () => {
    ...
    await tool.execute("call-hidden-hook", { path: "/tmp/hidden" }, undefined, undefined);
  ...
  it("exposes persisted capability snapshots to hook guard context", async () => {
    await withStateDirEnv("crawclaw-before-tool-call-capabilities-", async () => {
      const created = createRunningTaskRun({
        runtime: "subagent",
        ownerKey: "agent:main:main",
        scopeKind: "session",
        childSessionKey: "agent:worker:subagent:hook-capabilities",
        agentId: "worker",
        agentMetadata: {
          parentAgentId: "main",
          mode: "background",
        ...
        runId: "run-hook-capabilities",
        task: "Inspect guard capabilities",
      });
      ...
      await upsertAgentTaskRuntimeMetadata({
        ...
        mode: "background",
        spawnSource: "sessions_spawn",
        sessionKey: "agent:worker:subagent:hook-capabilities",
        capabilitySnapshot: {
          runtime: "subagent",
          agentId: "worker",
          parentAgentId: "main",
      ...
      registerAgentRuntimeRun({
        ...
        taskId: created.taskId,
        sessionKey: "agent:worker:subagent:hook-capabilities",
        sessionId: "ephemeral-hook-capabilities",
        agentId: "worker",
        runtime: "subagent",
        mode: "background",
        status: "running",
      ...
      await tool.execute("call-hook-capabilities", { path: "/tmp/file" }, undefined, undefined);
