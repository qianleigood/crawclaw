...
read_when:
  - Setting up Twitch chat integration for CrawClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. CrawClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
crawclaw plugins install @crawclaw/twitch
```

Local checkout (when running from a git repo):

```bash
crawclaw plugins install ./path/to/local/twitch-plugin
```
...
4. Configure the token:
   - Env: `CRAWCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "crawclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use CRAWCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
CRAWCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
