...
export const FIELD_HELP: Record<string, string> = {
  meta: "Metadata fields automatically maintained by CrawClaw to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
  "meta.lastTouchedVersion": "Auto-set when CrawClaw writes the config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for CrawClaw. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  wizard:
  ...
  "wizard.lastRunVersion":
    "CrawClaw version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version setup changes.",
  ...
  cli: "Terminal presentation controls for retained local diagnostic flows, such as banner and tagline style. Use this section to keep local output aligned with operator preference without changing runtime behavior.",
  "cli.language":
    "Human-facing terminal prompt/help language. Keeps command names, flags, config keys, and JSON output in English while translating prompts and help text for operators.",
  "cli.banner":
    "Terminal startup banner controls for title/version line and tagline style behavior. Keep banner enabled for fast version/context checks, then tune tagline mode to your preferred noise level.",
  "cli.banner.taglineMode":
    'Controls tagline style in the terminal startup banner: "random" (default) picks from the rotating tagline pool, "default" always shows the neutral default tagline, and "off" hides tagline text while keeping the banner version line.',
  update:
    "Update-channel and startup-check behavior for keeping CrawClaw runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  ...
  "talk.provider": "Active Talk provider id.",
  "talk.providers":
    "Provider-specific Talk settings keyed by provider id. Use this shape for Talk integrations so speech providers stay decoupled from core config schema.",
  ...
  "agents.list[].runtime":
    "Optional runtime descriptor for this agent. Use embedded for default CrawClaw execution or acp for external ACP harness defaults.",
  "agents.list[].runtime.type":
    'Runtime type for this agent: "embedded" (default CrawClaw runtime) or "acp" (ACP harness defaults).',
  ...
  "agents.list[].runtime.acp.agent":
    "Optional ACP harness agent id to use for this CrawClaw agent (for example codex, claude, cursor, gemini, crawclaw).",
  ...
  "browser.profiles":
    "Named managed browser profiles used for explicit routing and visual identity. Keep profile names consistent and avoid overlapping endpoint definitions.",
  "browser.profiles.*.cdpPort":
    "Per-profile local CDP port allocated for the managed browser profile. Use unique ports per profile to avoid connection collisions.",
  ...
  "discovery.mdns.mode":
    'mDNS broadcast mode ("minimal" default, "full" includes sshPort, "off" disables mDNS).',
  discovery:
  ...
  "discovery.wideArea.domain":
    "Optional unicast DNS-SD domain for wide-area discovery, such as crawclaw.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
  "discovery.mdns":
  ...
  web: "Web chat runtime settings for heartbeat and reconnect behavior when operating web-based chat surfaces. Use reconnect values tuned to your network reliability profile and expected uptime needs.",
  "web.enabled":
    "Enables the web chat runtime and related websocket lifecycle behavior. Keep disabled when web chat is unused to reduce active connection management overhead.",
  "web.heartbeatSeconds":
    "Heartbeat interval in seconds for web chat connectivity and liveness maintenance. Use shorter intervals for faster detection, or longer intervals to reduce keepalive chatter.",
  "web.reconnect":
    "Reconnect backoff policy for web chat reconnect attempts after transport failure. Keep bounded retries and jitter tuned to avoid thundering-herd reconnect behavior.",
  ...
  "web.reconnect.factor":
    "Exponential backoff multiplier used between reconnect attempts in web chat retry loops. Keep factor above 1 and tune with jitter for stable large-fleet reconnect behavior.",
  ...
  "gateway.reload.mode":
    'Controls how config edits are applied: "off" ignores live edits, "restart" forces a process restart for debugging/ops, "hot" applies in-process, and "hybrid" applies schema-owned config in-process by default.',
  ...
  "bindings[].match.guildId":
    "Optional channel-style guild/server ID constraint for binding evaluation in multi-server deployments. Use this when the same peer identifiers can appear across different guilds.",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $CRAWCLAW_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "tools.media.image.prompt":
    "Instruction template used for image understanding requests to shape extraction style and detail level. Keep prompts deterministic so outputs stay consistent across turns and chat surfaces.",
  ...
  "approvals.exec.targets[].threadId":
    "Optional thread/topic target for approval delivery. Use this to keep approval traffic contained in operational threads.",
  "approvals.plugin":
    "Groups plugin-approval forwarding behavior including enablement, routing mode, filters, and explicit targets. Independent of exec approval forwarding.",
  ...
  "models.providers.*.injectNumCtxForOpenAICompat":
    "Controls whether CrawClaw injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
  ...
  "memory.notebooklm.auth.degradedCooldownMs":
    "Cooldown window after NotebookLM enters a degraded or expired state. During this window CrawClaw reuses the cached degraded status instead of hammering the provider with repeated probes.",
  ...
  "plugins.slots":
    "Selects which plugins own exclusive runtime slots such as the memory slot so only one plugin provides that capability. Use explicit slot ownership to avoid overlapping providers with conflicting behavior.",
  "plugins.entries":
    "Per-plugin settings keyed by plugin ID including enablement and plugin-specific runtime configuration payloads. Use this for scoped plugin tuning without changing global loader policy.",
  "plugins.entries.*.enabled":
    "Per-plugin enablement override for a specific entry, applied on top of global plugin policy. The gateway swaps plugin runtime state during live reconfigure.",
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `crawclaw plugins update` to locate install sources).",
  ...
  "agents.defaults.runtime":
    "Rust agent runtime hardening controls for how workspace-local runtime settings are trusted and applied in CrawClaw sessions.",
  "agents.defaults.runtime.projectSettingsPolicy":
    'How the Rust agent runtime handles workspace-local settings: "sanitize" (default) strips shellPath/shellCommandPrefix, "ignore" disables project settings entirely, and "trusted" applies project settings as-is.',
  ...
  "commands.native":
    "Registers native slash/menu commands with channels that support command registration. Keep enabled for discoverability unless you intentionally run text-only command workflows.",
  ...
  "commands.mcp":
    "Allow /mcp chat command to manage CrawClaw MCP server config under mcp.servers (default: false).",
  "commands.plugins":
  ...
  mcp: "Global MCP server definitions managed by CrawClaw. Runtime adapters can consume these servers without storing them inside project settings.",
  "mcp.servers":
    "Named MCP server definitions. CrawClaw stores them in its own config and runtime adapters decide which transports are supported at execution time.",
  session:
  ...
  "session.typingIntervalSeconds":
    "Controls interval for repeated typing indicators while replies are being prepared. Increase to reduce chatty updates or decrease for more active typing feedback.",
  "session.typingMode":
  ...
  "session.parentForkMaxTokens":
    "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, CrawClaw starts a fresh thread session instead of forking; set 0 to disable this protection.",
  "session.mainKey":
  ...
  hooks:
    "Inbound webhook automation surface for mapping external events into wake or agent actions in CrawClaw. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
  "hooks.enabled":
  ...
  "hooks.defaultSessionKey":
    "Fallback session key used for hook deliveries when a request does not provide one. Use a stable but scoped key to avoid mixing unrelated automation conversations.",
  ...
  "messages.responsePrefix":
    "Prefix text prepended to outbound assistant replies. Use for lightweight branding/context tags and avoid long prefixes that reduce content density.",
  "messages.groupChat":
  ...
  "messages.statusReactions.enabled":
    "Enable lifecycle status reactions for channels that support them. When enabled, the ack reaction becomes the initial 'queued' state and progresses through thinking, tool, done/error automatically. Default: false.",
  "messages.statusReactions.emojis":
    "Override default status reaction emojis. Keys: thinking, compacting, tool, coding, web, done, error, stallSoft, stallHard. Must be valid channel reaction emojis.",
