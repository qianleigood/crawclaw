...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.CRAWCLAW_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.CRAWCLAW_GATEWAY_PASSWORD ?? configuredPassword;
  ...
  try {
    ...
    note(
      [
        "Docs:",
        "https://docs.crawclaw.ai/gateway/health",
        "https://docs.crawclaw.ai/gateway/troubleshooting",
      ].join("\n"),
      "Health check help",
    );
  }
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from crawclaw.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<CrawClawConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "CrawClaw update wizard" : "CrawClaw configure");
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("crawclaw doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: process.env.CRAWCLAW_GATEWAY_TOKEN ?? baseLocalProbeToken,
      password: process.env.CRAWCLAW_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    });
