...
describe("gateway auth", () => {
  ...
  it("resolves token/password from CRAWCLAW gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          CRAWCLAW_GATEWAY_TOKEN: "env-token",
          CRAWCLAW_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides CRAWCLAW_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        CRAWCLAW_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
