...
function resolveSubagentTaskRuntime(
  taskRuntime: TaskRuntime | null | undefined,
): Extract<TaskRuntime, "subagent" | "acp"> {
  return taskRuntime === "acp" ? "acp" : "subagent";
}
...
export function createSubagentRunManager(params: {
  ...
  const syncSubagentTaskMetadataBestEffort = async (
    taskId: string,
    registerParams: {
      runId: string;
      childSessionKey: string;
    ...
    const childAgentId = parseAgentSessionKey(registerParams.childSessionKey)?.agentId;
    const parentAgentId = parseAgentSessionKey(registerParams.requesterSessionKey)?.agentId;
    ...
    try {
      ...
      log.warn("Failed to persist subagent task runtime metadata", {
        taskId,
        runId: registerParams.runId,
  ...
  const registerSubagentRun = (registerParams: {
    ...
    if (taskRuntime !== "acp") {
      try {
        ...
        registerAgentRunContext(registerParams.runId, {
          sessionKey: registerParams.childSessionKey,
          agentId: childAgentId,
          parentAgentId,
        ...
        void syncSubagentTaskMetadataBestEffort(createdTask.taskId, {
          runId: registerParams.runId,
          childSessionKey: registerParams.childSessionKey,
          requesterSessionKey: registerParams.requesterSessionKey,
          taskRuntime,
