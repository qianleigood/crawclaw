...
IMAGE_NAME="crawclaw-openwebui-e2e"
OPENWEBUI_IMAGE="${OPENWEBUI_IMAGE:-ghcr.io/open-webui/open-webui:v0.8.10}"
PROFILE_FILE="${CRAWCLAW_PROFILE_FILE:-${CRAWCLAW_PROFILE_FILE:-$HOME/.profile}}"
MODEL="${CRAWCLAW_OPENWEBUI_MODEL:-${CRAWCLAW_OPENWEBUI_MODEL:-openai/gpt-5.4}}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${CRAWCLAW_OPENWEBUI_PROMPT:-${CRAWCLAW_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}}"
PORT="${CRAWCLAW_OPENWEBUI_GATEWAY_PORT:-${CRAWCLAW_OPENWEBUI_GATEWAY_PORT:-18789}}"
WEBUI_PORT="${CRAWCLAW_OPENWEBUI_PORT:-${CRAWCLAW_OPENWEBUI_PORT:-8080}}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${CRAWCLAW_OPENWEBUI_ADMIN_EMAIL:-${CRAWCLAW_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}}"
ADMIN_PASSWORD="${CRAWCLAW_OPENWEBUI_ADMIN_PASSWORD:-${CRAWCLAW_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}}"
NET_NAME="crawclaw-openwebui-e2e-$$"
GW_NAME="crawclaw-openwebui-gateway-$$"
OW_NAME="crawclaw-openwebui-$$"
...
AUTH_DIRS=()
if [[ -n "${CRAWCLAW_DOCKER_AUTH_DIRS:-${CRAWCLAW_DOCKER_AUTH_DIRS:-}}" ]]; then
...
docker run -d \
  ...
  -e "CRAWCLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "CRAWCLAW_OPENWEBUI_MODEL=$MODEL" \
  ...
  bash -lc '
    ...
    node "$entry" config set agents.defaults.model.primary "$CRAWCLAW_OPENWEBUI_MODEL" >/dev/null
...
docker run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="CrawClaw E2E" \
  -e WEBUI_SECRET_KEY="crawclaw-openwebui-e2e-secret" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="CrawClaw E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="crawclaw/default" \
...
echo "Running Open WebUI -> CrawClaw smoke..."
docker exec \
