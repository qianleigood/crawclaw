...
- `outputFormat`：macOS 上默认为 `pcm_44100`（设置 `mp3_*` 以强制 MP3 流式传输）
...
- TTS 使用带有 `ELEVENLABS_API_KEY` 的 ElevenLabs 流式 API，并在 macOS 上进行增量播放以降低延迟。
- `eleven_v3` 的 `stability` 验证为 `0.0`、`0.5` 或 `1.0`；其他模型接受 `0..1`。
- 设置时 `latency_tier` 验证为 `0..4`。
- 旧版移动端运行时曾使用额外输出格式；这些实现已不再随本仓库分发。
