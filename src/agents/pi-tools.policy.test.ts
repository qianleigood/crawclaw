...
describe("resolveSubagentToolPolicy depth awareness", () => {
  ...
  it("applies configured deny to web tools even though they are allowed by default", () => {
    const cfg = {
      agents: { defaults: { subagents: { maxSpawnDepth: 2 } } },
      tools: {
        subagents: {
    ...
    const policy = resolveSubagentToolPolicy(cfg, 1);
    expect(isToolAllowedByPolicyName("web_search", policy)).toBe(false);
  ...
  it("depth-1 orchestrator still denies gateway and cron but allows web tools", () => {
    const policy = resolveSubagentToolPolicy(baseCfg, 1);
  ...
  it("uses stored leaf role for flat depth-1 session keys", () => {
    const storePath = path.join(
      os.tmpdir(),
      `crawclaw-subagent-policy-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
