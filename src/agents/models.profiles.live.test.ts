...
const LIVE_HEARTBEAT_MS = Math.max(1_000, toInt(process.env.CRAWCLAW_LIVE_HEARTBEAT_MS, 30_000));
const LIVE_SETUP_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.CRAWCLAW_LIVE_SETUP_TIMEOUT_MS, 45_000),
);
  ...
  it(
    ...
    async () => {
      ...
      await withLiveStageTimeout(
        ensureCrawClawModelsJson(cfg),
        "[live-models] prepare models.json",
      );
      if (!DIRECT_ENABLED) {
        logProgress(
          "[live-models] skipping (set CRAWCLAW_LIVE_MODELS=modern|all|<list>; all=modern)",
        );
        return;
      }
      ...
      const rawModels = process.env.CRAWCLAW_LIVE_MODELS?.trim();
      const useModern = rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseModelFilter(rawModels) : null;
      const allowNotFoundSkip = useModern;
      const providers = parseProviderFilter(process.env.CRAWCLAW_LIVE_PROVIDERS);
      const perModelTimeoutMs = toInt(process.env.CRAWCLAW_LIVE_MODEL_TIMEOUT_MS, 30_000);
      const maxModels = toInt(process.env.CRAWCLAW_LIVE_MAX_MODELS, 0);
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[live-models] capped to ${selectedCandidates.length}/${candidates.length} via CRAWCLAW_LIVE_MAX_MODELS=${maxModels}`,
        );
      }
