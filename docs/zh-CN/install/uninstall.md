...
summary: 完全卸载 CrawClaw（CLI、服务、状态、工作区）
title: 卸载
x-i18n:
...
如果你将 `CRAWCLAW_CONFIG_PATH` 设置为状态目录外的自定义位置，也请删除该文件。
...
如果 Gateway 网关服务持续运行但 `crawclaw` 缺失，请使用此方法。

### macOS（launchd）

默认标签是 `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.crawclaw.*` 可能仍然存在）：
...
如果你使用了配置文件，请将标签和 plist 名称替换为 `bot.molt.<profile>`。如果存在任何旧版 `com.crawclaw.*` plist，请将其移除。

### Linux（systemd 用户单元）

默认单元名称是 `crawclaw-gateway.service`（或 `crawclaw-gateway-<profile>.service`）：
...
默认任务名称是 `CrawClaw Gateway`（或 `CrawClaw Gateway (<profile>)`）。
任务脚本位于你的状态目录下。
...
如果你使用了配置文件，请删除匹配的任务名称和 `~\.crawclaw-<profile>\gateway.cmd`。

## 普通安装 vs 源码检出

### 普通安装（install.sh / npm / pnpm / bun）

如果你使用了 `https://crawclaw.ai/install.sh` 或 `install.ps1`，CLI 是通过 `npm install -g crawclaw@latest` 安装的。
使用 `npm rm -g crawclaw` 移除（或 `pnpm remove -g` / `bun remove -g`，如果你是用那种方式安装的）。

### 源码检出（git clone）

如果你从仓库检出运行（`git clone` + `crawclaw ...` / `bun run crawclaw ...`）：
