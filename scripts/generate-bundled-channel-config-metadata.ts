...
const DEFAULT_OUTPUT_PATH = "src/generated/config/bundled-channel-config-metadata.generated.ts";
...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const crawclawMeta =
    ...
    "crawclaw" in source.packageJson
      ? (source.packageJson.crawclaw as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    crawclawMeta &&
    typeof crawclawMeta.channel === "object" &&
    crawclawMeta.channel &&
    !Array.isArray(crawclawMeta.channel)
      ? (crawclawMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
