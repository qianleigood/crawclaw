import type { SlackSlashCommandConfig } from "crawclaw/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "crawclaw");
  const name = normalizedName || "crawclaw";
  return {
