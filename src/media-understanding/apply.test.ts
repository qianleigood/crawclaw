...
const TEMP_MEDIA_PREFIX = "crawclaw-media-";
...
function createGroqAudioConfig(): CrawClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): CrawClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): CrawClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: CrawClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredCrawClawTmpDir();
