...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".crawclaw"].map((dir) => path.resolve(root, entry.name, dir));
...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for CRAWCLAW_STATE_DIR.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // CRAWCLAW_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".crawclaw");
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.crawclaw).",
        `  Set locally: CRAWCLAW_STATE_DIR=~/.crawclaw ${formatCliCommand("crawclaw doctor")}`,
      ].join("\n"),
    );
  }
  ...
  if (stateDirExists) {
    ...
    if (requireOAuthDir) {
      ...
      warnings.push(
        `- OAuth dir not present (${displayOauthDir}). Skipping create because no channel config is active.`,
      );
    }
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Verify sessions in store: ${formatCliCommand(`crawclaw sessions --store "${absoluteStorePath}"`)}`,
          `  Preview cleanup impact: ${formatCliCommand(`crawclaw sessions cleanup --store "${absoluteStorePath}" --dry-run`)}`,
          `  Prune missing entries: ${formatCliCommand(`crawclaw sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`,
        ].join("\n"),
      );
    }
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.crawclaw out of git; it contains credentials and session history.",
