...
const buildSessionLookup = (
  ...
  cfg: { session: { mainKey: "agent:main:main" } } as CrawClawConfig,
...
vi.mock("./session-utils.js", () => ({
  ...
  resolveSessionModelRef: vi.fn(
    (_cfg: CrawClawConfig, entry?: { model?: string; modelProvider?: string }) => ({
...
const requestMainSessionWakeNowMock = vi.mocked(requestMainSessionWakeNow);
...
describe("node exec events", () => {
  ...
  it("enqueues exec.finished events with output", async () => {
    ...
    expect(requestMainSessionWakeNowMock).toHaveBeenCalledWith({ reason: "exec-event" });
  });
...
describe("notifications changed events", () => {
  ...
  it("does not wake heartbeat when notifications.changed event is deduped", async () => {
    ...
    expect(requestMainSessionWakeNowMock).toHaveBeenCalledTimes(1);
  });
