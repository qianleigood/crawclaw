...
# CrawClaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://crawclaw.ai/install.sh | bash
...
GUM_VERSION="${CRAWCLAW_GUM_VERSION:-${CRAWCLAW_GUM_VERSION:-0.17.0}}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦀 CrawClaw Installer")"
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$CRAWCLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.crawclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_crawclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.crawclaw-* "$npm_root"/.crawclaw-* "$npm_root"/crawclaw "$npm_root"/crawclaw 2>/dev/null || true
}

extract_crawclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_crawclaw_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/crawclaw" && "$bin_path" != "$npm_bin/crawclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/crawclaw"|"/usr/local/bin/crawclaw"|"/opt/homebrew/bin/crawclaw"|"/usr/local/bin/crawclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/crawclaw/"* || "$target" == *"/node_modules/crawclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale crawclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing crawclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing CrawClaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_crawclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "CrawClaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -Eq "ENOTEMPTY: directory not empty, rename .*(crawclaw|crawclaw)" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_crawclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_crawclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "CrawClaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because a crawclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
TAGLINES+=("I'll season your workflow like a crab boil: messy, delicious, effective.")
...
pick_tagline() {
    ...
    if [[ -n "${CRAWCLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${CRAWCLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((CRAWCLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${CRAWCLAW_NO_ONBOARD:-${CRAWCLAW_NO_ONBOARD:-0}}
NO_PROMPT=${CRAWCLAW_NO_PROMPT:-${CRAWCLAW_NO_PROMPT:-0}}
DRY_RUN=${CRAWCLAW_DRY_RUN:-${CRAWCLAW_DRY_RUN:-0}}
INSTALL_METHOD=${CRAWCLAW_INSTALL_METHOD:-${CRAWCLAW_INSTALL_METHOD:-}}
CRAWCLAW_VERSION=${CRAWCLAW_VERSION:-${CRAWCLAW_VERSION:-latest}}
USE_BETA=${CRAWCLAW_BETA:-${CRAWCLAW_BETA:-0}}
GIT_DIR_DEFAULT="${HOME}/crawclaw"
GIT_DIR=${CRAWCLAW_GIT_DIR:-${CRAWCLAW_GIT_DIR:-$GIT_DIR_DEFAULT}}
...
print_usage() {
    cat <<EOF
CrawClaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://crawclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/crawclaw)
...
Environment variables:
  ...
  CRAWCLAW_VERIFY_INSTALL=1
  CRAWCLAW_DRY_RUN=1
  CRAWCLAW_NO_ONBOARD=1
  CRAWCLAW_VERBOSE=1
  CRAWCLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://crawclaw.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://crawclaw.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://crawclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected CrawClaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a CrawClaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_crawclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"crawclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
# Check for existing CrawClaw installation
check_existing_crawclaw() {
    if [[ -n "$(type -P crawclaw 2>/dev/null || true)" || -n "$(type -P crawclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing CrawClaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make crawclaw show as \"command not found\" in new terminals."
...
warn_crawclaw_not_found() {
    ui_warn "Installed, but crawclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t crawclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named crawclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_crawclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P crawclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/crawclaw" ]]; then
        echo "${npm_bin}/crawclaw"
        return 0
    fi
    if [[ -n "$npm_bin" && -x "${npm_bin}/crawclaw" ]]; then
        echo "${npm_bin}/crawclaw"
...
install_crawclaw_from_git() {
    ...
    if [[ -d "$repo_dir/.git" ]]; then
        ...
        ui_info "Installing CrawClaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning CrawClaw" git clone "$repo_url" "$repo_dir"
    fi
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view crawclaw dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:crawclaw/crawclaw#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_crawclaw() {
    local package_name="crawclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            CRAWCLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="crawclaw"
        else
            CRAWCLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${CRAWCLAW_VERSION}" ]]; then
        CRAWCLAW_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${CRAWCLAW_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${CRAWCLAW_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing CrawClaw v${resolved_version}"
    else
        ui_info "Installing CrawClaw (${CRAWCLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${CRAWCLAW_VERSION}")"

    if ! install_crawclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_crawclaw_paths
        install_crawclaw_npm "${install_spec}"
    fi

    if [[ "${CRAWCLAW_VERSION}" == "latest" && "${package_name}" == "crawclaw" ]]; then
        if ! resolve_crawclaw_bin &> /dev/null; then
            ui_warn "npm install crawclaw@latest failed; retrying crawclaw@next"
            cleanup_npm_crawclaw_paths
            install_crawclaw_npm "crawclaw@next"
        fi
    fi

    ensure_crawclaw_bin_link || true
...
run_doctor() {
    ...
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (crawclaw not on PATH yet)"
        warn_crawclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${CRAWCLAW_PROFILE:-${CRAWCLAW_PROFILE:-default}}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.crawclaw/workspace-${profile}"
    else
        echo "${HOME}/.crawclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run crawclaw onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but crawclaw not on PATH; skipping onboarding"
        warn_crawclaw_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run crawclaw onboard to retry"
        return
    }
}
...
resolve_crawclaw_version() {
    ...
    local claw="${CRAWCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v crawclaw &> /dev/null; then
        claw="$(command -v crawclaw)"
    fi
    if [[ -z "$claw" ]] && command -v crawclaw &> /dev/null; then
        claw="$(command -v crawclaw)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_crawclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/crawclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/crawclaw/package.json').version)" 2>/dev/null || true)
        elif [[ -n "$npm_root" && -f "$npm_root/crawclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/crawclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: crawclaw not on PATH yet"
        warn_crawclaw_not_found
        return 1
    fi

    run_quiet_step "Checking CrawClaw version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: crawclaw gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_crawclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found CrawClaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set CRAWCLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    ui_stage "Installing CrawClaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g crawclaw &>/dev/null || npm list -g crawclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g crawclaw 2>/dev/null || true
            npm uninstall -g crawclaw 2>/dev/null || true
        ...
        install_crawclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/crawclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/crawclaw"
    ...
    CRAWCLAW_BIN="$(resolve_crawclaw_bin || true)"
    ...
    installed_version=$(resolve_crawclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦀 CrawClaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦀 CrawClaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/crawclaw"
        ui_kv "Update command" "crawclaw update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://crawclaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running crawclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                CRAWCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                CRAWCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                CRAWCLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run crawclaw doctor and crawclaw plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (crawclaw not on PATH yet)"
                    warn_crawclaw_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run crawclaw onboard to finish setup"
            return 0
        fi
    fi

    if command -v crawclaw &> /dev/null || command -v crawclaw &> /dev/null; then
        local claw="${CRAWCLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_crawclaw_bin || true)"
        fi
...
if [[ "${CRAWCLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
