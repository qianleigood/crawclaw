---
summary: "CLI reference for `crawclaw onboard` (interactive onboarding)"
read_when:
...
Onboarding includes a Memory / Experience step for experience capture. CrawClaw
keeps a local sync queue so extracted experience is not lost while NotebookLM is
not ready, but prompt-facing experience recall is NotebookLM-only. If NotebookLM
recall and sync are enabled, onboarding asks for the NotebookLM CLI command and
notebook id so the provider can be checked immediately. If that provider check
recommends `crawclaw memory login`, onboarding can offer to run the login flow
near the end.
...
```bash
crawclaw onboard
crawclaw onboard --flow quickstart
crawclaw onboard --flow manual
crawclaw onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`CRAWCLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment
(legacy: `CRAWCLAW_ALLOW_INSECURE_PRIVATE_WS=1`).
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `crawclaw gateway run`.
