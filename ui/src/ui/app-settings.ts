...
type SettingsHost = {
  ...
  loadAgentInspection?: (params: {
    runId?: string | null;
    taskId?: string | null;
  }) => Promise<void>;
  resetAgentInspectionState?: () => void;
...
export function applySettings(host: SettingsHost, next: UiSettings) {
  const uiMode = next.uiMode === "advanced" ? "advanced" : "simple";
  ...
  const normalizedTab = normalizeTabForMode(host.tab, uiMode);
  if (host.tab !== normalizedTab) {
    host.tab = normalizedTab;
    syncUrlWithTab(host, normalizedTab, true);
    if (host.connected) {
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as CrawClawApp);
  }
  if (host.tab === "usage") {
    await loadUsage(host as unknown as CrawClawApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as CrawClawApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as CrawClawApp);
  }
  if (host.tab === "workflows") {
    await loadWorkflows(host as unknown as CrawClawApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as CrawClawApp);
    await loadConfig(host as unknown as CrawClawApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as CrawClawApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as CrawClawApp, agentId);
      if (host.agentsPanel === "files") {
        void loadAgentFiles(host as unknown as CrawClawApp, agentId);
      }
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as CrawClawApp, agentId);
      }
      if (host.agentsPanel === "inspect") {
        const activeSessionAgentId = host.sessionKey
          ? resolveAgentIdFromSessionKey(host.sessionKey)
          : null;
        if (host.chatRunId && activeSessionAgentId && activeSessionAgentId === agentId) {
          void host.loadAgentInspection?.({ runId: host.chatRunId });
        } else {
          host.resetAgentInspectionState?.();
        }
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as CrawClawApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as CrawClawApp);
    await loadDevices(host as unknown as CrawClawApp);
    await loadConfig(host as unknown as CrawClawApp);
    await loadExecApprovals(host as unknown as CrawClawApp);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as CrawClawApp);
    await loadConfig(host as unknown as CrawClawApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as CrawClawApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as CrawClawApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__CRAWCLAW_CONTROL_UI_BASE_PATH__;
...
function applyTabSelection(
  ...
  next = normalizeTabForMode(next, host.settings.uiMode ?? "simple");
  const prev = host.tab;
  if (host.tab !== next) {
    host.tab = next;
  }
  ...
  syncInspectPolling(host as unknown as Parameters<typeof syncInspectPolling>[0]);
  syncWorkflowPolling(host as unknown as Parameters<typeof syncWorkflowPolling>[0]);
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as CrawClawApp;
...
async function loadOverviewLogs(host: CrawClawApp) {
...
export function buildAttentionItems(host: CrawClawApp) {
  ...
  if (host.connected && host.feishuCliSupported === false) {
    items.push({
      severity: "warning",
      icon: "key",
      title: "Feishu user tools not loaded",
      description:
        "This gateway does not have the Feishu CLI user-identity plugin loaded. Enable plugins.entries.feishu-cli.enabled and restart the gateway.",
    });
  } else if (host.connected && host.feishuCliError) {
    items.push({
      severity: "warning",
      icon: "key",
      title: "Feishu user status unavailable",
      description: host.feishuCliError,
    });
  } else if (host.connected && host.feishuCliStatus?.authOk === false) {
    items.push({
      severity: "warning",
      icon: "key",
      title: "Feishu user auth required",
      description:
        host.feishuCliStatus.message ??
        "Feishu user tools are installed, but user authentication is not ready. Run crawclaw feishu-cli auth login first.",
    });
  }
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as CrawClawApp, true),
    loadConfigSchema(host as unknown as CrawClawApp),
    loadConfig(host as unknown as CrawClawApp),
  ]);
}
