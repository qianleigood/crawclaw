...
function resolveQianfanPreset(cfg: CrawClawConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: CrawClawConfig) => {
...
export function applyQianfanProviderConfig(cfg: CrawClawConfig): CrawClawConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: CrawClawConfig): CrawClawConfig {
