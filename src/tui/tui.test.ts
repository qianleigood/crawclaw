...
describe("resolveInitialTuiAgentId", () => {
  const cfg: CrawClawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/crawclaw" },
        { id: "ops", workspace: "/tmp/crawclaw/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/crawclaw/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run crawclaw devices list");
    expect(state.pairingHint).toContain("crawclaw devices list");
  });
