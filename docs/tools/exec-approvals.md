...
- requested exec policy in CrawClaw config (`tools.exec.*`)
- host-local approvals policy in `~/.crawclaw/exec-approvals.json`
...
When `tools.exec.strictInlineEval=true`, CrawClaw treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
editing in the active approvals UI. Patterns are **case-insensitive glob matches**.
...
If you explicitly opt `jq` into `safeBins`, CrawClaw still rejects the `env` builtin in safe-bin
...
When an async exec approval is denied, CrawClaw prevents the agent from reusing
