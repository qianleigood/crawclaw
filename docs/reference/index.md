---
summary: Entry point for reference docs, specs, release notes, and stable supporting material
read_when:
  - You need stable reference material rather than conceptual walkthroughs
  - You are looking for test, release, migration, RPC, or cost references
title: Reference Docs
---

# Reference Docs

`reference/` is for stable supporting material: specs, checklists, release notes, migration guides, cost references, and operational reference docs.

## Core Reference

- [Testing](/reference/test)
- [Release policy](/reference/RELEASING)
- [RPC](/reference/rpc)
- [Wizard reference](/reference/wizard)
- [Credits](/reference/credits)

## Runtime And Session Reference

- [Session management and compaction](/reference/session-management-compaction)
- [Transcript hygiene](/reference/transcript-hygiene)
- [Token use](/reference/token-use)
- [Prompt caching](/reference/prompt-caching)
- [Memory config](/reference/memory-config)

## Migration And Compatibility

- [CrawClaw migration](/reference/crawclaw-migration)
- [Open source release checklist](/reference/open-source-release-checklist)
- [Support matrix](/reference/support-matrix)

## Product And Platform Reference

- [API usage costs](/reference/api-usage-costs)
- [n8n workflow architecture](/reference/n8n-workflow-architecture)
- [SecretRef credential surface](/reference/secretref-credential-surface)
- [Skills audit](/reference/skills-audit)
- [Default AGENTS](/reference/AGENTS.default)

## Design Notes

- [Browser redesign Pinchtab plan](/reference/browser-redesign-pinchtab-plan)
