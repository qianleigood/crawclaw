...
describe("canvas a2ui copy", () => {
  async function withA2uiFixture(run: (dir: string) => Promise<void>) {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-a2ui-"));
    try {
      await run(dir);
    } finally {
  ...
  it("skips missing assets when CRAWCLAW_A2UI_SKIP_MISSING=1", async () => {
    await withA2uiFixture(async (dir) => {
      const previous = process.env.CRAWCLAW_A2UI_SKIP_MISSING;
      process.env.CRAWCLAW_A2UI_SKIP_MISSING = "1";
      try {
        await expect(
        ...
        if (previous === undefined) {
          delete process.env.CRAWCLAW_A2UI_SKIP_MISSING;
        } else {
          process.env.CRAWCLAW_A2UI_SKIP_MISSING = previous;
        }
      }
    });
  });
  ...
  it("copies bundled assets to dist", async () => {
    await withA2uiFixture(async (dir) => {
      ...
      await fs.writeFile(
        path.join(srcDir, "generated", "a2ui.bundle.js"),
        "console.log(1);",
        "utf8",
      );
      ...
      await expect(fs.stat(path.join(outDir, "generated", "a2ui.bundle.js"))).resolves.toBeTruthy();
    });
  });
});
