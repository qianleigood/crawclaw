...
const match = source.match(
  /function classifyDerivedCaptureCandidate\([\s\S]*?\n}\n\nasync function detectExtendMode/,
);
if (!match) {
...
vm.runInNewContext(`${fnSource}\nmodule.exports = classifyDerivedCaptureCandidate;`, sandbox, {
  filename: "grok_extend_capture_selftest.vm",
});
const classifyDerivedCaptureCandidate = sandbox.module.exports;

const sourceUrl = "https://grok.com/imagine/post/source123";
const baseline = new Set([sourceUrl, "https://grok.com/imagine/post/old-other-post"]);
...
const fixtures = [
  {
    ...
    check(result) {
      assert(
        result.verdict === "confirmed_derived",
        "auto submit primary navigation should be confirmed",
      );
    },
  },
  {
    ...
    check(result) {
      assert(
        result.verdict === "suspicious_redirect",
        "manual handoff same-tab navigation must not become derived lineage",
      );
      assert(
        result.reasons.includes("same_tab_navigation_not_trusted_for_lineage"),
        "same-tab suspicion reason should be recorded",
      );
    },
  },
  {
    ...
    check(result) {
      ...
      assert(
        result.reasons.includes("candidate_seen_before_watch_window"),
        "preexisting drift reason should be recorded",
      );
    },
  },
  {
