import { MAX_IMAGE_BYTES } from "crawclaw/plugin-sdk/media-runtime";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";

vi.mock("crawclaw/plugin-sdk/media-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("crawclaw/plugin-sdk/media-runtime")>();
  return {
    ...actual,
...
describe("tlon monitor media", () => {
  async function loadMediaModule() {
    const mediaRuntime = await import("crawclaw/plugin-sdk/media-runtime");
  ...
  it("stores fetched media through the shared inbound media store with the image cap", async () => {
    ...
    saveMediaBuffer.mockResolvedValue({
      id: "photo---uuid.png",
      path: "/tmp/crawclaw/media/inbound/photo---uuid.png",
    ...
    expect(result).toEqual({
      localPath: "/tmp/crawclaw/media/inbound/photo---uuid.png",
      contentType: "image/png",
