---
summary: "CLI reference for `crawclaw secrets` (reload, audit, configure, apply)"
read_when:
...
Use `crawclaw secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
Scan CrawClaw state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `crawclaw.json` refs)
...
```bash
crawclaw secrets audit
crawclaw secrets audit --check
crawclaw secrets audit --json
crawclaw secrets audit --allow-exec
```
...
```bash
crawclaw secrets configure
crawclaw secrets configure --plan-out /tmp/crawclaw-secrets-plan.json
crawclaw secrets configure --apply --yes
crawclaw secrets configure --providers-only
crawclaw secrets configure --skip-provider-setup
crawclaw secrets configure --agent ops
crawclaw secrets configure --json
```
...
- `configure` targets secret-bearing fields in `crawclaw.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, CrawClaw fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
crawclaw secrets apply --from /tmp/crawclaw-secrets-plan.json --json
```
...
- `crawclaw.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.crawclaw/.env` known secret keys whose values were migrated
