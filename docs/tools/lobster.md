---
title: Lobster
summary: "Typed workflow runtime for CrawClaw with resumable approval gates."
read_when:
...
Lobster is one authoring layer above detached background work. For flow orchestration above individual tasks, see [Task Flow](/automation/taskflow) (`crawclaw tasks flow`). For the task activity ledger, see [`crawclaw tasks`](/automation/tasks).
...
- **One call instead of many**: CrawClaw runs one Lobster tool call and gets a structured result.
...
CrawClaw launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In CrawClaw tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the CrawClaw Gateway (see the [Lobster repo](https://github.com/crawclaw/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), CrawClaw keeps core tools enabled. To restrict core
...
→ crawclaw calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ crawclaw calls gmail.send
...
One public example: a “second brain” CLI + Lobster pipelines that manage three Markdown vaults (personal, partner, shared). The CLI emits JSON for stats, inbox listings, and stale scans; Lobster chains those commands into workflows like `weekly-review`, `inbox-triage`, `dream`, and `shared-task-sync`, each with approval gates. AI handles judgment (categorization) when available and falls back to deterministic rules when not.
