...
describe("resolveGatewayRuntimeConfig", () => {
  describe("trusted-proxy auth mode", () => {
    ...
    it.each([
      ...
      const host = cfg.gateway.bind === "loopback" ? "127.0.0.1" : undefined;
  ...
  describe("token/password auth modes", () => {
    ...
    beforeEach(() => {
      originalToken = process.env.CRAWCLAW_GATEWAY_TOKEN;
      delete process.env.CRAWCLAW_GATEWAY_TOKEN;
    });

    afterEach(() => {
      if (originalToken !== undefined) {
        process.env.CRAWCLAW_GATEWAY_TOKEN = originalToken;
      } else {
        delete process.env.CRAWCLAW_GATEWAY_TOKEN;
      }
    });
