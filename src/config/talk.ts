...
export function normalizeTalkSection(value: unknown): TalkConfig | undefined {
  if (!isPlainObject(value)) {
    return undefined;
  }
...
export function normalizeTalkConfig(config: CrawClawConfig): CrawClawConfig {
  if (!config.talk) {
    return config;
  }
...
export function buildTalkConfigResponse(value: unknown): TalkConfigResponse | undefined {
  ...
  } else if (typeof resolved?.provider === "string") {
