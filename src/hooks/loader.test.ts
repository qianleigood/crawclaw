...
describe("loader", () => {
  ...
  describe("loadInternalHooks", () => {
    ...
    const expectNoCommandHookRegistration = async (cfg: CrawClawConfig) => {
    ...
    it("should treat missing hooks.internal.enabled (default-on)", async () => {
      ...
      for (const cfg of [
        {} satisfies CrawClawConfig,
        { hooks: {} } satisfies CrawClawConfig,
        { hooks: { internal: {} } } satisfies CrawClawConfig,
      ]) {
        const count = await loadInternalHooks(cfg, tmpDir);
        expect(count).toBe(0);
      }
