...
Inspired by Simon Willison's [Running CrawClaw in Docker](https://til.simonwillison.net/llms/crawclaw-docker).
...
Canonical docs page: https://docs.crawclaw.ai/install/clawdock
...
On first command, ClawDock auto-detects your CrawClaw directory:

- Checks common paths (`~/crawclaw`, `~/workspace/crawclaw`, etc.)
...
| `clawdock-config`      | Open the CrawClaw config directory        |
...
| --------------------------- | ------------------------------------------------ | ------------------------------------------------------------------- |
| `<project>/.env`            | **Docker infra** — image, ports, gateway token   | `CRAWCLAW_GATEWAY_TOKEN`, `CRAWCLAW_IMAGE`, `CRAWCLAW_GATEWAY_PORT` |
| `~/.crawclaw/.env`          | **Secrets** — API keys and bot tokens            | `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `TELEGRAM_BOT_TOKEN`         |
| `~/.crawclaw/crawclaw.json` | **Behavior config** — models, channels, policies | Model selection, WhatsApp allowlists, agent settings                |

**Do NOT** put API keys or bot tokens in `crawclaw.json`. Use `~/.crawclaw/.env` for all secrets.

### Initial Setup

`./docker-setup.sh` (in the project root) handles first-time Docker configuration:

- Builds the `crawclaw:local` image from `Dockerfile`
- Creates `<project>/.env` from `.env.example` with a generated gateway token
- Sets up `~/.crawclaw` directories if they don't exist
...
- `CRAWCLAW_DOCKER_APT_PACKAGES` — extra apt packages to install (e.g. `ffmpeg`)
- `CRAWCLAW_INSTALL_BROWSER=1` — pre-install Chromium for browser automation (adds ~300MB, but skips the 60-90s Playwright install on each container start)
...
- `~/.crawclaw/.env` is available inside the container at `/home/node/.crawclaw/.env`
- `~/.crawclaw/crawclaw.json` is available at `/home/node/.crawclaw/crawclaw.json`, and the gateway hot-reloads most changes
...
The project `.env` feeds Docker Compose directly (gateway token, image name, ports). The `~/.crawclaw/.env` feeds the CrawClaw process inside the container.

### Example `~/.crawclaw/.env`
...
```bash
CRAWCLAW_CONFIG_DIR=/Users/you/.crawclaw
CRAWCLAW_WORKSPACE_DIR=/Users/you/.crawclaw/workspace
CRAWCLAW_GATEWAY_PORT=18789
CRAWCLAW_BRIDGE_PORT=18790
CRAWCLAW_GATEWAY_BIND=lan
CRAWCLAW_GATEWAY_TOKEN=<generated-by-docker-setup>
CRAWCLAW_IMAGE=crawclaw:local
```

### Env Precedence

CrawClaw loads env vars in this order (highest wins, never overrides existing):
...
3. **`~/.crawclaw/.env`** — global secrets (API keys, bot tokens)
4. **`crawclaw.json` `env` block** — inline vars, applied only if still missing
5. **Shell env import** — optional login-shell scrape (`CRAWCLAW_LOAD_SHELL_ENV=1`)

## Common Workflows

### Update CrawClaw

> **Important:** `crawclaw update` does not work inside Docker.
...
```bash
crawclaw channels login --channel whatsapp --verbose
```
...
2. Configure it in the CrawClaw config
...
- Bash or Zsh shell
- CrawClaw project (run `scripts/docker/setup.sh`)
