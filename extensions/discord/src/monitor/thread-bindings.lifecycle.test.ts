...
const acpRuntime = await import("crawclaw/plugin-sdk/acp-runtime");
...
describe("thread binding lifecycle", () => {
  ...
  it("persists touched activity timestamps across restart when persistence is enabled", async () => {
    ...
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-thread-bindings-"));
  ...
  it("reuses webhook credentials after unbind when rebinding in the same channel", async () => {
    ...
    const first = await manager.bindTarget({
      ...
      webhookToken: "tok-explicit",
    });
    expect(first).not.toBeNull();
    expect(first?.webhookId).toBe("wh-explicit");
    expect(first?.webhookToken).toBe("tok-explicit");
    ...
    expect(second?.webhookId).toBe("wh-explicit");
    expect(second?.webhookToken).toBe("tok-explicit");
  });
  ...
  it("binds current Discord DMs as direct conversation bindings", async () => {
    ...
    const bound = await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:crawclaw-codex-app-server:dm",
      targetKind: "session",
      conversation: {
        channel: "discord",
        accountId: "default",
