...
describe("Cron issue regressions", () => {
  ...
  it("treats persisted jobs with missing enabled during update()", async () => {
    const store = makeStorePath();
    const now = Date.parse("2026-02-06T10:05:00.000Z");
    await writeCronStoreSnapshot(store.storePath, [
      {
  ...
  it("respects abort signals while retrying one-shot main-session wake-now heartbeat runs", async () => {
    ...
    const state = createCronServiceState({
      cronEnabled: true,
      storePath: "/tmp/crawclaw-cron-abort-test/jobs.json",
      log: noopLogger,
      nowMs: () => Date.now(),
      enqueueSystemEvent,
  ...
  it("finishes recurring wake-now main jobs quickly when the main lane is busy (#58833)", async () => {
    ...
    const state = createCronServiceState({
      cronEnabled: true,
      storePath: "/tmp/crawclaw-cron-busy-main-test/jobs.json",
      log: noopLogger,
      nowMs,
      enqueueSystemEvent,
      requestMainSessionWake,
