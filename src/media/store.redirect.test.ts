...
const HOME = path.join(os.tmpdir(), "crawclaw-home-redirect");
...
describe("media store redirects", () => {
  ...
  beforeAll(async () => {
    envSnapshot = captureEnv(["CRAWCLAW_STATE_DIR"]);
    await fs.rm(HOME, { recursive: true, force: true });
    process.env.CRAWCLAW_STATE_DIR = HOME;
  });
  ...
  it("strips sensitive headers when a redirect crosses origins", async () => {
    ...
    expect(secondHeaders.get("user-agent")).toBe("CrawClaw-Test/1.0");
  });
