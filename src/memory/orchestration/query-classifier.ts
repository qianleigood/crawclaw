import type { UnifiedQueryClassification, UnifiedQueryClassificationInput, UnifiedQueryRouteWeights, UnifiedRecallIntent, UnifiedRecallLayer, UnifiedSkillFamily } from "../types/orchestration.ts";
import { normalizeRecallText, tokenizeRecallText } from "../recall/query-analysis.ts";

const STOPWORDS = new Set([
  "the", "and", "for", "with", "that", "this", "from", "into", "about", "what", "which", "when", "where",
  "一个", "这个", "那个", "怎么", "如何", "一下", "还有", "以及", "现在", "当时", "后来", "之前", "上次", "为什么", "如果", "按什么",
]);

const INTENT_PATTERNS: Record<UnifiedRecallIntent, RegExp[]> = {
  decision: [/(为什么这样设计|为什么这么设计|trade ?off|decision|架构决策|设计决策|怎么定的|为什么定成|为何采用|为什么.*兜底|为什么.*默认保留|为什么.*回退|为什么.*保留|为何.*回退|why.*fallback)/i],
  sop: [/(sop|runbook|playbook|步骤|流程|怎么做|怎么排查|操作手册|处理手册|标准流程|怎么回滚|如何回滚|怎么恢复|如何恢复|部署|发布|上线|安装|配置|启用|启动)/i],
  preference: [/(偏好|preference|prefer|默认.*倾向|默认.*喜欢|习惯|不喜欢|always|never|默认保留|默认走)/i],
...
const SOURCE_CUES = {
  ...
  notebooklm: /(notebooklm|笔记|wiki|文档|知识库|research notebook|source pack|长期知识)/i,
...
function pushIntent(intent: UnifiedRecallIntent, scores: Map<UnifiedRecallIntent, number>, boost: number) {
...
function extractEntityHints(query: string, tokens: string[]): string[] {
  ...
  const special = tokens.filter((token) => (
    token.includes("/")
    || token.includes(".")
    || token.includes("-")
  ...
  const filtered = tokens.filter((token) => token.length >= 3 && !STOPWORDS.has(token.toLowerCase()));
  return unique([...quoted, ...special, ...filtered]).slice(0, 8);
}

function buildKeywords(tokens: string[]): string[] {
  return unique(tokens.filter((token) => token.length >= 2 && !STOPWORDS.has(token.toLowerCase()))).slice(0, 10);
}
...
function rankIntents(scores: Map<UnifiedRecallIntent, number>): { intent: UnifiedRecallIntent; secondaryIntents: UnifiedRecallIntent[]; confidence: number } {
  ...
  return {
    intent: winner[0],
    secondaryIntents: ranked.slice(1).filter(([, score]) => score >= Math.max(0.9, winner[1] - 0.6)).map(([intent]) => intent).slice(0, 3),
...
function buildWeights(intent: UnifiedRecallIntent, normalized: string): UnifiedQueryRouteWeights {
  ...
  if (SOURCE_CUES.graph.test(normalized)) {weights.graph += 0.2;}
  if (SOURCE_CUES.notebooklm.test(normalized)) {weights.notebooklm += 0.24;}
  if (SOURCE_CUES.nativeMemory.test(normalized)) {weights.nativeMemory += 0.24;}
  if (SOURCE_CUES.execution.test(normalized)) {weights.execution += 0.24;}
...
function buildTargetLayers(intent: UnifiedRecallIntent, normalized: string): UnifiedRecallLayer[] {
  ...
  if (intent === "decision" || /决策|architecture|trade ?off|why|为什么|为何/i.test(normalized)) {layers.add("key_decisions");}
  if (intent === "sop" || /sop|runbook|步骤|流程|排查|部署|发布|上线|安装|配置/i.test(normalized)) {layers.add("sop");}
  if (intent === "preference" || /偏好|prefer|习惯|默认/i.test(normalized)) {layers.add("preferences");}
  if (intent === "runtime" || intent === "history" || /状态|刚刚|最新|上次|之前|runtime|signal/i.test(normalized)) {layers.add("runtime_signals");}
...
function buildTemporalHints(normalized: string): string[] {
  const hints: string[] = [];
  if (/(刚刚|现在|当前|最新|目前|recent|latest)/i.test(normalized)) {hints.push("recent");}
  if (/(上次|之前|历史|以前|past|history|previous)/i.test(normalized)) {hints.push("historical");}
  return hints;
}

function buildSkillFamily(intent: UnifiedRecallIntent, normalized: string): UnifiedSkillFamily | undefined {
  if (/(image|multimodal|audio|video|截图|图片|多模态)/i.test(normalized)) {return "multimodal";}
  if (/(deploy|release|发布|部署)/i.test(normalized)) {return "operations";}
  if (/(incident|outage|recover|restore|rollback|runtime|故障|异常|恢复|回滚|线上)/i.test(normalized)) {return "incident";}
  switch (intent) {
    case "decision":
      return "architecture";
    case "sop":
...
export function classifyUnifiedQuery(input: UnifiedQueryClassificationInput): UnifiedQueryClassification {
  ...
  for (const [intent, patterns] of Object.entries(INTENT_PATTERNS) as Array<[UnifiedRecallIntent, RegExp[]]>) {
    if (patterns.some((pattern) => pattern.test(normalizedContext))) {pushIntent(intent, scores, 1.2);}
  }

  if (entityHints.length >= 2) {pushIntent("entity_lookup", scores, 0.35);}
  if (keywords.length >= 6) {pushIntent("broad", scores, 0.18);}
  if (/偏好|默认|always|never/i.test(normalizedContext) && /怎么做|步骤|sop|runbook/i.test(normalizedContext)) {
  ...
  if (entityHints.length) {rationale.push(`entityHints=${entityHints.slice(0, 4).join(",")}`);}
  if (temporalHints.length) {rationale.push(`temporal=${temporalHints.join(",")}`);}
