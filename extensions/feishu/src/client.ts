...
export const FEISHU_HTTP_TIMEOUT_ENV_VAR = "CRAWCLAW_FEISHU_HTTP_TIMEOUT_MS";
export const FEISHU_HTTP_TIMEOUT_ENV_VAR_FALLBACK = "CRAWCLAW_FEISHU_HTTP_TIMEOUT_MS";
...
function resolveConfiguredHttpTimeoutMs(creds: FeishuClientCredentials): number {
  ...
  const envRaw =
    process.env[FEISHU_HTTP_TIMEOUT_ENV_VAR] ?? process.env[FEISHU_HTTP_TIMEOUT_ENV_VAR_FALLBACK];
  if (envRaw) {
