...
import {
  isApiKeyBundledWebSearchPluginId,
  isApiKeylessBundledWebSearchPluginId,
} from "./web-search-provider-policy.js";
import {
...
function mapRegistryWebSearchProviders(params: {
  ...
  return sortWebSearchProviders(
    params.registry.webSearchProviders
      .filter(
        (entry) =>
          (!onlyPluginIdSet || onlyPluginIdSet.has(entry.pluginId)) &&
          !isApiKeyBundledWebSearchPluginId(entry.pluginId),
      )
      .map((entry) => ({
        ...entry.provider,
...
export function resolvePluginWebSearchProviders(params: {
  ...
  const resolved = mapRegistryWebSearchProviders({
    registry:
      resolveCompatibleRuntimePluginRegistry(loadOptions) ?? loadCrawClawPlugins(loadOptions),
  });
  if (cacheOwnerConfig && shouldMemoizeSnapshot) {
