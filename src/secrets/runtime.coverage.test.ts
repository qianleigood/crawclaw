...
function createTestProvider(params: {
  id: "brave" | "gemini" | "grok" | "kimi" | "perplexity";
  pluginId: string;
  order: number;
...
function resolveCoverageEnvId(_entry: SecretRegistryEntry, fallbackEnvId: string): string {
...
function buildConfigForCrawClawTarget(entry: SecretRegistryEntry, envId: string): CrawClawConfig {
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every crawclaw.json registry target when configured as active", async () => {
    const entries = listSecretTargetRegistryEntries().filter(
      (entry) => entry.configFile === "crawclaw.json",
    );
    for (const [index, entry] of entries.entries()) {
      const envId = `CRAWCLAW_SECRET_TARGET_${index}`;
      const runtimeEnvId = resolveCoverageEnvId(entry, envId);
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
        config: buildConfigForCrawClawTarget(entry, envId),
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const [index, entry] of entries.entries()) {
      const envId = `CRAWCLAW_AUTH_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
