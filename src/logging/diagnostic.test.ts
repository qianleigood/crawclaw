...
describe("diagnostic session state pruning", () => {
  ...
  it("keeps a bounded recent history of channel streaming decisions", () => {
    for (let i = 0; i < 15; i += 1) {
      recordDiagnosticChannelStreamingDecision(
        { sessionKey: "agent:main:test" },
        {
          ts: 100 + i,
          channel: "feishu",
          surface: i % 2 === 0 ? "card_stream" : "none",
          enabled: i % 2 === 0,
          reason: i % 2 === 0 ? "enabled" : "disabled_for_thread_reply",
        },
      );
    }

    const state = getDiagnosticSessionState({ sessionKey: "agent:main:test" });
    expect(state.recentChannelStreamingDecisions).toHaveLength(12);
    expect(state.recentChannelStreamingDecisions?.[0]?.ts).toBe(103);
    expect(state.recentChannelStreamingDecisions?.at(-1)?.ts).toBe(114);
  });
});
