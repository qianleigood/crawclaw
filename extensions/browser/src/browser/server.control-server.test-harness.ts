...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.CRAWCLAW_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "crawclaw",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/crawclaw" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    crawclaw: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchCrawClawChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "crawclaw";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.CRAWCLAW_GATEWAY_PORT;
  process.env.CRAWCLAW_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.CRAWCLAW_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.CRAWCLAW_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.CRAWCLAW_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.CRAWCLAW_GATEWAY_PASSWORD;
  } else {
    process.env.CRAWCLAW_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
