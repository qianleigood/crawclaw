#!/usr/bin/env bash
# ClawDock - Docker helpers for CrawClaw
# Inspired by Simon Willison's Docker helper workflow
# https://til.simonwillison.net/llms/crawclaw-docker
#
# Installation:
...
# Common paths to check for CrawClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/crawclaw"
  "${HOME}/workspace/crawclaw"
  "${HOME}/projects/crawclaw"
  "${HOME}/dev/crawclaw"
  "${HOME}/code/crawclaw"
  "${HOME}/src/crawclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found CrawClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/crawclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ CrawClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/qianleigood/crawclaw.git ~/crawclaw"
    echo "  cd ~/crawclaw && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^CRAWCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d crawclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart crawclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f crawclaw-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.crawclaw"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show crawclaw.json
  if [[ -f "${config_dir}/crawclaw.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/crawclaw.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/crawclaw.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No crawclaw.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec crawclaw-gateway \
    bash -c 'echo "alias crawclaw=\"./crawclaw.mjs\"" > /tmp/.bashrc_crawclaw && bash --rcfile /tmp/.bashrc_crawclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm crawclaw-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build crawclaw-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d crawclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build crawclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "CRAWCLAW_GATEWAY_TOKEN=$token" crawclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" crawclaw-gateway \
    bash -c './crawclaw.mjs config set gateway.remote.token "$TOKEN" && ./crawclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec crawclaw-gateway \
    bash -c "./crawclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart crawclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec crawclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'crawclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for CrawClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (crawclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "${_CLR_BOLD}${_CLR_MAGENTA}🔐 Pairing & Devices${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to crawclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.crawclaw)${_CLR_RESET}"
  ...
  echo -e "${_CLR_CYAN}  3.${_CLR_RESET} $(_cmd clawdock-devices)        ${_CLR_DIM}# If pairing needed${_CLR_RESET}"
  echo -e "${_CLR_CYAN}  4.${_CLR_RESET} $(_cmd clawdock-approve) ${_CLR_CYAN}<id>${_CLR_RESET}   ${_CLR_DIM}# Approve pairing${_CLR_RESET}"
  echo ""

  echo -e "${_CLR_BOLD}${_CLR_GREEN}💬 WhatsApp Setup${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'crawclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'crawclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.crawclaw.ai${_CLR_RESET}"
  echo ""
}
