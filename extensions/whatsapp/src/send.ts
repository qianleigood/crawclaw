...
import { generateSecureUuid } from "crawclaw/plugin-sdk/core";
import { normalizePollInput, type PollInput } from "crawclaw/plugin-sdk/media-runtime";
import { createSubsystemLogger } from "crawclaw/plugin-sdk/runtime-env";
import { getChildLogger } from "crawclaw/plugin-sdk/text-runtime";
import { redactIdentifier } from "crawclaw/plugin-sdk/text-runtime";
import { convertMarkdownTables } from "crawclaw/plugin-sdk/text-runtime";
import { markdownToWhatsApp } from "crawclaw/plugin-sdk/text-runtime";
import { toWhatsappJid } from "crawclaw/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: CrawClawConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);
