...
import { clearFeishuRuntime, setFeishuRuntime } from "../runtime.js";
...
export function setFeishuLifecycleStateDir(prefix: string) {
  process.env.CRAWCLAW_STATE_DIR = `/tmp/${prefix}-${randomUUID()}`;
}
...
export function cleanupFeishuLifecycleTestState(originalStateDir: string | undefined) {
  vi.useRealTimers();
  clearFeishuRuntime();
