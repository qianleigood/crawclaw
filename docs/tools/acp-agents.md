---
summary: "Use ACP runtime sessions for Codex, Claude Code, Cursor, Gemini CLI, CrawClaw ACP, and other harness agents"
read_when:
...
Task-backed ACP runs now persist:

- the ACP wrapper session transcript
- task runtime metadata
- a task trajectory with completion evidence and completion-guard output

## Context and memory boundaries

ACP runs are task-backed from CrawClaw's point of view, but their internal
runtime is still owned by the ACP backend.

- CrawClaw persists the wrapper session, task metadata, trajectory, and guard /
  loop / completion state for the ACP run.
- Follow-up messages can stay bound to the same ACP session key and backend
  workspace.
...
Use ACP when you want an external harness runtime. Use sub-agents when you want CrawClaw-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | CrawClaw native sub-agent runtime  |
| Session key   | `agent:<agentId>:acp:<uuid>`          | `agent:<agentId>:subagent:<uuid>`  |
| Main commands | `/acp ...`                            | `/subagents ...`                   |
| Spawn tool    | `sessions_spawn` with `runtime:"acp"` | `sessions_spawn` (default runtime) |

ACP oneshot sessions are still represented as real CrawClaw sessions. Their
wrapper transcript is materialized on completion so task metadata, session
history, and replay can all point at a concrete JSONL transcript.
...
- CrawClaw keeps owning the channel transport, auth, safety, and delivery.
...
- `/acp spawn codex --thread auto`: CrawClaw may create a child thread/topic and bind the ACP session there
...
- Generic current-conversation binds use the shared CrawClaw binding store and survive normal gateway restarts.
...
- CrawClaw binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, CrawClaw returns a clear unsupported/unavailable message.
...
- CrawClaw ensures the configured ACP session exists before use.
- Messages in that channel or topic route to the configured ACP session.
- In bound conversations, `/new` resets the ACP session key in place.
...
- If `agentId` is omitted, CrawClaw uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, CrawClaw may default to persistent behavior per runtime path
...
- `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new CrawClaw session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, not inside the CrawClaw sandbox.
...
If no target resolves, CrawClaw returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, CrawClaw returns a clear unsupported-control error.
...
When CrawClaw uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal CrawClaw `agentId` path).
...
- Relative paths resolve from CrawClaw workspace directory.
...
- CrawClaw startup remains non-blocking while the backend health check runs.
...
By default, ACPX sessions do **not** expose CrawClaw plugin-registered tools to
the ACP harness.
...
These ACPX harness permissions are separate from CrawClaw exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.
...
> **Important:** CrawClaw currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
