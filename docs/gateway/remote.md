...
- Local mode defaults:
  ...
  - password: `CRAWCLAW_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `CRAWCLAW_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
Note: if you have a leftover `com.crawclaw.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
