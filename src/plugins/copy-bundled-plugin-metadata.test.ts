...
const excludeOptionalEnv = { CRAWCLAW_INCLUDE_OPTIONAL_BUNDLED: "0" } as const;
...
function createPlugin(
  ...
  params: {
    ...
    packageCrawClaw?: Record<string, unknown>;
  },
) {
  const pluginDir = path.join(repoRoot, "extensions", params.id);
  fs.mkdirSync(pluginDir, { recursive: true });
  writeJson(path.join(pluginDir, "crawclaw.plugin.json"), {
    id: params.id,
  ...
  writeJson(path.join(pluginDir, "package.json"), {
    ...
    ...(params.packageCrawClaw ? { crawclaw: params.packageCrawClaw } : {}),
  });
  return pluginDir;
}
...
function readBundledManifest(repoRoot: string, pluginId: string) {
  return JSON.parse(
    fs.readFileSync(
      path.join(repoRoot, "dist", "extensions", pluginId, "crawclaw.plugin.json"),
      "utf8",
    ),
  ) as { skills?: string[] };
}
...
describe("copyBundledPluginMetadata", () => {
  it("copies plugin manifests, package metadata, and local skill directories", () => {
    ...
    expect(
      fs.existsSync(path.join(repoRoot, "dist", "extensions", "acpx", "crawclaw.plugin.json")),
    ).toBe(true);
    expect(
      fs.readFileSync(
  ...
  it("retries transient skill copy races from concurrent runtime postbuilds", () => {
    const repoRoot = makeRepoRoot("crawclaw-bundled-plugin-retry-");
    const pluginDir = createPlugin(repoRoot, {
      id: "diffs",
  ...
  it("removes generated outputs for plugins no longer present in source", () => {
    const repoRoot = makeRepoRoot("crawclaw-bundled-plugin-removed-");
    const staleBundledSkillDir = path.join(
      repoRoot,
      "dist",
      "extensions",
    ...
    writeJson(path.join(repoRoot, "dist", "extensions", "removed-plugin", "crawclaw.plugin.json"), {
      id: "removed-plugin",
      configSchema: { type: "object" },
      skills: ["./bundled-skills/@scope/skill"],
    });
    writeJson(path.join(repoRoot, "dist", "extensions", "removed-plugin", "package.json"), {
      name: "@crawclaw/removed-plugin",
    });
  ...
  it("removes stale dist outputs when a source extension directory no longer has a manifest", () => {
    const repoRoot = makeRepoRoot("crawclaw-bundled-plugin-manifestless-source-");
    ...
    writeJson(path.join(staleDistDir, "crawclaw.plugin.json"), {
      id: "google-gemini-cli-auth",
      configSchema: { type: "object" },
    });
    writeJson(path.join(staleDistDir, "package.json"), {
      name: "@crawclaw/google-gemini-cli-auth",
    });
  ...
  ] as const)("$name", ({ pluginId, packageName, packageCrawClaw, env, expectedExists }) => {
    const repoRoot = makeRepoRoot(`crawclaw-bundled-plugin-${pluginId}-`);
  ...
  it("preserves manifest-less speech runtime support package outputs and copies package metadata", () => {
    ...
    fs.mkdirSync(path.join(repoRoot, "dist", "extensions", "speech-core"), {
      recursive: true,
    });
    fs.writeFileSync(
