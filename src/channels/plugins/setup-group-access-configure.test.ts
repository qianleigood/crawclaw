...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: CrawClawConfig, policy: ChannelAccessPolicy) => CrawClawConfig;
  resolveAllowlist: (params: { cfg: CrawClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: CrawClawConfig; resolved: TResolved }) => CrawClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: CrawClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: CrawClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: CrawClawConfig, policy: ChannelAccessPolicy): CrawClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: CrawClawConfig, policy: ChannelAccessPolicy): CrawClawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: CrawClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: CrawClawConfig; resolved: string[] }) => {
      calls.push("apply");
