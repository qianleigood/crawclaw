...
let resetInboundDedupe: typeof import("crawclaw/plugin-sdk/reply-runtime").resetInboundDedupe;
...
beforeAll(async () => {
  ({ resetInboundDedupe } = await import("crawclaw/plugin-sdk/reply-runtime"));
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\bcrawclaw\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("crawclaw: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("crawclaw: hello <@U2>");
  });
