...
function expectGeneratedAuthEnvVarModuleState(params: {
  ...
  const result = writeBundledProviderAuthEnvVarModule({
    ...
    outputPath: "src/generated/plugins/bundled-provider-auth-env-vars.generated.ts",
    check: true,
  });
...
describe("bundled provider auth env vars", () => {
  ...
  it("supports check mode for stale generated artifacts", () => {
    const tempRoot = createGeneratedPluginTempRoot("crawclaw-provider-auth-env-vars-");
    ...
    fs.writeFileSync(
      path.join(tempRoot, "src/generated/plugins/bundled-provider-auth-env-vars.generated.ts"),
      "// stale\n",
      "utf8",
    );
