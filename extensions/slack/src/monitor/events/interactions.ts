...
// Prefix for CrawClaw-generated action IDs to scope our handler
const CRAWCLAW_ACTION_PREFIX = "crawclaw:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${CRAWCLAW_ACTION_PREFIX}`);

  // Handle CrawClaw modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
