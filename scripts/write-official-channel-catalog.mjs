...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.crawclaw) ? packageJson.crawclaw : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.crawclaw?.channel?.id) || left.name;
    const rightId = trimString(right.crawclaw?.channel?.id) || right.name;
