...
This page lists every configuration knob for CrawClaw memory search. For
...
`crawclaw.json` unless noted otherwise.
...
When `provider` is not set, CrawClaw selects the first available:
...
Remote embeddings require an API key. CrawClaw resolves from:
...
When sqlite-vec is unavailable, CrawClaw falls back to in-process cosine
...
| Key                   | Type     | Default                               | Description                                 |
| --------------------- | -------- | ------------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.crawclaw/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |
...
The context archive keeps per-run metadata and blob files under `memory.contextArchive`.
Retention is best-effort and runs automatically after archive writes.

It is the replay/export-facing truth layer for:
...
- session transcripts, which are product-facing logs and may be rewritten later
