...
const APPROVALS_PATH = "~/.crawclaw/plugin-binding-approvals.json";
...
const CODEX_PLUGIN_SESSION_PREFIXES = [
  "crawclaw-app-server:thread:",
  "crawclaw-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("crawclaw.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("crawclaw.plugins.binding.global-state");
...
function isLegacyPluginBindingRecord(params: {
  ...
  return (
    ...
    CODEX_PLUGIN_SESSION_PREFIXES.some((prefix) => targetSessionKey.startsWith(prefix))
  );
}
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to CrawClaw instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
