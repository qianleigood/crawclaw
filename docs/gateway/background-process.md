...
CrawClaw runs shell commands through the `exec` tool and keeps long-running tasks in memory. The `process` tool manages those background sessions.
...
- Spawned exec commands receive `CRAWCLAW_SHELL=exec` for context-aware shell/profile rules.
...
- `tools.exec.notifyOnExit` (default true): enqueue a system event and request a main-session wake when a backgrounded exec exits.
