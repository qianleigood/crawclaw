...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure CrawClaw HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `crawclaw pairing approve slack <code>`.
...
- The interactive callback values are CrawClaw-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, CrawClaw falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while CrawClaw is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "CrawClaw",
    "description": "Slack connector for CrawClaw"
  },
  "features": {
    "bot_user": {
      "display_name": "CrawClaw",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/crawclaw",
        "description": "Send a message to CrawClaw",
        "should_escape": false
      }
    ]
  },
...
Exec approval prompts can route natively through Slack using interactive buttons and interactions, instead of falling back to the terminal. Approver authorization is enforced: only users identified as approvers can approve or deny requests through Slack.
...
CrawClaw supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, CrawClaw falls back to normal delivery for remaining payloads.
