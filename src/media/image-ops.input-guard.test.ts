...
describe("image input pixel guard", () => {
  ...
  it("fails closed when sips cannot determine image dimensions", async () => {
    const previousCanonicalBackend = process.env.CRAWCLAW_IMAGE_BACKEND;
    ...
    try {
      ...
      if (previousCanonicalBackend === undefined) {
        delete process.env.CRAWCLAW_IMAGE_BACKEND;
      } else {
        process.env.CRAWCLAW_IMAGE_BACKEND = previousCanonicalBackend;
      }
    }
  });
});
