...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.CRAWCLAW_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: CrawClawConfig) {
...
function resolveRegistryDefaultSpeechProviderId(cfg?: CrawClawConfig): TtsProvider {
...
export function resolveTtsConfig(cfg: CrawClawConfig): ResolvedTtsConfig {
...
function resolveEffectiveTtsAutoState(params: { cfg: CrawClawConfig; sessionAuto?: string }): {
...
export function buildTtsSystemPromptHint(cfg: CrawClawConfig): string | undefined {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredCrawClawTmpDir();
