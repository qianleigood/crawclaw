...
read_when:
  - 添加或修改 CLI 命令或选项时
  - 为新的命令界面编写文档时
summary: "`crawclaw` 命令、子命令和选项的 CrawClaw CLI 参考"
title: CLI 参考
x-i18n:
...
- `--dev`：将状态隔离到 `~/.crawclaw-dev` 下，并变更默认端口。
- `--profile <name>`：将状态隔离到 `~/.crawclaw-<name>` 下。
- `--no-color`：禁用 ANSI 颜色。
- `--update`：`crawclaw update` 的简写（仅适用于源码安装）。
- `-V`, `--version`, `-v`：打印版本并退出。
...
- `crawclaw security audit` — 审计配置 + 本地状态中常见的安全陷阱。
- `crawclaw security audit --deep` — 尽力进行实时 Gateway 网关探测。
- `crawclaw security audit --fix` — 收紧安全默认值并对状态 / 配置执行 chmod。

## 密钥

- `crawclaw secrets reload` — 重新解析引用，并以原子方式替换运行时快照。
- `crawclaw secrets audit` — 扫描明文残留、未解析引用和优先级漂移（`--allow-exec` 可在审计时执行 exec provider）。
- `crawclaw secrets configure` — 用于提供商设置 + SecretRef 映射 + 预检 / 应用的交互式助手（`--allow-exec` 可在预检和包含 exec 的应用流程中执行 exec provider）。
- `crawclaw secrets apply --from <plan.json>` — 应用先前生成的计划（支持 `--dry-run`；使用 `--allow-exec` 可在 dry-run 或包含 exec 的写入计划中执行 exec provider）。

## 插件

管理扩展及其配置：

- `crawclaw plugins list` — 发现插件（机器输出请使用 `--json`）。
- `crawclaw plugins inspect <id>` — 显示插件详情（`info` 是别名）。
- `crawclaw plugins install <path|.tgz|npm-spec|plugin@marketplace>` — 安装插件（或将插件路径添加到 `plugins.load.paths`）。
- `crawclaw plugins marketplace list <marketplace>` — 安装前先列出 marketplace 条目。
- `crawclaw plugins enable <id>` / `disable <id>` — 切换 `plugins.entries.<id>.enabled`。
- `crawclaw plugins doctor` — 报告插件加载错误。

大多数插件更改都需要重启 gateway。参见 [/plugin](/tools/plugin)。

## 内存

用于管理 NotebookLM 知识访问和记忆提示词诊断：

- `crawclaw memory status` — 显示 NotebookLM provider 状态。
- `crawclaw memory refresh` — 从配置好的 cookie fallback 刷新 NotebookLM 认证。
...
task-backed agent runtime 的检查/导出入口：

- `crawclaw agent inspect --run-id <id>|--task-id <id>` — 查看 runtime/task/guard/completion/archive 状态、query-context 诊断和归档 lifecycle timeline。
- `crawclaw agent export-context --run-id <id>|--task-id <id>` — 导出 Context Archive 记录用于 replay/debug。
- `crawclaw agents status` — 查看跨 agent 运行的运维汇总。
...
- `--workspace <dir>`：智能体工作区路径（默认 `~/.crawclaw/workspace`）。
- `--wizard`：运行新手引导。
...
非交互式配置助手（get/set/unset/file/validate）。直接运行 `crawclaw config` 而不带
子命令会启动向导。
...
- `channels status`：检查 gateway 可达性和渠道健康状态（`--probe` 会运行额外检查；gateway 健康探测请使用 `crawclaw health` 或 `crawclaw status --deep`）。
- 提示：如果能够检测到常见配置错误，`channels status` 会打印带建议修复方式的警告（随后指向 `crawclaw doctor`）。
...
- `--session-id <id>`
- `--agent <id>`（覆盖路由绑定，直接指定 agent）
- `--thinking <off|minimal|low|medium|high|xhigh>`（提供商支持各不相同，CLI 不做模型级限制）
- `--verbose <on|off>`
- `--channel <channel>`（投递渠道；省略时使用主会话渠道）
- `--reply-to <target>`（独立于会话路由的投递目标覆盖）
...
显示面向运维的 agent runtime/task/guard 汇总。
...
绑定规范使用 `channel[:accountId]`。省略 `accountId` 时，CrawClaw 可能通过渠道默认值 / 插件 hook 解析账户作用域；否则这就是不带显式账户作用域的渠道绑定。
...
通过显式参数或 `IDENTITY.md` 更新 agent identity。
...
离线 harness 报告与 promotion check。
...
在 OAuth / API 凭据可用时，CrawClaw 可以显示提供商用量 / 配额。

展示位置：

- `/status`（可用时添加一行简短的提供商用量信息）
- `crawclaw status --usage`（打印完整的提供商明细）
...
- `gateway status` 还能在检测到时显示旧版或额外的 gateway 服务（`--deep` 会增加系统级扫描）。带 profile 名称的 CrawClaw 服务会被视为一等公民，不会标记为“额外”。
...
`crawclaw models` 是 `models status` 的别名。
...
将一个系统事件加入队列，并可选择触发一次主会话唤醒（Gateway 网关 RPC）。
...
浏览器控制 CLI（专用 Chrome/Brave/Edge/Chromium）。参见 [`crawclaw browser`](/cli/browser) 和 [Browser 工具](/tools/browser)。
