...
function candidateBinDirs(opts: EnsureCrawClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.CRAWCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "crawclaw"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `crawclaw`
