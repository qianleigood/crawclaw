...
async function expectSharedOperatorScopesCleared(
  ...
  try {
    ...
    const adminRes = await rpcReq(ws, "secrets.reload");
...
describe("gateway auth compatibility baseline", () => {
  describe("token mode", () => {
    ...
    test("returns stable token-missing details for browser client without token", async () => {
      const ws = await openWs(port, { origin: originForPort(port) });
      try {
        const res = await connectReq(ws, {
          skipDefaultAuth: true,
          client: { ...BROWSER_CLIENT_CLIENT },
        });
    ...
    test("keeps local backend device-token reconnects out of pairing", async () => {
      const identityPath = path.join(
        os.tmpdir(),
        `crawclaw-backend-device-${process.pid}-${port}.json`,
      );
