#!/bin/bash
# Setup CrawClaw Auth Management System
...
echo "=== CrawClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'crawclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  CrawClaw message: Send warning via CrawClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/crawclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/crawclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/crawclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now crawclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status crawclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
