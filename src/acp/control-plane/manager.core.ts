import { upsertAgentTaskRuntimeMetadata } from "../../agents/runtime/agent-metadata-store.js";
import {
...
export class AcpSessionManager {
  ...
  resolveSession(params: { cfg: CrawClawConfig; sessionKey: string }): AcpSessionResolution {
  ...
  getObservabilitySnapshot(cfg: CrawClawConfig): AcpManagerObservabilitySnapshot {
      ...
      async () => {
        ...
        const taskContext =
          input.mode === "prompt"
            ...
            : null;
        if (taskContext) {
          this.createBackgroundTaskRecord(input.cfg, taskContext, turnStartedAt);
        }
  ...
  private resolveTurnTimeoutMs(params: { cfg: CrawClawConfig; meta: SessionAcpMeta }): number {
  ...
  private enforceConcurrentSessionLimit(params: { cfg: CrawClawConfig; sessionKey: string }): void {
  ...
  private async evictIdleRuntimeHandles(params: { cfg: CrawClawConfig }): Promise<void> {
  ...
  private createBackgroundTaskRecord(
    ...
    try {
      const childAgentIdRaw = resolveAcpAgentFromSessionKey(context.childSessionKey);
      const parentAgentIdRaw = resolveAcpAgentFromSessionKey(context.requesterSessionKey);
      const childAgentId = childAgentIdRaw ? normalizeAgentId(childAgentIdRaw) : undefined;
      const parentAgentId = parentAgentIdRaw ? normalizeAgentId(parentAgentIdRaw) : undefined;
      const childEntry = this.deps.readSessionEntry({
        cfg,
        sessionKey: context.childSessionKey,
      })?.entry;
      const storePath = childAgentId
        ? resolveStorePath(cfg.session?.store, {
            agentId: childAgentId,
          })
        : undefined;
      const createdTask = createRunningTaskRun({
        runtime: "acp",
        ...
        agentId: childAgentId || undefined,
        agentMetadata: {
          ...(parentAgentId ? { parentAgentId } : {}),
          mode: "background",
          spawnSource: "acp_manager",
        },
        runId: context.runId,
        label: context.label,
        task: context.task,
      ...
      void upsertAgentTaskRuntimeMetadata({
        ...
        sessionFile: childEntry?.sessionFile,
        storePath,
        ...
        logVerbose(
          `acp-manager: failed persisting runtime metadata for ${context.runId}: ${String(error)}`,
        );
      });
      registerAgentRunContext(context.runId, {
        sessionKey: context.childSessionKey,
        sessionId: childEntry?.sessionId,
        agentId: childAgentId || undefined,
    ...
    try {
      if (params.status === "succeeded") {
        ...
        markAgentRunCompleted({
          runId,
          endedAt: params.endedAt,
          summary: params.progressSummary ?? params.terminalSummary ?? "ACP task completed",
        });
        return;
      }
      failTaskRunByRunId({
      ...
      markAgentRunFailed({
        ...
        error: params.error,
        summary:
          params.error ?? params.terminalSummary ?? params.progressSummary ?? "ACP task failed",
      });
    } catch (error) {
      logVerbose(`acp-manager: failed updating background task for ${runId}: ${String(error)}`);
    }
