...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("crawclaw tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("crawclaw tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: CrawClawConfig) {
