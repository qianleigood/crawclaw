import { DEFAULT_ACCOUNT_ID } from "crawclaw/plugin-sdk/account-id";
import type { DiscordGuildEntry } from "crawclaw/plugin-sdk/config-runtime";
import type { CrawClawConfig } from "crawclaw/plugin-sdk/config-runtime";
import { createEnvPatchedAccountSetupAdapter } from "crawclaw/plugin-sdk/setup-adapter-runtime";
import type {
  ChannelSetupAdapter,
  ChannelSetupDmPolicy,
  ChannelSetupWizard,
} from "crawclaw/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "crawclaw/plugin-sdk/setup-runtime";
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: CrawClawConfig; accountId: string }) => {
          const account = inspectDiscordSetupAccount({ cfg, accountId });
          return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: CrawClawConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: CrawClawConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: CrawClawConfig; accountId: string }) =>
    ...
    disable: (cfg: CrawClawConfig) => setSetupChannelEnabled(cfg, channel, false),
