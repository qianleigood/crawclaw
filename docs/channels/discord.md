...
You will need to create a new application with a bot, add the bot to your server, and pair it to CrawClaw. We recommend adding your bot to your own private server. If you don't have one yet, [create one first](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (choose **Create My Own > For me and my friends**).

<Steps>
  <Step title="Create a Discord application and bot">
    Go to the [Discord Developer Portal](https://discord.com/developers/applications) and click **New Application**. Name it something like "CrawClaw".

    Click **Bot** on the sidebar. Set the **Username** to whatever you call your CrawClaw agent.
  ...
  <Step title="Enable Developer Mode and collect your IDs">
    ...
    Save your **Server ID** and **User ID** alongside your Bot Token — you'll send all three to CrawClaw in the next step.
  ...
  <Step title="Allow DMs from server members">
    ...
    This lets server members (including bots) send you DMs. Keep this enabled if you want to use Discord DMs with CrawClaw. If you only plan to use guild channels, you can disable DMs after pairing.

  </Step>

  <Step title="Set your bot token securely (do not send it in chat)">
    Your Discord bot token is a secret (like a password). Set it on the machine running CrawClaw before messaging your agent.

```bash
export DISCORD_BOT_TOKEN="YOUR_BOT_TOKEN"
crawclaw config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
crawclaw config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
crawclaw config set channels.discord.enabled true --strict-json
crawclaw gateway
```

    If CrawClaw is already running as a background service, restart it via the CrawClaw Mac app or by stopping and restarting the `crawclaw gateway run` process.

  </Step>

  <Step title="Configure CrawClaw and pair">

    <Tabs>
      <Tab title="Ask your agent">
        Chat with your CrawClaw agent on any existing channel (e.g. Telegram) and tell it. If Discord is your first channel, use the CLI / config tab instead.
...
```bash
crawclaw pairing list discord
crawclaw pairing approve discord <CODE>
```
...
Discord forum and media channels only accept thread posts. CrawClaw supports two ways to create them:
...
- Use `crawclaw message thread create` to create a thread directly. Do not pass `--message-id` for forum channels.

Example: send to forum parent to create a thread

```bash
crawclaw message send --channel discord --target channel:<forumId> \
  --message "Topic title\nBody of the post"
```
...
```bash
crawclaw message thread create --channel discord --target channel:<forumId> \
...
CrawClaw supports Discord components v2 containers for agent messages. Use the message tool with a `components` payload. Interaction results are routed back to the agent as normal inbound messages and follow the existing Discord `replyToMode` settings.
...
- CrawClaw adds a trigger button automatically
...
<Tabs>
  ...
  <Tab title="Guild policy">
    ...
    - names/tags are supported for `users`, but IDs are safer; `crawclaw security audit` warns when name/tag entries are used
...
<AccordionGroup>
  ...
  <Accordion title="Copy IDs">
    ...
    Prefer numeric IDs in CrawClaw config for reliable audits and probes.
...
- Commands may still be visible in Discord UI for users who are not authorized; execution still enforces CrawClaw auth and returns "not authorized".
...
<AccordionGroup>
  ...
  <Accordion title="Live stream preview">
    CrawClaw can stream draft replies by sending a temporary message and editing it as text arrives.
...
```
    ...
    enabled for Discord, CrawClaw skips the preview stream to avoid double streaming.
...
```
    ...
    - In a bound channel or thread, `/new` resets the bound ACP session in place.
    - Temporary thread bindings still work and can override target resolution while active.
    - `spawnAcpSessions` is only required when CrawClaw needs to create/bind a child thread via `--thread auto|here`. It is not required for `/acp spawn ... --bind here` in the current channel.
  ...
  <Accordion title="Ack reactions">
    `ackReaction` sends an acknowledgement emoji while CrawClaw is processing an inbound message.
...
```json5
{
  channels: {
    discord: {
      ...
      activityType: 1,
      activityUrl: "https://twitch.tv/crawclaw",
    },
  },
}
```
...
```
  ...
  <Accordion title="Exec approvals in Discord">
    ...
    - env-first local auth (`CRAWCLAW_GATEWAY_TOKEN` / `CRAWCLAW_GATEWAY_PASSWORD` then `gateway.auth.*`)
...
CrawClaw uses Discord components v2 for exec approvals and cross-context markers. Discord message actions can also accept `components` for custom UI (advanced; requires constructing a component payload via the discord tool), while legacy `embeds` remain available but are not recommended.
...
CrawClaw can join Discord voice channels for realtime, continuous conversations. This is separate from voice message attachments.
...
- CrawClaw also watches receive decrypt failures and auto-recovers by leaving/rejoining the voice channel after repeated failures in a short window.
...
Discord voice messages show a waveform preview and require OGG/Opus audio plus metadata. CrawClaw generates the waveform automatically, but it needs `ffmpeg` and `ffprobe` available on the gateway host to inspect and convert audio files.

Requirements and constraints:

- Provide a **local file path** (URLs are rejected).
- Omit text content (Discord does not allow text + voice message in the same payload).
- Any audio format is accepted; CrawClaw converts to OGG/Opus when needed.
...
```
  ...
  <Accordion title="Voice STT drops with DecryptionFailed(...)">
    ...
    - keep CrawClaw current (`crawclaw update`) so the Discord voice receive recovery logic is present
...
- If command deploy/state is stale, restart gateway and re-check with `crawclaw channels status --probe`.
