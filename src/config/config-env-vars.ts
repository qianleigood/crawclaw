...
function collectConfigEnvVarsByTarget(cfg?: CrawClawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: CrawClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: CrawClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.CRAWCLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
