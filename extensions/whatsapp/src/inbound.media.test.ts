...
vi.mock("crawclaw/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("crawclaw/plugin-sdk/media-runtime")>(
    "crawclaw/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `crawclaw-inbound-media-${crypto.randomUUID()}`);
