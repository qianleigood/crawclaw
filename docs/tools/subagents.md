...
Task-backed sub-agent runs now persist:
...
Sub-agents are isolated, but not in every dimension:

- They start a new CrawClaw session and do **not** inherit the full parent
  transcript.
- They receive a child-specific task prompt plus lineage metadata
...
- Durable memory can still be shared when parent and child resolve to the same
  `agentId` scope.
- Hindsight experience recall is queried from the same configured backend during
...
For runtime inspection after spawn/completion, use `/subagents info`,
`/subagents log`, CrawClaw Desktop task/session views, or Gateway/session
surfaces such as `subagents`, `sessions.list`, and `sessions.history`.
...
For task-backed runs, the associated task metadata also records the trajectory
path used by completion evaluation and replay.
...
- For manual spawns, delivery is resilient:
  - CrawClaw tries direct `agent` delivery first with a stable idempotency key.
...
- Completion evidence and the completion-guard result are stored in the task
  trajectory, so successful detached work can be audited without replaying the
  full chat transcript.
- Context Archive can also preserve model-visible context, tool decisions, and
  post-turn state for replay/debug without treating the transcript as the only
  truth source.
...
- Default run timeout: if `sessions_spawn.runTimeoutSeconds` is omitted, CrawClaw uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout).
...
- QQBot (currently the only supported channel): supports persistent thread-bound subagent sessions (`sessions_spawn` with `thread: true`), manual thread controls (`/focus`, `/unfocus`, `/agents`, `/session idle`, `/session max-age`), and adapter keys `channels.qqbot.threadBindings.enabled`, `channels.qqbot.threadBindings.idleHours`, `channels.qqbot.threadBindings.maxAgeHours`, and `channels.qqbot.threadBindings.spawnSubagentSessions`.

Quick flow:

1. Spawn with `sessions_spawn` using `thread: true` (and optionally `mode: "session"`).
2. CrawClaw creates or binds a thread to that session target in the active channel.
...
- Otherwise delivery depends on requester depth:
  ...
  - if a nested requester subagent session is gone, CrawClaw falls back to that session's requester when available
...
For task-backed runs, announce delivery also updates task runtime state and the
persisted trajectory/completion record.

## Tool Policy (sub-agent tools)

By default, sub-agents do **not** get these session/control tools:
...
When `maxSpawnDepth >= 2`, depth-1 orchestrator sub-agents can additionally receive `sessions_spawn`, `subagents`, `sessions_list`, and `sessions_history` so they can manage their children.
