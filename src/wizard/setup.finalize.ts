...
export async function finalizeSetupWizard(
    ...
    if (gatewayProbe.ok) {
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("crawclaw gateway run")}`,
          `Or rerun with: ${formatCliCommand("crawclaw onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("crawclaw onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.crawclaw.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.crawclaw.ai/security",
    "Security",
  );
    ...
    if (!entry) {
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("crawclaw configure --section web")}`,
          "",
          "Docs: https://docs.crawclaw.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://crawclaw.ai/showcase ("What People Are Building").',
    "What now",
  );
