---
title: "记忆架构重设计：Hindsight 原生"
summary: "CrawClaw 记忆和 Hindsight Retain/Recall/Reflect 集成的历史重设计笔记"
status: historical
x-i18n:
  generated_at: "2026-06-10T12:43:23Z"
  model: codex
  provider: openai
  source_hash: a478c9666c0b303b0016df0e1cf323d353e7957e70a93a0425cd76440dfbec0c
  source_path: debug/memory-architecture-redesign-zh.md
  workflow: 15
---

# 记忆架构重设计：Hindsight 原生

> 历史说明：本文记录迁移前的设计基线和拟议 rollout。当前运行时不再暴露下文部分章节提到的旧本地体验发件箱或 file-backed durable-memory tool surface。
...
## 2. 不变的层：Session Summary

### 2.1 为什么 session-summary 不需要改

session-summary 的设计是好的，它解决的问题与 Hindsight 解决的问题不同：

| 维度                 | session-summary                      | Hindsight 记忆层 |
| -------------------- | ------------------------------------ | ---------------- |
| 作用域               | 单会话                               | 跨会话           |
| 目的                 | 会话内连续性 + compaction 边界       | 跨会话知识积累   |
| 存储                 | 本地文件（summary.md）               | Hindsight banks  |
| 检索方式             | 总是作为上下文加载                   | 语义搜索按需召回 |
| 与 compaction 的关系 | 紧耦合（summary 是 compaction 边界） | 无关             |

session-summary 的关键设计优势：

1. **单一事实来源**：`summary.md` 是唯一的持久化 session-summary 来源
2. **Compaction 边界**：summary 是 compaction 的边界，compaction 保留 summary 之后的 tail
3. **生命周期感知**：从 run-loop 的 parent fork context 运行，跳过缺失的情况
4. **两种维护模式**：Light profile（快速，关键段落）和 Full profile（完整）
5. **Prompt assembly 不注入 summary**：compaction 前连续性来自 transcript；compaction 后来自 compact summary message + tail

把 session-summary 移到 Hindsight 会：

- 增加不必要的复杂度
- 丢失与 compaction 的紧耦合
- 增加延迟（Hindsight API vs 本地文件）
- 没有明确收益（session summary 不需要语义搜索，它总是作为上下文加载）

**结论**：session-summary 保持不变，继续使用本地文件 + background agent。
...
会话摘要             session-summary/*.md           -->  不变（保持本地文件 + background agent）
...
- **会话消息**：SQLite 存储，会话隔离，无需语义搜索
- **会话摘要**：本地文件 + background agent，与 compaction 紧耦合，会话隔离的连续性机制，设计良好无需改动
- **上下文归档**：本地文件，用于 replay/export/debug
```
...
+--- 会话摘要（不变） ------------------------------+
|  background session-summary agent 维护 summary.md  |
|  与 compaction 紧耦合，提供会话内连续性             |
|  不经过 Hindsight，保持本地文件存储                |
+---------------------------------------------------+
```
...
```json5
{
  memory: {
    ...
    sessionSummary: { enabled: true, minTokensToInit: 500 }, // 不变，保持本地文件 + background agent
...
crates/crawclaw-runtime/src/memory/
  ...
  session_summary_store.rs # 会话摘要（不变）
