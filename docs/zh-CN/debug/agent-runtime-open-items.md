...
read_when:
  ...
  - 想决定当前版本下一步先做什么时
  - 想看 review、Action Feed、Context Archive 还差哪些工作时
...
- `/review` 作为公开两阶段 review 入口
- Context Archive 基础能力
- Action Feed 基础能力
- durable auto-write 的后台 `memory_extractor` 主链
- shared special-agent substrate，已覆盖 `session_summary`、`memory_extractor`、`dream` 和 review
...
两阶段 review flow 现在能用，但还是 MVP。

- [x] 把 review 输出从 `VERDICT + summary` 升级成结构化报告。
  - 已落地字段：
    - `verdict`
    - `summary`
    - `checks[]`
  ...
  - 结构化报告现在已经接进：
    - review parsing
    - `/review` 工具返回
    - 父 Action Feed 的 review detail
    - task trajectory 的 completion detail / archive payload
- [ ] 让 review 失败正式影响 completion。
  - 现在 `PASS` 已经能回流 evidence。
  - 但 `FAIL / PARTIAL` 还没有正式成为父任务 completion 的一等信号。
- [ ] 增加 review policy。
  - 明确哪些任务默认要求验证。
  - 建议先从 `fix` 和 `code` 任务开始。
- [ ] 让 completion policy 可以自动触发 review。
  - 现在主要还是靠手动 `/review`。
  - 后续应该支持命中 `review_missing` 时自动拉起 review。
- [ ] 继续收紧 review capability。
  - 保持只读
  - 不允许改代码
  - 不允许继续 spawn 子 agent
- [ ] 强化 review 动作回流。
  - 父聊天现在能看到高层状态。
  - 但还看不到 review 子任务里的关键 checks。
...
- [ ] 在 Browser client 里增加 agent runtime 详情面板。
  - runtime state
  - trajectory
  - completion
  - review result
