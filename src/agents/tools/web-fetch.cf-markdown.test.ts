...
describe("web_fetch Cloudflare Markdown for Agents", () => {
  ...
  it("falls back to direct fetch when runtime metadata points at an unavailable provider", async () => {
    const fetchSpy = vi
      .fn()
      .mockResolvedValue(
        htmlResponse(
    ...
    await tool?.execute?.("call", { url: "https://example.com/runtime-provider-off" });

    expect(fetchSpy).toHaveBeenCalled();
    expect(fetchSpy.mock.calls[0]?.[0]).toBe("https://example.com/runtime-provider-off");
  });
