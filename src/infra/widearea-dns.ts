...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate =
    params?.configDomain ?? env.CRAWCLAW_WIDE_AREA_DOMAIN ?? env.CRAWCLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*crawclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "crawclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "crawclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "crawclaw-gw");
  ...
  records.push(`_crawclaw-gw._tcp IN PTR ${instanceLabel}._crawclaw-gw._tcp`);
  records.push(`${instanceLabel}._crawclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._crawclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; crawclaw-content-hash: ${contentHash}\n${contentBody}`;
}
