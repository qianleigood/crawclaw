...
export async function executeNodeHostCommand(
  ...
  if (requiresAsk) {
    const initialWarningText = params.warnings.length ? `${params.warnings.join("\n")}\n\n` : "";
    const guardUnavailable = execHostShared.resolveExecApprovalGuardUnavailable({
      runId: params.runId,
      agentId: runAgentId,
      sessionKey: runSessionKey,
    });
    if (guardUnavailable) {
      if (
        execHostShared.shouldResolveExecApprovalUnavailableInline({
          ...
          unavailableReason: guardUnavailable.unavailableReason,
        ...
        return execHostShared.buildExecApprovalUnavailableToolResult({
          host: "node",
          command: params.command,
          cwd: params.workdir,
          nodeId,
          warningText: execHostShared.mergeExecApprovalWarningText(
            initialWarningText,
            guardUnavailable.warningText,
          ),
          unavailableReason: guardUnavailable.unavailableReason,
        });
      }
  ...
  if (requiresAsk && inlineApprovalDecision === null && !inlineApprovedByAsk) {
