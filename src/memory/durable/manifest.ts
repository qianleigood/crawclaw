...
import { resolveDurableMemoryIndexPath, resolveDurableMemoryRootDir, type DurableMemoryScope } from "./scope.ts";
...
function parseIndexNotePaths(indexText: string): string[] {
  const paths = new Set<string>();
  const add = (value: string) => {
    const cleaned = value.trim().replace(/\\/g, "/").replace(/^\.?\/?/, "");
    if (!cleaned || !/\.md$/i.test(cleaned) || cleaned === "MEMORY.md") {return;}
    paths.add(cleaned);
  };

  for (const match of indexText.matchAll(/\[[^\]]+\]\(([^)]+\.md)\)/gi)) {
    if (match[1]) {add(match[1]);}
  }
  for (const match of indexText.matchAll(/(?:^|\s)([^\s()]+\.md)(?:\s|$)/gi)) {
    if (match[1]) {add(match[1]);}
  }
  return [...paths];
}

async function listCandidateNotePaths(scope: DurableMemoryScope): Promise<string[]> {
  const scopeDir =
    scope.rootDir ??
    path.join(
  ...
  if (indexExists) {
    ...
    const indexed = parseIndexNotePaths(indexText);
  ...
  for (const absolutePath of markdownFiles) {
    ...
    if (notePath === "MEMORY.md") {continue;}
    candidatePaths.add(notePath);
  }
...
export async function scanDurableMemoryManifest(params: {
  ...
  const candidatePaths = await listCandidateNotePaths(params.scope);
  ...
  return entries
    .toSorted((left, right) => right.updatedAt - left.updatedAt || left.notePath.localeCompare(right.notePath))
    .slice(0, maxFiles);
}
