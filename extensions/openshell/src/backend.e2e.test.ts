...
import { createSandboxTestContext } from "crawclaw/plugin-sdk/testing";
...
const CRAWCLAW_OPENSHELL_E2E = process.env.CRAWCLAW_E2E_OPENSHELL === "1";
const CRAWCLAW_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const CRAWCLAW_OPENSHELL_COMMAND =
  process.env.CRAWCLAW_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "crawclaw-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && CRAWCLAW_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: CRAWCLAW_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(CRAWCLAW_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-openshell-e2e-"));
      ...
      const gatewayName = `crawclaw-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `crawclaw-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/crawclaw-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: CRAWCLAW_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("crawclaw-openshell-e2e");
