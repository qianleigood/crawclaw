...
function cloneTaskRecord(record: TaskRecord): TaskRecord {
  return {
    ...record,
    ...(record.agentMetadata ? { agentMetadata: { ...record.agentMetadata } } : {}),
  };
}
...
function normalizeAgentTaskMetadata(
...
function agentTaskMetadataEquals(
  left: AgentTaskMetadata | undefined,
  right: AgentTaskMetadata | undefined,
): boolean {
  return (
    left?.parentAgentId === right?.parentAgentId &&
    left?.mode === right?.mode &&
    left?.transcriptRef === right?.transcriptRef &&
    left?.runtimeStateRef === right?.runtimeStateRef &&
    left?.trajectoryRef === right?.trajectoryRef &&
    left?.capabilitySnapshotRef === right?.capabilitySnapshotRef &&
    left?.spawnSource === right?.spawnSource
  );
}

function mergeAgentTaskMetadata(
  current: AgentTaskMetadata | undefined,
  incoming: AgentTaskMetadata | undefined,
): AgentTaskMetadata | undefined {
...
function addAgentIdIndex(taskId: string, task: Pick<TaskRecord, "agentId">) {
  const key = normalizeAgentId(task.agentId);
  if (!key) {
    return;
  }
  addIndexedKey(taskIdsByAgentId, key, taskId);
}

function deleteAgentIdIndex(taskId: string, task: Pick<TaskRecord, "agentId">) {
  const key = normalizeAgentId(task.agentId);
  if (!key) {
    return;
  }
  deleteIndexedKey(taskIdsByAgentId, key, taskId);
}

function addParentAgentIdIndex(taskId: string, task: Pick<TaskRecord, "agentMetadata">) {
  const key = normalizeParentAgentId(task.agentMetadata);
  if (!key) {
    return;
  }
  addIndexedKey(taskIdsByParentAgentId, key, taskId);
}

function deleteParentAgentIdIndex(taskId: string, task: Pick<TaskRecord, "agentMetadata">) {
  const key = normalizeParentAgentId(task.agentMetadata);
  if (!key) {
    return;
  }
  deleteIndexedKey(taskIdsByParentAgentId, key, taskId);
}
...
function rebuildAgentIdIndex() {
  taskIdsByAgentId.clear();
  for (const [taskId, task] of tasks.entries()) {
    addAgentIdIndex(taskId, task);
  }
}

function rebuildParentAgentIdIndex() {
  taskIdsByParentAgentId.clear();
  for (const [taskId, task] of tasks.entries()) {
    addParentAgentIdIndex(taskId, task);
  }
}
...
function mergeExistingTaskForCreate(
  ...
  const mergedAgentMetadata = mergeAgentTaskMetadata(
    existing.agentMetadata,
    normalizeAgentTaskMetadata(params.agentMetadata),
  );
  if (!agentTaskMetadataEquals(existing.agentMetadata, mergedAgentMetadata)) {
    patch.agentMetadata = mergedAgentMetadata;
  }
...
function updateTask(taskId: string, patch: Partial<TaskRecord>): TaskRecord | null {
  ...
  const hasAgentMetadataPatch = Object.prototype.hasOwnProperty.call(patch, "agentMetadata");
  const next: TaskRecord = {
    ...current,
    ...patch,
    ...(hasAgentMetadataPatch
      ? { agentMetadata: normalizeAgentTaskMetadata(patch.agentMetadata) }
      : {}),
  };
  ...
  const agentIdIndexChanged = normalizeAgentId(current.agentId) !== normalizeAgentId(next.agentId);
  const parentAgentIndexChanged =
    normalizeParentAgentId(current.agentMetadata) !== normalizeParentAgentId(next.agentMetadata);
  ...
  if (agentIdIndexChanged) {
    deleteAgentIdIndex(taskId, current);
    addAgentIdIndex(taskId, next);
  }
  if (parentAgentIndexChanged) {
    deleteParentAgentIdIndex(taskId, current);
    addParentAgentIdIndex(taskId, next);
  }
...
export function mergeTaskAgentMetadataById(params: {
  taskId: string;
  agentMetadata?: AgentTaskMetadata | null;
}): TaskRecord | null {
  ensureTaskRegistryReady();
  ...
  const merged = mergeAgentTaskMetadata(
    current.agentMetadata,
    normalizeAgentTaskMetadata(params.agentMetadata),
  );
  if (agentTaskMetadataEquals(current.agentMetadata, merged)) {
    return cloneTaskRecord(current);
  }
...
export function createTaskRecord(params: {
  ...
  const agentMetadata = normalizeAgentTaskMetadata(params.agentMetadata);
  ...
  addAgentIdIndex(taskId, record);
  addParentAgentIdIndex(taskId, record);
...
export function findLatestTaskForAgentId(agentId: string): TaskRecord | undefined {
  const task = listTasksForAgentId(agentId)[0];
  return task ? cloneTaskRecord(task) : undefined;
}
...
export function listTasksForAgentId(agentId: string): TaskRecord[] {
  ensureTaskRegistryReady();
  const lookup = normalizeAgentId(agentId);
  if (!lookup) {
    return [];
  }
  return listTasksFromIndex(taskIdsByAgentId, lookup);
}

export function findLatestTaskForParentAgentId(parentAgentId: string): TaskRecord | undefined {
  const task = listTasksForParentAgentId(parentAgentId)[0];
  return task ? cloneTaskRecord(task) : undefined;
}

export function listTasksForParentAgentId(parentAgentId: string): TaskRecord[] {
  ensureTaskRegistryReady();
  const lookup = normalizeAgentId(parentAgentId);
  if (!lookup) {
    return [];
  }
  return listTasksFromIndex(taskIdsByParentAgentId, lookup);
}
