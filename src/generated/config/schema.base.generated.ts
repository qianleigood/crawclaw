...
import type { BaseConfigSchemaResponse } from "../../config/schema-base.js";
...
export const GENERATED_BASE_CONFIG_SCHEMA = {
  ...
  uiHints: {
    ...
    update: {
      ...
      group: "Update",
      order: 25,
      help: "Update-channel and startup-check behavior for keeping CrawClaw runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
      tags: ["advanced"],
    },
    cli: {
      label: "CLI",
    ...
    meta: {
      label: "Metadata",
      help: "Metadata fields automatically maintained by CrawClaw to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
      tags: ["advanced"],
    },
    "meta.lastTouchedVersion": {
      label: "Config Last Touched Version",
      help: "Auto-set when CrawClaw writes the config.",
      tags: ["media"],
    },
    "meta.lastTouchedAt": {
    ...
    "env.vars": {
      ...
      help: "Explicit key/value environment variable overrides merged into runtime process environment for CrawClaw. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
      tags: ["advanced"],
    },
    "wizard.lastRunAt": {
    ...
    "wizard.lastRunVersion": {
      ...
      help: "CrawClaw version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version setup changes.",
      tags: ["advanced"],
    },
    ...
    "diagnostics.cacheTrace.filePath": {
      ...
      help: "JSONL output path for cache trace logs (default: $CRAWCLAW_STATE_DIR/logs/cache-trace.jsonl).",
      tags: ["observability", "storage"],
    },
    ...
    "agents.list[].runtime": {
      label: "Agent Runtime",
      help: "Optional runtime descriptor for this agent. Use embedded for default CrawClaw execution or acp for external ACP harness defaults.",
      tags: ["advanced"],
    },
    "agents.list[].runtime.type": {
      label: "Agent Runtime Type",
      help: 'Runtime type for this agent: "embedded" (default CrawClaw runtime) or "acp" (ACP harness defaults).',
      tags: ["advanced"],
    },
    ...
    "agents.list[].runtime.acp.agent": {
      ...
      help: "Optional ACP harness agent id to use for this CrawClaw agent (for example codex, claude, cursor, gemini, crawclaw).",
      tags: ["advanced"],
    },
    ...
    "gateway.nodes.allowCommands": {
      ...
      help: "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `crawclaw security audit`.",
      tags: ["access", "network"],
    },
    ...
    "nodeHost.browserProxy.allowProfiles": {
      ...
      help: "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, CrawClaw enforces least-privilege profile access and blocks persistent profile create/delete through the proxy.",
    ...
    "agents.defaults.memorySearch.store.path": {
      ...
      help: "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.crawclaw/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
      tags: ["storage"],
    },
    ...
    "memory.notebooklm.auth.profile": {
      label: "NotebookLM Auth Profile",
      help: "NotebookLM profile name used by the local wrapper or CLI integration. Keep this stable unless you intentionally manage multiple NotebookLM identities.",
      tags: ["storage"],
    },
    "memory.notebooklm.auth.cookieFile": {
      label: "NotebookLM Cookie File",
    ...
    "memory.notebooklm.auth.degradedCooldownMs": {
      label: "NotebookLM Auth Degraded Cooldown (ms)",
      help: "Cooldown window after NotebookLM enters a degraded or expired state. During this window CrawClaw reuses the cached degraded status instead of hammering the provider with repeated probes.",
      tags: ["storage"],
    },
    "memory.notebooklm.auth.refreshCooldownMs": {
      label: "NotebookLM Auth Refresh Cooldown (ms)",
      help: "Cooldown window after an explicit NotebookLM refresh failure. This limits repeated refresh attempts until the next eligible recovery window or a forced manual refresh.",
      tags: ["storage"],
    },
    ...
    "memory.notebooklm.auth.heartbeat.minIntervalMs": {
      label: "NotebookLM Heartbeat Min Interval (ms)",
      help: "Lower bound for the randomized NotebookLM auth heartbeat interval in milliseconds. Use a moderate floor to avoid noisy provider probes.",
      tags: ["performance", "storage", "automation"],
    },
    "memory.notebooklm.auth.heartbeat.maxIntervalMs": {
      label: "NotebookLM Heartbeat Max Interval (ms)",
    ...
    "memory.notebooklm.cli.notebookId": {
      ...
      help: "Optional NotebookLM notebook identifier injected into `{notebookId}` placeholders for scoped CLI retrieval. Leave empty when your command performs cross-notebook search without a fixed notebook.",
      tags: ["storage"],
    },
    ...
    "models.providers.*.injectNumCtxForOpenAICompat": {
      ...
      help: "Controls whether CrawClaw injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
      tags: ["models"],
    },
    ...
    "agents.defaults.sandbox.browser.network": {
      ...
      help: "Docker network for sandbox browser containers (default: crawclaw-sandbox-browser). Avoid bridge if you need stricter isolation.",
      tags: ["storage"],
    },
    ...
    "commands.mcp": {
      label: "Allow /mcp",
      help: "Allow /mcp chat command to manage CrawClaw MCP server config under mcp.servers (default: false).",
      tags: ["advanced"],
    },
    "commands.plugins": {
      label: "Allow /plugins",
    ...
    "session.parentForkMaxTokens": {
      ...
      help: "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, CrawClaw starts a fresh thread session instead of forking; set 0 to disable this protection.",
    ...
    "discovery.wideArea.domain": {
      ...
      help: "Optional unicast DNS-SD domain for wide-area discovery, such as crawclaw.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
      tags: ["network"],
    },
    "discovery.mdns": {
    ...
    "agents.list[].identity.avatar": {
      ...
      placeholder: "avatars/crawclaw.png",
      tags: ["advanced"],
    },
    ...
    "plugins.slots": {
      label: "Plugin Slots",
      help: "Selects which plugins own exclusive runtime slots such as the memory slot so only one plugin provides that capability. Use explicit slot ownership to avoid overlapping providers with conflicting behavior.",
      tags: ["advanced"],
    },
    "plugins.entries": {
    ...
    "plugins.installs": {
      ...
      help: "CLI-managed install metadata (used by `crawclaw plugins update` to locate install sources).",
      tags: ["advanced"],
    },
