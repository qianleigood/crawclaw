...
export function createCronStoreHarness(options?: { prefix?: string }) {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), options?.prefix ?? "crawclaw-cron-"));
  });
...
export function createStartedCronServiceWithFinishedBarrier(params: {
  ...
  return { cron, enqueueSystemEvent, requestMainSessionWake, finished };
}
