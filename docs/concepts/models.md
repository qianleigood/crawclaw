...
- `agents.defaults.models` is the allowlist/catalog of models CrawClaw can use (plus aliases).
...
`crawclaw models` (no subcommand) is a shortcut for `models status`.
...
`crawclaw models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.crawclaw/agents/<agentId>/agent/models.json`). This file
...
This applies whenever CrawClaw regenerates `models.json`, including command-driven paths like `crawclaw agent`.
