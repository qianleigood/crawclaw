...
summary: CrawClaw 如何使用会话记忆、持久记忆、NotebookLM 知识召回和上下文归档
title: 记忆概览
x-i18n:
...
- **知识召回** 由 NotebookLM 提供支持，在提示词组装期间查询
...
<Tip>
如果希望智能体长期记住某些内容，请明确告诉它。它可以根据事实的类型写入持久记忆或知识笔记。
</Tip>

## 知识召回

知识召回是一个提供商支持的层。NotebookLM 是当前的默认提供商，但提示词组装与知识提供商注册表对话，而非直接调用 NotebookLM CLI。CrawClaw 可以：

- 查询 NotebookLM 获取相关知识
- 通过 `write_knowledge_note` 直接写入结构化知识笔记
- 通过 `crawclaw memory` 管理登录、刷新和提供商状态
- 通过 `crawclaw memory prompt-journal-summary` 汇总夜间记忆提示词诊断

知识召回在每个智能体轮次的上下文组装阶段运行。运行时首先对用户查询进行分类，然后从该分类构建知识查询计划：

- 仅偏好提示词被路由到持久记忆并跳过知识提供商查询
- SOP 和操作手册提示词可以借用少量提供商搜索预算，以免弱元数据导致运营知识饥饿
- 成功的 `write_knowledge_note` 调用更新小型本地基线索引，因此最近写入的知识在实时提供商搜索无结果时仍可被召回
- 本地基线命中保持自己的 `local_knowledge_index` 来源，因此检查和提示词诊断可以将它们与实时 NotebookLM 命中区分开来
- 选中的知识召回仍由记忆提示词预算限制；层分配是软指导，但组装的知识部分必须适合该轮次的全局知识预算
- 选中的目标层、提供商 ID、原因和限制被写入记忆召回诊断，以便检查/调试流程可以解释知识被查询或跳过的原因

如果当前轮次没有可用的提示词，运行时完全跳过知识提供商查询。

`write_knowledge_note` 是当前运行时中唯一的 NotebookLM 写入路径。它在模式和安全验证后直接通过工具路径写入。
...
- **知识召回** 在运行中跨会话使用相同的已配置 NotebookLM 后端；
  不按会话 ID 分区。
