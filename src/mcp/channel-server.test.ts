...
import { z } from "zod";
import { CrawClawChannelBridge, createCrawClawChannelMcpServer } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createCrawClawChannelMcpServer({
...
describe("crawclaw channel mcp server", () => {
  describe("gateway-backed flows", () => {
    ...
    test("sendMessage normalizes route metadata for gateway send", async () => {
      const bridge = new CrawClawChannelBridge({} as never, {
        claudeChannelMode: "off",
        verbose: false,
      });
