...
import { promptYesNo } from "../terminal/prompt.js";
...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.CRAWCLAW_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for CrawClaw. You can keep running the web gateway without it: \`${formatCliCommand("crawclaw gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
