...
type TransportSettingsManager = {
  getGlobalSettings: () => { transport?: unknown };
  getProjectSettings: () => { transport?: unknown };
};
...
function supportsPromptCachePayload(modelApi?: string): boolean {
  return (
...
export function resolveAgentTransportOverride(params: {
  settingsManager: TransportSettingsManager;
...
function createStreamWithExtraParams(
  ...
  if (Object.hasOwn(extraParams, "toolChoice")) {
    streamParams.toolChoice = extraParams.toolChoice;
  } else if (Object.hasOwn(extraParams, "tool_choice")) {
  ...
  const skipCacheWrite = extraParams.skipCacheWrite === true;
  if (cacheRetention) {
    streamParams.cacheRetention = cacheRetention;
  }
  if (skipCacheWrite) {
    streamParams.skipCacheWrite = true;
    streamParams.cacheRetention = "none";
  }
  const promptCacheKey =
    normalizeOptionalText(extraParams.promptCacheKey) ??
    normalizeOptionalText(extraParams.prompt_cache_key);
  if (promptCacheKey && supportsPromptCachePayload(modelApi)) {
    streamParams.promptCacheKey = promptCacheKey;
  }
  const promptCacheRetention =
    normalizeOptionalText(extraParams.promptCacheRetention) ??
    normalizeOptionalText(extraParams.prompt_cache_retention);
  if (!skipCacheWrite && promptCacheRetention && supportsPromptCachePayload(modelApi)) {
    streamParams.promptCacheRetention = promptCacheRetention;
  }
  ...
  const wrappedStreamFn: StreamFn = (model, context, options) => {
    ...
    const toolChoice = Object.hasOwn(providerOptions, "toolChoice")
      ...
      : undefined;
    const promptCacheKey = normalizeOptionalText(providerOptions.promptCacheKey);
    const promptCacheRetention = normalizeOptionalText(providerOptions.promptCacheRetention);
