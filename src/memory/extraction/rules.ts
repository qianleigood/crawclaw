...
export function correctEdgeType(
  edge: ExtractedEdge,
  nameToType: Map<string, NodeType>,
): ExtractedEdge | null {
  ...
  if (fromType === "TASK" && isProcedureNodeType(toType) && type !== "USED_SKILL") {
    type = "USED_SKILL";
  }
  if (fromType === "EVENT" && isProcedureNodeType(toType) && type !== "SOLVED_BY") {
    type = "SOLVED_BY";
  }

  // 常见模型误判修正：把 EVENT->TASK / TASK->EVENT 尽量折算到 EVENT->SKILL 或 TASK->SKILL 不现实时直接丢弃
  if (fromType === "EVENT" && toType === "TASK" && type === "SOLVED_BY") {
    return null;
  }
  if (fromType === "TASK" && toType === "EVENT" && type === "PATCHES") {
    return null;
  }
  if (fromType === "TASK" && toType === "TASK" && type === "REQUIRES") {
    return null;
  }
  if (fromType === "EVENT" && toType === "TASK" && type === "CONFLICTS_WITH") {
    return null;
  }
  if (fromType === "TASK" && toType === "TASK" && type === "SOLVED_BY") {
    return null;
  }
  if (
    isProcedureNodeType(fromType) &&
    toType === "EVENT" &&
    (type === "PATCHES" || type === "SOLVED_BY")
  ) {
    return null;
  }
  ...
  if (!EDGE_FROM_CONSTRAINT[type].has(fromType)) {
    return null;
  }
  if (!EDGE_TO_CONSTRAINT[type].has(toType)) {
    return null;
  }
