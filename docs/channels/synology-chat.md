---
summary: "Synology Chat webhook setup and CrawClaw config"
read_when:
  - Setting up Synology Chat with CrawClaw
...
```bash
crawclaw plugins install ./path/to/local/synology-chat-plugin
```
...
1. Install and enable the Synology Chat plugin.
   - `crawclaw onboard` now shows Synology Chat in the same channel setup list as `crawclaw channels add`.
   - Non-interactive setup: `crawclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your CrawClaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in CrawClaw.
   - Guided: `crawclaw onboard`
   - Direct: `crawclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `crawclaw pairing list synology-chat`
  - `crawclaw pairing approve synology-chat <CODE>`
...
```bash
crawclaw message send --channel synology-chat --target 123456 --text "Hello from CrawClaw"
crawclaw message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. CrawClaw now rejects duplicate exact paths
