...
metadata: { "crawclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == CrawClaw `sessions_spawn`
- **File I/O** == CrawClaw `read`/`write`
- **Remote fetch** == CrawClaw `web_fetch` (or `exec` with curl when POST is required)
