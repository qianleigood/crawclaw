...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredCrawClawTmpDir: vi.fn(() => path.join(os.tmpdir(), "crawclaw-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredCrawClawTmpDir: pluginApiMocks.resolvePreferredCrawClawTmpDir,
...
function createApi(params?: {
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as CrawClawPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: CrawClawPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as CrawClawPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredCrawClawTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredCrawClawTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the CrawClaw iOS app:");
    expect(text).toContain("![CrawClaw pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the CrawClaw iOS app:");
