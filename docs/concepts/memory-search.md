...
CrawClaw runs two retrieval paths in parallel and merges the results:
...
**No results?** Run `crawclaw memory status` to check the index. If empty, run
`crawclaw memory index --force`.

**Only keyword matches?** Your embedding provider may not be configured. Check
`crawclaw memory status --deep`.
