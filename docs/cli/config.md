---
summary: "CLI reference for `crawclaw config` (get/set/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `crawclaw.json`: get/set/unset/file/schema/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `crawclaw configure`).

## Examples

```bash
crawclaw config file
...
crawclaw config validate
crawclaw config validate --json
```

### `config schema`

Print the generated JSON schema for `crawclaw.json` to stdout as plain text.
...
```bash
crawclaw config schema > crawclaw.schema.json
```
...
`crawclaw config set` supports four assignment styles:

1. Value mode: `crawclaw config set <path> <value>`
2. SecretRef builder mode:

```bash
crawclaw config set channels.discord.token \
...
```bash
crawclaw config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/crawclaw-vault \
  --provider-arg read \
...
```bash
crawclaw config set --batch-json '[
  {
...
crawclaw config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/crawclaw/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `crawclaw.json`.
...
```json
{
  "ok": true,
  "operations": 1,
  "configPath": "~/.crawclaw/crawclaw.json",
  "inputModes": ["builder"],
  "checks": {
    "schema": false,
    "resolvability": true,
...
- `config file`: Print the active config file path (resolved from `CRAWCLAW_CONFIG_PATH` or default location). Legacy `CRAWCLAW_CONFIG_PATH` remains supported only for migration compatibility.
