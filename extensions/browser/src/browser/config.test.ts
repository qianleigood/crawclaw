...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("crawclaw");
    expect(profile?.driver).toBe("crawclaw");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const crawclaw = resolveProfile(resolved, "crawclaw");
    expect(crawclaw?.driver).toBe("crawclaw");
    expect(crawclaw?.cdpPort).toBe(18800);
    expect(crawclaw?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from CRAWCLAW_GATEWAY_PORT when unset", () => {
    withEnv({ CRAWCLAW_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const crawclaw = resolveProfile(resolved, "crawclaw");
      expect(crawclaw?.cdpPort).toBe(19012);
      expect(crawclaw?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ CRAWCLAW_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const crawclaw = resolveProfile(resolved, "crawclaw");
      expect(crawclaw?.cdpPort).toBe(19022);
      expect(crawclaw?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(crawclaw?.cdpPort).toBe(19000);
    expect(crawclaw?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("sets usesChromeMcp false for managed profiles", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        work: { cdpPort: 18801, color: "#0066CC" },
      },
    ...
    const managed = resolveProfile(resolved, "crawclaw")!;
  ...
  describe("default profile preference", () => {
    it("defaults to crawclaw profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("crawclaw");
    });

    it("keeps crawclaw default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("crawclaw");
    ...
    it("keeps crawclaw default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("crawclaw");
    ...
    it("keeps crawclaw default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
