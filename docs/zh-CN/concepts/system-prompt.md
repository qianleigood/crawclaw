...
- `full`（默认）：包含上述所有部分。
- `minimal`：用于子智能体；省略 **Skills**、**Memory Recall**、**CrawClaw Self-Update**、**Model Aliases**、**User Identity**、**Reply Tags**、**Messaging**、**Silent Replies** 和 **Heartbeats**。Tooling、**Safety**、Workspace、Sandbox、Current Date & Time（已知时）、Runtime 和注入的上下文仍然可用。
- `none`：仅返回基本身份行。
...
> **注意：** `memory/*.md` 每日文件不会自动注入。它们只会在模型显式调用 `memory_search` / `memory_get` 等工具时进入上下文，因此不会默认占用上下文窗口。
...
如果可用，系统提示词包含一个 **Documentation** 部分，指向本地 CrawClaw 文档目录（仓库工作区中的 `docs/` 或打包的 npm 包文档），并注明公共镜像、源仓库、社区 Discord 和 ClawHub (https://clawhub.com) 用于 Skills 发现。提示词指示模型首先查阅本地文档了解 CrawClaw 行为、命令、配置或架构，并尽可能自己运行 `crawclaw status`（仅在无法访问时询问用户）。
