...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: CrawClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: CrawClawConfig) => { changed: boolean; next: CrawClawConfig };
  defaultModel: string;
  overrideConfig: CrawClawConfig;
  alreadyDefaultConfig: CrawClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: CrawClawConfig) => config,
      applyDefaultConfig: (config: CrawClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: CrawClawConfig = { agents: { defaults: {} } };
