...
export type FeishuStreamingPolicyReason = ChannelStreamingDecisionReason;

export function resolveFeishuStreamingPolicy(params: {
  renderMode: "auto" | "raw" | "card";
  streamingEnabled: boolean | undefined;
  threadReplyMode: boolean;
}): ChannelStreamingDecision {
  if (params.threadReplyMode) {
    return { enabled: false, surface: "none", reason: "disabled_for_thread_reply" };
  }
  if (params.streamingEnabled === false) {
    return { enabled: false, surface: "none", reason: "disabled_by_config" };
  }
  if (params.renderMode === "raw") {
    return { enabled: false, surface: "none", reason: "disabled_for_render_mode" };
  }
  return { enabled: true, surface: "card_stream", reason: "enabled" };
}
...
export function createFeishuReplyDispatcher(params: CreateFeishuReplyDispatcherParams) {
  ...
  const streamingPolicy = resolveFeishuStreamingPolicy({
    renderMode,
    streamingEnabled: account.config?.streaming,
    threadReplyMode,
  });
  const streamingEnabled = streamingPolicy.enabled;
  if (!streamingEnabled) {
    params.runtime.log?.(
      `feishu[${account.accountId}] streaming ${formatChannelStreamingDecision({
        ...streamingPolicy,
        renderMode,
      })}`,
    );
  ...
  if (isDiagnosticsEnabled(cfg)) {
    emitDiagnosticEvent({
      ...
      sessionKey,
      chatId,
      enabled: streamingPolicy.enabled,
      surface: streamingPolicy.surface,
      reason: streamingPolicy.reason,
    });
  }
  if (sessionKey) {
