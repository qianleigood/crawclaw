...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".crawclaw-profile-decorated");
}
 ...
 */
export function decorateCrawClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_CRAWCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_CRAWCLAW_BROWSER_COLOR).toUpperCase();
