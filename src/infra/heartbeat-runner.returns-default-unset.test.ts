...
vi.mock("../memory/index.js", () => ({
  resolveMemoryRuntime: vi.fn(async () => null),
}));
...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-heartbeat-suite-"));
});
  ...
  it.each([
    {
      ...
      message: "Group alert",
      applyOverride: ({ cfg, sessionKey }: { cfg: CrawClawConfig; sessionKey: string }) => {
