...
describe("getApiKeyForModel", () => {
  it("reads oauth auth-profiles entries from auth-profiles.json via explicit profile", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-oauth-"));

    try {
      const agentDir = path.join(tempDir, "agent");
      await withEnvAsync(
        {
          CRAWCLAW_STATE_DIR: tempDir,
          CRAWCLAW_AGENT_DIR: agentDir,
        ...
        async () => {
          ...
          const store = ensureAuthProfileStore(process.env.CRAWCLAW_AGENT_DIR, {
  ...
  it("reports the configured auth store when only Codex OAuth is configured", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-auth-"));
    ...
    try {
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          expect(String(error)).toContain("auth-profiles.json");
          expect(String(error)).toContain("crawclaw agents add <id>");
        },
      );
    } finally {
      await fs.rm(tempDir, { recursive: true, force: true });
  ...
  it("resolveEnvApiKey('anthropic-vertex') accepts GOOGLE_APPLICATION_CREDENTIALS with project_id", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-adc-"));
