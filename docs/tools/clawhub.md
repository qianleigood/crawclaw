...
4. Users browse, download, and install skills in CrawClaw.
...
If you want to add new capabilities to your CrawClaw agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
current working directory. If a CrawClaw workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). CrawClaw loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.crawclaw/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches CrawClaw how to perform a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to CrawClaw workspace).
...
Code plugins must include the required CrawClaw metadata in `package.json`:
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/crawclaw/skills` and `~/.crawclaw/skills`). This is designed to find older skill installs without extra flags.
