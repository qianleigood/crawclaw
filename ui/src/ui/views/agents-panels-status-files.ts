...
function renderAgentContextCard(
  ...
  return html`
    <section class="card">
      <div class="card-title">${uiLiteral("Agent Context")}</div>
      <div class="card-sub">${uiLiteral(subtitle)}</div>
      <div class="agents-overview-grid" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">${uiLiteral("Workspace")}</div>
          <div>
            <button
              type="button"
              class="workspace-link mono"
              @click=${() => onSelectPanel("files")}
              title=${uiLiteral("Open Files tab")}
            >
              ${context.workspace}
            </button>
          </div>
        ...
        <div class="agent-kv">
          <div class="label">${uiLiteral("Primary Model")}</div>
          <div class="mono">${context.model}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${uiLiteral("Identity Name")}</div>
          <div>${context.identityName}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${uiLiteral("Identity Avatar")}</div>
          <div>${context.identityAvatar}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${uiLiteral("Skills Filter")}</div>
          <div>${context.skillsLabel}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${uiLiteral("Default")}</div>
          <div>${context.isDefault ? uiLiteral("yes") : uiLiteral("no")}</div>
        </div>
      </div>
    </section>
  `;
    ...
    <section class="grid grid-cols-2">
      ...
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">${uiLiteral("Channels")}</div>
            <div class="card-sub">${uiLiteral("Gateway-wide channel status snapshot.")}</div>
          </div>
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? uiLiteral("Refreshing…") : uiLiteral("Refresh")}
          </button>
        </div>
        <div class="muted" style="margin-top: 8px;">${uiLiteral("Last refresh")}: ${lastSuccessLabel}</div>
        ${params.error
        ...
        ${!params.snapshot
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${uiLiteral("Load channels to see live status.")}
              </div>
            `
          : nothing}
        ${entries.length === 0
          ? html` <div class="muted" style="margin-top: 16px">${uiLiteral("No channels found.")}</div> `
          : html`
              <div class="list" style="margin-top: 16px;">
                ${entries.map((entry) => {
                  const summary = summarizeChannelAccounts(entry.accounts);
                  const status = summary.total
                    ? `${summary.connected}/${summary.total} ${uiLiteral("connected")}`
                    : uiLiteral("no accounts");
                  const configLabel = summary.configured
                    ? `${summary.configured} ${uiLiteral("configured")}`
                    : uiLiteral("not configured");
                  const enabled = summary.total ? `${summary.enabled} ${uiLiteral("enabled")}` : uiLiteral("disabled");
