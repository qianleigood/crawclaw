...
let buildMemoryRuntimeMaintenanceRuntimeContext: typeof import("./memory-runtime-maintenance.js").buildMemoryRuntimeMaintenanceRuntimeContext;
let runMemoryRuntimeMaintenance: typeof import("./memory-runtime-maintenance.js").runMemoryRuntimeMaintenance;
...
async function loadFreshMemoryRuntimeMaintenanceModuleForTest() {
  vi.resetModules();
  ({ buildMemoryRuntimeMaintenanceRuntimeContext, runMemoryRuntimeMaintenance } =
    await import("./memory-runtime-maintenance.js"));
}

describe("buildMemoryRuntimeMaintenanceRuntimeContext", () => {
  beforeEach(async () => {
    rewriteTranscriptEntriesInSessionManagerMock.mockClear();
    rewriteTranscriptEntriesInSessionFileMock.mockClear();
    await loadFreshMemoryRuntimeMaintenanceModuleForTest();
  });

  it("adds a transcript rewrite helper that targets the current session file", async () => {
    const runtimeContext = buildMemoryRuntimeMaintenanceRuntimeContext({
      sessionId: "session-1",
  ...
  it("reuses the active session manager when one is provided", async () => {
    const sessionManager = { appendMessage: vi.fn() } as unknown as Parameters<
      typeof buildMemoryRuntimeMaintenanceRuntimeContext
    >[0]["sessionManager"];
    const runtimeContext = buildMemoryRuntimeMaintenanceRuntimeContext({
      sessionId: "session-1",
      sessionKey: "agent:main:session-1",
...
describe("runMemoryRuntimeMaintenance", () => {
  beforeEach(async () => {
    rewriteTranscriptEntriesInSessionManagerMock.mockClear();
    rewriteTranscriptEntriesInSessionFileMock.mockClear();
    await loadFreshMemoryRuntimeMaintenanceModuleForTest();
  ...
  it("passes a rewrite-capable runtime context into maintain()", async () => {
    ...
    const result = await runMemoryRuntimeMaintenance({
      memoryRuntime: {
