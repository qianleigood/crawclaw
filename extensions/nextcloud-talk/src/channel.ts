import { describeWebhookAccountSnapshot } from "crawclaw/plugin-sdk/account-helpers";
...
import { createAllowlistProviderRouteAllowlistWarningCollector } from "crawclaw/plugin-sdk/channel-policy";
import { createChatChannelPlugin } from "crawclaw/plugin-sdk/core";
import { runStoppablePassiveMonitor } from "crawclaw/plugin-sdk/extension-shared";
import {
          ...
          if (changed) {
            if (nextSection && Object.keys(nextSection).length > 0) {
              ...
              if (Object.keys(nextChannels).length > 0) {
                nextCfg.channels = nextChannels as CrawClawConfig["channels"];
              } else {
