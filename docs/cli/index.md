---
summary: "CrawClaw CLI reference for `crawclaw` commands, subcommands, and options"
read_when:
...
CrawClaw uses a crab palette for CLI output.
...
Palette source of truth: `src/terminal/palette.ts` (the “crab palette”).
...
- `crawclaw security audit` — audit config + local state for common security foot-guns.
- `crawclaw security audit --deep` — best-effort live Gateway probe.
- `crawclaw security audit --fix` — tighten safe defaults and chmod state/config.

## Secrets

- `crawclaw secrets reload` — re-resolve refs and atomically swap the runtime snapshot.
- `crawclaw secrets audit` — scan for plaintext residues, unresolved refs, and precedence drift (`--allow-exec` to execute exec providers during audit).
- `crawclaw secrets configure` — interactive helper for provider setup + SecretRef mapping + preflight/apply (`--allow-exec` to execute exec providers during preflight and exec-containing apply flows).
- `crawclaw secrets apply --from <plan.json>` — apply a previously generated plan (`--dry-run` supported; use `--allow-exec` to permit exec providers in dry-run and exec-containing write plans).

## Plugins

Manage extensions and their config:

- `crawclaw plugins list` — discover plugins (use `--json` for machine output).
- `crawclaw plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `crawclaw plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`).
- `crawclaw plugins marketplace list <marketplace>` — list marketplace entries before install.
...
Experience memory access and memory prompt diagnostics:
...
Task-backed agent runtime inspection/export surfaces:

- `crawclaw agent inspect --run-id <id>|--task-id <id>` — inspect runtime/task/guard/completion/archive state, query-context diagnostics, and archived lifecycle timeline.
- `crawclaw agent export-context --run-id <id>|--task-id <id>` — export Context Archive records for replay/debug.
- `crawclaw agents status` — show ops-oriented summaries across agent runs.
...
- `--workspace <dir>`: agent workspace path (default `~/.crawclaw/workspace`).
...
Non-interactive config helpers (get/set/unset/file/schema/validate). Running `crawclaw config` with no
...
- `config set --dry-run`: validate assignments without writing `crawclaw.json` (exec SecretRef checks are skipped by default).
...
- `config schema`: print the generated JSON schema for `crawclaw.json`.
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use `crawclaw health` or `crawclaw status --deep` for gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `crawclaw doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, CrawClaw moves account-scoped values into `channels.<channel>.accounts.default` before writing the new account.
...
Tip: use `crawclaw skills search`, `crawclaw skills install`, and `crawclaw skills update` for ClawHub-backed skills.
...
Inspect a task-backed agent run.
...
Show an ops-oriented agent runtime/task/guard summary.
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, CrawClaw may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
Update an agent identity from explicit flags or an `IDENTITY.md` file.
...
Offline harness reporting and promotion checks.

Subcommands:

- `agents harness report`
- `agents harness promote-check`
...
CrawClaw can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `crawclaw status --usage` (prints full provider breakdown)
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named CrawClaw services are treated as first-class and aren't flagged as "extra".
...
crawclaw logs --limit 200
crawclaw logs --plain
crawclaw logs --json
crawclaw logs --no-color
```
...
`crawclaw models` is an alias for `models status`.
