...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a CrawClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`crawclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to CrawClaw channel
conversations instead of hosting an ACP harness session, use
[`crawclaw mcp serve`](/cli/mcp) instead.
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the CrawClaw gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
crawclaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
crawclaw acp --url wss://gateway-host:18789 --token-file ~/.crawclaw/gateway.token

# Attach to an existing session key
crawclaw acp --session agent:main:main

# Attach by label (must already exist)
crawclaw acp --session-label "support inbox"

# Reset the session key before the first prompt
crawclaw acp --session agent:main:main --reset-session
```
...
crawclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.crawclaw/gateway.token

# Override the server command (default: crawclaw)
crawclaw acp client --server "node" --server-args crawclaw.mjs acp --url ws://127.0.0.1:19001
```
...
- ACP only auto-approves narrow readonly classes: scoped `read` calls under the active cwd plus readonly search tools (`search`, `web_search`). Unknown/non-core tools, out-of-scope reads, exec-capable tools, control-plane tools, mutating tools, and interactive flows always require explicit prompt approval.
- Server-provided `toolCall.kind` is treated as untrusted metadata (not an authorization source).
- This ACP bridge policy is separate from ACPX harness permissions. If you run CrawClaw through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive a CrawClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `crawclaw acp` over stdio.

Example config (persisted):

```bash
crawclaw config set gateway.remote.url wss://gateway-host:18789
crawclaw config set gateway.remote.token <token>
```
...
```bash
crawclaw acp --session agent:main:main
crawclaw acp --session agent:design:main
crawclaw acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see CrawClaw plugin tools, enable the
...
CrawClaw bot over ACP, use `acpx` with its built-in `crawclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx crawclaw` at `crawclaw acp`.
3. Target the CrawClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default CrawClaw ACP session
acpx crawclaw exec "Summarize the active CrawClaw session state."

# Persistent named session for follow-up turns
acpx crawclaw sessions ensure --name codex-bridge
acpx crawclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my CrawClaw work agent for recent context relevant to this repo."
```

If you want `acpx crawclaw` to target a specific Gateway and session key every
time, override the `crawclaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "crawclaw": {
      "command": "env CRAWCLAW_HIDE_BANNER=1 CRAWCLAW_SUPPRESS_NOTES=1 crawclaw acp --url ws://127.0.0.1:18789 --token-file ~/.crawclaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local CrawClaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env CRAWCLAW_HIDE_BANNER=1 CRAWCLAW_SUPPRESS_NOTES=1 node crawclaw.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from a CrawClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select “CrawClaw ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`CRAWCLAW_GATEWAY_TOKEN`, `CRAWCLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`CRAWCLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `CRAWCLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `crawclaw acp client` sets `CRAWCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `crawclaw`).
