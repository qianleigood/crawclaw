...
export function triggerCrawClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.CRAWCLAW_SYSTEMD_UNIT,
      process.env.CRAWCLAW_PROFILE,
    );
  ...
  const label =
    process.env.CRAWCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.CRAWCLAW_PROFILE);
