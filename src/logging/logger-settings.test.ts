...
let originalCrawClawLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.CRAWCLAW_TEST_FILE_LOG;
  originalCrawClawLogLevel = process.env.CRAWCLAW_LOG_LEVEL;
  delete process.env.CRAWCLAW_TEST_FILE_LOG;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.CRAWCLAW_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalCrawClawLogLevel === undefined) {
    delete process.env.CRAWCLAW_LOG_LEVEL;
  } else {
    process.env.CRAWCLAW_LOG_LEVEL = originalCrawClawLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
