---
summary: 面向维护者的仓库和运行时文档入口
read_when:
  - 你在维护 CrawClaw 内部实现
  - 你需要仓库结构和维护者 ownership 指引
title: 维护者文档
x-i18n:
  generated_at: "2026-06-10T10:32:09Z"
  model: codex
  provider: openai
  source_hash: 4b06849e0eccdc1f6c7cc1627b36933115a766968fe2d855df2ea7ac12f43643
  source_path: maintainers/index.md
  workflow: 15
---

# 维护者文档

`maintainers/` 面向贡献者和维护者，关注仓库结构、ownership 和工程约定。

## 核心维护文档

- [Plugin 依赖维护](/maintainers/plugin-dependency-maintenance)
- [Provider 和 plugin 元数据漂移](/maintainers/provider-plugin-metadata-drift)
- [仓库结构](/maintainers/repo-structure)
- [Runtime surface](/maintainers/runtime-surface)
- [Skills catalog](/maintainers/skills-catalog)

## 推荐搭配阅读

维护系统时，下面这些章节通常需要一起看：

- [概念文档](/concepts)
- [调试文档](/debug)
- [参考文档](/reference)
