...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => CrawClawConfig>(),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<CrawClawConfig["gateway"]>["auth"]> | undefined;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: CrawClawConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
