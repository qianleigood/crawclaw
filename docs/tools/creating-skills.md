...
| `metadata.crawclaw.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.crawclaw.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.crawclaw.requires.config` | No       | Required config keys                        |
...
| `~/.crawclaw/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with CrawClaw) | Lowest     | Global                |
