---
summary: "Deploy CrawClaw on Render with Infrastructure-as-Code"
read_when:
  - Deploying CrawClaw to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy CrawClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/qianleigood/crawclaw)
...
repository configures everything needed to run CrawClaw:
...
services:
  - type: web
    ...
    envVars:
      - key: CRAWCLAW_GATEWAY_PORT
        value: "8080"
      - key: CRAWCLAW_STATE_DIR
        value: /data/.crawclaw
      - key: CRAWCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: CRAWCLAW_GATEWAY_TOKEN
...
Use a supported gateway client against `https://<your-service>.onrender.com/`.

Connect using the `CRAWCLAW_GATEWAY_TOKEN` value that was auto-generated during deploy (find it in **Dashboard → your service → Environment**).
...
If you use the original CrawClaw repository, Render will not auto-deploy your CrawClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For CrawClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive you can restore on any CrawClaw host. See [Backup](/cli/backup) for details.
...
- Missing `CRAWCLAW_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `CRAWCLAW_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export your config via `crawclaw backup create` in the Render shell.
