 ...
 * `crawclaw update`. Use it to verify the completion installation flow
...
\`crawclaw update\`, \`crawclaw doctor\`, and \`crawclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# CrawClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`crawclaw completion --install\` later to enable.`));
    return;
  }
