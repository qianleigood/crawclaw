...
- **Global `--dev` (profile):** isolates state under `~/.crawclaw-dev` and
...
1. **Profile isolation** (global `--dev`)
   ...
   - `CRAWCLAW_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`CRAWCLAW_SKIP_CHANNELS=1`).
...
CrawClaw can log the **raw assistant stream** before any filtering/formatting.
