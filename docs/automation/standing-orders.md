...
The agent loads these instructions every session via the workspace bootstrap layer (by default this is just `AGENTS.md`) and executes against them, combined with [cron jobs](/automation/cron-jobs) for time-based enforcement.

<Tip>
Put standing orders in `AGENTS.md` to guarantee they're loaded every session. Other workspace files can still exist, but they are no longer part of the default per-turn bootstrap unless a hook or workflow explicitly injects them.
</Tip>
