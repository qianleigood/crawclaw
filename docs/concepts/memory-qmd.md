...
alongside CrawClaw. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, CrawClaw falls back to the
...
- macOS and Linux work out of the box. Windows support uses the native Windows path.
...
CrawClaw creates a self-contained QMD home under
`~/.crawclaw/agents/<agentId>/qmd/` and manages the sidecar lifecycle
automatically -- collections, updates, and embedding runs are handled for you.

## How the sidecar works

- CrawClaw creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). If a mode fails, CrawClaw retries with `qmd query`.
- If QMD fails entirely, CrawClaw falls back to the builtin SQLite engine.
...
collection under `~/.crawclaw/agents/<id>/qmd/sessions/`.
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If CrawClaw
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs CrawClaw uses.
...
CrawClaw's builtin symlink rules. Keep temporary monorepo checkouts under
