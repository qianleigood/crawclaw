...
import { parseCrawClawVersion } from "./version.js";

async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-config-"));
  try {
    await run(home);
  } finally {
...
async function writeConfig(
  home: string,
  dirname: ".crawclaw" = ".crawclaw",
  port: number,
  filename: string = "crawclaw.json",
) {
...
async function expectNoNewerVersionWarning(touchedVersion: string) {
  await withTempHome(async (home) => {
    ...
    expect(logger.warn).not.toHaveBeenCalledWith(
      expect.stringContaining("Config was last written by a newer CrawClaw"),
    );
...
describe("config io paths", () => {
  it("uses ~/.crawclaw/crawclaw.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".crawclaw", 19001);
  ...
  it("defaults to ~/.crawclaw/crawclaw.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".crawclaw", "crawclaw.json"));
    });
  });

  it("uses CRAWCLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { CRAWCLAW_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".crawclaw", "crawclaw.json"));
    });
  });

  it("honors explicit CRAWCLAW_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".crawclaw", 20002, "custom.json");
      const io = createIoForHome(home, { CRAWCLAW_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("does not warn when config was last touched by a same-base correction publish", async () => {
    const parsedVersion = parseCrawClawVersion(VERSION);
    if (!parsedVersion) {
