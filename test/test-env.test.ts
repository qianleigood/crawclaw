...
function createTempHome(): string {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-test-env-real-home-"));
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-crawclaw.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.crawclaw/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.crawclaw/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".crawclaw", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".crawclaw", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.CRAWCLAW_LIVE_TEST_QUIET = "1";
    process.env.CRAWCLAW_CONFIG_PATH = "~/custom-crawclaw.json5";
    process.env.CRAWCLAW_TEST_HOME = priorIsolatedHome;
    process.env.CRAWCLAW_STATE_DIR = path.join(priorIsolatedHome, ".crawclaw");
    ...
    expect(process.env.CRAWCLAW_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".crawclaw", "crawclaw.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".crawclaw", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".crawclaw", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.CRAWCLAW_LIVE_TEST = "1";
    process.env.CRAWCLAW_LIVE_USE_REAL_HOME = "1";
    process.env.CRAWCLAW_LIVE_TEST_QUIET = "1";
