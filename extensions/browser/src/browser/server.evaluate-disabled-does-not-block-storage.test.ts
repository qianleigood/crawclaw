  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          crawclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  ...
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.CRAWCLAW_GATEWAY_PORT;
    process.env.CRAWCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.CRAWCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.CRAWCLAW_GATEWAY_PASSWORD;
