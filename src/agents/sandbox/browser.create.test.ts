...
function buildConfig(enableNoVnc: boolean): SandboxConfig {
  return {
    ...
    browser: {
      ...
      containerPrefix: "crawclaw-sbx-browser-",
...
describe("ensureSandboxBrowser create args", () => {
  ...
  it("publishes noVNC on loopback and injects noVNC password env", async () => {
    ...
    expect(envEntries).toContain("CRAWCLAW_BROWSER_NO_SANDBOX=1");
    const passwordEntry = envEntries.find((entry) =>
      entry.startsWith("CRAWCLAW_BROWSER_NOVNC_PASSWORD="),
    );
    expect(passwordEntry).toMatch(/^CRAWCLAW_BROWSER_NOVNC_PASSWORD=[A-Za-z0-9]{8}$/);
  ...
  it("does not inject noVNC password env when noVNC is disabled", async () => {
    ...
    const createArgs = findDockerArgsCall(dockerMocks.execDocker.mock.calls, "create");
    const envEntries = collectDockerFlagValues(createArgs ?? [], "-e");
    expect(envEntries.some((entry) => entry.startsWith("CRAWCLAW_BROWSER_NOVNC_PASSWORD="))).toBe(
      false,
    );
    expect(result?.noVncUrl).toBeUndefined();
  });

  it("publishes PinchTab when browser.provider=pinchtab", async () => {
    ...
    try {
      const result = await ensureSandboxBrowser({
        ...
        agentWorkspaceDir: "/tmp/workspace",
        cfg: buildConfig(false),
        pinchTab: { enabled: true, token: "secret-token" },
      });

      const createArgs = findDockerArgsCall(dockerMocks.execDocker.mock.calls, "create");
      expect(createArgs).toBeDefined();
      expect(createArgs).toContain("127.0.0.1::9867");
      const envEntries = collectDockerFlagValues(createArgs ?? [], "-e");
      expect(envEntries).toContain("CRAWCLAW_BROWSER_ENABLE_PINCHTAB=1");
      expect(envEntries).toContain("CRAWCLAW_BROWSER_PINCHTAB_PORT=9867");
      expect(envEntries).toContain("CRAWCLAW_BROWSER_PINCHTAB_TOKEN=secret-token");
      expect(result?.pinchTabUrl).toBe("http://127.0.0.1:49102");
    } finally {
      vi.unstubAllGlobals();
    }
  });
  ...
  it("stamps the mount format version label on browser containers", async () => {
    ...
    expect(labels).toContain(`crawclaw.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  });
});
