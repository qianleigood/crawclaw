...
describe("command-registry", () => {
  ...
  it("does not narrow to the primary command when help is requested", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "crawclaw", "doctor", "--help"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "crawclaw", "vitest"]);
    program.exitOverride();
    await withProcessArgv(["node", "crawclaw", "status"], async () => {
      await program.parseAsync(["node", "crawclaw", "status"]);
    });
