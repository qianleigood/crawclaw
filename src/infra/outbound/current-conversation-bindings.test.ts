...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.CRAWCLAW_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-current-bindings-"));
    process.env.CRAWCLAW_STATE_DIR = testStateDir;
