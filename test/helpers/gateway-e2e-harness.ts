...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `crawclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".crawclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          CRAWCLAW_SKIP_PROVIDERS: "1",
          CRAWCLAW_SKIP_GMAIL_WATCHER: "1",
          CRAWCLAW_SKIP_CRON: "1",
          CRAWCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
