...
import type { ChannelSetupWizard } from "crawclaw/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "crawclaw/plugin-sdk/setup-tools";
...
function mergeWhatsAppConfig(
  cfg: CrawClawConfig,
  patch: Partial<NonNullable<NonNullable<CrawClawConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: CrawClawConfig, dmPolicy: DmPolicy): CrawClawConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: CrawClawConfig, allowFrom?: string[]): CrawClawConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: CrawClawConfig, selfChatMode: boolean): CrawClawConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: CrawClawConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so CrawClaw can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for CrawClaw" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("crawclaw channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
