...
describe("doctor legacy toolsBySender helpers", () => {
  it("finds untyped legacy sender keys", () => {
    ...
    expect(hits).toEqual([
      {
        toolsBySenderPath: ["channels", "weixin", "groups", "123@g.us", "toolsBySender"],
        pathLabel: "channels.weixin.groups.123@g.us.toolsBySender",
        key: "owner",
        targetKey: "id:owner",
      },
    ]);
  ...
  it("migrates legacy sender keys to typed id entries", () => {
    ...
    expect(result.config.channels?.weixin?.groups?.["123@g.us"]?.toolsBySender).toEqual({
      "id:owner": { allow: ["fs.read"] },
      "id:alice": { deny: ["exec"] },
    });
  });
  ...
  it("formats legacy sender key warnings", () => {
    ...
    expect(warnings).toEqual([
      ...
      expect.stringContaining('Run "crawclaw doctor --fix"'),
    ]);
  });
});
