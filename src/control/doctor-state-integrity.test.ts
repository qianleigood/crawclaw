...
type EnvSnapshot = {
  HOME?: string;
  CRAWCLAW_HOME?: string;
  CRAWCLAW_STATE_DIR?: string;
  CRAWCLAW_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    CRAWCLAW_HOME: process.env.CRAWCLAW_HOME,
    CRAWCLAW_STATE_DIR: process.env.CRAWCLAW_STATE_DIR,
    CRAWCLAW_OAUTH_DIR: process.env.CRAWCLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: CrawClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: CrawClawConfig) {
...
async function runStateIntegrityText(cfg: CrawClawConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-doctor-state-integrity-"));
    ...
    fs.mkdirSync(process.env.CRAWCLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("does not prompt for oauth dir when no explicit OAuth config is active", async () => {
  ...
  it("prompts for oauth dir when CRAWCLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.CRAWCLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: CrawClawConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
  ...
  it("prints crawclaw verification hints when recent sessions are missing transcripts", async () => {
    const cfg: CrawClawConfig = {};
    ...
    expect(text).toMatch(/crawclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/crawclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /crawclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
