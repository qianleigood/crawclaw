...
describe("createCacheTrace", () => {
  ...
  it("honors diagnostics cache trace config and expands file paths", () => {
    ...
    const trace = createCacheTrace({
      cfg: {
        diagnostics: {
          cacheTrace: {
            enabled: true,
            filePath: "~/.crawclaw/logs/cache-trace.jsonl",
          },
        },
      },
      env: {},
    ...
    expect(trace?.filePath).toBe(resolveUserPath("~/.crawclaw/logs/cache-trace.jsonl"));
  ...
  it("records decision codes when provided", () => {
    const lines: string[] = [];
    const trace = createCacheTrace({
      cfg: {
        diagnostics: {
    ...
    trace?.recordStage("session:after", {
      decisionCodes: {
        promptCache: "prompt_cache_parent_prefix_reused",
        runOutcome: "run_outcome_success",
      },
    });

    const event = JSON.parse(lines[0]?.trim() ?? "{}") as Record<string, unknown>;
    expect(event.decisionCodes).toEqual({
      promptCache: "prompt_cache_parent_prefix_reused",
      runOutcome: "run_outcome_success",
    });
  });
