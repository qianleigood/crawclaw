...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.CRAWCLAW_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "CRAWCLAW_TEST_FAST", value: process.env.CRAWCLAW_TEST_FAST },
    ...
    { key: "CRAWCLAW_STATE_DIR", value: process.env.CRAWCLAW_STATE_DIR },
    { key: "CRAWCLAW_CONFIG_PATH", value: process.env.CRAWCLAW_CONFIG_PATH },
    { key: "CRAWCLAW_GATEWAY_PORT", value: process.env.CRAWCLAW_GATEWAY_PORT },
    { key: "CRAWCLAW_BRIDGE_ENABLED", value: process.env.CRAWCLAW_BRIDGE_ENABLED },
    { key: "CRAWCLAW_BRIDGE_HOST", value: process.env.CRAWCLAW_BRIDGE_HOST },
    { key: "CRAWCLAW_BRIDGE_PORT", value: process.env.CRAWCLAW_BRIDGE_PORT },
    { key: "CRAWCLAW_CANVAS_HOST_PORT", value: process.env.CRAWCLAW_CANVAS_HOST_PORT },
    { key: "CRAWCLAW_TEST_HOME", value: process.env.CRAWCLAW_TEST_HOME },
    { key: "CRAWCLAW_AGENT_DIR", value: process.env.CRAWCLAW_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.CRAWCLAW_TEST_HOME = tempHome;
  ...
  delete process.env.CRAWCLAW_BRIDGE_ENABLED;
  delete process.env.CRAWCLAW_BRIDGE_HOST;
  delete process.env.CRAWCLAW_BRIDGE_PORT;
  delete process.env.CRAWCLAW_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.CRAWCLAW_STATE_DIR = path.join(tempHome, ".crawclaw");
  }
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.CRAWCLAW_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".crawclaw");
  const priorIsolatedHome = params.env.CRAWCLAW_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".crawclaw")
  ) {
    realStateDir = path.join(params.realHome, ".crawclaw");
  }
  const tempStateDir = path.join(params.tempHome, ".crawclaw");
  fs.mkdirSync(tempStateDir, { recursive: true });

  const realConfigPath = params.env.CRAWCLAW_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.CRAWCLAW_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "crawclaw.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "crawclaw.json"),
...
export function installTestEnv(): { cleanup: () => void; tempHome: string } {
  const live =
    process.env.LIVE === "1" ||
    process.env.CRAWCLAW_LIVE_TEST === "1" ||
    process.env.CRAWCLAW_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.CRAWCLAW_LIVE_USE_REAL_HOME);
