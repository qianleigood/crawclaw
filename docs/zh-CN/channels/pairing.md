...
"配对"是 CrawClaw 的显式**所有者批准**步骤。它用于两个地方：
...
历史说明：旧版移动端节点也使用同一套设备配对握手，但对应源码已从本仓库移除。
...
- 旧版 `node.pair.*` API（CLI：`crawclaw nodes pending/approve`）是一个单独的 Gateway 网关拥有的配对存储。WS 节点仍然需要设备配对。
