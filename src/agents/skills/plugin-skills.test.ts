    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "crawclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "crawclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "crawclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("crawclaw-");
  const acpxRoot = await tempDirs.make("crawclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("crawclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("crawclaw-");
  const pluginRoot = await tempDirs.make("crawclaw-plugin-");
  const outsideDir = await tempDirs.make("crawclaw-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("crawclaw-");
    const pluginRoot = await tempDirs.make("crawclaw-claude-bundle-");
