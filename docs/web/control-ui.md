...
If the page fails to load, start the Gateway first: `crawclaw gateway`.
...
created. Re-run `crawclaw devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `crawclaw devices revoke --device <id> --role <role>`. See
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop crawclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. CrawClaw
...
CrawClaw **blocks** Control UI connections without device identity.
