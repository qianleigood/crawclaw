...
read_when:
  - 你遇到连接/认证问题，需要引导式修复
  - 你更新后想进行完整性检查
summary: "`crawclaw doctor` 的 CLI 参考（健康检查 + 引导式修复）"
title: doctor
x-i18n:
...
- 交互式提示（如钥匙串/OAuth 修复）仅在 stdin 是 TTY 且**未**设置 `--non-interactive` 时运行。无头运行（cron、Feishu、无终端）将跳过提示。
- `--fix`（`--repair` 的别名）会将备份写入 `~/.crawclaw/crawclaw.json.bak`，并删除未知的配置键，同时列出每个删除项。

## macOS：`launchctl` 环境变量覆盖

如果你之前运行过 `launchctl setenv CRAWCLAW_GATEWAY_TOKEN ...`（或遗留的 `CRAWCLAW_GATEWAY_TOKEN`，以及对应密码变量），该值会覆盖你的配置文件，并可能导致持续的"未授权"错误。
