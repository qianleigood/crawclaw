...
CrawClaw loads skills from these sources:

1. **Extra skill folders**: configured with `skills.load.extraDirs`
2. **Bundled skills**: shipped with the install (npm package or CrawClaw.app)
3. **Managed/local skills**: `~/.crawclaw/skills`
...
`<workspace>/skills` (highest) → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.crawclaw/skills` → bundled skills → `skills.load.extraDirs` (lowest)
...
- **Shared skills** live in `~/.crawclaw/skills` (managed/local) and are visible
...
`crawclaw.plugin.json` (paths relative to the plugin root). Plugin skills load
...
You can gate them via `metadata.crawclaw.requires.config` on the plugin’s config
...
- Optional frontmatter keys:
  - `homepage` — URL surfaced as “Website” in the macOS Skills UI (also supported via `metadata.crawclaw.homepage`).
...
CrawClaw **filters skills at load time** using `metadata` (single-line JSON):
...
Fields under `metadata.crawclaw`:
...
- `requires.config` — list of `crawclaw.json` paths that must be truthy.
- `primaryEnv` — env var name associated with `skills.entries.<name>.apiKey`.
- `install` — optional array of installer specs used by the macOS Skills UI (brew/node/go/uv/download).

- `requires.bins` is checked on the **host** at skill load time.
  Install it through the runtime's normal provisioning flow before enabling the skill there.
...
- If all installers are `download`, CrawClaw lists each entry so you can see the available artifacts.
- Installer specs can include `os: ["darwin"|"linux"|"win32"]` to filter options by platform.
- Node installs honor `skills.install.nodeManager` in `crawclaw.json` (default: npm; options: npm/pnpm/yarn/bun).
  This only affects **skill installs**; the Gateway runtime should still be Node
  (Bun is not recommended for Weixin/Feishu).
- Go installs: if `go` is missing and `brew` is available, the gateway installs Go via Homebrew first and sets `GOBIN` to Homebrew’s `bin` when possible.
- Download installs: `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`), `extract` (default: auto when archive detected), `stripComponents`, `targetDir` (default: `~/.crawclaw/tools/<skillKey>`).

If no `metadata.crawclaw` is present, the skill is always eligible (unless
disabled in config or blocked by `skills.allowBundled` for bundled skills).

## Config overrides (`~/.crawclaw/crawclaw.json`)
...
Skill examples here are for custom or third-party workflows.

For native image analysis, use the `image` tool with `agents.defaults.imageModel`.

Config keys match the **skill name** by default. If a skill defines
`metadata.crawclaw.skillKey`, use that key under `skills.entries`.

Rules:

- `enabled: false` disables the skill even if it’s bundled/installed.
- `env`: injected **only if** the variable isn’t already set in the process.
- `apiKey`: convenience for skills that declare `metadata.crawclaw.primaryEnv`.
...
When an agent run starts, CrawClaw:
...
By default, CrawClaw watches skill folders and bumps the skills snapshot when `SKILL.md` files change. Configure this under `skills.load`:
...
CrawClaw ships a baseline set of skills as **bundled skills** as part of the
install (npm package or CrawClaw.app). `~/.crawclaw/skills` exists for local
