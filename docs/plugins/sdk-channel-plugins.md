...
summary: "Step-by-step guide to building a messaging channel plugin for CrawClaw"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect CrawClaw to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects CrawClaw to a
...
<Info>
  If you have not built any CrawClaw plugin before, read
...
Channel plugins do not need their own send/edit/react tools. CrawClaw keeps one
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `crawclaw/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization and transport hooks. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, `createApproverRestrictedNativeApprovalCapability`, and `createChannelNativeApprovalRuntime` from `crawclaw/plugin-sdk/approval-runtime` so core owns request filtering, routing, dedupe, expiry, and gateway subscription.
...
<Steps>
    ...
    ```json package.json
    {
      ...
      "crawclaw": {
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect CrawClaw to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Add a setup entry">
    ...
    CrawClaw loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    CrawClaw. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to CrawClaw.
...
├── package.json              # crawclaw.channel metadata
├── crawclaw.plugin.json      # Manifest with config schema
