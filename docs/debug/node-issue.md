...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in CrawClaw
...
Running CrawClaw via Node with `tsx` fails at startup with:

```
[crawclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), CrawClaw Desktop or the local Gateway API and `desktop Gateway dev mode` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch crawclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node crawclaw.mjs status` works on Node 25.
