...
type PluginConfig = NonNullable<CrawClawConfig["plugins"]>;
...
function createSingleNpmInstallConfig(installPath: string): CrawClawConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "crawclaw-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-crawclaw-plugin-install/my-plugin",
      },
      extensionsDir,
    });
