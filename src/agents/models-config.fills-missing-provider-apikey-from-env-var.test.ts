...
function createOpenAiConfigWithResolvedApiKey(mergeMode = false): CrawClawConfig {
  return {
    models: {
...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureCrawClawModelsJson(validated.config);
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureCrawClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
  ...
  it("preserves explicit moonshot model fields while keeping explicit token limits", async () => {
    await withTempHome(async () => {
      await withEnvVar("MOONSHOT_API_KEY", "sk-moonshot-test", async () => {
  ...
  it("retains explicit moonshot token limits even when they are invalid", async () => {
    await expectMoonshotTokenLimits({
      contextWindow: 0,
      maxTokens: -1,
