...
import { buildWebSearchProviderConfig, withTempHome, writeCrawClawConfig } from "./test-helpers.js";
import { CrawClawSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = CrawClawSchema.safeParse({
      $schema: "https://docs.crawclaw.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://docs.crawclaw.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = CrawClawSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = CrawClawSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeCrawClawConfig(home, {
        gateway: { bind: "${CRAWCLAW_BIND}" },
      });

      const prev = process.env.CRAWCLAW_BIND;
      process.env.CRAWCLAW_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        ...
        if (prev === undefined) {
          delete process.env.CRAWCLAW_BIND;
        } else {
          process.env.CRAWCLAW_BIND = prev;
        }
      }
    });
  });
