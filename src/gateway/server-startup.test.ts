...
const ensureCrawClawModelsJsonMock = vi.fn<
...
vi.mock("../agents/agent-paths.js", () => ({
  resolveCrawClawAgentDir: () => "/tmp/agent",
}));

vi.mock("../agents/models-config.js", () => ({
  ensureCrawClawModelsJson: (config: unknown, agentDir: unknown) =>
...
describe("gateway startup primary model warmup", () => {
  ...
  beforeEach(() => {
    ensureCrawClawModelsJsonMock.mockClear();
  ...
  it("prewarms an explicit configured primary model", async () => {
    ...
    expect(ensureCrawClawModelsJsonMock).toHaveBeenCalledWith(cfg, "/tmp/agent");
  ...
  it("skips warmup when no explicit primary model is configured", async () => {
    ...
    expect(ensureCrawClawModelsJsonMock).not.toHaveBeenCalled();
