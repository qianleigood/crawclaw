...
describe("statusJsonCommand", () => {
  ...
  it("includes Feishu CLI user status in JSON output as a separate field", async () => {
    const { runtime, logs } = createRuntimeCapture();
    ...
    expect(payload).toHaveProperty("feishuCli.supported", true);
    expect(payload).toHaveProperty("feishuCli.status.status", "ready");
    expect(payload).toHaveProperty("feishuCli.status.version", "1.0.7");
  });
});
