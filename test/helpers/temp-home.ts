...
type EnvSnapshot = {
  ...
  crawclawStateDir: string | undefined;
};
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    crawclawStateDir: process.env.CRAWCLAW_STATE_DIR,
  };
}
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("CRAWCLAW_STATE_DIR", snapshot.crawclawStateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked explicit runtime homes.
  delete process.env.CRAWCLAW_HOME;
  process.env.CRAWCLAW_STATE_DIR = path.join(base, ".crawclaw");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "crawclaw-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".crawclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
