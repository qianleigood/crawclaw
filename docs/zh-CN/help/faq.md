---
summary: 关于 CrawClaw 安装、配置和使用的常见问题
title: 常见问题
x-i18n:
...
- [快速开始与首次运行设置](#quick-start-and-firstrun-setup)
  ...
  - [无法访问 docs.crawclaw.ai（SSL 错误），怎么办？](#i-cant-access-docscrawclawai-ssl-error-what-now)
...
- [如何安装 beta 版本，beta 和 dev 有什么区别？](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  ...
  - [Windows 安装提示找不到 git 或无法识别 crawclaw](#windows-install-says-git-not-found-or-crawclaw-not-recognized)
  - [文档没有解答我的问题——如何获得更好的答案？](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [如何在 Linux 上安装 CrawClaw？](#how-do-i-install-crawclaw-on-linux)
  - [如何在 VPS 上安装 CrawClaw？](#how-do-i-install-crawclaw-on-a-vps)
  - [云/VPS 安装指南在哪里？](#where-are-the-cloudvps-install-guides)
  - [可以让 CrawClaw 自行更新吗？](#can-i-ask-crawclaw-to-update-itself)
  - [新手引导具体做了什么？](#新手引导具体做了什么)
  - [运行 CrawClaw 需要 Claude 或 OpenAI 订阅吗？](#do-i-need-a-claude-or-openai-subscription-to-run-this)
  ...
  - [在专用机器上运行 CrawClaw 有多重要？](#how-important-is-it-to-run-crawclaw-on-a-dedicated-machine)
  - [VPS 的最低要求和推荐操作系统是什么？](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [可以在虚拟机中运行 CrawClaw 吗？有什么要求？](#can-i-run-crawclaw-in-a-vm-and-what-are-the-requirements)
  - [什么是 CrawClaw？](#what-is-crawclaw)
  - [用一段话描述 CrawClaw？](#what-is-crawclaw-in-one-paragraph)
  - [价值主张是什么？](#whats-the-value-proposition)
  - [刚设置好，应该先做什么？](#i-just-set-it-up-what-should-i-do-first)
  - [CrawClaw 日常最常用的五个场景是什么？](#what-are-the-top-five-everyday-use-cases-for-crawclaw)
  - [CrawClaw 能否帮助 SaaS 进行获客、外联、广告和博客？](#can-crawclaw-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills 与自动化](#skills-and-automation)
  ...
  - [CrawClaw 能否按计划或在后台持续运行任务？](#can-crawclaw-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [磁盘上的文件位置](#where-things-live-on-disk)
  - [CrawClaw 使用的所有数据都保存在本地吗？](#is-all-data-used-with-crawclaw-saved-locally)
  - [CrawClaw 将数据存储在哪里？](#where-does-crawclaw-store-its-data)
  ...
  - [如何完全卸载 CrawClaw？](#how-do-i-completely-uninstall-crawclaw)
...
- [环境变量和 .env 加载](#env-vars-and-env-loading)
  - [CrawClaw 如何加载环境变量？](#how-does-crawclaw-load-environment-variables)
...
- [会话与多聊天](#sessions-multiple-chats)
  ...
  - [能否创建一个 CrawClaw 实例团队——一个 CEO 和多个智能体？](#is-there-a-way-to-make-a-team-of-crawclaw-instances-one-ceo-and-many-agents)
  - [为什么上下文在任务中途被截断了？如何防止？](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [如何完全重置 CrawClaw 但保留安装？](#how-do-i-completely-reset-crawclaw-but-keep-it-installed)
...
- [模型：默认值、选择、别名、切换](#models-defaults-selection-aliases-switching)
  ...
  - [CrawClaw、Flawd 和 Krill 使用什么模型？](#what-do-crawclaw-flawd-and-krill-use-for-models)
...
- [Gateway 网关：端口、“已在运行”和远程模式](#gateway-ports-already-running-and-remote-mode)
  ...
  - [为什么 `crawclaw gateway status` 显示 `Runtime: running` 但 `RPC probe: failed`？](#why-does-crawclaw-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [为什么 `crawclaw gateway status` 显示 `Config (cli)` 和 `Config (service)` 不同？](#why-does-crawclaw-gateway-status-show-config-cli-and-config-service-different)
  - ["another gateway instance is already listening"是什么意思？](#what-does-another-gateway-instance-is-already-listening-mean)
  - [如何以远程模式运行 CrawClaw（客户端连接到其他位置的 Gateway 网关）？](#how-do-i-run-crawclaw-in-remote-mode-client-connects-to-a-gateway-elsewhere)
  - [Gateway 客户端显示"unauthorized"（或持续重连），怎么办？](#the-browser-client-says-unauthorized-or-keeps-reconnecting-what-now)
...
- [安全与访问控制](#security-and-access-control)
  - [将 CrawClaw 暴露给入站私信安全吗？](#is-it-safe-to-expose-crawclaw-to-inbound-dms)
...
7. **Gateway 网关快照**
   ...
   crawclaw health --verbose   # 出错时显示目标 URL + 配置路径
   ```
...
这会从 **git checkout** 安装 CrawClaw，这样智能体可以读取代码 + 文档，并推理你正在运行的确切版本。你可以随时通过不带 `--install-method git` 重新运行安装程序切回稳定版。

提示：要求智能体**计划并监督**修复（逐步进行），然后只执行必要的命令。这样改动较小，更容易审查。

如果你发现了真正的 bug 或修复方案，请提交 GitHub issue 或发送 PR：
https://github.com/qianleigood/crawclaw/issues
https://github.com/qianleigood/crawclaw/pulls
...
- `crawclaw status`：Gateway 网关/智能体健康状况 + 基本配置的快速快照。
- `crawclaw models status`：检查提供商认证 + 模型可用性。
- `crawclaw doctor`：验证并修复常见的配置/状态问题。

其他有用的 CLI 检查：`crawclaw status --all`、`crawclaw logs --follow`、
...
新手引导完成后，可以直接运行 `crawclaw tui` 连接 Gateway 网关。如果没有自动进入本地界面，请在同一台机器上运行 `crawclaw tui`。
...
参阅 [远程访问](/gateway/remote) 了解绑定模式和认证详情。
...
1. 在新机器上安装 CrawClaw。
2. 从旧机器复制 `$CRAWCLAW_STATE_DIR`（默认：`~/.crawclaw`）。
...
相关：[迁移](/install/migrating)、[磁盘上的文件位置](/help/faq#where-does-crawclaw-store-its-data)、
...
查看 GitHub 变更日志：
https://github.com/qianleigood/crawclaw/blob/main/CHANGELOG.md

最新条目在顶部。如果顶部部分标记为 **Unreleased**，则下一个带日期的部分是最新发布版本。条目按**亮点**、**变更**和**修复**分组（需要时还有文档/其他部分）。

### 无法访问 docs.crawclaw.ai（SSL 错误），怎么办

一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 错误地拦截了 `docs.crawclaw.ai`。禁用该功能或将 `docs.crawclaw.ai` 加入白名单，然后重试。
请帮助我们在此处报告以解除封锁：https://spa.xfinity.com/check_url_status。

如果仍然无法访问该网站，文档在 GitHub 上有镜像：
https://github.com/qianleigood/crawclaw/tree/main/docs
...
**2) crawclaw is not recognized（安装后）**
...
Windows 使用原生 PowerShell 安装器。文档：[Windows](/platforms/windows)。
...
在云端的工作方式：**Gateway 网关运行在服务器上**，你通过 TUI、Gateway 客户端或 Tailscale/SSH 从笔记本/手机访问。你的状态 + 工作区位于服务器上，因此将主机视为数据来源并做好备份。

你可以将**节点**（Mac/无头）配对到云端 Gateway 网关，以访问本地屏幕/摄像头/画布或在笔记本上执行命令，同时 Gateway 网关保持在云端。
...
`crawclaw onboard` 是推荐的设置路径。在**本地模式**下，它引导你完成：
...
### 运行 CrawClaw 需要 Claude 或 OpenAI 订阅吗

不需要。你可以使用 **API 密钥**（Anthropic/OpenAI/其他）或**纯本地模型**运行 CrawClaw，这样你的数据留在你的设备上。订阅（Claude Pro/Max 或 OpenAI Codex）是这些提供商的可选认证方式。
...
`claude setup-token` 通过 Claude Code CLI 生成一个**令牌字符串**（在 Web 控制台中不可用）。你可以在**任何机器**上运行它。在新手引导中选择 **Anthropic token (paste setup-token)** 或使用 `crawclaw models auth paste-token --provider anthropic` 粘贴。令牌作为 **anthropic** 提供商的认证配置文件存储，像 API 密钥一样使用（无自动刷新）。更多详情：[OAuth](/concepts/oauth)。
...
复制它打印的令牌，然后在新手引导中选择 **Anthropic token (paste setup-token)**。如果你想在 Gateway 网关主机上运行，使用 `crawclaw models auth setup-token --provider anthropic`。如果你在其他地方运行了 `claude setup-token`，在 Gateway 网关主机上使用 `crawclaw models auth paste-token --provider anthropic` 粘贴。参阅 [Anthropic](/providers/anthropic)。

### 是否支持 Claude 订阅认证（Claude Pro/Max）

是的——通过 **setup-token**。CrawClaw 不再复用 Claude Code CLI OAuth 令牌；请使用 setup-token 或 Anthropic API 密钥。在任何地方生成令牌并在 Gateway 网关主机上粘贴。参阅 [Anthropic](/providers/anthropic) 和 [OAuth](/concepts/oauth)。
...
提示：设置一个**备用模型**，这样 CrawClaw 在某个提供商被限速时仍能继续回复。
...
CrawClaw 通过 OAuth（ChatGPT 登录）支持 **OpenAI Code (Codex)**。新手引导可以运行 OAuth 流程，并在适当时将默认模型设置为 `openai-codex/gpt-5.2`。参阅[模型提供商](/concepts/model-providers)和[CLI 新手引导](/start/wizard)。

### 是否支持 OpenAI 订阅认证（Codex OAuth）

是的。CrawClaw 完全支持 **OpenAI Code (Codex) 订阅 OAuth**。新手引导可以为你运行 OAuth 流程。

参阅 [OAuth](/concepts/oauth)、[模型提供商](/concepts/model-providers)和[CLI 新手引导](/start/wizard)。

### 如何设置 Gemini CLI OAuth

Gemini CLI 使用**插件认证流程**，而不是 `crawclaw.json` 中的 client id 或 secret。

步骤：

1. 启用插件：`crawclaw plugins enable google`
2. 登录：`crawclaw models auth login --provider google-gemini-cli --set-default`

这会在 Gateway 网关主机上将 OAuth 令牌存储为认证配置文件。详情：[模型提供商](/concepts/model-providers)。

### 本地模型适合日常聊天吗

通常不适合。CrawClaw 需要大上下文 + 强安全性；小显卡会截断且泄漏。如果必须使用，请在本地运行你能运行的**最大** MiniMax M2.1 版本（LM Studio），参阅 [/gateway/local-models](/gateway/local-models)。较小/量化的模型会增加提示注入风险——参阅[安全](/gateway/security)。
...
- 使用 `crawclaw nodes status` / `crawclaw nodes list` 查看它。
...
- 给你的机器人发私信，然后运行 `crawclaw logs --follow` 并读取 `from.id`。
...
如果你通过 systemd 运行 CrawClaw，确保服务 PATH 包含 `/home/linuxbrew/.linuxbrew/bin`（或你的 brew 前缀），以便 `brew` 安装的工具在非登录 shell 中可解析。
...
这**不会删除你的数据**——它只改变 CrawClaw 代码的安装位置。你的状态
（`~/.crawclaw`）和工作区（`~/.crawclaw/workspace`）保持不变。
...
CrawClaw 是轻量级的。对于基本的 Gateway 网关 + 一个聊天渠道：
...
如果你使用 Windows，请参阅 [Windows](/platforms/windows)。如果你使用 VPS 或虚拟机，请参阅 [VPS 托管](/vps)。
...
CrawClaw 不是“只是一个 Claude 包装器”。它是一个**本地优先的控制平面**，让你在**自己的硬件**上运行强大的助手，可从你已经使用的聊天应用访问，具有有状态会话、记忆和工具——无需将工作流程的控制权交给托管 SaaS。
...
### CrawClaw 日常最常用的五个场景是什么

日常收益通常包括：

- **个人简报：** 收件箱、日历和你关心的新闻摘要。
- **研究和起草：** 快速研究、摘要以及邮件或文档的初稿。
- **提醒和跟进：** cron、hook 和主会话唤醒驱动的提醒和检查清单。
- **浏览器自动化：** 填写表单、收集数据和重复性网页任务。
- **跨设备协调：** 从手机发送任务，让 Gateway 网关在服务器上运行，然后在聊天中获取结果。

### CrawClaw 能否帮助 SaaS 进行获客、外联、广告和博客

可以用于**调研、筛选和起草**。它可以扫描网站、建立候选名单、总结潜在客户，并撰写外联或广告文案草稿。

对于**外联或广告投放**，请保持人工审核。避免垃圾邮件，遵守当地法律和平台政策，在发送之前审查所有内容。最安全的模式是让 CrawClaw 起草，由你批准。

文档：[安全](/gateway/security)。

### 相比 Claude Code，在 Web 开发方面有什么优势

CrawClaw 是一个**个人助手**和协调层，不是 IDE 替代品。使用 Claude Code 或 Codex 在仓库中进行最快的直接编码循环。当你需要持久记忆、跨设备访问和工具编排时，使用 CrawClaw。
...
展示：https://crawclaw.ai/showcase

## Skills 与自动化

### 如何自定义 Skills 而不弄脏仓库

使用托管覆盖而不是编辑仓库副本。将你的更改放在 `~/.crawclaw/skills/<name>/SKILL.md`（或通过 `~/.crawclaw/crawclaw.json` 中的 `skills.load.extraDirs` 添加文件夹）。优先级是 `<workspace>/skills` > `~/.crawclaw/skills` > 内置，所以托管覆盖优先生效而不会修改 git。只有值得上游合并的编辑才应该放在仓库中并作为 PR 提交。

### 可以从自定义文件夹加载 Skills 吗

可以。通过 `~/.crawclaw/crawclaw.json` 中的 `skills.load.extraDirs` 添加额外目录（最低优先级）。默认优先级保持不变：`<workspace>/skills` → `~/.crawclaw/skills` → 内置 → `skills.load.extraDirs`。`clawhub` 默认安装到 `./skills`，CrawClaw 将其视为 `<workspace>/skills`。
...
- 确认 cron 已启用（`cron.enabled`）且未设置 `CRAWCLAW_SKIP_CRON`。
...
文档：[定时任务](/automation/cron-jobs)、[自动化与任务](/automation)。
...
### CrawClaw 能否按计划或在后台持续运行任务

可以。使用 Gateway 网关调度器：

- **定时任务**用于计划或重复任务（跨重启持久化）。
- **主会话 cron jobs** 用于需要当前对话上下文的计划工作。
...
不能直接运行。macOS Skills 受 `metadata.crawclaw.os` 和所需二进制文件限制，Skills 只有在 **Gateway 网关主机**上符合条件时才会出现在系统提示中。在 Linux 上，`darwin` 专用 Skills（如 `apple-notes`、`apple-reminders`、`things-mac`）不会加载，除非你覆盖限制。

你有三种支持的模式：

**方案 A - 在 Mac 上运行 Gateway 网关（最简单）。**
在 macOS 二进制文件所在的地方运行 Gateway 网关，然后从 Linux 通过[远程模式](#how-do-i-run-crawclaw-in-remote-mode-client-connects-to-a-gateway-elsewhere)或 Tailscale 连接。Skills 正常加载，因为 Gateway 网关主机是 macOS。

**方案 B - 使用 macOS 节点（无需 SSH）。**
在 Linux 上运行 Gateway 网关，配对一个 macOS 节点主机，并在 Mac 上将**节点运行命令**设置为“始终询问”或“始终允许”。当所需二进制文件存在于节点上时，CrawClaw 可以将 macOS 专用 Skills 视为符合条件。智能体通过 `nodes` 工具运行这些 Skills。如果你选择“始终询问”，在提示中批准“始终允许”会将该命令添加到允许列表。
...
3. 覆盖 Skills 元数据（工作区或 `~/.crawclaw/skills`）以允许 Linux：
   ```markdown
   ---
   name: imsg
...
ClawHub 安装到当前目录下的 `./skills`（或回退到你配置的 CrawClaw 工作区）；CrawClaw 在下一个会话中将其视为 `<workspace>/skills`。对于跨智能体共享的 Skills，将它们放在 `~/.crawclaw/skills/<name>/SKILL.md`。某些 Skills 期望通过 Homebrew 安装二进制文件；在 Linux 上意味着 Linuxbrew（参阅上面的 Homebrew Linux 常见问题条目）。参阅[Skills](/tools/skills)和 [ClawHub](/tools/clawhub)。
...
```bash
crawclaw browser extension install
crawclaw browser extension path
```
...
CrawClaw 记忆就是智能体工作区中的 Markdown 文件：

- 每日笔记在 `memory/YYYY-MM-DD.md`
- 精选的长期笔记在 `MEMORY.md`（仅限主/私密会话）

CrawClaw 还会运行**静默的预压缩记忆刷新**，以提醒模型在自动压缩之前写入持久笔记。这只在工作区可写时运行（只读沙箱会跳过）。参阅[记忆](/concepts/memory)。
...
你的**工作区**（AGENTS.md、记忆文件、Skills 等）是独立的，通过 `agents.defaults.workspace` 配置（默认：`~/.crawclaw/workspace`）。

### AGENTS.md / SOUL.md / USER.md / MEMORY.md 应该放在哪里

这些文件位于**智能体工作区**中，而不是 `~/.crawclaw`。

- **工作区（按智能体）**：`AGENTS.md`、`SOUL.md`、`IDENTITY.md`、`USER.md`、
  `MEMORY.md`（或 `memory.md`）、`memory/YYYY-MM-DD.md`、可选的 `HEARTBEAT.md`。
- **状态目录（`~/.crawclaw`）**：配置、凭据、认证配置文件、会话、日志和共享 Skills（`~/.crawclaw/skills`）。

默认工作区是 `~/.crawclaw/workspace`，可通过以下方式配置：
...
**不要**提交 `~/.crawclaw` 下的任何内容（凭据、会话、令牌）。如果你需要完整恢复，将工作区和状态目录分别备份（参阅上面的迁移问题）。
...
可以。工作区是**默认 cwd** 和记忆锚点，不是硬沙箱。相对路径在工作区内解析，但绝对路径可以访问其他主机位置，除非启用了沙箱。如果你需要隔离，使用 [`agents.defaults.sandbox`](/gateway/sandboxing) 或按智能体的沙箱设置。如果你希望某个仓库作为默认工作目录，将该智能体的 `workspace` 指向仓库根目录。CrawClaw 仓库只是源代码；除非你有意要让智能体在其中工作，否则保持工作区独立。
...
CrawClaw 从 `$CRAWCLAW_CONFIG_PATH`（默认：`~/.crawclaw/crawclaw.json`）读取可选的 **JSON5** 配置：

```
$CRAWCLAW_CONFIG_PATH
```

如果文件不存在，使用安全的默认值（包括默认工作区 `~/.crawclaw/workspace`）。

### 我设置了 gateway.bind: "lan"（或 "tailnet"），现在什么都监听不了 / UI 显示未授权

非 local loopback 绑定**需要认证**。配置 `gateway.auth.mode` + `gateway.auth.token`（或使用 `CRAWCLAW_GATEWAY_TOKEN`）。
...
- Gateway 客户端通过 `connect.params.auth.token` 进行认证。避免将令牌放在 URL 中。

### 为什么现在在 localhost 也需要令牌

向导默认生成 Gateway 网关令牌（即使在 local loopback 上），因此**本地 WS 客户端必须认证**。这阻止了其他本地进程调用 Gateway 网关。在你的客户端配置中设置令牌以连接。

如果你**确实**想要开放 local loopback，从配置中移除 `gateway.auth`。Doctor 可以随时为你生成令牌：`crawclaw doctor --generate-gateway-token`。
...
`web_fetch` 无需 API 密钥即可工作。`web_search` 需要所选提供商的 API 密钥。**推荐：** 运行 `crawclaw configure --section web`。新的提供商专属配置会存储在 `plugins.entries.<plugin>.config.webSearch.*` 下。环境变量替代方案：为 Gateway 网关进程设置相应的提供商环境变量。
...
- 守护进程从 `~/.crawclaw/.env`（或服务环境）读取环境变量。
...
- 从备份恢复（git 或复制的 `~/.crawclaw/crawclaw.json`）。
- 如果没有备份，重新运行 `crawclaw doctor` 并重新配置渠道/模型。
- 如果这是意外情况，提交 bug 并附上你最后已知的配置或任何备份。
- 本地编码智能体通常可以从日志或历史中重建工作配置。

避免方法：

- 对小更改使用 `crawclaw config set`。
- 对交互式编辑使用 `crawclaw configure`。
...
文档：[节点](/nodes)、[远程访问](/gateway/remote)、[多智能体路由](/concepts/multi-agent)、[子智能体](/tools/subagents)、[TUI](/cli/tui)。
...
参阅[浏览器](/tools/browser#browser-selection)中的完整配置示例。
...
4. 在本地运行节点主机并通过 SSH 隧道或 tailnet 连接，使其可以注册为节点。
5. 在 Gateway 网关上批准节点：
   ```bash
   crawclaw nodes pending
   crawclaw nodes approve <requestId>
   ```
...
文档：[节点](/nodes)、[Gateway 网关协议](/gateway/protocol)、[macOS 远程模式](/gateway/remote)、[安全](/gateway/security)。

### Tailscale 已连接但收不到回复，怎么办

检查基础项：

- Gateway 网关正在运行：`crawclaw gateway status`
- Gateway 网关健康：`crawclaw status`
- 渠道健康：`crawclaw channels status`
...
有——节点是从远程 Gateway 网关到达你笔记本的首选方式，它们解锁的不仅仅是 shell 访问。Gateway 网关运行在 macOS、Linux 或 Windows 上且是轻量级的（小型 VPS 或 Raspberry Pi 级别的设备就够用；4 GB RAM 足够），所以常见设置是一个常开主机加上你的笔记本作为节点。
...
不会。每台主机上应该只运行**一个 Gateway 网关**，除非你有意运行隔离的配置文件（参阅[多 Gateway 网关](/gateway/multiple-gateways)）。节点是连接到 Gateway 网关的外围设备（节点主机）。对于无头节点主机和 CLI 控制，参阅[节点主机 CLI](/cli/node)。

`gateway` 和 `discovery` 的更改需要完全重启。
...
如果你想要无 SSH 的 Gateway 访问，在 VPS 上使用 Tailscale Serve：
...
Serve 暴露 **Gateway 网关 HTTP/API + WS**。节点通过同一个 Gateway 网关 WS 端点连接。

推荐设置：

1. **确保 VPS + Mac 在同一个 tailnet 上**。
2. **使用节点主机连接远程 Gateway 网关**（SSH 目标可以是 tailnet 主机名，或直接使用 tailnet）。
3. **在 Gateway 网关上批准节点**：
   ```bash
   crawclaw nodes pending
   crawclaw nodes approve <requestId>
...
文档：[Gateway 网关协议](/gateway/protocol)、[发现](/gateway/discovery)、[macOS 远程模式](/gateway/remote)。

## 环境变量和 .env 加载

### CrawClaw 如何加载环境变量

CrawClaw 从父进程（shell、launchd/systemd、CI 等）读取环境变量，并额外加载：

- 当前工作目录下的 `.env`
- `~/.crawclaw/.env`（即 `$CRAWCLAW_STATE_DIR/.env`）的全局回退 `.env`
...
1. 将缺失的密钥放在 `~/.crawclaw/.env` 中，这样即使服务不继承你的 shell 环境也能被获取。
...
`crawclaw models status` 报告的是 **shell 环境导入**是否启用。"Shell env: off"**不**意味着你的环境变量缺失——它只意味着 CrawClaw 不会自动加载你的登录 shell。

如果 Gateway 网关作为服务（launchd/systemd）运行，它不会继承你的 shell 环境。通过以下方式之一修复：

1. 将令牌放在 `~/.crawclaw/.env` 中：
   ```
...
### 能否创建一个 CrawClaw 实例团队——一个 CEO 和多个智能体
...
- 如果你使用了配置文件（`--profile` / `CRAWCLAW_PROFILE`），重置每个状态目录（默认为 `~/.crawclaw-<profile>`）。
- 开发重置：`crawclaw gateway --dev --reset`（仅限开发；清除开发配置 + 凭据 + 会话 + 工作区）。
...
- **磁盘增长：** 会话 + 记录位于 `~/.crawclaw/agents/<agentId>/sessions/` 下。
...
模型以 `provider/model` 引用（示例：`anthropic/claude-opus-4-5`）。如果你省略提供商，CrawClaw 目前假设 `anthropic` 作为临时弃用回退——但你仍然应该**明确**设置 `provider/model`。
...
- 聊天中的 `/model`（快速，按会话）
- `crawclaw models set ...`（只更新模型配置）
- `crawclaw configure --section models`（交互式）
- 编辑 `~/.crawclaw/crawclaw.json` 中的 `agents.defaults.model`

避免使用部分对象执行 `config.apply`，除非你打算替换整个配置。如果你确实覆盖了配置，从备份恢复或重新运行 `crawclaw doctor` 来修复。

文档：[模型](/concepts/models)、[Configure](/cli/configure)、[Config](/cli/config)、[Doctor](/gateway/doctor)。

### CrawClaw、Flawd 和 Krill 使用什么模型

- **CrawClaw + Flawd：** Anthropic Opus（`anthropic/claude-opus-4-5`）——参阅 [Anthropic](/providers/anthropic)。
...
是的。CrawClaw 内置了一些默认简写（仅在模型存在于 `agents.defaults.models` 中时应用）：
...
- 运行 `crawclaw agents add <id>` 并在向导中配置认证。
...
冷却期适用于失败的配置文件（指数退避），因此 CrawClaw 即使在提供商被限速或临时失败时也能继续响应。
...
- **确认认证配置文件的位置**（新路径 vs 旧路径）
  - 当前：`~/.crawclaw/agents/<agentId>/agent/auth-profiles.json`
  - 旧版：`~/.crawclaw/agent/*`（通过 `crawclaw doctor` 迁移）
- **确认环境变量被 Gateway 网关加载**
  - 如果你在 shell 中设置了 `ANTHROPIC_API_KEY` 但通过 systemd/launchd 运行 Gateway 网关，它可能不会继承。将其放在 `~/.crawclaw/.env` 中或启用 `env.shellEnv`。
- **确保你编辑的是正确的智能体**
  - 多智能体设置意味着可能有多个 `auth-profiles.json` 文件。
- **完整性检查模型/认证状态**
  - 使用 `crawclaw models status` 查看已配置的模型以及提供商是否已认证。
...
- **使用 setup-token**
  - 运行 `claude setup-token`，然后用 `crawclaw models auth setup-token --provider anthropic` 粘贴。
  - 如果令牌在另一台机器上创建，使用 `crawclaw models auth paste-token --provider anthropic`。
- **如果你想使用 API 密钥**
  - 在 **Gateway 网关主机**上将 `ANTHROPIC_API_KEY` 放入 `~/.crawclaw/.env`。
  - 清除任何强制缺失配置文件的固定顺序：
    ```bash
...
如果你的模型配置包含 Google Gemini 作为回退（或你切换到了 Gemini 简写），CrawClaw 会在模型回退期间尝试它。如果你没有配置 Google 凭据，你会看到 `No API key found for provider "google"`。
...
修复：CrawClaw 现在为 Google Antigravity Claude 剥离未签名的 thinking 块。如果仍然出现，开始**新会话**或为该智能体设置 `/thinking off`。
...
如果某个配置文件处于短期**冷却**（速率限制/超时/认证失败）或较长的**禁用**状态（计费/额度不足），CrawClaw 可能会临时跳过它。要检查这一点，运行 `crawclaw models status --json` 并查看 `auth.unusableProfiles`。调优：`auth.cooldowns.billingBackoffHours*`。
...
`gateway.port` 控制用于 WebSocket + HTTP（Gateway API、钩子等）的单个复用端口。

优先级：

```
--port > CRAWCLAW_GATEWAY_PORT > gateway.port > 默认 18789
```

### 为什么 crawclaw gateway status 显示 Runtime: running 但 RPC probe: failed

因为"running"是 **supervisor** 的视角（launchd/systemd/schtasks）。RPC 探测是 CLI 实际连接到 Gateway 网关 WebSocket 并调用 `status`。

使用 `crawclaw gateway status` 并关注这些行：

- `Probe target:`（探测实际使用的 URL）
- `Listening:`（端口上实际绑定的内容）
- `Last gateway error:`（进程存活但端口未监听时的常见根因）

### 为什么 crawclaw gateway status 显示 Config (cli) 和 Config (service) 不同

你正在编辑一个配置文件，而服务运行的是另一个（通常是 `--profile` / `CRAWCLAW_STATE_DIR` 不匹配）。
...
CrawClaw 通过在启动时立即绑定 WebSocket 监听器来强制运行时锁（默认 `ws://127.0.0.1:18789`）。如果绑定因 `EADDRINUSE` 失败，它会抛出 `GatewayLockError` 表示另一个实例已在监听。

修复：停止另一个实例，释放端口，或使用 `crawclaw gateway --port <port>` 运行。

### 如何以远程模式运行 CrawClaw（客户端连接到其他位置的 Gateway 网关）
...
- `crawclaw gateway` 仅在 `gateway.mode` 为 `local` 时启动（或你传递覆盖标志）。
- 客户端在配置更改后需要重新连接。

### Gateway 客户端显示"unauthorized"或持续重连，怎么办

你的 Gateway 网关运行时启用了认证（`gateway.auth.*`），但客户端没有发送匹配的令牌/密码。
...
- 最快：运行 `crawclaw tui` 直接连接 Gateway 网关。
...
- 在你使用的客户端里配置相同的令牌。
- 仍然卡住？运行 `crawclaw status --all` 并按 [故障排除](/gateway/troubleshooting) 操作。参阅 [远程访问](/gateway/remote) 了解认证详情。
...
- `CRAWCLAW_CONFIG_PATH`（每实例配置）
- `CRAWCLAW_STATE_DIR`（每实例状态）
- `agents.defaults.workspace`（工作区隔离）
- `gateway.port`（唯一端口）

快速设置（推荐）：

- 每实例使用 `crawclaw --profile <name> …`（自动创建 `~/.crawclaw-<name>`）。
- 在每个配置文件配置中设置唯一的 `gateway.port`（或手动运行时传 `--port`）。
- 安装每配置文件的服务：`crawclaw --profile <name> gateway install`。

配置文件还会为服务名称添加后缀（`bot.molt.<profile>`；旧版 `com.crawclaw.*`、`crawclaw-gateway-<profile>.service`、`CrawClaw Gateway 网关 (<profile>)`）。
...
- macOS：`$CRAWCLAW_STATE_DIR/logs/gateway.log` 和 `gateway.err.log`（默认：`~/.crawclaw/logs/...`；配置文件使用 `~/.crawclaw-<profile>/logs/...`）
- Linux：`journalctl --user -u crawclaw-gateway[-<profile>].service -n 200 --no-pager`
- Windows：`schtasks /Query /TN "CrawClaw Gateway 网关 (<profile>)" /V /FO LIST`
...
如果你手动运行 Gateway 网关，`crawclaw gateway --force` 可以回收端口。参阅 [Gateway 网关](/gateway)。

### 我在 Windows 上关闭了终端——如何重启 CrawClaw
...
文档：[Windows](/platforms/windows)、[Gateway 网关服务运维手册](/gateway)。
...
文档：[TUI](/cli/tui)、[斜杠命令](/tools/slash-commands)。
...
### 通俗解释：crawclaw gateway restart 与 crawclaw gateway

- `crawclaw gateway restart`：重启**后台服务**（launchd/systemd）。
- `crawclaw gateway`：在这个终端会话中**前台**运行 Gateway 网关。

如果你安装了服务，使用 Gateway 网关命令。想要一次性前台运行时使用 `crawclaw gateway`。
...
智能体的出站附件必须包含 `MEDIA:<path-or-url>` 行（独占一行）。参阅 [CrawClaw 助手设置](/start/crawclaw)和 [Agent send](/tools/agent-send)。
...
### 将 CrawClaw 暴露给入站私信安全吗

将入站私信视为不可信输入。默认设计旨在降低风险：

- 支持私信的渠道上的默认行为是**配对**：
  - 未知发送者会收到配对码；机器人不处理他们的消息。
  - 批准方式：`crawclaw pairing approve <channel> <code>`
  - 每个渠道的待处理请求上限为 **3 个**；如果没收到代码，检查 `crawclaw pairing list <channel>`。
- 公开开放私信需要明确选择加入（`dmPolicy: "open"` 且允许列表 `"*"`）。

运行 `crawclaw doctor` 以发现有风险的私信策略。
...
如果仍然嘈杂，检查会话设置并将 verbose 设为**继承**。同时确认你没有使用在配置中将 `verboseDefault` 设为 `on` 的机器人配置文件。
