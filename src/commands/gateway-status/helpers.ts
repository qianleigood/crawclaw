...
import type { CrawClawConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveTargets(cfg: CrawClawConfig, explicitUrl?: string): GatewayStatusTarget[] {
    ...
    issues: issuesRaw
      .filter(
        (i): i is { path: string; message: string } =>
          i != null && typeof i.path === "string" && typeof i.message === "string",
      )
...
export function buildNetworkHints(cfg: CrawClawConfig) {
