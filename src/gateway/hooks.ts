...
export function resolveHooksConfig(cfg: CrawClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: CrawClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-crawclaw-token"] === "string"
      ? req.headers["x-crawclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-crawclaw-idempotency-key"]) ||
