...
type GatewaySecretDefaults = NonNullable<CrawClawConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${CRAWCLAW_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.CRAWCLAW_GATEWAY_TOKEN);
}

export function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.CRAWCLAW_GATEWAY_PASSWORD);
}
