...
import { buildOauthProviderAuthResult } from "crawclaw/plugin-sdk/provider-auth";
import { loginOpenAICodexOAuth } from "crawclaw/plugin-sdk/provider-auth-login";
import {
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `crawclaw models auth login --provider openai-codex` or `crawclaw configure`.";
}
