...
const crawclawShell = process.env.CRAWCLAW_SHELL || "";
...
const emitDoneAndMaybeHang = (payload = { type: "done", stopReason: "end_turn" }) => {
  emitJson(payload);
  if (process.env.MOCK_ACPX_PROMPT_HANG_AFTER_DONE === "1") {
    setInterval(() => {}, 1000);
    return;
  }
  flushAndExit(0);
...
async function ensureMockCliScriptPath(): Promise<string> {
  ...
  sharedMockCliScriptPath = (async () => {
    const dir = await mkdtemp(
      path.join(resolvePreferredCrawClawTmpDir(), "crawclaw-acpx-runtime-test-"),
    );
    tempDirs.push(dir);
