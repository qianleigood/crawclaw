...
vi.mock("../channels/deliverable-target.js", () => ({
  resolveDeliverableTarget: (target: { channel?: string; to?: string } | undefined) => {
    if (target?.channel === "task-channel" && target.to?.trim()) {
      return { channel: "task-channel", to: target.to.trim() };
    }
    return null;
  },
}));
...
async function withTaskRegistryTempDir<T>(run: (root: string) => Promise<T>): Promise<T> {
  return await withTempDir({ prefix: "crawclaw-task-registry-" }, async (root) => {
    process.env.CRAWCLAW_STATE_DIR = root;
    resetTaskRegistryForTests();
...
describe("task-registry", () => {
  ...
  it("indexes agent tasks by agentId and parentAgentId and merges metadata on repeated create", () => {
    const created = createTaskRecord({
      runtime: "acp",
      ownerKey: "agent:main:main",
      scopeKind: "session",
      childSessionKey: "agent:child:acp:session",
      agentId: "agent-child",
      agentMetadata: {
        mode: "background",
        transcriptRef: "agents/child/transcript.jsonl",
      },
      runId: "run-agent-task",
      label: "Child agent",
    ...
    const merged = createTaskRecord({
      ...
      agentMetadata: {
        ...
        runtimeStateRef: "agents/child/runtime.json",
      },
      runId: "run-agent-task",
      label: "Child agent",
      task: "Investigate regression",
    ...
    expect(merged.taskId).toBe(created.taskId);
    expect(findLatestTaskForAgentId("agent-child")).toMatchObject({
    ...
    expect(listTasksForParentAgentId("agent-parent")).toMatchObject([
      expect.objectContaining({
        taskId: created.taskId,
        agentId: "agent-child",
      }),
    ]);
    expect(findLatestTaskForParentAgentId("agent-parent")?.taskId).toBe(created.taskId);
  });
  ...
  it("records delivery failure and queues a session fallback when direct delivery misses", async () => {
    await withTaskRegistryTempDir(async (root) => {
      process.env.CRAWCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      hoisted.sendMessageMock.mockRejectedValueOnce(new Error("task-channel unavailable"));
  ...
  it("cancels ACP-backed tasks through the ACP session manager", async () => {
    await withTempDir({ prefix: "crawclaw-task-registry-" }, async (root) => {
