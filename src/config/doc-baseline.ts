...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.CRAWCLAW_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveCrawClawPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    CRAWCLAW_STATE_DIR: path.join(os.tmpdir(), "crawclaw-config-doc-baseline-state"),
    CRAWCLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
