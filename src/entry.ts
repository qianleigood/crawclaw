...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "crawclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "crawclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureCliRespawnReady(): boolean {
    ...
    child.once("error", (error) => {
      console.error(
        "[crawclaw] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `CrawClaw ${VERSION} (${commit})` : `CrawClaw ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[crawclaw] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
...
export function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[crawclaw] Failed to display help:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
