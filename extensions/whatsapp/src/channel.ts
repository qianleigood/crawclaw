import { buildDmGroupAccountAllowlistAdapter } from "crawclaw/plugin-sdk/allowlist-config-edit";
import { resolveReactionMessageId } from "crawclaw/plugin-sdk/channel-actions";
import { createChatChannelPlugin } from "crawclaw/plugin-sdk/core";
import { chunkText } from "crawclaw/plugin-sdk/reply-runtime";
import {
...
function areWhatsAppAgentReactionsEnabled(params: { cfg: CrawClawConfig; accountId?: string }) {
...
function hasAnyWhatsAppAccountWithAgentReactionsEnabled(cfg: CrawClawConfig) {
...
function resolveWhatsAppAgentReactionGuidance(params: { cfg: CrawClawConfig; accountId?: string }) {
  if (!params.cfg.channels?.whatsapp) {
    return undefined;
  }
  const gate = createActionGate(params.cfg.channels.whatsapp.actions);
