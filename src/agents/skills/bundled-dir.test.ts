...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["CRAWCLAW_BUNDLED_SKILLS_DIR", "CRAWCLAW_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns CRAWCLAW_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-bundled-override-"));
    process.env.CRAWCLAW_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "crawclaw" }));
