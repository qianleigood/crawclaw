...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop crawclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@crawclaw_bot", { botUsername: "crawclaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@crawclaw_bot", { botUsername: "crawclaw_bot" })).toBe(true);
