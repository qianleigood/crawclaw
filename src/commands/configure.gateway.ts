...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "CRAWCLAW_GATEWAY_TOKEN",
          placeholder: "CRAWCLAW_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = (value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like CRAWCLAW_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. CrawClaw will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: CrawClaw trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.crawclaw.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
