...
function resolveSystemdServiceName(env: GatewayServiceEnv): string {
  ...
  return resolveGatewaySystemdServiceName(env.CRAWCLAW_PROFILE);
}
...
async function activateSystemdService(params: { env: GatewayServiceEnv }) {
  const serviceName = resolveGatewaySystemdServiceName(params.env.CRAWCLAW_PROFILE);
...
export async function uninstallSystemdService({
  ...
  await assertSystemdAvailable(env);
  const serviceName = resolveGatewaySystemdServiceName(env.CRAWCLAW_PROFILE);
  const unitName = `${serviceName}.service`;
