...
import type { CrawClawConfig } from "../../../../src/plugin-sdk/discord.js";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: CrawClawConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): CrawClawConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("crawclaw/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("crawclaw/plugin-sdk/acp-runtime")>(
    "crawclaw/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("crawclaw/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("crawclaw/plugin-sdk/command-auth")>(
...
vi.mock("crawclaw/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("crawclaw/plugin-sdk/reply-runtime")>(
    "crawclaw/plugin-sdk/reply-runtime",
  );
  return {
    ...actual,
...
vi.mock("crawclaw/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("crawclaw/plugin-sdk/config-runtime")>(
...
vi.mock("crawclaw/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("crawclaw/plugin-sdk/runtime-env")>(
    "crawclaw/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
