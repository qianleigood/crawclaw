...
const { createCrawClawCodingToolsMock } = vi.hoisted(() => ({
  createCrawClawCodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(async () => {
    vi.restoreAllMocks();
    vi.resetModules();
    createCrawClawCodingToolsMock.mockClear();
    createCrawClawCodingToolsMock.mockReturnValue([]);
    const piTools = await import("../../agents/pi-tools.js");
    vi.spyOn(piTools, "createCrawClawCodingTools").mockImplementation(
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    expect(createCrawClawCodingToolsMock).toHaveBeenCalledWith(
