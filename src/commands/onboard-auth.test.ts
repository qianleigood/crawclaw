...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under CRAWCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("crawclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-oauth-sync-"));
    ...
    process.env.CRAWCLAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-oauth-nosync-"));
    process.env.CRAWCLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside CRAWCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-oauth-external-"));
    process.env.CRAWCLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* CRAWCLAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to CRAWCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("crawclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
