...
import { maxBytesForKind } from "crawclaw/plugin-sdk/media-runtime";
...
import type { PollInput } from "crawclaw/plugin-sdk/media-runtime";
import { resolveChunkMode } from "crawclaw/plugin-sdk/reply-runtime";
import type { RetryConfig } from "crawclaw/plugin-sdk/retry-runtime";
...
import { loadWebMediaRaw } from "crawclaw/plugin-sdk/web-media";
...
async function materializeVoiceMessageInput(mediaUrl: string): Promise<{ filePath: string }> {
  ...
  const tempDir = resolvePreferredCrawClawTmpDir();
