...
describe("session hook context wiring", () => {
  ...
  it("passes sessionKey to session_start hook context", async () => {
    const sessionKey = "agent:main:feishu:direct:123";
    const storePath = await createStorePath("crawclaw-session-hook-start");
  ...
  it("passes sessionKey to session_end hook context on reset", async () => {
    const sessionKey = "agent:main:feishu:direct:123";
    const storePath = await createStorePath("crawclaw-session-hook-end");
    await writeStore(storePath, {
      [sessionKey]: {
