...
describe("cron service state seam coverage", () => {
  it("threads main-session and session-store dependencies into internal state", () => {
    ...
    expect(state.deps.requestMainSessionWake).toBe(requestMainSessionWake);
    expect(state.deps.runMainSessionOnce).toBe(runMainSessionOnce);
