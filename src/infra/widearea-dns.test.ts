...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "crawclaw.internal", expected: "crawclaw.internal." },
    { value: "crawclaw.internal.", expected: "crawclaw.internal." },
    { value: "  crawclaw.internal  ", expected: "crawclaw.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { CRAWCLAW_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { CRAWCLAW_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("crawclaw.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "crawclaw.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      ...
      `_crawclaw-gw._tcp IN PTR studio-london._crawclaw-gw._tcp`,
      `studio-london._crawclaw-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (CrawClaw)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/crawclaw`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN crawclaw.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._crawclaw-gw._tcp IN TXT`,
        `displayName=Mac Studio (CrawClaw)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("crawclaw.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("crawclaw.internal."),
