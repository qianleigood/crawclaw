---
read_when:
  - 你想在 GCP 上 24/7 运行 CrawClaw
  - 你想要在自己的 VM 上运行生产级、常驻的 Gateway 网关
  - 你想完全控制持久化、二进制文件和重启行为
summary: 在 GCP Compute Engine VM（Docker）上 24/7 运行 CrawClaw Gateway 网关并持久化状态
title: GCP
x-i18n:
...
# 在 GCP Compute Engine 上运行 CrawClaw（Docker，生产 VPS 指南）

## 目标

使用 Docker 在 GCP Compute Engine VM 上运行持久化的 CrawClaw Gateway 网关，具有持久状态、内置二进制文件和安全的重启行为。

如果你想要"CrawClaw 24/7 大约 $5-12/月"，这是在 Google Cloud 上的可靠设置。
...
```bash
gcloud projects create my-crawclaw-project --name="CrawClaw Gateway"
gcloud config set project my-crawclaw-project
```
...
```bash
gcloud compute instances create crawclaw-gateway \
  --zone=us-central1-a \
...
```bash
gcloud compute ssh crawclaw-gateway --zone=us-central1-a
```
...
```bash
CRAWCLAW_IMAGE=crawclaw:latest
CRAWCLAW_GATEWAY_TOKEN=change-me-now
CRAWCLAW_GATEWAY_BIND=lan
CRAWCLAW_GATEWAY_PORT=18789

CRAWCLAW_CONFIG_DIR=/home/$USER/.crawclaw
CRAWCLAW_WORKSPACE_DIR=/home/$USER/.crawclaw/workspace

GOG_KEYRING_PASSWORD=change-me-now
XDG_CONFIG_HOME=/home/node/.crawclaw
```
...
services:
  crawclaw-gateway:
    ...
    environment:
      ...
      - NODE_ENV=production
      - TERM=xterm-256color
      - CRAWCLAW_GATEWAY_BIND=${CRAWCLAW_GATEWAY_BIND}
      - CRAWCLAW_GATEWAY_PORT=${CRAWCLAW_GATEWAY_PORT}
      - CRAWCLAW_GATEWAY_TOKEN=${CRAWCLAW_GATEWAY_TOKEN}
    ...
    ports:
      ...
      - "127.0.0.1:${CRAWCLAW_GATEWAY_PORT}:18789"

      # 可选：仅当你针对此 VM 运行节点客户端并需要 Canvas 主机时。
...
```bash
docker compose exec crawclaw-gateway which gog
docker compose exec crawclaw-gateway which goplaces
docker compose exec crawclaw-gateway which wacli
```
...
```bash
docker compose logs -f crawclaw-gateway
```
...
CrawClaw 在 Docker 中运行，但 Docker 不是真实来源。
所有长期状态必须在重启、重建和重启后仍然存在。

| 组件             | 位置                              | 持久化机制    | 说明                        |
| ---------------- | --------------------------------- | ------------- | --------------------------- |
| Gateway 网关配置 | `/home/node/.crawclaw/`           | 主机卷挂载    | 包括 `crawclaw.json`、令牌  |
| 模型认证配置文件 | `/home/node/.crawclaw/`           | 主机卷挂载    | OAuth 令牌、API 密钥        |
| Skill 配置       | `/home/node/.crawclaw/skills/`    | 主机卷挂载    | Skill 级别状态              |
| 智能体工作区     | `/home/node/.crawclaw/workspace/` | 主机卷挂载    | 代码和智能体产物            |
| WhatsApp 会话    | `/home/node/.crawclaw/`           | 主机卷挂载    | 保留 QR 登录                |
| Gmail 密钥环     | `/home/node/.crawclaw/`           | 主机卷 + 密码 | 需要 `GOG_KEYRING_PASSWORD` |
...
```bash
# 首先停止 VM
gcloud compute instances stop crawclaw-gateway --zone=us-central1-a

# 更改机器类型
gcloud compute instances set-machine-type crawclaw-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# 启动 VM
gcloud compute instances start crawclaw-gateway --zone=us-central1-a
```
...
1. 创建服务账户：
   ...
   ```bash
   gcloud iam service-accounts create crawclaw-deploy \
     --display-name="CrawClaw Deployment"
   ```

2. 授予 Compute Instance Admin 角色（或更窄的自定义角色）：
   ```bash
   gcloud projects add-iam-policy-binding my-crawclaw-project \
     --member="serviceAccount:crawclaw-deploy@my-crawclaw-project.iam.gserviceaccount.com" \
