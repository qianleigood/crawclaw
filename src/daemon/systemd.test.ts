...
const TEST_MANAGED_HOME = "/tmp/crawclaw-test-home";
const GATEWAY_SERVICE = "crawclaw-gateway.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/crawclaw gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/crawclaw", "gateway", "run"]);
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/crawclaw-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "crawclaw-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "crawclaw-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/crawclaw-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled crawclaw-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when CRAWCLAW_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/crawclaw-gateway.service",
    },
    {
      name: "uses profile-specific service name when CRAWCLAW_PROFILE is set to a custom value",
      env: { HOME: "/home/test", CRAWCLAW_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/crawclaw-gateway-jbphoenix.service",
    },
    {
      name: "prefers CRAWCLAW_SYSTEMD_UNIT over CRAWCLAW_PROFILE",
      env: {
        HOME: "/home/test",
        CRAWCLAW_PROFILE: "jbphoenix",
        CRAWCLAW_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles CRAWCLAW_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        CRAWCLAW_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from CRAWCLAW_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        CRAWCLAW_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/crawclaw gateway start --name "My Bot"')).toEqual([
      "/usr/bin/crawclaw",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('crawclaw --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["crawclaw", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('crawclaw --path "C:\\\\Program Files\\\\CrawClaw"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["crawclaw", "--path", "C:\\\\Program Files\\\\CrawClaw"]);

    expect(
      splitArgsPreservingQuotes('crawclaw --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["crawclaw", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/crawclaw gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads CRAWCLAW_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/crawclaw gateway run", "EnvironmentFile=%h/.crawclaw/.env"],
      { [`${TEST_SERVICE_HOME}/.crawclaw/.env`]: "CRAWCLAW_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.CRAWCLAW_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/crawclaw gateway run",
        "EnvironmentFile=%h/.crawclaw/.env",
        'Environment="CRAWCLAW_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.crawclaw/.env`]: "CRAWCLAW_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.CRAWCLAW_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.CRAWCLAW_GATEWAY_TOKEN).toBe("file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.crawclaw/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/crawclaw-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/crawclaw gateway run",
          'EnvironmentFile=%h/.crawclaw/first.env "%h/.crawclaw/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.crawclaw/first.env") {
        return "CRAWCLAW_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.crawclaw/second env.env") {
        return 'CRAWCLAW_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      ...
      CRAWCLAW_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "CRAWCLAW_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "CRAWCLAW_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "CRAWCLAW_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      CRAWCLAW_GATEWAY_TOKEN: "override-token",
      CRAWCLAW_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/crawclaw-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/crawclaw gateway run",
          "EnvironmentFile=%h/.crawclaw/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.crawclaw/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'CRAWCLAW_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "CRAWCLAW_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      CRAWCLAW_GATEWAY_TOKEN: "quoted token",
      CRAWCLAW_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      CRAWCLAW_GATEWAY_TOKEN: "file",
      CRAWCLAW_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "crawclaw-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ CRAWCLAW_PROFILE: "work" });
  });
