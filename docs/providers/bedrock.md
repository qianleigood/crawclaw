---
summary: "Use Amazon Bedrock (Converse API) models with CrawClaw"
read_when:
  - You want to use Amazon Bedrock models with CrawClaw
...
If AWS credentials are detected, CrawClaw can automatically discover Bedrock
...
When running CrawClaw on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, CrawClaw's credential detection currently only checks for environment
...
- CrawClaw surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
