...
import { normalizeE164 } from "crawclaw/plugin-sdk/text-runtime";
import {
...
export const signalDmPolicy = createTopLevelChannelDmPolicy({
  ...
  getCurrent: (cfg: CrawClawConfig) => cfg.channels?.signal?.dmPolicy ?? "pairing",
...
export const signalCompletionNote = {
  ...
  lines: [
    'Link device with: signal-cli link -n "CrawClaw"',
    "Scan QR in Signal -> Linked Devices",
    `Then run: ${formatCliCommand("crawclaw gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
