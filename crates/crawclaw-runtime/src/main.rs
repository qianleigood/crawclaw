...
fn emit_provider_auth_choices(args: Vec<String>) {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            other => {
                eprintln!("unsupported emit-provider-auth-choices option: {other}");
                std::process::exit(2);
            }
        }
    }
    let Some(output_path) = output else {
        eprintln!(
            "usage: crawclaw-runtime emit-provider-auth-choices --output <path> [--check|--write]"
        );
        std::process::exit(2);
    };
    match crawclaw_runtime::write_provider_auth_choices_module(&output_path, check) {
        Ok(result) => {
            if check {
                if result.changed {
                    eprintln!(
                        "[provider-auth-choices] stale generated output at {}",
