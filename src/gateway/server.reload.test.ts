...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.CRAWCLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.CRAWCLAW_SKIP_GMAIL_WATCHER;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    prevGeminiApiKey = process.env.GEMINI_API_KEY;
    process.env.CRAWCLAW_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.CRAWCLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.CRAWCLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.CRAWCLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  ...
  async function writeConfigFile(config: unknown) {
    ...
    if (!configPath) {
      throw new Error("CRAWCLAW_CONFIG_PATH is not set");
    }
  ...
  async function writeAuthProfileEnvRefStore() {
    ...
    if (!stateDir) {
      throw new Error("CRAWCLAW_STATE_DIR is not set");
    }
      ...
      `${JSON.stringify(
        {
          ...
          profiles: {
            missing: {
              type: "api_key",
              provider: "openai",
              keyRef: { source: "env", provider: "default", id: "MISSING_CRAWCLAW_AUTH_REF" },
            },
          },
  ...
  it("fails startup when auth-profile secret refs are unresolved", async () => {
    ...
    delete process.env.MISSING_CRAWCLAW_AUTH_REF;
    try {
      await expect(withGatewayServer(async () => {})).rejects.toThrow(
        'Environment variable "MISSING_CRAWCLAW_AUTH_REF" is missing or empty.',
      );
    } finally {
  ...
  it("does not emit secrets reloader events for web search secret reload transitions", async () => {
    ...
    await withGatewayServer(async () => {
              ...
              config: {
                webSearch: {
                  apiKey: {
                    source: "env",
                    provider: "default",
                    id: "CRAWCLAW_TEST_MISSING_GEMINI_API_KEY",
                  },
                },
              },
            },
      ...
      delete process.env.GEMINI_API_KEY;
      delete process.env.CRAWCLAW_TEST_MISSING_GEMINI_API_KEY;
