import { DEFAULT_ACCOUNT_ID } from "crawclaw/plugin-sdk/routing";
import type { RuntimeEnv } from "crawclaw/plugin-sdk/runtime-env";
import { createWhatsAppPollFixture, expectWhatsAppPollSent } from "crawclaw/plugin-sdk/testing";
...
const hoisted = vi.hoisted(() => ({
  ...
  listWhatsAppAccountIds: vi.fn((cfg: CrawClawConfig) => {
  ...
  resolveWhatsAppAuthDir: vi.fn(() => ({
    authDir: "/tmp/crawclaw-whatsapp-test",
  })),
}));
...
vi.mock("crawclaw/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("crawclaw/plugin-sdk/setup")>(
    "crawclaw/plugin-sdk/setup",
  );
  return {
    ...actual,
...
describe("whatsapp setup wizard", () => {
  ...
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/crawclaw-whatsapp-test" });
  });
  ...
  it("skips relink note when already linked and relink is declined", async () => {
    ...
    expect(harness.note).not.toHaveBeenCalledWith(
      expect.stringContaining("crawclaw channels login"),
      "WhatsApp",
    );
  });
