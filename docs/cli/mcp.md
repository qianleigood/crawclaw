---
summary: "Expose CrawClaw channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to CrawClaw-backed channels
  - Running `crawclaw mcp serve`
  - Managing CrawClaw-saved MCP server definitions
title: "mcp"
---

# mcp

`crawclaw mcp` has two jobs:

- run CrawClaw as an MCP server with `crawclaw mcp serve`
- manage CrawClaw-owned outbound MCP server definitions with `list`, `show`,
  `set`, and `unset`

In other words:

- `serve` is CrawClaw acting as an MCP server
- `list` / `show` / `set` / `unset` is CrawClaw acting as an MCP client-side
  registry for other MCP servers its runtimes may consume later

Use [`crawclaw acp`](/cli/acp) when CrawClaw should host a coding harness
session itself and route that runtime through ACP.

## CrawClaw as an MCP server

This is the `crawclaw mcp serve` path.

## When to use `serve`

Use `crawclaw mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to
  CrawClaw-backed channel conversations
- you already have a local or remote CrawClaw Gateway with routed sessions
- you want one MCP server that works across CrawClaw's channel backends instead
  of running separate per-channel bridges

Use [`crawclaw acp`](/cli/acp) instead when CrawClaw should host the coding
runtime itself and keep the agent session inside CrawClaw.

## How it works

`crawclaw mcp serve` starts a stdio MCP server. The MCP client owns that
process. While the client keeps the stdio session open, the bridge connects to a
local or remote CrawClaw Gateway over WebSocket and exposes routed channel
conversations over MCP.

Lifecycle:

1. the MCP client spawns `crawclaw mcp serve`
...
conversations. A conversation appears when CrawClaw already has session state
...
# Remote Gateway
crawclaw mcp serve --url wss://gateway-host:18789 --token-file ~/.crawclaw/gateway.token

# Remote Gateway with password auth
crawclaw mcp serve --url wss://gateway-host:18789 --password-file ~/.crawclaw/gateway.password

# Enable verbose bridge logs
crawclaw mcp serve --verbose

# Disable Claude-specific push notifications
crawclaw mcp serve --claude-channel-mode off
```
...
CrawClaw equivalent of a Claude Code channel adapter: standard MCP tools remain
...
- sender allowlists, pairing, and channel-level trust still belong to the
  underlying CrawClaw channel configuration
...
## CrawClaw as an MCP client registry

This is the `crawclaw mcp list`, `show`, `set`, and `unset` path.

These commands do not expose CrawClaw over MCP. They manage CrawClaw-owned MCP
server definitions under `mcp.servers` in CrawClaw config.

Those saved definitions are for runtimes that CrawClaw launches or configures
later, such as embedded Pi and other runtime adapters. CrawClaw stores the
definitions centrally so those runtimes do not need to keep their own duplicate
MCP server lists.

Important behavior:

- these commands only read or write CrawClaw config
...
CrawClaw also stores a lightweight MCP server registry in config for surfaces
that want CrawClaw-managed MCP definitions.

Commands:

- `crawclaw mcp list`
- `crawclaw mcp show [name]`
- `crawclaw mcp set <name> <json>`
- `crawclaw mcp unset <name>`

Examples:

```bash
crawclaw mcp list
crawclaw mcp show context7 --json
crawclaw mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
crawclaw mcp set docs '{"url":"https://mcp.example.com"}'
crawclaw mcp unset context7
```
