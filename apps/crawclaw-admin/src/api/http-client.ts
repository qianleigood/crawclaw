...
export class ApiClient {
  ...
  private resolveRpcErrorMessage(value: unknown): string {
    if (!value || typeof value !== 'object') {return 'RPC call failed'}
  ...
  async rpc<T = unknown>(method: string, params?: unknown): Promise<T> {
    ...
    if (!result.ok) {
      throw new Error(this.resolveRpcErrorMessage(result))
    }
