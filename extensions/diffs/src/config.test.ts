...
function compileManifestConfigSchema() {
  const manifest = JSON.parse(
    fs.readFileSync(new URL("../crawclaw.plugin.json", import.meta.url), "utf8"),
...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/crawclaw/",
      }),
    ).toBe("https://example.com/crawclaw");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/crawclaw?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/crawclaw#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/crawclaw/" })).toBe(true);
  });
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/crawclaw",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/crawclaw/plugins/diffs/view/id/token");
  });
...
describe("renderDiffDocument", () => {
  it("renders before/after input into a complete viewer document", async () => {
    ...
    expect(rendered.html).toContain("data-crawclaw-diff-root");
  ...
  it("resolves viewer assets under an optional base path", async () => {
    ...
    expect(
      new URL(loaderSrc ?? "", "https://example.com/crawclaw/plugins/diffs/view/id/token").pathname,
    ).toBe("/crawclaw/plugins/diffs/assets/viewer.js");
  });
  ...
  it("downgrades invalid language hints to plain text", async () => {
    ...
    const payloads = [...html.matchAll(/data-crawclaw-diff-payload>(.*?)<\/script>/g)].map(
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("crawclawDiffsReady");
