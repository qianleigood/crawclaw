...
export DISPLAY=:1
export HOME=/tmp/crawclaw-home
export XDG_CONFIG_HOME="${HOME}/.config"
export XDG_CACHE_HOME="${HOME}/.cache"

CDP_PORT="${CRAWCLAW_BROWSER_CDP_PORT:-9222}"
CDP_SOURCE_RANGE="${CRAWCLAW_BROWSER_CDP_SOURCE_RANGE:-}"
VNC_PORT="${CRAWCLAW_BROWSER_VNC_PORT:-5900}"
NOVNC_PORT="${CRAWCLAW_BROWSER_NOVNC_PORT:-6080}"
PINCHTAB_PORT="${CRAWCLAW_BROWSER_PINCHTAB_PORT:-9867}"
ENABLE_PINCHTAB="${CRAWCLAW_BROWSER_ENABLE_PINCHTAB:-0}"
PINCHTAB_TOKEN="${CRAWCLAW_BROWSER_PINCHTAB_TOKEN:-}"
ENABLE_NOVNC="${CRAWCLAW_BROWSER_ENABLE_NOVNC:-1}"
HEADLESS="${CRAWCLAW_BROWSER_HEADLESS:-0}"
ALLOW_NO_SANDBOX="${CRAWCLAW_BROWSER_NO_SANDBOX:-0}"
NOVNC_PASSWORD="${CRAWCLAW_BROWSER_NOVNC_PASSWORD:-}"
DISABLE_GRAPHICS_FLAGS="${CRAWCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS:-1}"
DISABLE_EXTENSIONS="${CRAWCLAW_BROWSER_DISABLE_EXTENSIONS:-1}"
RENDERER_PROCESS_LIMIT="${CRAWCLAW_BROWSER_RENDERER_PROCESS_LIMIT:-2}"
...
if [[ "${ENABLE_PINCHTAB}" == "1" ]] && command -v pinchtab >/dev/null 2>&1; then
  ...
  pinchtab &
elif [[ "${ENABLE_PINCHTAB}" == "1" ]]; then
  echo "PinchTab requested but binary not present in sandbox image; continuing without PinchTab" >&2
fi
