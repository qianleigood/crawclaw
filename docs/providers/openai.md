...
OpenAI API usage. CrawClaw forwards both through the `openai/*` Responses path.
CrawClaw intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
...
For `openai/*` (Responses API), CrawClaw also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. CrawClaw enables it by default for
...
CrawClaw, set `agents.defaults.models["<provider>/<model>"].params.serviceTier`
...
CrawClaw forwards `params.serviceTier` to both direct `openai/*` Responses
...
- if you route either provider through another base URL or proxy, CrawClaw leaves `service_tier` untouched

### OpenAI fast mode

CrawClaw exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, CrawClaw maps it to OpenAI priority processing:
...
`baseUrl` on `api.openai.com`), CrawClaw now auto-enables OpenAI server-side
