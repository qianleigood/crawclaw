...
const fixedWindowRequest = resolveTimelineRequest(
  { request: {}, manifest: {} },
  {
...
assert(
  fixedWindowRequest.fixedWindowMode,
  "fixed-window mode should activate when fallback duration is known",
);
assert(
  fixedWindowRequest.targetStartPct === 30,
  "fixed-window target start should honor requested start",
);
assert(
  fixedWindowRequest.targetEndPct === 60,
  "fixed-window target end should be derived from current window duration",
);
