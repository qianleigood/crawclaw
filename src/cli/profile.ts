...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.crawclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  if (!env.CRAWCLAW_CONFIG_PATH?.trim()) {
    env.CRAWCLAW_CONFIG_PATH = configPath;
  }
  env.CRAWCLAW_CONFIG_PATH = env.CRAWCLAW_CONFIG_PATH?.trim() || configPath;

  if (profile === "dev") {
    const gatewayPort = env.CRAWCLAW_GATEWAY_PORT?.trim() || "19001";
    env.CRAWCLAW_GATEWAY_PORT = env.CRAWCLAW_GATEWAY_PORT?.trim() || gatewayPort;
  }
}
