#!/usr/bin/env bash
# Rootless CrawClaw in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-crawclaw-podman.sh launch        # Start gateway
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      CRAWCLAW_GATEWAY_TOKEN|CRAWCLAW_PODMAN_CONTAINER|CRAWCLAW_PODMAN_IMAGE|CRAWCLAW_IMAGE|CRAWCLAW_PODMAN_PULL|CRAWCLAW_PODMAN_GATEWAY_HOST_PORT|CRAWCLAW_GATEWAY_PORT|CRAWCLAW_PODMAN_BRIDGE_HOST_PORT|CRAWCLAW_BRIDGE_PORT|CRAWCLAW_GATEWAY_BIND|CRAWCLAW_PODMAN_USERNS|CRAWCLAW_BIND_MOUNT_OPTIONS|CRAWCLAW_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-crawclaw-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${CRAWCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.crawclaw}"
ENV_FILE="${CRAWCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${CRAWCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.crawclaw}"
ENV_FILE="${CRAWCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${CRAWCLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${CRAWCLAW_PODMAN_CONTAINER:-crawclaw}"
CRAWCLAW_IMAGE="${CRAWCLAW_PODMAN_IMAGE:-${CRAWCLAW_IMAGE:-crawclaw:local}}"
PODMAN_PULL="${CRAWCLAW_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${CRAWCLAW_PODMAN_GATEWAY_HOST_PORT:-${CRAWCLAW_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${CRAWCLAW_PODMAN_BRIDGE_HOST_PORT:-${CRAWCLAW_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${CRAWCLAW_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$CRAWCLAW_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  CRAWCLAW_CONTAINER="" CRAWCLAW_CONFIG_DIR="$config_dir" \
    crawclaw config get gateway.bind 2>/dev/null || true
}
...
# CRAWCLAW_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${CRAWCLAW_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'CRAWCLAW_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
if [[ -z "${CRAWCLAW_GATEWAY_TOKEN:-}" ]]; then
  export CRAWCLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "CRAWCLAW_GATEWAY_TOKEN" "$CRAWCLAW_GATEWAY_TOKEN"
  echo "Generated CRAWCLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/crawclaw.json"
...
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported CRAWCLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (CRAWCLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${CRAWCLAW_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${CRAWCLAW_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$CRAWCLAW_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.crawclaw/.npm \
    -e CRAWCLAW_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.crawclaw:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.crawclaw/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$CRAWCLAW_IMAGE" \
