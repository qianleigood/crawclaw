...
IMAGE_NAME="${CRAWCLAW_IMAGE:-${CRAWCLAW_IMAGE:-crawclaw-mcp-channels-e2e}}"
PORT="18789"
TOKEN="mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="crawclaw-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t crawclaw-mcp-client-log.XXXXXX)"
...
docker run --rm \
  ...
  -e "GW_TOKEN=$TOKEN" \
  -e "CRAWCLAW_ALLOW_INSECURE_PRIVATE_WS=1" \
  "$IMAGE_NAME" \
