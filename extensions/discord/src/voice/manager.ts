...
import { agentCommandFromIngress } from "crawclaw/plugin-sdk/agent-runtime";
import { resolveTtsConfig, type ResolvedTtsConfig } from "crawclaw/plugin-sdk/agent-runtime";
import type { CrawClawConfig } from "crawclaw/plugin-sdk/config-runtime";
import type { DiscordAccountConfig, TtsConfig } from "crawclaw/plugin-sdk/config-runtime";
import { transcribeAudioFile } from "crawclaw/plugin-sdk/media-understanding-runtime";
...
import { parseTtsDirectives } from "crawclaw/plugin-sdk/speech";
import { textToSpeech } from "crawclaw/plugin-sdk/speech-runtime";
...
function resolveVoiceTtsConfig(params: { cfg: CrawClawConfig; override?: TtsConfig }): {
  cfg: CrawClawConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredCrawClawTmpDir(), "discord-voice-"));
