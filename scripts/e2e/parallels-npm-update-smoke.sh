...
PACKAGE_SPEC=""
JSON_OUTPUT=0
RUN_DIR="$(mktemp -d /tmp/crawclaw-parallels-npm-update.XXXXXX)"
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: crawclaw@latest
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/crawclaw-main-$CURRENT_HEAD_SHORT.tgz"
  cp "$MAIN_TGZ_DIR/$pkg" "$MAIN_TGZ_PATH"
}

write_windows_update_script() {
  WINDOWS_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/crawclaw-main-update.ps1"
...
function Invoke-Logged {
  ...
  try {
    ...
    $PSNativeCommandUseErrorActionPreference = $false
    # Merge native stderr into stdout before logging so npm/crawclaw warnings do not
...
try {
  ...
  $tgz = Join-Path $env:TEMP 'crawclaw-main-update.tgz'
  ...
  $version = Invoke-CaptureLogged 'crawclaw --version' { & $crawclaw --version }
  ...
  Invoke-Logged 'crawclaw models set' { & $crawclaw models set $ModelId }
  ...
  Invoke-Logged 'crawclaw gateway restart' { & $crawclaw gateway restart }
  Start-Sleep -Seconds 5
  Write-ProgressLog 'update.gateway-status'
  Invoke-Logged 'crawclaw gateway status' { & $crawclaw gateway status --deep --require-rpc }
  Write-ProgressLog 'update.agent-turn'
  Invoke-CaptureLogged 'crawclaw agent' { & $crawclaw agent --agent main --session-id $SessionId --message 'Reply with exact ASCII text OK only.' --json } | Out-Null
  $exitCode = $LASTEXITCODE
  if ($null -eq $exitCode) {
    $exitCode = 0
  }
...
start_server() {
  ...
  ) >/tmp/crawclaw-parallels-npm-update-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
matches = re.findall(r"CrawClaw [^\r\n]+", text)
matches = [match for match in matches if re.search(r"CrawClaw \d", match)]
...
run_windows_script_via_log() {
  ...
  runner_name="crawclaw-update-$RANDOM-$RANDOM.ps1"
  log_name="crawclaw-update-$RANDOM-$RANDOM.log"
  done_name="crawclaw-update-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/crawclaw-update-log-state.XXXXXX")"
  : >"$log_state_path"
...
run_macos_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$MACOS_VM" --current-user /usr/bin/tee /tmp/crawclaw-main-update.sh >/dev/null
set -euo pipefail
...
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/crawclaw-main-update.tgz
/opt/homebrew/bin/npm install -g /tmp/crawclaw-main-update.tgz
version="\$(/opt/homebrew/bin/crawclaw --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
/opt/homebrew/bin/crawclaw models set "$MODEL_ID"
/opt/homebrew/bin/crawclaw gateway status --deep --require-rpc
/usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /opt/homebrew/bin/crawclaw agent --agent main --session-id parallels-npm-update-macos-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$MACOS_VM" --current-user /bin/bash /tmp/crawclaw-main-update.sh
}
...
run_linux_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$LINUX_VM" /usr/bin/tee /tmp/crawclaw-main-update.sh >/dev/null
set -euo pipefail
export HOME=/root
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/crawclaw-main-update.tgz
npm install -g /tmp/crawclaw-main-update.tgz --no-fund --no-audit
version="\$(crawclaw --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
crawclaw models set "$MODEL_ID"
crawclaw agent --local --agent main --session-id parallels-npm-update-linux-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$LINUX_VM" /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /bin/bash /tmp/crawclaw-main-update.sh
}
...
if [[ -z "$PACKAGE_SPEC" ]]; then
  PACKAGE_SPEC="crawclaw@$LATEST_VERSION"
fi
