...
function collectInstalledRuntimeClosure(rootNodeModulesDir, dependencyNames) {
  const packageCache = new Map();
  const closure = new Set();
  const queue = dependencyNames.map((depName) => ({ depName, optional: false }));

  while (queue.length > 0) {
    const { depName, optional } = queue.shift();
    ...
    for (const [childName, childSpec] of Object.entries(packageJson.dependencies ?? {})) {
      void childSpec;
      queue.push({ depName: childName, optional: false });
    }
    for (const [childName, childSpec] of Object.entries(packageJson.optionalDependencies ?? {})) {
      void childSpec;
      queue.push({ depName: childName, optional: true });
    }
  }
...
function sanitizeBundledManifestForRuntimeInstall(pluginDir) {
  ...
  if (packageJson.peerDependencies?.crawclaw) {
    const nextPeerDependencies = { ...packageJson.peerDependencies };
    delete nextPeerDependencies.crawclaw;
  ...
  if (packageJson.peerDependenciesMeta?.crawclaw) {
    const nextPeerDependenciesMeta = { ...packageJson.peerDependenciesMeta };
    delete nextPeerDependenciesMeta.crawclaw;
  ...
  if (packageJson.devDependencies?.crawclaw) {
    const nextDevDependencies = { ...packageJson.devDependencies };
    delete nextDevDependencies.crawclaw;
...
function resolveRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".crawclaw-runtime-deps-stamp.json");
}
...
function stageInstalledRootRuntimeDeps(params) {
  ...
  const stagedNodeModulesDir = path.join(
    makeTempDir(
      os.tmpdir(),
      `crawclaw-runtime-deps-${sanitizeTempPrefixSegment(path.basename(pluginDir))}-`,
    ),
    "node_modules",
  );
...
function installPluginRuntimeDeps(params) {
  ...
  const tempInstallDir = makeTempDir(
    os.tmpdir(),
    `crawclaw-runtime-deps-${sanitizeTempPrefixSegment(pluginId)}-`,
  );
