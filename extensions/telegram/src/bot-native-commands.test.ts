...
import { TELEGRAM_COMMAND_NAME_PATTERN } from "crawclaw/plugin-sdk/config-runtime";
...
import { STATE_DIR } from "crawclaw/plugin-sdk/state-paths";
    ...
    expect(firstDeliverRepliesCall?.[0]).toEqual(
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([
          expect.stringMatching(/[\\/]\.crawclaw[\\/]workspace-work$/),
        ]),
      }),
    );
