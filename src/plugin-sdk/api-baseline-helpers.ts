...
export function renderPluginSdkApiBaselineData(params: {
  repoRoot: string;
  generatedBy: PluginSdkApiBaseline["generatedBy"];
}): {
  ...
  validateMetadata();
  const { checker, printer, program } = createCompilerContext(params.repoRoot);
...
export async function loadCurrentFile(filePath: string): Promise<string | null> {
  try {
