...
- `coding`：`group:fs`、`group:runtime`、`group:web`、`group:sessions`、`group:memory`、`image`
...
- `host=node` 可以针对 macOS 配套应用或无头节点主机（`crawclaw node run`）。
...
- 快速冒烟测试：`crawclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`。
