...
function validateIdentityAvatar(config: CrawClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: CrawClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: CrawClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = CrawClawSchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as CrawClawConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: CrawClawConfig | null | undefined;

  const ensureCompatConfig = (): CrawClawConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native CrawClaw config schema.
