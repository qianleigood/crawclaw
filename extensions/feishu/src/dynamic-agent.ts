...
export async function maybeCreateDynamicAgent(params: {
  ...
  const hasBinding = existingBindings.some(
    (b: NonNullable<CrawClawConfig["bindings"]>[number]) =>
  ...
  const existingAgent = (cfg.agents?.list ?? []).find(
    (a: NonNullable<NonNullable<CrawClawConfig["agents"]>["list"]>[number]) => a.id === agentId,
  );
  if (existingAgent) {
    // Agent exists but binding doesn't - just add the binding
    log(`feishu: agent "${agentId}" exists, adding missing binding for ${senderOpenId}`);

    const updatedCfg: CrawClawConfig = {
      ...cfg,
      bindings: [
        ...existingBindings,
        {
  ...
  const workspaceTemplate = dynamicCfg.workspaceTemplate ?? "~/.crawclaw/workspace-{agentId}";
  const agentDirTemplate = dynamicCfg.agentDirTemplate ?? "~/.crawclaw/agents/{agentId}/agent";
