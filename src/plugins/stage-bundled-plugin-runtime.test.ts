...
describe("stageBundledPluginRuntime", () => {
  ...
  it("keeps plugin command registration on the canonical dist graph when loaded from dist-runtime", async () => {
    const repoRoot = makeRepoRoot("crawclaw-stage-bundled-runtime-commands-");
    ...
    fs.writeFileSync(
      ...
      [
        "const registry = globalThis.__crawclawTestPluginCommands ??= new Map();",
  ...
  it("preserves package metadata needed for bundled plugin discovery from dist-runtime", () => {
    const repoRoot = makeRepoRoot("crawclaw-stage-bundled-runtime-discovery-");
    ...
    const env = {
      ...process.env,
      CRAWCLAW_BUNDLED_PLUGINS_DIR: runtimeExtensionsDir,
    };
  ...
  it("removes stale runtime plugin directories that are no longer in dist", () => {
    const repoRoot = makeRepoRoot("crawclaw-stage-bundled-runtime-stale-");
  ...
  it("tolerates EEXIST when an identical runtime symlink is materialized concurrently", () => {
    const repoRoot = makeRepoRoot("crawclaw-stage-bundled-runtime-eexist-");
