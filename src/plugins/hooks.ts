...
import type {
  ...
  PluginHookBeforeSkillsPromptBuildEvent,
  PluginHookBeforeSkillsPromptBuildResult,
  PluginHookDiscoverSkillsForStepEvent,
...
export function createHookRunner(registry: PluginRegistry, options: HookRunnerOptions = {}) {
  ...
  const mergeBeforeSkillsPromptBuild = (
    acc: PluginHookBeforeSkillsPromptBuildResult | undefined,
  ...
  const mergeDiscoverSkillsForStep = (
    acc: PluginHookDiscoverSkillsForStepResult | undefined,
    next: PluginHookDiscoverSkillsForStepResult,
  ): PluginHookDiscoverSkillsForStepResult => ({
    discoveredSkillNames: [
      ...(acc?.discoveredSkillNames ?? []),
      ...(next.discoveredSkillNames ?? []),
    ].filter((name, index, list) => Boolean(name) && list.indexOf(name) === index),
    reason: firstDefined(acc?.reason, next.reason),
    confidence: firstDefined(acc?.confidence, next.confidence),
  });
  ...
  const mergeBeforePromptBuild = (
    ...
    queryContextPatch:
      acc?.queryContextPatch || next.queryContextPatch
        ? compactPluginQueryContextPatch({
            replaceUserPrompt: firstDefined(
              acc?.queryContextPatch?.replaceUserPrompt,
              next.queryContextPatch?.replaceUserPrompt,
            ),
            clearSystemContextSections:
              acc?.queryContextPatch?.clearSystemContextSections === true ||
              next.queryContextPatch?.clearSystemContextSections === true,
            replaceSystemPromptSections: firstDefined(
              acc?.queryContextPatch?.replaceSystemPromptSections,
              next.queryContextPatch?.replaceSystemPromptSections,
            ),
            prependUserContextSections: [
              ...(acc?.queryContextPatch?.prependUserContextSections ?? []),
              ...(next.queryContextPatch?.prependUserContextSections ?? []),
            ],
            appendUserContextSections: [
              ...(acc?.queryContextPatch?.appendUserContextSections ?? []),
              ...(next.queryContextPatch?.appendUserContextSections ?? []),
            ],
            prependSystemContextSections: [
              ...(acc?.queryContextPatch?.prependSystemContextSections ?? []),
              ...(next.queryContextPatch?.prependSystemContextSections ?? []),
            ],
            appendSystemContextSections: [
              ...(acc?.queryContextPatch?.appendSystemContextSections ?? []),
              ...(next.queryContextPatch?.appendSystemContextSections ?? []),
            ],
          })
        : undefined,
  });

  const compactPluginQueryContextPatch = (
    patch: NonNullable<PluginHookBeforePromptBuildResult["queryContextPatch"]>,
  ): NonNullable<PluginHookBeforePromptBuildResult["queryContextPatch"]> => ({
  ...
  async function runBeforeSkillsPromptBuild(
    event: PluginHookBeforeSkillsPromptBuildEvent,
    ctx: PluginHookAgentContext,
  ): Promise<PluginHookBeforeSkillsPromptBuildResult | undefined> {
    return runModifyingHook<"before_skills_prompt_build", PluginHookBeforeSkillsPromptBuildResult>(
      "before_skills_prompt_build",
      event,
      ctx,
      {
        mergeResults: mergeBeforeSkillsPromptBuild,
      },
    );
  }

  async function runDiscoverSkillsForStep(
    event: PluginHookDiscoverSkillsForStepEvent,
    ctx: PluginHookAgentContext,
  ): Promise<PluginHookDiscoverSkillsForStepResult | undefined> {
    return runModifyingHook<"discover_skills_for_step", PluginHookDiscoverSkillsForStepResult>(
      "discover_skills_for_step",
      event,
      ctx,
      {
        mergeResults: mergeDiscoverSkillsForStep,
      },
    );
  }
