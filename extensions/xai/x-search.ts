...
import { getRuntimeConfigSnapshot } from "crawclaw/plugin-sdk/config-runtime";
...
const X_SEARCH_CACHE_KEY = Symbol.for("crawclaw.xai.x-search.cache");
...
function readLegacyGrokApiKey(cfg?: CrawClawConfig): string | undefined {
...
function readPluginXaiWebSearchApiKey(cfg?: CrawClawConfig): string | undefined {
  return readConfiguredSecretString(
...
function resolveFallbackXaiApiKey(cfg?: CrawClawConfig): string | undefined {
  return readPluginXaiWebSearchApiKey(cfg) ?? readLegacyGrokApiKey(cfg);
}

function resolveXSearchConfig(cfg?: CrawClawConfig): Record<string, unknown> | undefined {
...
export function createXSearchTool(options?: {
  ...
  runtimeConfig?: CrawClawConfig | null;
}) {
