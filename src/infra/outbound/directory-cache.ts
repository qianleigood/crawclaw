...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: CrawClawConfig | null = null;
  ...
  get(key: string, cfg: CrawClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: CrawClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: CrawClawConfig): void {
