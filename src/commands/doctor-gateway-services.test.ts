...
async function runRepair(cfg: CrawClawConfig) {
    ...
    issues: [
      {
        ...
        message: "Gateway service CRAWCLAW_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    fsMocks.realpath.mockImplementation(async (value: string) => value);
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: CrawClawConfig, env) => {
      const configToken =
        typeof cfg.gateway?.auth?.token === "string" ? cfg.gateway.auth.token.trim() : undefined;
      const envToken = env.CRAWCLAW_GATEWAY_TOKEN?.trim() || undefined;
  ...
  it("uses CRAWCLAW_GATEWAY_TOKEN when config token is missing", async () => {
      ...
      realpath: async (value: string) => {
        if (value.includes("/global/5/node_modules/crawclaw/")) {
          return value.replace(
            "/global/5/node_modules/crawclaw/",
            "/global/5/node_modules/.pnpm/crawclaw@2026.3.12/node_modules/crawclaw/",
          );
        }
        return value;
      },
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            CRAWCLAW_GATEWAY_TOKEN: "env-file-token",
          },
