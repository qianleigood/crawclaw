...
function parseInstallSpec(input: unknown): HookInstallSpec | undefined {
  const parsed = parseCrawClawManifestInstallBase(input, ["bundled", "npm", "git"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyCrawClawManifestInstallCommonFields<HookInstallSpec>(
    {
...
): CrawClawHookMetadata | undefined {
