...
import { danger, logVerbose, shouldLogVerbose, warn } from "crawclaw/plugin-sdk/runtime-env";
 ...
 *   exec ssh -T crawclaw@192.168.64.3 /opt/homebrew/bin/imsg "$@"
...
async function detectRemoteHostFromCliPath(cliPath: string): Promise<string | undefined> {
  try {
    ...
    // Match user@host pattern first (e.g., crawclaw@192.168.64.3)
