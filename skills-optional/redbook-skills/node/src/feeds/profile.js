...
async function humanScrollPage(page, remaining, pauseSeconds = 1.2) {
  ...
  await page.evaluate(
    ...
    },
    {
      ...
      pauseMs: Math.max(200, Math.floor(Number(pauseSeconds || 1.2) * 1000)),
    },
  );
}

async function getMyProfileFeeds(
  page,
  {
    profileUrl = "",
  ...
  if (
    ["请求太频繁，请稍后再试", "请求太频繁", "验证", "captcha"].includes(
      String(profilePageIssue || "").trim(),
    )
  ) {
    throw new XhsCliError(
      `Current account profile page is rate-limited or blocked: ${String(profilePageIssue).trim()}`,
      {
  ...
  for (let roundIndex = 0; roundIndex < rounds; roundIndex += 1) {
    const rawCards = await page.evaluate(() => {
      ...
      return cards
        .map((section, index) => {
          const profileLink =
            Array.from(section.querySelectorAll('a[href*="/user/profile/"]')).find((a) =>
              (a.href || "").includes("xsec_token="),
            ) || null;
          const exploreLink =
            Array.from(section.querySelectorAll('a[href*="/explore/"]'))[0] || null;
    ...
    if (Array.isArray(rawCards)) {
      for (const card of rawCards) {
        ...
        const xsecToken = profileNoteUrl
    ...
    const currentBottom =
      Number(scrollMetrics.scrollY || 0) + Number(scrollMetrics.innerHeight || 0);
