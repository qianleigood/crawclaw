...
export async function dispatchPreparedSlackMessage(prepared: PreparedSlackMessage) {
  ...
  const streamingDecision = resolveSlackStreamingDecision({
    mode: slackStreaming.mode,
    nativeStreaming: slackStreaming.nativeStreaming,
    isDirectMessage: prepared.isDirectMessage,
    threadTs: streamThreadHint,
  ...
  runtime.log?.(
    `slack[${account.accountId}] streaming ${formatChannelStreamingDecision(streamingDecision)}`,
  );
  if (isDiagnosticsEnabled(cfg)) {
    emitDiagnosticEvent({
      ...
      enabled: streamingDecision.enabled,
      surface: streamingDecision.surface,
      reason: streamingDecision.reason,
    });
  }
  if (route.sessionKey) {
