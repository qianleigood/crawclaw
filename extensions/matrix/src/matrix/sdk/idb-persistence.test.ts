...
describe("Matrix IndexedDB persistence", () => {
  ...
  it("persists and restores database contents for the selected prefix", async () => {
    ...
    await seedDatabase({
      name: "crawclaw-matrix-test::matrix-sdk-crypto",
      storeName: "sessions",
    ...
    await persistIdbToDisk({
      snapshotPath,
      databasePrefix: "crawclaw-matrix-test",
    });
  ...
  it("serializes concurrent persist operations via file lock", async () => {
    ...
    await Promise.all([
      persistIdbToDisk({ snapshotPath, databasePrefix: "crawclaw-matrix-test" }),
      persistIdbToDisk({ snapshotPath, databasePrefix: "crawclaw-matrix-test" }),
    ]);
  ...
  it("releases lock after persist completes", async () => {
    ...
    await persistIdbToDisk({ snapshotPath, databasePrefix: "crawclaw-matrix-test" });
