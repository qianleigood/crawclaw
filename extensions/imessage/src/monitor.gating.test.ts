...
function baseCfg(): CrawClawConfig {
  return {
    channels: {
      imessage: {
        dmPolicy: "open",
...
function buildDispatchContextPayload(params: { cfg: CrawClawConfig; message: IMessagePayload }) {
...
describe("imessage monitor gating + envelope builders", () => {
  ...
  it("blocks group messages when imessage.groups is set without a wildcard", () => {
    ...
    const decision = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@crawclaw hello",
      bodyText: "@crawclaw hello",
      allowFrom: ["*"],
      groupAllowFrom: [],
      groupPolicy: "open",
      dmPolicy: "open",
  ...
  it("honors group allowlist and ignores pairing-store senders in groups", () => {
    ...
    const denied = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@crawclaw hi",
      bodyText: "@crawclaw hi",
      allowFrom: ["*"],
    ...
    const allowed = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@crawclaw ok",
      bodyText: "@crawclaw ok",
      allowFrom: ["*"],
      groupAllowFrom: ["chat_id:101"],
      groupPolicy: "allowlist",
      dmPolicy: "pairing",
