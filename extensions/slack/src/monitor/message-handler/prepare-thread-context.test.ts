...
import { resolveEnvelopeFormatOptions } from "crawclaw/plugin-sdk/channel-inbound";
...
describe("resolveSlackThreadContextData", () => {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-slack-thread-context-"));
  });
  ...
  it("omits non-allowlisted starter text and thread history messages", async () => {
    ...
    const result = await resolveSlackThreadContextData({
      ...
      envelopeOptions: resolveEnvelopeFormatOptions({} as CrawClawConfig),
