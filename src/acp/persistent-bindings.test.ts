...
type ConfiguredBinding = NonNullable<CrawClawConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<CrawClawConfig>,
): CrawClawConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: CrawClawConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/crawclaw" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
