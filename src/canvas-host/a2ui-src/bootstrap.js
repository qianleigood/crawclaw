...
import "@a2ui/lit/ui";
import { themeContext } from "@crawclaw/a2ui-theme-context";
...
class CrawClawA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.crawclawA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["crawclaw:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.crawclawCanvasA2UIAction ??
      globalThis.crawclawCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        if (handler === globalThis.crawclawCanvasA2UIAction) {
...
if (!customElements.get("crawclaw-a2ui-host")) {
  customElements.define("crawclaw-a2ui-host", CrawClawA2UIHost);
}
