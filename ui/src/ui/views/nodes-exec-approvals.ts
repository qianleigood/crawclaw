...
const SECURITY_OPTIONS: Array<{ value: ExecSecurity; label: string }> = [
  { value: "deny", label: uiLiteral("Deny") },
  { value: "allowlist", label: uiLiteral("Allowlist") },
  { value: "full", label: uiLiteral("Full") },
];

const ASK_OPTIONS: Array<{ value: ExecAsk; label: string }> = [
  { value: "off", label: uiLiteral("Off") },
  { value: "on-miss", label: uiLiteral("On miss") },
  { value: "always", label: uiLiteral("Always") },
];
  ...
  return html`
    <div class="list" style="margin-top: 12px;">
      ...
      ${state.target === "node" && !hasNodes
        ? html` <div class="muted">${uiLiteral("No nodes advertise exec approvals yet.")}</div> `
        : nothing}
    </div>
  `;
}
...
function renderExecApprovalsTabs(state: ExecApprovalsState) {
  return html`
    <div class="row" style="margin-top: 12px; gap: 8px; flex-wrap: wrap;">
      <span class="label">${uiLiteral("Scope")}</span>
...
function renderExecApprovalsPolicy(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 16px;">
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${securityValue === "__default__"}>
                    ${uiLiteral("Use default")} (${uiLiteral(defaults.security)})
                  </option>`
                : nothing}
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${askValue === "__default__"}>
                    ${uiLiteral("Use default")} (${uiLiteral(defaults.ask)})
                  </option>`
                : nothing}
              ${ASK_OPTIONS.map(
                (option) =>
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${uiLiteral("Ask fallback")}</div>
          <div class="list-sub">
            ${isDefaults
              ? uiLiteral("Applied when the UI prompt is unavailable.")
              : `${uiLiteral("Default")}: ${uiLiteral(defaults.askFallback)}.`}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${uiLiteral("Fallback")}</span>
            <select
              ?disabled=${state.disabled}
              @change=${(event: Event) => {
                const target = event.target as HTMLSelectElement;
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${askFallbackValue === "__default__"}>
                    ${uiLiteral("Use default")} (${uiLiteral(defaults.askFallback)})
                  </option>`
                : nothing}
              ${SECURITY_OPTIONS.map(
                (option) =>
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${uiLiteral("Auto-allow skill CLIs")}</div>
          <div class="list-sub">
            ${isDefaults
              ? uiLiteral("Allow skill executables listed by the Gateway.")
              : autoIsDefault
                ? `${uiLiteral("Using default")} (${uiLiteral(defaults.autoAllowSkills ? "on" : "off")}).`
                : `${uiLiteral("Override")} (${uiLiteral(autoEffective ? "on" : "off")}).`}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${uiLiteral("Enabled")}</span>
            <input
              type="checkbox"
              ?disabled=${state.disabled}
...
function renderExecApprovalsAllowlist(state: ExecApprovalsState) {
  ...
  return html`
    <div class="row" style="margin-top: 18px; justify-content: space-between;">
      <div>
        <div class="card-title">${uiLiteral("Allowlist")}</div>
        <div class="card-sub">${uiLiteral("Case-insensitive glob patterns.")}</div>
      </div>
      <button
        class="btn btn--sm"
        ?disabled=${state.disabled}
      ...
      >
        ${uiLiteral("Add pattern")}
      </button>
    </div>
    <div class="list" style="margin-top: 12px;">
      ${entries.length === 0
        ? html` <div class="muted">${uiLiteral("No allowlist entries yet.")}</div> `
