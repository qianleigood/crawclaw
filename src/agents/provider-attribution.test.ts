...
describe("provider attribution", () => {
  it("resolves the canonical CrawClaw product and runtime version", () => {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { CRAWCLAW_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "crawclaw",
        version: "2026.3.22",
        "User-Agent": "crawclaw/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { CRAWCLAW_VERSION: "2026.3.22" })).toEqual({
      originator: "crawclaw",
      version: "2026.3.22",
      "User-Agent": "crawclaw/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { CRAWCLAW_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ CRAWCLAW_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
