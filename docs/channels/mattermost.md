---
summary: "Mattermost bot setup and CrawClaw config"
read_when:
...
```bash
crawclaw plugins install @crawclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
crawclaw plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure CrawClaw and start the gateway.
...
Native slash commands are opt-in. When enabled, CrawClaw registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, CrawClaw derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as CrawClaw.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to CrawClaw.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from CrawClaw, not `404`.
...
- Approve via:
  - `crawclaw pairing list mattermost`
  - `crawclaw pairing approve mattermost <CODE>`
...
Use these target formats with `crawclaw message send` or cron/webhooks:
...
CrawClaw resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), CrawClaw sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When CrawClaw sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, CrawClaw derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and CrawClaw run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="crawclaw-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"crawclaw-mattermost-interactions",
...
`crawclaw message send` and cron/webhook deliveries.
