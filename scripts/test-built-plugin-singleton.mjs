...
const warningFilterKey = Symbol.for("crawclaw.warning-filter");
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@crawclaw/build-smoke-plugin",
      type: "module",
      crawclaw: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "crawclaw.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'crawclaw/plugin-sdk';",
...
const registry = loadCrawClawPlugins({
  ...
  env: {
    ...process.env,
    CRAWCLAW_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
