...
import { normalizeGoogleModelId } from "../generated/providers/model-normalization.generated.js";
...
const PROVIDERS = parseFilter(process.env.CRAWCLAW_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.CRAWCLAW_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.CRAWCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.CRAWCLAW_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.CRAWCLAW_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["CRAWCLAW_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.CRAWCLAW_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.CRAWCLAW_LIVE_MAX_MODELS, 0));
}
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.CRAWCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.CRAWCLAW_LIVE_MODEL_TIMEOUT_MS,
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<CrawClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    ...
    stateDir: process.env.CRAWCLAW_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveCrawClawAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-live-state-"));
  process.env.CRAWCLAW_STATE_DIR = tempStateDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.crawclaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveCrawClawAgentDir();
  const sanitizedCfg: CrawClawConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-live-"));
  const tempConfigPath = path.join(tempDir, "crawclaw.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.CRAWCLAW_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            (async () => {
              ...
              for (
                ...
                toolText = await requestGatewayAgentText({
                  ...
                  modelKey,
                  message: strictReply
                    ? "CrawClaw live tool probe (local, safe): " +
    ...
    process.env.CRAWCLAW_CONFIG_PATH = previous.configPath;
    process.env.CRAWCLAW_GATEWAY_TOKEN = previous.token;
    ...
    process.env.CRAWCLAW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        const rawModels = process.env.CRAWCLAW_LIVE_GATEWAY_MODELS?.trim();
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via CRAWCLAW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const authStorage = discoverAuthStorage(agentDir);
    const modelRegistry = discoverModels(authStorage, agentDir);
    const anthropic = modelRegistry.find("anthropic", "claude-opus-4-6");
    const zai = modelRegistry.find("zai", "glm-4.7");
