...
import type { AgentTaskMetadata, TaskDeliveryState, TaskRecord } from "./task-registry.types.js";
...
function rowToTaskRecord(row: TaskRegistryRow): TaskRecord {
  ...
  const agentMetadata = parseJsonValue<AgentTaskMetadata>(row.agent_metadata_json);
  return {
    taskId: row.task_id,
    runtime: row.runtime,
...
function bindTaskRecord(record: TaskRecord) {
  return {
    ...
    agent_metadata_json: serializeJson(record.agentMetadata),
...
function ensureSchema(db: DatabaseSync) {
  ...
  if (!hasTaskRunsColumn(db, "agent_metadata_json")) {
    db.exec(`ALTER TABLE task_runs ADD COLUMN agent_metadata_json TEXT;`);
  }
  db.exec(`
