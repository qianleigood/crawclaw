...
function resolveSandboxSession(params: { config?: CrawClawConfig; sessionKey?: string }) {
  ...
  const browser =
    resolvedCfg.browser.enabled && backend.capabilities?.browser === true
      ? await ensureSandboxBrowser({
          ...
          bridgeAuth,
          pinchTab:
            params.config?.browser?.provider === "pinchtab"
              ? { enabled: true, token: bridgeAuth?.token }
              : undefined,
        })
      : null;
