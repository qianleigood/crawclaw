...
are allowed to join. UIs and other clients are just frontends that approve or
...
crawclaw nodes reject <requestId>
crawclaw nodes status
crawclaw nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
## Auto-approval (managed clients)

A managed client can optionally attempt a **silent approval** when:

- the request is marked `silent`, and
- the client can verify an SSH connection to the gateway host using the same user.

If silent approval fails, it falls back to the normal “Approve/Reject” prompt.

## Storage (local, private)

Pairing state is stored under the Gateway state directory (default `~/.crawclaw`):

- `~/.crawclaw/nodes/paired.json`
- `~/.crawclaw/nodes/pending.json`

If you override `CRAWCLAW_STATE_DIR`, the `nodes/` folder moves with it.
