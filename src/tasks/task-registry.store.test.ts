...
describe("task-registry store runtime", () => {
  ...
  it("persists agent metadata with task rows", () => {
    const created = createTaskRecord({
      runtime: "subagent",
      ownerKey: "agent:main:main",
      scopeKind: "session",
      ...
      agentMetadata: {
        ...
        capabilitySnapshotRef: "agents/child/capabilities.json",
        spawnSource: "sessions_spawn",
      },
      runId: "run-agent-metadata",
      task: "Persist agent metadata",
      status: "running",
      deliveryStatus: "pending",
    });
    ...
    expect(findTaskByRunId("run-agent-metadata")).toMatchObject({
      taskId: created.taskId,
      agentId: "agent-child",
      agentMetadata: {
        parentAgentId: "agent-parent",
  ...
  it("hardens the sqlite task store directory and file modes", () => {
    ...
    const stateDir = mkdtempSync(path.join(os.tmpdir(), "crawclaw-task-store-"));
  ...
  it("migrates legacy ownerless cron rows to system scope", () => {
    const stateDir = mkdtempSync(path.join(os.tmpdir(), "crawclaw-task-store-legacy-"));
    process.env.CRAWCLAW_STATE_DIR = stateDir;
    const sqlitePath = resolveTaskRegistrySqlitePath(process.env);
