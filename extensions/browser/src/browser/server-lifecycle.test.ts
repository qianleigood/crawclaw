...
const { stopCrawClawChromeMock } = vi.hoisted(() => ({
  stopCrawClawChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopCrawClawChrome: stopCrawClawChromeMock,
}));
...
beforeEach(() => {
  ...
  stopCrawClawChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["crawclaw", "user"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      crawclaw: vi.fn(async () => {}),
      user: vi.fn(async () => {
    ...
    expect(stopMap.crawclaw).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopCrawClawChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("crawclaw browser stop failed: Error: oops");
  });
});
