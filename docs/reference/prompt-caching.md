...
- With Anthropic API-key auth profiles, CrawClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, CrawClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
CrawClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
- `CRAWCLAW_CACHE_TRACE=1` enables cache tracing.
- `CRAWCLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `CRAWCLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `CRAWCLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `CRAWCLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
