...
import type {
  RegisteredExecApprovalRequestContext,
...
vi.mock("./bash-tools.exec-host-shared.js", () => ({
  ...
  buildExecApprovalUnavailableToolResult: buildExecApprovalUnavailableToolResultMock,
  ...
  mergeExecApprovalWarningText: mergeExecApprovalWarningTextMock,
  resolveApprovalDecisionOrUndefined: vi.fn(async () => undefined),
  resolveExecApprovalGuardUnavailable: resolveExecApprovalGuardUnavailableMock,
...
describe("processGatewayAllowlist", () => {
  ...
  beforeEach(() => {
    ...
    buildExecApprovalUnavailableToolResultMock.mockReset();
    buildExecApprovalFollowupTargetMock.mockReset();
    resolveExecApprovalGuardUnavailableMock.mockReset();
    mergeExecApprovalWarningTextMock.mockClear();
    buildExecApprovalFollowupTargetMock.mockReturnValue(null);
    buildExecApprovalPendingToolResultMock.mockReturnValue({
      details: { status: "approval-pending" },
      content: [],
    });
    buildExecApprovalUnavailableToolResultMock.mockReturnValue({
    ...
    } satisfies RegisteredExecApprovalRequestContext);
    resolveExecApprovalGuardUnavailableMock.mockReturnValue(null);
  });
  ...
  it("returns approval-unavailable immediately when agent guard disables interactive exec approvals", async () => {
    resolveExecApprovalGuardUnavailableMock.mockReturnValue({
      warningText:
    ...
    expect(buildExecApprovalUnavailableToolResultMock).toHaveBeenCalledWith(
      expect.objectContaining({
        host: "gateway",
        command: "echo ok",
        unavailableReason: "background",
    ...
    expect(result.pendingResult?.details.status).toBe("approval-unavailable");
  });
});
