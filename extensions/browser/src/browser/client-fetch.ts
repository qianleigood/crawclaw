...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-crawclaw-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-crawclaw-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-crawclaw-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
  ...
  if (looksLikeTimeout) {
    return new Error(
      appendBrowserToolModelHint(
        `Can't reach the CrawClaw browser control service (timed out after ${timeoutMs}ms). ${operatorHint}`,
      ),
    );
  }
  return new Error(
    appendBrowserToolModelHint(
      `Can't reach the CrawClaw browser control service. ${operatorHint} (${msg})`,
    ),
  );
}
...
export async function fetchBrowserJson<T>(
  ...
  try {
    ...
    throw new Error(
      "Relative browser dispatcher paths are no longer supported; use an explicit browser bridge or loopback HTTP URL.",
    );
  } catch (err) {
