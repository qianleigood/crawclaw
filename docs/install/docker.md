---
summary: "Optional Docker-based setup and onboarding for CrawClaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run CrawClaw on a host without local installs.
...
<Steps>
  ...
  <Step title="Complete onboarding">
    ...
    `crawclaw-gateway` directly. `crawclaw-cli` is for commands you run after
...
<Note>
Run `docker compose` from the repo root. If you enabled `CRAWCLAW_EXTRA_MOUNTS`
or `CRAWCLAW_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `crawclaw-cli` shares `crawclaw-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d crawclaw-gateway`, run onboarding
and setup-time config writes through `crawclaw-gateway` with
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `CRAWCLAW_IMAGE`               | Use a remote image instead of building locally                   |
| `CRAWCLAW_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `CRAWCLAW_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `CRAWCLAW_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `CRAWCLAW_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `CRAWCLAW_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `CRAWCLAW_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
`scripts/docker/setup.sh` defaults `CRAWCLAW_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `CRAWCLAW_CONFIG_DIR` to `/home/node/.crawclaw` and
`CRAWCLAW_WORKSPACE_DIR` to `/home/node/.crawclaw/workspace`, so those paths
...
and rolling file logs under `/tmp/crawclaw/`.
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export CRAWCLAW_SANDBOX=1
    export CRAWCLAW_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm crawclaw-cli gateway probe
    docker compose run -T --rm crawclaw-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `crawclaw-cli` uses `network_mode: "service:crawclaw-gateway"` so CLI
    ...
    `no-new-privileges` on `crawclaw-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.crawclaw`, make sure your host bind mounts are owned by uid 1000:
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export CRAWCLAW_HOME_VOLUME="crawclaw_home"`
    2. **Bake system deps**: `export CRAWCLAW_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `CRAWCLAW_HOME_VOLUME` or `CRAWCLAW_EXTRA_MOUNTS`.
...
<AccordionGroup>
  ...
  <Accordion title="Custom tools not found in sandbox">
    CrawClaw runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm crawclaw-cli config set gateway.mode local
    docker compose run --rm crawclaw-cli config set gateway.bind lan
    docker compose run --rm crawclaw-cli devices list --url ws://127.0.0.1:18789
    ```
