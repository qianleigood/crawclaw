...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: CrawClawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/crawclaw" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: CrawClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: CrawClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: CrawClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: CrawClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: CrawClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: CrawClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: CrawClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/crawclaw");
  });

  it("uses CRAWCLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "crawclaw-home");
    vi.stubEnv("CRAWCLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as CrawClawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".crawclaw", "workspace"));
  });

  it("uses CRAWCLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "crawclaw-home");
    vi.stubEnv("CRAWCLAW_HOME", home);
    // Clear state dir so it falls back to CRAWCLAW_HOME
    vi.stubEnv("CRAWCLAW_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as CrawClawConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".crawclaw", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/crawclaw-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/crawclaw-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-agent-scope-"));
