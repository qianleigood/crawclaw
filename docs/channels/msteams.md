...
```bash
crawclaw plugins install @crawclaw/msteams
```

Local checkout (when running from a git repo):

```bash
crawclaw plugins install ./path/to/local/msteams-plugin
```

If you choose Teams during setup and a git checkout is detected,
CrawClaw will offer the local install path automatically.
...
3. Configure CrawClaw with those credentials.
...
- Talk to CrawClaw via Teams DMs, group chats, or channels.
...
- On startup, CrawClaw resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.crawclaw/crawclaw.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring CrawClaw, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `crawclaw-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `crawclaw plugins install @crawclaw/msteams`
   - From a local checkout: `crawclaw plugins install ./path/to/local/msteams-plugin`
...
CrawClaw exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, role) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "CrawClaw in Teams", full: "CrawClaw in Teams" },
...
CrawClaw handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, CrawClaw only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/CrawClawShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

CrawClaw sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `crawclaw message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.crawclaw/msteams-polls.json`.
...
```bash
crawclaw message send --channel msteams \
...
crawclaw message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
crawclaw message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
crawclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
crawclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
