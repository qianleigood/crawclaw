...
vi.mock("./app-action-feed.ts", () => ({
  handleAgentActionEvent: vi.fn(),
  resetActionFeed: vi.fn(),
}));
  ...
  it("returns serverVersion for same-origin websocket targets", () => {
    expect(
      resolveControlUiClientVersion({
        ...
        pageUrl: "http://localhost:8787/crawclaw/",
      }),
    ).toBe("2026.3.7");
  });
  ...
  it("returns serverVersion for same-origin relative targets", () => {
    expect(
      resolveControlUiClientVersion({
        gatewayUrl: "/ws",
        serverVersion: "2026.3.7",
        pageUrl: "https://control.example.com/crawclaw/",
      }),
    ).toBe("2026.3.7");
  });
