...
describe("doctor bundled plugin runtime deps", () => {
  it("skips source checkouts", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-doctor-bundled-"));
    fs.mkdirSync(path.join(root, ".git"));
    fs.mkdirSync(path.join(root, "src"));
    fs.mkdirSync(path.join(root, "extensions"));
    writeJson(path.join(root, "dist", "extensions", "qqbot", "package.json"), {
      dependencies: {
        "dep-one": "1.0.0",
      },
    });
  ...
  it("reports missing deps and conflicts", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "crawclaw-doctor-bundled-"));
    writeJson(path.join(root, "package.json"), { name: "crawclaw" });
