...
describe("loadGatewayStartupPlugins browser plugin integration", () => {
  ...
  it("loads the bundled browser plugin without compatibility gateway/service ownership", () => {
  ...
  it("still omits compatibility gateway ownership when the bundled browser plugin is disabled", () => {
    const loaded = loadGatewayStartupPlugins({
      cfg: {
        plugins: {
          allow: ["browser"],
