...
export function isLocalCheckEnabled(env) {
  const raw = env.CRAWCLAW_LOCAL_CHECK?.trim().toLowerCase();
...
export function applyLocalTsgoPolicy(args, env) {
  ...
  if (nextEnv.CRAWCLAW_TSGO_PPROF_DIR && !hasFlag(nextArgs, "--pprofDir")) {
    insertBeforeSeparator(nextArgs, "--pprofDir", nextEnv.CRAWCLAW_TSGO_PPROF_DIR);
  }
...
export function acquireLocalHeavyCheckLockSync(params) {
  ...
  const locksDir = path.join(commonDir, "crawclaw-local-checks");
  const lockDir = path.join(locksDir, `${params.lockName ?? "heavy-check"}.lock`);
  const ownerPath = path.join(lockDir, "owner.json");
  const timeoutMs = readPositiveInt(
    env.CRAWCLAW_HEAVY_CHECK_LOCK_TIMEOUT_MS,
    DEFAULT_LOCK_TIMEOUT_MS,
  );
  const pollMs = readPositiveInt(env.CRAWCLAW_HEAVY_CHECK_LOCK_POLL_MS, DEFAULT_LOCK_POLL_MS);
  const staleLockMs = readPositiveInt(
    env.CRAWCLAW_HEAVY_CHECK_STALE_LOCK_MS,
