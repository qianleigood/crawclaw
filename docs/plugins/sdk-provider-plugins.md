...
summary: "Step-by-step guide to building a model provider plugin for CrawClaw"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to CrawClaw
...
(LLM) to CrawClaw. By the end you will have a provider with a model catalog,
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so CrawClaw can detect
    credentials without loading your plugin runtime. If you publish the
    provider on ClawHub, those `crawclaw.compat` and `crawclaw.build` fields
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "crawclaw/plugin-sdk/provider-auth";
    ...
    `crawclaw/plugin-sdk/provider-onboard`. The narrowest helpers are
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="All available provider hooks">
      CrawClaw calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    CrawClaw classifies this as a **hybrid-capability** plugin. This is the
...
├── package.json              # crawclaw.providers metadata
├── crawclaw.plugin.json      # Manifest with providerAuthEnvVars
