...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): CrawClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as CrawClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): CrawClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: CrawClawConfig, accountId = "media") {
    await handleTelegramAction(
