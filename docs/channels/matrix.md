...
Matrix is the Matrix channel plugin for CrawClaw.
It uses the official `matrix-js-sdk` and supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Plugin required

Matrix is a plugin and is not bundled with core CrawClaw.

Install from npm:

```bash
crawclaw plugins install @crawclaw/matrix
```

Install from a local checkout:

```bash
crawclaw plugins install ./path/to/local/matrix-plugin
```
...
crawclaw configure --section channels
```
...
- To resolve room names before saving them, use `crawclaw channels resolve --channel matrix "Project Room"`.
...
```json5
{
  channels: {
    matrix: {
      ...
      deviceName: "CrawClaw Gateway",
    },
  },
}
```

Matrix stores cached credentials in `~/.crawclaw/credentials/matrix/`.
...
Set `channels.matrix.streaming` to `"partial"` when you want CrawClaw to send a single draft reply,
...
- `streaming: "off"` is the default. CrawClaw waits for the final reply and sends it once.
...
- If the preview no longer fits in one Matrix event, CrawClaw stops preview streaming and falls back to normal final delivery.
- Media replies still send attachments normally. If a stale preview can no longer be reused safely, CrawClaw redacts it before sending the final media reply.
...
By default, Matrix messages from other configured CrawClaw Matrix accounts are ignored.
...
- CrawClaw still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix does not expose a native bot flag here; CrawClaw treats "bot-authored" as "sent by another configured Matrix account on this CrawClaw gateway".
...
```bash
crawclaw matrix verify status --include-recovery-key --json
```
...
```bash
crawclaw matrix verify bootstrap --verbose
```

Force a fresh cross-signing identity reset before bootstrapping:

```bash
crawclaw matrix verify bootstrap --force-reset-cross-signing
```
...
```bash
crawclaw matrix verify device "<your-recovery-key>" --verbose
```
...
```bash
crawclaw matrix verify backup restore
```

Verbose restore diagnostics:

```bash
crawclaw matrix verify backup restore --verbose
```
...
```bash
crawclaw matrix verify status --account assistant
crawclaw matrix verify backup restore --account assistant
crawclaw matrix devices list --account assistant
```
...
CrawClaw treats this Matrix device as verified only when it is verified by your own cross-signing identity.
In practice, `crawclaw matrix verify status --verbose` exposes three trust signals:
...
Local trust by itself is not enough for CrawClaw to treat the device as fully verified.

### What bootstrap does

`crawclaw matrix verify bootstrap` is the repair and setup command for encrypted Matrix accounts.
...
If the homeserver requires interactive auth to upload cross-signing keys, CrawClaw tries the upload without auth first, then with `m.login.dummy`, then with `m.login.password` when `channels.matrix.password` is configured.
...
If you intentionally want to discard the current room-key backup and start a new backup baseline for future messages, use `crawclaw matrix verify backup reset --yes`.
...
If startup finds broken bootstrap state and `channels.matrix.password` is configured, CrawClaw can attempt a stricter repair path.
If the current device is already owner-signed, CrawClaw preserves that identity instead of resetting it automatically.

Upgrading from the previous public Matrix plugin:

- CrawClaw automatically reuses the same Matrix account, access token, and device identity when possible.
- Before any actionable Matrix migration changes run, CrawClaw creates or reuses a recovery snapshot under `~/Backups/crawclaw-migrations/`.
- If you use multiple Matrix accounts, set `channels.matrix.defaultAccount` before upgrading from the old flat-store layout so CrawClaw knows which account should receive that shared legacy state.
- If the previous plugin stored a Matrix room-key backup decryption key locally, startup or `crawclaw doctor --fix` will import it into the new recovery-key flow automatically.
...
- If the Matrix access token changes later for the same account, homeserver, and user, CrawClaw now prefers reusing the most complete existing token-hash storage root instead of starting from an empty Matrix state directory.
- On the next gateway start, backed-up room keys are restored automatically into the new crypto store.
- If the old plugin had local-only room keys that were never backed up, CrawClaw will warn clearly. Those keys cannot be exported automatically from the previous rust crypto store, so some old encrypted history may remain unavailable until recovered manually.
...
`~/.crawclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/`.
...
When the token changes but the account identity stays the same, CrawClaw reuses the best existing
...
CrawClaw currently provides that in Node by:
...
Under CrawClaw's security model, the gateway host and local CrawClaw state directory are already inside the trusted operator boundary, so this is primarily an operational durability concern rather than a separate remote trust boundary.

Planned improvement:

- add SecretRef support for persistent Matrix key material so recovery keys and related store-encryption secrets can be sourced from CrawClaw secrets providers instead of only local files
...
```bash
crawclaw matrix profile set --name "CrawClaw Assistant"
crawclaw matrix profile set --avatar-url https://cdn.example.org/avatar.png
```

Add `--account <id>` when you want to target a named Matrix account explicitly.

Matrix accepts `mxc://` avatar URLs directly. When you pass an `http://` or `https://` avatar URL, CrawClaw uploads it to Matrix first and stores the resolved `mxc://` URL back into `channels.matrix.avatarUrl` (or the selected account override).
...
Incoming verification requests from another Matrix client are tracked and auto-accepted by CrawClaw.
For self-verification flows, CrawClaw also starts the SAS flow automatically when emoji verification becomes available and confirms its own side.
For verification requests from another Matrix user/device, CrawClaw auto-accepts the request and then waits for the SAS flow to proceed normally.
...
CrawClaw does not auto-accept self-initiated duplicate flows blindly. Startup skips creating a new request when a self-verification request is already pending.

Verification protocol/system notices are not forwarded to the agent chat pipeline, so they do not produce `NO_REPLY`.

### Device hygiene

Old CrawClaw-managed Matrix devices can accumulate on the account and make encrypted-room trust harder to reason about.
List them with:

```bash
crawclaw matrix devices list
```

Remove stale CrawClaw-managed devices with:

```bash
crawclaw matrix devices prune-stale
```

### Direct Room Repair

If direct-message state gets out of sync, CrawClaw can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer with:

```bash
crawclaw matrix direct inspect --user-id @alice:example.org
```

Repair it with:

```bash
crawclaw matrix direct repair --user-id @alice:example.org
```
...
- `/new` resets the bound ACP session in place.
- `/acp close` closes the ACP session and removes the binding.

Notes:

- `--bind here` does not create a child Matrix thread.
- `threadBindings.spawnAcpSessions` is only required for `/acp spawn --thread auto|here`, where CrawClaw needs to create or bind a child Matrix thread.
...
Ack reactions use the standard CrawClaw resolution order:
...
- Matrix room history is pending-only: CrawClaw buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
...
```bash
crawclaw pairing list matrix
crawclaw pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging you before approval, CrawClaw reuses the same pending pairing code and may send a reminder reply again after a short cooldown instead of minting a new code.
...
Partial shared auth defaults do not create a separate implicit default account by themselves. CrawClaw only synthesizes the top-level `default` account when that default has fresh auth (`homeserver` plus `accessToken`, or `homeserver` plus `userId` and `password`); named accounts can still stay discoverable from `homeserver` plus `userId` when cached credentials satisfy auth later.
Set `defaultAccount` when you want CrawClaw to prefer one named Matrix account for implicit routing, probing, and CLI operations.
If you configure multiple named accounts, set `defaultAccount` or pass `--account <id>` for CLI commands that rely on implicit account selection.
Pass `--account <id>` to `crawclaw matrix verify ...` and `crawclaw matrix devices ...` when you want to override that implicit selection for one command.

## Private/LAN homeservers

By default, CrawClaw blocks private/internal Matrix homeservers for SSRF protection unless you
...
CrawClaw uses the same proxy setting for runtime Matrix traffic and account status probes.

## Target resolution

Matrix accepts these target forms anywhere CrawClaw asks you for a room or user target:
...
- `allowBots`: allow messages from other configured CrawClaw Matrix accounts (`true` or `"mentions"`).
...
- `autoJoinAllowlist`: rooms/aliases allowed when `autoJoin` is `allowlist`. Alias entries are resolved to room IDs during invite handling; CrawClaw does not trust alias state claimed by the invited room.
