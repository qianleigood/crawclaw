...
const CLI_PREFIX_RE =
  /^(?:pnpm|npm|bunx|npx)\s+(?:crawclaw|crawclaw)\b|^(?:crawclaw|crawclaw)\b/;
const CONTAINER_FLAG_RE = /(?:^|\s)--container(?:\s|=|$)/;
const PROFILE_FLAG_RE = /(?:^|\s)--profile(?:\s|=|$)/;
const DEV_FLAG_RE = /(?:^|\s)--dev(?:\s|$)/;
const UPDATE_COMMAND_RE =
  /^(?:pnpm|npm|bunx|npx)\s+(?:crawclaw|crawclaw)\b.*(?:^|\s)update(?:\s|$)|^(?:crawclaw|crawclaw)\b.*(?:^|\s)update(?:\s|$)/;
...
export function formatCliCommand(
  ...
  const container = env.CRAWCLAW_CONTAINER_HINT?.trim();
