...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-version-"));
  try {
...
describe("version resolution", () => {
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "crawclaw", version: "2.3.4" });
  ...
  it("ignores non-crawclaw package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers CRAWCLAW_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        CRAWCLAW_VERSION: "9.9.9",
        CRAWCLAW_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale CRAWCLAW_VERSION for compatibility checks", () => {
    const previous = process.env.CRAWCLAW_VERSION;
    const previousService = process.env.CRAWCLAW_SERVICE_VERSION;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        CRAWCLAW_VERSION: "2026.3.99",
        CRAWCLAW_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers runtime VERSION over service/package markers and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        CRAWCLAW_VERSION: "   ",
        CRAWCLAW_SERVICE_VERSION: "  2.0.0  ",
        npm_package_version: "1.0.0",
      }),
    ).toBe(resolveUsableRuntimeVersion(VERSION) ?? "2.0.0");
    ...
    ).toBe(resolveUsableRuntimeVersion(VERSION) ?? "1.0.0-package");
    ...
    ).toBe(resolveUsableRuntimeVersion(VERSION) ?? "fallback");
  });
});
