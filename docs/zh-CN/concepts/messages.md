...
渠道可能在重新连接后重复投递同一消息。CrawClaw 保持一个短期缓存，以渠道/账户/对端/会话/消息 ID 为键，因此重复投递不会触发另一次智能体运行。
...
多个设备/渠道可以映射到同一会话，但历史记录不会完全同步回每个客户端。建议：对长对话使用一个主设备，以避免上下文分歧。TUI 和 Gateway 客户端始终显示 Gateway 网关支持的会话记录，因此它们是事实来源。
