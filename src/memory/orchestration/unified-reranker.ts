...
function recencyBoost(updatedAt?: number): number {
  ...
  if (ageHours <= 6) {return 0.12;}
  if (ageHours <= 24) {return 0.08;}
  if (ageHours <= 72) {return 0.05;}
  if (ageHours <= 168) {return 0.02;}
  return 0;
}
...
function inferLayer(item: UnifiedRecallItem): UnifiedRecallLayer {
  ...
  if (item.memoryKind === "preference") {return "preferences";}
  if (item.memoryKind === "decision") {return "key_decisions";}
  if (item.memoryKind === "procedure") {return "sop";}
  if (item.memoryKind === "reference") {return "sources";}
  if (item.memoryKind === "runtime_pattern") {return "runtime_signals";}
  const text = `${item.title} ${item.summary}`.toLowerCase();
  if (/(decision|trade ?off|架构|为什么这样|why)/i.test(text)) {return "key_decisions";}
  if (/(sop|runbook|playbook|步骤|流程|排查|procedure)/i.test(text)) {return "sop";}
  if (/(偏好|prefer|默认|习惯|always|never)/i.test(text) || item.source === "native_memory") {return "preferences";}
  if (/(runtime|状态|incident|signal|recent|latest|execution)/i.test(text) || item.source === "execution") {return "runtime_signals";}
  if (item.source === "notebooklm") {return "key_decisions";}
...
function exactTitleBoost(query: string, title: string): number {
  ...
  if (normalizedQuery === normalizedTitle) {return 0.22;}
  if (normalizedQuery.includes(normalizedTitle) || normalizedTitle.includes(normalizedQuery)) {return 0.14;}
  return 0;
}

function keywordBoost(queryTokens: string[], item: UnifiedRecallItem): number {
  if (!queryTokens.length) {return 0;}
  const haystack = new Set(tokenizeRecallText(`${item.title} ${item.summary} ${item.content ?? ""}`.toLowerCase()));
...
function entityBoost(item: UnifiedRecallItem, selectedEntities: UnifiedEntityCandidate[]): number {
  if (!selectedEntities.length) {return 0;}
  const entityIds = new Set(selectedEntities.map((entity) => entity.id));
  const canonicalIds = new Set(selectedEntities.map((entity) => entity.canonicalId).filter((value): value is string => Boolean(value)));
  const refs = new Set(item.entityRefs ?? []);
  let boost = 0;
  for (const id of entityIds) {
    if (refs.has(id)) {boost += 0.12;}
  }
  if (item.canonicalKey && canonicalIds.has(item.canonicalKey)) {boost += 0.08;}
  return clamp(boost, 0, 0.24);
}

function layerPrior(layer: UnifiedRecallLayer, targets: UnifiedRecallLayer[]): number {
  if (targets.includes(layer)) {return 0.16;}
  if (layer === "sources") {return 0;}
  return 0.04;
}
...
function memoryKindPrior(kind: MemoryKind, input: UnifiedRerankInput): number {
  ...
  switch (intent) {
    case "decision":
      if (kind === "decision") {return 0.14;}
      if (kind === "reference") {return 0.08;}
      if (kind === "procedure") {return 0.04;}
      return 0;
    case "sop":
      if (kind === "procedure") {return 0.14;}
      if (kind === "preference") {return 0.07;}
      if (kind === "runtime_pattern") {return 0.05;}
      return 0;
    case "preference":
      if (kind === "preference") {return 0.14;}
      if (kind === "procedure") {return 0.05;}
      return 0;
    case "runtime":
    case "history":
      if (kind === "runtime_pattern") {return 0.12;}
      if (kind === "procedure") {return 0.06;}
      if (kind === "reference") {return 0.04;}
      return 0;
    default:
      if (kind === "decision" || kind === "procedure") {return 0.04;}
      return 0;
  }
}
...
function parseLifecycleWeight(item: UnifiedRecallItem): number {
  ...
  if (!signal) {return 0;}
  if (typeof signal.recallWeight === "number") {return clamp(signal.recallWeight, -0.2, 0.12);}
  const stage = typeof signal.stage === "string" ? signal.stage : "candidate";
  if (stage === "formalized") {return 0.08;}
  if (stage === "formalizing") {return 0.02;}
  if (stage === "stale") {return -0.1;}
  if (stage === "failed") {return -0.18;}
  return 0;
}
...
function sourcePrior(source: UnifiedRecallSource, input: UnifiedRerankInput): number {
  ...
  if (source === "graph") {return weights.graph * 0.3;}
  if (source === "notebooklm") {return weights.notebooklm * 0.3;}
  if (source === "native_memory") {return weights.nativeMemory * 0.3;}
  return weights.execution * 0.3;
}

function mediaBoost(item: UnifiedRecallItem, input: UnifiedRerankInput): number {
  if (!input.queryHasImage) {return 0;}
  const hasImage = item.metadata?.hasImage === true;
  const hasVisualSummary = typeof item.metadata?.visualSummary === "string" && item.metadata.visualSummary.length > 0;
  const mediaIds = Array.isArray(item.metadata?.mediaIds) ? item.metadata.mediaIds : [];
  if (hasImage && hasVisualSummary) {return 0.14;}
  if (hasImage || mediaIds.length > 0) {return 0.1;}
  return 0;
}

function dedupeKey(item: UnifiedRecallItem): string {
  return item.canonicalKey ? normalizeKey(item.canonicalKey) : normalizeKey(`${item.source}:${item.title}`);
}
...
function flattenItems(input: UnifiedRerankInput): { items: UnifiedRecallItem[]; counts: Record<UnifiedRecallSource, number> } {
  ...
  const counts = {
    ...
    notebooklm: input.notebooklmItems?.length ?? 0,
...
export function rerankUnifiedResults(input: UnifiedRerankInput): UnifiedRerankResult {
  ...
  const targets = input.classification?.targetLayers ?? ["key_decisions", "runtime_signals", "sources"];
  ...
  for (const item of items) {
    ...
    if (!existing) {
      ...
      deduped.set(key, { item: { ...item, memoryKind, layer: inferLayer({ ...item, memoryKind }) }, support: [{ ...item, memoryKind }] });
      continue;
    }
  ...
  const ranked = [...deduped.values()].map(({ item, support }): UnifiedRankedItem => {
    const memoryKind = projectMemoryKind(item);
    ...
    const supportScore = clamp((new Set(support.map((entry) => entry.source)).size - 1) * 0.04, 0, 0.12);
    ...
    const finalScore = retrieval + sourceScore + layerScore + kindScore + entityScore + keywordScore + exactScore + recencyScore + importanceScore + supportScore + lifecycleBoost + mediaScore - penalty;
