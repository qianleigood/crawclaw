...
summary: "`crawclaw configure` 的 CLI 参考（交互式配置提示）"
title: configure
x-i18n:
...
提示：不带子命令的 `crawclaw config` 会打开相同的向导。使用 `crawclaw config get|set|unset` 进行非交互式编辑。
...
- 面向渠道的服务（DingTalk/QQBot/Matrix/QQBot）在设置期间会提示输入频道/房间允许列表。你可以输入名称或 ID；向导会尽可能将名称解析为 ID。

## 示例

```bash
crawclaw configure
crawclaw configure --section models --section channels
```
