...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("crawclaw doctor")}\` ${message}`;
}
...
function collectDryRunSchemaErrors(config: CrawClawConfig): ConfigSetDryRunError[] {
...
export async function runConfigSet(opts: {
  ...
  try {
    ...
    const nextConfig = next as CrawClawConfig;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "crawclaw.json";
