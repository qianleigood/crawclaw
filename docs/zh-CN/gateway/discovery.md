...
- 服务类型：
  - `_crawclaw-gw._tcp`（Gateway 网关传输信标）
- TXT 键（非机密）：
  - `role=gateway`
...
- `CRAWCLAW_DISABLE_BONJOUR=1` 禁用广播。
- `~/.crawclaw/crawclaw.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
- `CRAWCLAW_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（默认为 22）。
- `CRAWCLAW_TAILNET_DNS` 发布 `tailnetDns` 提示（MagicDNS）。
- `CRAWCLAW_CLI_PATH` 覆盖广播的 CLI 路径。
