...
#[serde(rename_all = "camelCase")]
pub struct SpecialAgentRunResponse {
    pub status: String,
    pub run_id: String,
    pub kind: String,
    pub execution_mode: SpecialAgentExecutionMode,
...
#[derive(Clone, Debug)]
pub struct SpecialAgentRunner {
    runtime_root: PathBuf,
}

impl SpecialAgentRunner {
    ...
    pub fn run(&self, request: SpecialAgentRunRequest) -> Result<SpecialAgentRunResponse, String> {
        ...
        let scope = request.scope.as_deref().unwrap_or("main");
        let task = request.task.unwrap_or_default();
        let result = match definition.id {
            "review-spec" | "review-quality" => self.run_review(definition, &task),
            "durable-memory" => self.run_durable_memory(scope, &task)?,
            "dream" => self.dream_store().run(scope, task.as_str())?,
            "session-summary" => self.session_summary_store().refresh(scope, task.as_str())?,
            "experience" => self.experience_store().write_note(
                scope,
                "special-agent",
                task.as_str(),
                "generated",
            )?,
            _ => json!({ "status": "skipped" }),
        };

        Ok(SpecialAgentRunResponse {
            status: "completed".to_string(),
            run_id: format!("special-{}-{}", definition.id, now_millis()),
            kind: definition.id.to_string(),
    ...
    pub fn run_review_task(&self, task: &str, quality: bool) -> Result<Value, String> {
        let kind = if quality {
            "review-quality"
        } else {
            "review-spec"
        };
        self.run(SpecialAgentRunRequest {
    ...
    fn run_review(&self, definition: &SpecialAgentDefinition, task: &str) -> Value {
        json!({
            "status": "completed",
            "summary": task.trim(),
            "findings": [],
    ...
    fn run_durable_memory(&self, scope: &str, task: &str) -> Result<Value, String> {
        let tools = SpecialAgentMemoryTools::new(self.runtime_root.clone());
        let manifest = tools.read_manifest(scope)?;
        Ok(json!({
            "status": "completed",
            "summary": task.trim(),
            "manifest": manifest
