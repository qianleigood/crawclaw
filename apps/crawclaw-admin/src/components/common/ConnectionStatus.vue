...
const displayLatestVersion = computed(() => {
  return wsStore.updateAvailable?.latestVersion ?? latestVersion.value
})

function applyGatewayUpdateAvailable() {
  const updateAvailable = wsStore.updateAvailable
  if (!updateAvailable) {
    return false
  }

  latestVersion.value = updateAvailable.latestVersion
  versionOptions.value = [
    {
      label: updateAvailable.latestVersion,
      value: updateAvailable.latestVersion,
    },
  ]
  selectedVersion.value = updateAvailable.latestVersion
  return true
}
...
async function fetchNpmVersions() {
  ...
  try {
    ...
    const versions: string[] = Array.isArray(data.versions)
      ? (data.versions as unknown[]).filter((version): version is string => typeof version === 'string')
      : []
    const registryLatest = typeof data.latestVersion === 'string' ? data.latestVersion : null
    console.log('[ConnectionStatus] Fetched versions:', versions.slice(0, 5))
    
    if (versions.length > 0) {
      const nextLatestVersion = registryLatest || versions[0]
      ...
      selectedVersion.value = nextLatestVersion
    } else {
