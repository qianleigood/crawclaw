...
export async function loadStatusScanModuleForTest(
  ...
  if (options.fastJson) {
    ...
    vi.doMock("../terminal/command-secret-targets.js", () => ({
      getStatusCommandSecretTargetIds: mocks.getStatusCommandSecretTargetIds,
    }));
  } else {
    vi.doMock("../terminal/progress.js", () => ({
  ...
  vi.doMock("../terminal/command-secret-gateway.js", () => ({
  ...
  vi.doMock("../terminal/plugin-registry.js", () => createStatusPluginRegistryModuleMock(mocks));
...
export function createStatusScanConfig<T extends object = CrawClawConfig>(
  overrides: T = {} as T,
): CrawClawConfig & T {
  return {
    session: {},
