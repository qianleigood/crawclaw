...
function normalizeExtendDuration(value) {
  ...
  if (text === "6" || text === "+6" || text === "6s" || text === "+6s") {
    return "6s";
  }
  if (text === "10" || text === "+10" || text === "10s" || text === "+10s") {
    return "10s";
  }
  return "";
}
...
function summarizeTimelineRequest(timelineRequest) {
  return {
    ...
    requestedStartPct:
      timelineRequest && timelineRequest.requestedStartPct != null
        ? timelineRequest.requestedStartPct
        : null,
    requestedEndPct:
      timelineRequest && timelineRequest.requestedEndPct != null
        ? timelineRequest.requestedEndPct
        : null,
    targetStartPct:
      timelineRequest && timelineRequest.targetStartPct != null
        ? timelineRequest.targetStartPct
        : null,
    targetEndPct:
      timelineRequest && timelineRequest.targetEndPct != null ? timelineRequest.targetEndPct : null,
    tolerancePct:
      timelineRequest && Number.isFinite(timelineRequest.tolerancePct)
        ? timelineRequest.tolerancePct
        : null,
    validationErrors: Array.isArray(timelineRequest && timelineRequest.validationErrors)
      ? timelineRequest.validationErrors
      : [],
  };
}
...
function resolveTimelineRequest(job, args = {}, fallbackProbe = null) {
  ...
  const fallbackSelection =
    fallbackProbe && fallbackProbe.currentSelection ? fallbackProbe.currentSelection : {};
  const fallbackDurationPct =
    fallbackProbe &&
    fallbackProbe.automationModel &&
    Number.isFinite(fallbackProbe.automationModel.fixedWindowDurationPct)
  ...
  let targetStartPct =
    requestedStartPct != null
      ? requestedStartPct
      : Number.isFinite(fallbackSelection.startPct)
        ? roundPct(fallbackSelection.startPct)
        : null;
  let targetEndPct =
    requestedEndPct != null
      ? requestedEndPct
      : Number.isFinite(fallbackSelection.endPct)
        ? roundPct(fallbackSelection.endPct)
        : null;
  ...
  if (requested) {
    ...
    if (
      requestedStartPct != null &&
      requestedEndPct != null &&
      Number.isFinite(fallbackDurationPct)
    ) {
...
function buildTimelineAdjustmentPlan(timelineProbe, timelineRequest) {
  const plan = {
    ...
    reasons: [],
    targetRange:
      timelineRequest && timelineRequest.requested
        ? {
  ...
  if (
    !current ||
    !current.detected ||
    !Number.isFinite(current.startPct) ||
    !Number.isFinite(current.endPct)
  ) {
  ...
  const startHandle =
    model.handles && model.handles.start && model.handles.end ? model.handles.start : null;
  const endHandle =
    model.handles && model.handles.start && model.handles.end ? model.handles.end : null;
  const singleWindowHandle = model.handles && model.handles.end ? model.handles.end : null;
  const singleEndTrim =
    model.trimControls && model.trimControls.end ? model.trimControls.end : null;
  const fixedWindowDurationPct = Number.isFinite(model.fixedWindowDurationPct)
    ? model.fixedWindowDurationPct
    : null;
  const startRange =
    model.rangeInputs && model.rangeInputs.start && model.rangeInputs.end
      ? model.rangeInputs.start
      : null;
  const endRange =
    model.rangeInputs && model.rangeInputs.start && model.rangeInputs.end
      ? model.rangeInputs.end
      : null;

  if (
    (singleEndTrim && singleEndTrim.selector && Number.isFinite(fixedWindowDurationPct)) ||
    (singleWindowHandle && singleWindowHandle.selector && Number.isFinite(fixedWindowDurationPct))
  ) {
    const fixedWindowControl =
      singleEndTrim && singleEndTrim.selector ? singleEndTrim : singleWindowHandle;
    ...
    plan.actions.push({
      edge: "end",
      control: "handle",
      selector: endHandle.selector,
      fromPct: current.endPct,
      toPct: targetEndPct,
    });
...
function evaluateTimelineAdjustment({ timelineRequest, beforeProbe, afterProbe }) {
  ...
  const result = {
    ...
    moved: {
      startPct:
        Number.isFinite(before.startPct) && Number.isFinite(after.startPct)
          ? roundPct(after.startPct - before.startPct)
          : null,
      endPct:
        Number.isFinite(before.endPct) && Number.isFinite(after.endPct)
          ? roundPct(after.endPct - before.endPct)
          : null,
      startSec:
        Number.isFinite(before.startSec) && Number.isFinite(after.startSec)
          ? roundPct(after.startSec - before.startSec)
          : null,
      endSec:
        Number.isFinite(before.endSec) && Number.isFinite(after.endSec)
          ? roundPct(after.endSec - before.endSec)
          : null,
    },
  ...
  const startDiff = Number.isFinite(result.afterRange.startPct)
    ? Math.abs(result.afterRange.startPct - timelineRequest.targetStartPct)
    : null;
  const endDiff = Number.isFinite(result.afterRange.endPct)
    ? Math.abs(result.afterRange.endPct - timelineRequest.targetEndPct)
    : null;
  const startDirectionExpected =
    Number.isFinite(result.beforeRange.startPct) && Number.isFinite(timelineRequest.targetStartPct)
      ? Math.sign(timelineRequest.targetStartPct - result.beforeRange.startPct)
      : 0;
  const endDirectionExpected =
    Number.isFinite(result.beforeRange.endPct) && Number.isFinite(timelineRequest.targetEndPct)
      ? Math.sign(timelineRequest.targetEndPct - result.beforeRange.endPct)
      : 0;
  const startDirectionActual = Number.isFinite(result.moved.startPct)
    ? Math.sign(result.moved.startPct)
    : 0;
  const endDirectionActual = Number.isFinite(result.moved.endPct)
    ? Math.sign(result.moved.endPct)
    : 0;

  const fixedWindowMode = Boolean(
    timelineRequest &&
    timelineRequest.fixedWindowMode &&
    Number.isFinite(timelineRequest.fixedWindowDurationPct),
  );
  const hasSecondRangeSignals =
    Number.isFinite(result.beforeRange.startSec) &&
    Number.isFinite(result.beforeRange.endSec) &&
    Number.isFinite(result.afterRange.startSec) &&
    Number.isFinite(result.afterRange.endSec);
  const startDirectionOk =
    startDirectionExpected === 0 || startDirectionExpected === startDirectionActual;
  ...
  const startChanged =
    Number.isFinite(result.moved.startPct) && Math.abs(result.moved.startPct) >= 0.5;
  ...
  const fixedWindowMaintainedByPct = fixedWindowMode
    ? Number.isFinite(result.afterRange.startPct) &&
      ...
      Math.abs(
        result.afterRange.endPct -
          result.afterRange.startPct -
          timelineRequest.fixedWindowDurationPct,
      ) <= tolerancePct
    : true;
  const fixedWindowMaintainedBySeconds =
    fixedWindowMode && hasSecondRangeSignals
      ? Math.abs(
          result.afterRange.endSec -
            result.afterRange.startSec -
            (result.beforeRange.endSec - result.beforeRange.startSec),
        ) <= 0.75
      : false;
  ...
  const secondChangeConfirmed = hasSecondRangeSignals
    ...
    : false;
  result.credibleChange = Boolean(
    (startChanged || endChanged || secondChangeConfirmed) &&
    startDirectionOk &&
    endDirectionOk &&
    secondDirectionOk &&
    fixedWindowMaintained,
  );
  result.achieved = Boolean(
    Number.isFinite(startDiff) &&
    Number.isFinite(endDiff) &&
    startDiff <= tolerancePct &&
    endDiff <= tolerancePct &&
    fixedWindowMaintained,
  );
  ...
  if (
    fixedWindowMode &&
    hasSecondRangeSignals &&
    secondChangeConfirmed &&
    secondDirectionOk &&
    fixedWindowMaintainedBySeconds
  ) {
    result.outcome = "success";
  ...
  result.failureReason = result.credibleChange
    ? "selection_changed_but_target_not_reached"
    : "selection_change_not_confirmed";
  result.reasons.push(result.failureReason);
  return result;
}
...
async function performTimelineMouseDrag(page, action, timelineProbe) {
  const container =
    timelineProbe && timelineProbe.automationModel ? timelineProbe.automationModel.container : null;
  if (
    !container ||
    !container.rect ||
    !Number.isFinite(container.rect.left) ||
    !Number.isFinite(container.rect.width) ||
    container.rect.width <= 0
  ) {
  ...
  const clampedTargetX = Math.max(
    container.rect.left + 1,
    Math.min(container.rect.left + container.rect.width - 1, targetX),
  );
  const endY = startY;
...
const KNOWN_SECONDARY_MENU_TRIGGER_PROBES = [
  {
    xpath:
      "/html/body/div[2]/div/div[2]/div/div/div/div[1]/div/main/article/div/div[3]/div[2]/button[6]",
    closestSelector: "button",
    source: "known_secondary_menu_trigger_xpath",
  },
  {
    xpath:
      "/html/body/div[2]/div/div[2]/div/div/div/div[1]/div/main/article/div/div[3]/div[2]/button[6]/svg",
    closestSelector: "button",
...
const KNOWN_EXTEND_SUBMIT_PROBES = [
  {
    xpath:
      "/html/body/div[2]/div/div[2]/div/div/div/div[2]/div/form/div/div/div/div[2]/div[2]/button",
    closestSelector: "button",
    source: "known_extend_submit_button_xpath",
  },
  {
    xpath:
      "/html/body/div[2]/div/div[2]/div/div/div/div[2]/div/form/div/div/div/div[2]/div[2]/button/div/svg",
    closestSelector: "button",
    ...
    .evaluate((attrName) => {
      ...
      const norm = (value) =>
        String(value || "")
          .replace(/\s+/g, " ")
          .trim();
      const positive = [/^extend video$/i, /^扩展视频$/, /^延长视频$/];
      ...
      const scored = nodes
        .map((node, index) => {
          ...
          if (!label || !positive.some((re) => re.test(label))) {
            return null;
          }
      ...
      const scored = nodes
        .map((node, index) => {
          const probe = [
            ...
            .toLowerCase();
          let score = 100 - index;
          if (/prompt|描述|描述视频|写点什么|what should|describe/i.test(probe)) {
            score += 25;
          }
          if (node.tagName === "TEXTAREA") {
            score += 10;
          }
          if (String(node.getAttribute("contenteditable") || "").toLowerCase() === "true") {
            score += 8;
          }
          return { node, score };
        })
...
async function fillExtendPrompt(page, prompt, logger) {
  ...
  if (!value) {
    return { ok: false, selector: "", note: "extend_prompt_empty" };
  }
  const target = await markVisiblePromptTarget(page);
  if (!target.found) {
    return { ok: false, selector: "", note: "extend_prompt_input_not_found" };
  }
  try {
    ...
    const writeResult = await page.evaluate(
      ({ selector, value }) => {
        ...
        if (!node) {
          return { ok: false, note: "node_missing", probe: "" };
        }
        node.focus?.();
        if ("value" in node) {
          node.value = value;
        ...
        return {
          ok: Boolean(probe),
          note: probe ? "extend_prompt_written" : "extend_prompt_probe_empty",
          probe,
        };
      },
    ...
    if (logger) {
      logger.warn("extend.prompt_fill_failed", {
        phase: "extend_prompt_fill",
        selector: target.selector,
...
async function collectAvailableExtendDurations(page) {
  const found = [];
  if (await detectDurationControl(page, "6s")) {
    found.push("6s");
  }
  if (await detectDurationControl(page, "10s")) {
    found.push("10s");
  }
  return found;
}
...
function classifyDerivedCaptureCandidate({
  sourceResultUrl,
  stage,
  candidateUrl,
  currentUrl,
  ...
  const current = String(currentUrl || "")
    ...
    .replace(/[\\]+$/g, "");
  const baseline =
    baselineUrls && typeof baselineUrls.has === "function"
      ? baselineUrls
  ...
  if (stage === "auto_submit" && (primaryPageNavigated || contextPageOpened)) {
    reasons.push(
      primaryPageNavigated
        ? "primary_page_navigated_to_new_post"
        : "new_context_page_opened_to_new_post",
    );
...
async function detectExtendMode(page, extendEntry) {
  ...
  const labels = await collectVisibleLabels(
    page,
    [
      "button",
      '[role="button"]',
  ...
  if (availableDurations.length) {
    signals.push(`duration:${availableDurations.join(",")}`);
  }
  if (timelineProbe.detected) {
    signals.push(`timeline:${timelineProbe.timelineMode}`);
  }
  if (submitCandidate) {
    signals.push(`submit:${submitCandidate.label || submitCandidate.selector}`);
  }
  if (extendEntry && extendEntry.text) {
    signals.push(`entry:${extendEntry.text}`);
  }
...
async function detectFinalSubmitControl(page, options = {}) {
  ...
  if (xpathCandidate) {
    const candidateMeta = await page
      .evaluate(
        ({ selector }) => {
          const norm = (value) =>
            String(value || "")
  ...
  const candidate = await page
    .evaluate(
      ({ entryLabel: rawEntryLabel, includeExtendLabel: allowExtendLabel }) => {
        const isVisible = (node) => {
          if (!(node instanceof Element)) {
            return false;
          }
        ...
        const scored = nodes
          .map((node, index) => {
            ...
            if (negative.some((re) => re.test(label))) {
              return null;
            }
            ...
            if (!allowExtendLabel && entryLabel && label === entryLabel) {
              return null;
            }
            ...
            if (
              /primary|accent|submit|confirm|generate|create|extend/i.test(node.className || "")
            ) {
              score += 4;
            }
            ...
            if (node.closest('[role="dialog"], dialog, [aria-modal="true"]')) {
              score += 4;
            }
...
async function observeDerivedResult({
  page,
  context,
  sourceResultUrl,
  stage = "initial",
  ...
  const baselineUrls =
    watchState && watchState.baselineUrls instanceof Set
      ? watchState.baselineUrls
      : new Set(
          Array.isArray(watchState && watchState.baselineUrls) ? watchState.baselineUrls : [],
        );
  ...
  for (let index = 0; index < pages.length; index += 1) {
    ...
    const candidates = Array.from(
      new Set([currentUrl, ...(await scanResultUrls(current))].filter(Boolean)),
    );
    ...
    if (
      !best.derivedUrl &&
      observation.candidateVerdict !== "no_candidate" &&
      observation.candidateVerdict !== "source_page"
    ) {
      best = observation;
    }
  ...
  return {
    ...
    derivedVerdict:
      best && best.derivedUrl
        ? "confirmed_derived"
        : fallbackVerdict
          ? fallbackVerdict.verdict
          : "no_candidate",
    derivedVerdictReasons:
      best && best.derivedUrl
...
async function waitForDerivedResult({
  page,
  context,
  sourceResultUrl,
  timeoutMs = 25000,
  ...
  let lastDerived = "";
  let lastSignals = [];
  const baselineSignalsSet = new Set(
    (Array.isArray(baselineStartedSignals) ? baselineStartedSignals : [])
  ...
  const watchState = {
    ...
    baselineUrls: new Set(
      baselinePageSummaries.map((item) => String(item.url || "").trim()).filter(Boolean),
    ),
  };

  while (Date.now() - started <= timeoutMs) {
    const observation = await observeDerivedResult({
      page,
      context,
      sourceResultUrl,
      stage,
    ...
    const completion =
      ...
      (await detectCompletion(page).catch(() => ({
        status: "unknown",
        progressSignals: [],
        completionSignals: [],
      })));
    const rawStartedSignals = Array.from(new Set(observation.startedSignals || []));
    const startedSignals = rawStartedSignals.filter(
      (item) => !baselineSignalsSet.has(String(item || "").trim()),
    );
    const pageSummaries = observation.pageSummaries || [];
    const pagesKey = JSON.stringify(
      pageSummaries.map((item) => [item.index, item.closed, item.url]),
    );
    const verdictPayload = {
      derivedVerdict:
        observation.derivedVerdict || (derivedUrl ? "confirmed_derived" : "no_candidate"),
  ...
  const finalObservation = await observeDerivedResult({
    page,
    context,
    sourceResultUrl,
    stage,
  ...
  return {
    ...
    derivedVerdictReasons: finalObservation.derivedVerdictReasons || [
      "watch_timeout_without_trusted_candidate",
    ],
...
function buildExtendHandoff({
  ...
  const samePageHint =
    payload.extendEntry && payload.extendEntry.found
  ...
  return {
    ...
    status: relation.newResultUrl
      ...
      : payload.handoff && payload.handoff.required
        ? watching
          ? payload.capture && payload.capture.timeout
            ? "watch_timed_out"
            : "watching_or_ready"
    ...
    timelineBoundary:
      payload.timelineProbe && payload.timelineProbe.boundary
        ? payload.timelineProbe.boundary
        : null,
    timelineAdjustment: payload.timelineAdjustment || null,
    finalSubmit: payload.finalSubmit,
    capture: {
      active: watching,
      captureStage: payload.capture
        ? payload.capture.captureStage || payload.capture.stage || ""
        : "",
    ...
    instructions: relation.newResultUrl
      ...
      : [
          ...
          payload.timelineAdjustment && payload.timelineAdjustment.requested
            ...
            : payload.timelineProbe &&
                payload.timelineProbe.boundary &&
                payload.timelineProbe.boundary.summary
...
async function main() {
  ...
  job.files.extendHandoffPath =
    job.files.extendHandoffPath || path.join(job.stateDir, "extend-handoff.json");
  ...
  writeJson(job.files.extendHandoffPath, {
    ...
    timelineRequest: initialTimelineRequestSummary,
    currentUrl: sourceResultUrl,
    instructions: ["脚本正在准备 extend handoff；完成 probe 后会把当前人工接管说明写回这个文件。"],
    stateFile: job.files.extendHandoffPath,
  });
  ...
  try {
    ...
    if (!sourceReady) {
        ...
        handoff: {
          ...
          reason:
            sourceCompletion.status === "blocked"
              ? "source_result_page_blocked"
              : "source_result_not_completed",
      ...
      updateWorkflowStatus(job, {
        ...
        phase:
          sourceCompletion.status === "blocked"
            ? "extend_source_blocked"
            : "extend_source_not_ready",
      ...
      appendWorkflowCheckpoint(job, {
        ...
        resultUrl: sourceResultUrl,
        note:
          (sourceCompletion.blockerSignals || sourceCompletion.progressSignals || []).join(", ") ||
          sourceBlock.status,
        actionType: "extend_video",
        lineage: sourceLineage.current || {},
      });
    ...
    if (!primaryExtendEntry) {
      ...
      try {
        ...
        secondaryExtendEntry = await waitForLocator(page, detectSecondaryExtendEntry, {
          timeoutMs: 3500,
          intervalMs: 250,
        });
    ...
    const preferredDuration =
      requestedDuration ||
      (availableDurations.includes("6s") ? "6s" : availableDurations.includes("10s") ? "10s" : "");
    ...
    logger.info("extend.timeline_probe", {
      ...
      boundaryAutomationReady: Boolean(
        timelineProbe.boundary && timelineProbe.boundary.canAutoAdjust,
      ),
    ...
    if (timelineRequest.requested) {
      if (!timelinePlan.ok) {
        ...
        timelineAdjustment.manualHandoffRequired =
          timelineAdjustment.evaluation.outcome !== "success";
      } else {
        timelineAdjustment.afterProbe = timelineProbe;
        timelineAdjustment.evaluation = {
      ...
      logger[timelineAdjustment.evaluation.outcome === "success" ? "info" : "warn"](
        "extend.timeline_adjustment",
        {
    ...
    let newResultUrl =
      manualRecordedUrl || pickDerivedResultUrl(sourceResultUrl, capture.observedResultUrls) || "";

    const timelineReadyForSubmit =
    ...
    if (
      ...
      try {
        ...
        const baselineStartedSignals = Array.from(
          new Set([
            ...(
              (baselineCompletionBeforeSubmit && baselineCompletionBeforeSubmit.progressSignals) ||
              []
            ).map((item) => `progress:${item}`),
            ...(
              (baselineCompletionBeforeSubmit &&
                baselineCompletionBeforeSubmit.completionSignals) ||
              []
        ...
        : extendEntry
          ? timelineRequest.requested && timelineAdjustment.evaluation.outcome !== "success"
            ? timelineAdjustment.evaluation.outcome === "failed"
              ? "timeline_adjust_failed"
              : "timeline_adjust_unresolved"
            : finalSubmit.clicked
      ...
      handoff: {
        ...
        reason: newResultUrl
          ...
          : extendEntry
            ...
            : secondaryEntryFailReason ||
              primaryEntryFailReason ||
              "extend_entry_not_found_or_ui_drift",
        note: newResultUrl
    ...
    appendJsonl(job.files.extendHistoryPath, {
      ...
      timelineManualHandoffRequired: Boolean(
        payload.timelineBoundary && payload.timelineBoundary.manualHandoffRequired,
      ),
      timelineAdjustmentRequested: Boolean(
        payload.timelineAdjustment && payload.timelineAdjustment.requested,
      ),
      timelineAdjustmentOutcome:
        payload.timelineAdjustment && payload.timelineAdjustment.evaluation
          ? payload.timelineAdjustment.evaluation.outcome
          : "",
      timelineAdjustmentFailureReason:
        payload.timelineAdjustment && payload.timelineAdjustment.evaluation
          ? payload.timelineAdjustment.evaluation.failureReason
          : "",
      timelineAdjustmentCredibleChange: Boolean(
        payload.timelineAdjustment &&
        payload.timelineAdjustment.evaluation &&
        payload.timelineAdjustment.evaluation.credibleChange,
      ),
      timelineAdjustmentAchieved: Boolean(
        payload.timelineAdjustment &&
        payload.timelineAdjustment.evaluation &&
        payload.timelineAdjustment.evaluation.achieved,
      ),
      timelineAdjustmentTargetRange: payload.timelineAdjustment
        ? payload.timelineAdjustment.targetRange || null
        : null,
      timelineAdjustmentBeforeRange:
        payload.timelineAdjustment && payload.timelineAdjustment.evaluation
          ? payload.timelineAdjustment.evaluation.beforeRange
          : null,
      timelineAdjustmentAfterRange:
        payload.timelineAdjustment && payload.timelineAdjustment.evaluation
          ? payload.timelineAdjustment.evaluation.afterRange
          : null,
      timelineAdjustmentDragActions:
        payload.timelineAdjustment && payload.timelineAdjustment.drag
          ? payload.timelineAdjustment.drag.actions || []
          : [],
      ...
      captureActivePages: (payload.capture.pageSummaries || []).filter((item) => !item.closed)
        .length,
    });
    ...
    updateManifest(job, {
      ...
      extend: {
        ...
        timelineRequestErrors: payload.timelineRequest
          ? payload.timelineRequest.validationErrors || []
          : [],
        availableExtendDurations: availableDurations,
        selectedExtendDuration: selectedDuration || "",
    ...
    if (newResultUrl) {
      ...
      updateWorkflowStatus(job, {
        status:
          timelineRequest.requested &&
          payload.timelineAdjustment &&
          payload.timelineAdjustment.evaluation &&
          payload.timelineAdjustment.evaluation.outcome !== "success"
            ? payload.timelineAdjustment.evaluation.outcome === "failed"
              ? "failed"
              : "running"
            : payload.finalSubmit.clicked || payload.capture.started
              ? "queued"
              : "running",
        blocked: false,
        phase:
          ...
          payload.timelineAdjustment.evaluation.outcome !== "success"
            ? payload.timelineAdjustment.evaluation.outcome === "failed"
              ? "extend_timeline_adjust_failed"
              : "extend_timeline_adjust_unresolved"
            : payload.finalSubmit.clicked || payload.capture.started
              ? "extend_submit_capture_pending"
              : "extend_handoff_ready",
      ...
      setWorkflowBlockReason(job, {
        ...
        summary:
          secondaryEntryFailReason === "secondary_extend_entry_not_found_after_primary_click"
