...
- [src/gateway](src/gateway)：控制面、协议、认证、methods、gateway 服务
- [src/agents](src/agents)：agent runtime、工具注册、sandbox、provider 集成、subagents
- [src/memory](src/memory)：记忆引擎、提取、持久存储、编排、prompt assembly
- [src/workflows](src/workflows)：workflow registry、版本管理、n8n 编译与执行桥接
- [extensions](extensions)：为渠道、provider、浏览器后端等提供扩展能力的插件式包
...
1. **摄取与提取**
   ...
   - 相关模块在
     [src/memory/extraction](src/memory/extraction)
     和
...
2. **存储**
   ...
   - 相关模块：
     [src/memory/runtime](src/memory/runtime)
     和
...
3. **召回与排序**
   ...
   - 相关模块：
     ...
     [src/memory/search](src/memory/search)、
...
4. **Prompt assembly 与 compaction**
   ...
   - 相关模块：
     [src/memory/context](src/memory/context)
     和
