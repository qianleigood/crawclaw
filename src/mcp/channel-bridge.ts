...
export class CrawClawChannelBridge {
  ...
  constructor(
    private readonly cfg: CrawClawConfig,
  ...
  async start(): Promise<void> {
    ...
    const gatewayUrlOverrideSource =
      connection.urlSource === "cli --url"
        ? "cli"
        : connection.urlSource.startsWith("env ")
          ? "env"
          : undefined;
    ...
    this.gateway = new GatewayClient({
      ...
      clientDisplayName: "CrawClaw MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`crawclaw mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `crawclaw mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
