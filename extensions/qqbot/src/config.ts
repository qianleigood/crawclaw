...
export function listQQBotAccountIds(cfg: CrawClawConfig): string[] {
...
export function resolveDefaultQQBotAccountId(cfg: CrawClawConfig): string {
  const qqbot = cfg.channels?.qqbot as QQBotChannelConfig | undefined;
  if (qqbot?.appId || process.env.QQBOT_APP_ID) {
...
/** Apply account config updates back into the CrawClaw config object. */
