...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    crawclaw: process.env.CRAWCLAW_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.crawclaw === undefined) {
      delete process.env.CRAWCLAW_GATEWAY_TOKEN;
    } else {
      process.env.CRAWCLAW_GATEWAY_TOKEN = envSnapshot.crawclaw;
    }
  });
  ...
  it("uses CRAWCLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.CRAWCLAW_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
