...
let resetLogger: typeof import("crawclaw/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("crawclaw/plugin-sdk/runtime-env").setLoggerOverride;
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("crawclaw/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `crawclaw-outbound-${crypto.randomUUID()}.log`);
