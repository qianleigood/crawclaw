...
describe("gateway pre-auth hardening", () => {
  ...
  it("closes idle unauthenticated sockets after the handshake timeout", async () => {
    const previousConfigured = process.env.CRAWCLAW_HANDSHAKE_TIMEOUT_MS;
    const previous = process.env.CRAWCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
    delete process.env.CRAWCLAW_HANDSHAKE_TIMEOUT_MS;
    process.env.CRAWCLAW_TEST_HANDSHAKE_TIMEOUT_MS = "200";
    cleanupEnv.push(() => {
      if (previousConfigured === undefined) {
        delete process.env.CRAWCLAW_HANDSHAKE_TIMEOUT_MS;
      } else {
        process.env.CRAWCLAW_HANDSHAKE_TIMEOUT_MS = previousConfigured;
      }
      if (previous === undefined) {
        delete process.env.CRAWCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      } else {
        process.env.CRAWCLAW_TEST_HANDSHAKE_TIMEOUT_MS = previous;
      }
    });
  ...
  it("rejects excess simultaneous unauthenticated sockets from the same client ip", async () => {
    const previousConfigured = process.env.CRAWCLAW_MAX_PREAUTH_CONNECTIONS_PER_IP;
    const previous = process.env.CRAWCLAW_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP;
    delete process.env.CRAWCLAW_MAX_PREAUTH_CONNECTIONS_PER_IP;
    process.env.CRAWCLAW_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP = "1";
    cleanupEnv.push(() => {
      if (previousConfigured === undefined) {
        delete process.env.CRAWCLAW_MAX_PREAUTH_CONNECTIONS_PER_IP;
      } else {
        process.env.CRAWCLAW_MAX_PREAUTH_CONNECTIONS_PER_IP = previousConfigured;
      }
      if (previous === undefined) {
        delete process.env.CRAWCLAW_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP;
      } else {
        process.env.CRAWCLAW_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP = previous;
      }
    });
