...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(
      process.env.CRAWCLAW_PROFILE ?? process.env.CRAWCLAW_PROFILE,
    );
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("crawclaw gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
