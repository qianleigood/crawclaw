...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): CrawClawConfig {
  return {
    messages: {
      tts: {
        provider: "openai",
...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      process.env.CRAWCLAW_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
    ...
    const taggedCfg: CrawClawConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
