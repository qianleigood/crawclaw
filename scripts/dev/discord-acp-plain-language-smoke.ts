...
type DriverMode = "token" | "webhook" | "crawclaw";
...
function resolveStateDir(): string {
  const override = process.env.CRAWCLAW_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.CRAWCLAW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".crawclaw");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver crawclaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) CrawClaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|crawclaw> Driver transport mode (default: token)\n" +
    ...
    "  --crawclaw-bin <path>        CrawClaw CLI binary for driver=crawclaw (default: crawclaw)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  CRAWCLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  CRAWCLAW_DISCORD_SMOKE_DRIVER\n" +
    "  CRAWCLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  CRAWCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  CRAWCLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  CRAWCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  CRAWCLAW_DISCORD_SMOKE_AGENT\n" +
    "  CRAWCLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  CRAWCLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  CRAWCLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  CRAWCLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  CRAWCLAW_DISCORD_SMOKE_CRAWCLAW_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.CRAWCLAW_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.CRAWCLAW_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "crawclaw"
        ? "crawclaw"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.CRAWCLAW_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.CRAWCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.CRAWCLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.CRAWCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.CRAWCLAW_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.CRAWCLAW_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.CRAWCLAW_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.CRAWCLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.CRAWCLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.CRAWCLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const crawclawBin =
    resolveArg("--crawclaw-bin") || process.env.CRAWCLAW_DISCORD_SMOKE_CRAWCLAW_BIN || "crawclaw";
...
async function crawclawCliJson<T>(params: { crawclawBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.crawclawBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`crawclaw ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await crawclawCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    crawclawBin: params.crawclawBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "crawclaw") {
    return await readMessagesWithOpenclaw({
      crawclawBin: params.args.crawclawBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `crawclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await crawclawCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("crawclaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from CrawClaw.`,
        diagnostics: {
          bindingCandidates: [
            {
