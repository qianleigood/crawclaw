...
function mockContextDeps(params: {
  ...
  const ensureCrawClawModelsJson = vi.fn(async () => {});
  ...
  vi.doMock("./agent-paths.js", () => ({
    resolveCrawClawAgentDir: () => "/tmp/crawclaw-agent",
  }));
  ...
  return { ensureCrawClawModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real crawclaw startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "crawclaw", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "crawclaw", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "crawclaw", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureCrawClawModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await importContextModule();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureCrawClawModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
