...
vi.mock("../infra/crawclaw-root.js", () => ({
  resolveCrawClawPackageRoot: vi.fn().mockResolvedValue("/tmp/crawclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/crawclaw/pnpm-lock.yaml",
      markerPath: "/tmp/crawclaw/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints JSON when requested", async () => {
    ...
    expect(payload.sessions.defaults.model).toBeNull();
    expect(payload.sessions.defaults.contextTokens).toBeNull();
    expect(payload.sessions.recent).toEqual([]);
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("crawclaw status --all") ||
          line.includes("crawclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  });
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("CRAWCLAW_GATEWAY_TOKEN", "abcd1234", async () => {
  ...
  it("surfaces channel runtime errors from the gateway", async () => {
        ...
        weixin: [
          {
            ...
            configured: true,
            running: false,
            lastError: "weixin permission denied",
          },
        ],
      },
    });
    ...
    expect(joined).toMatch(/Weixin/i);
