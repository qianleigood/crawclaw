...
read_when:
  - You want to connect CrawClaw to LINE
...
LINE connects to CrawClaw via the LINE Messaging API. The plugin runs as a webhook
...
```bash
crawclaw plugins install @crawclaw/line
```

Local checkout (when running from a git repo):

```bash
crawclaw plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so CrawClaw applies strict pre-auth body limits and timeout before verification.
- CrawClaw processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
crawclaw pairing list line
crawclaw pairing approve line <CODE>
```
