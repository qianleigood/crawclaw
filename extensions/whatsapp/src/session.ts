...
import { VERSION } from "crawclaw/plugin-sdk/cli-runtime";
import { danger, success } from "crawclaw/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "crawclaw/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "crawclaw/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("crawclaw channels login")}`,
              ),
            );
          }
        }
