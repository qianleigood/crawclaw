...
const emitDiagnosticEventMock = vi.hoisted(() => vi.fn());
const isDiagnosticsEnabledMock = vi.hoisted(() => vi.fn(() => false));
...
vi.mock("crawclaw/plugin-sdk/diagnostics-otel", () => ({
  emitDiagnosticEvent: emitDiagnosticEventMock,
  isDiagnosticsEnabled: isDiagnosticsEnabledMock,
}));
...
import { createFeishuReplyDispatcher, resolveFeishuStreamingPolicy } from "./reply-dispatcher.js";
...
describe("createFeishuReplyDispatcher streaming behavior", () => {
  ...
  function createRuntimeLogger() {
    return {
      ...
      exit: vi.fn<(...args: unknown[]) => void>(),
    } satisfies RuntimeLoggerStub;
  }
  ...
  it("logs why streaming is disabled for raw render mode", () => {
    ...
    createDispatcherHarness({ runtime });

    expect(runtime.log).toHaveBeenCalledWith(
      expect.stringContaining('streaming disabled because renderMode="raw"'),
    );
  });

  it("logs why streaming is disabled for thread replies", () => {
    const runtime = createRuntimeLogger();
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      expect.stringContaining("streaming disabled for thread/topic replies"),
    );
  });
...
describe("resolveFeishuStreamingPolicy", () => {
  it("enables streaming by default in auto mode", () => {
    expect(
      resolveFeishuStreamingPolicy({
        renderMode: "auto",
        streamingEnabled: undefined,
        threadReplyMode: false,
      }),
    ).toEqual({
      enabled: true,
      surface: "card_stream",
  ...
  it("disables streaming when explicitly turned off", () => {
    expect(
      resolveFeishuStreamingPolicy({
        renderMode: "card",
  ...
  it("disables streaming in raw render mode", () => {
    expect(
      resolveFeishuStreamingPolicy({
        renderMode: "raw",
  ...
  it("disables streaming for thread replies even when config enables it", () => {
    expect(
      resolveFeishuStreamingPolicy({
        renderMode: "card",
        streamingEnabled: true,
        threadReplyMode: true,
      }),
    ).toEqual({
      enabled: false,
      surface: "none",
