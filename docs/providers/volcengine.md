...
read_when:
  - You want to use Volcano Engine or Doubao models with CrawClaw
...
Volcengine, paste your Volcano Engine API key, and save either a general
`volcengine/<model>` profile or a coding `volcengine-plan/<model>` profile.
...
For headless hosts, set `VOLCANO_ENGINE_API_KEY` in the Gateway environment or
patch the Volcengine provider API key to an `env`, `file`, or `exec` SecretRef
with `config.patch`. Set `agents.defaults.model.primary` to either
`volcengine/<model>` or `volcengine-plan/<model>`.
...
`~/.crawclaw/.env` or via `env.shellEnv`).
