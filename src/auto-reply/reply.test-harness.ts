...
type HomeEnvSnapshot = {
  ...
  CRAWCLAW_STATE_DIR: string | undefined;
  CRAWCLAW_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    CRAWCLAW_AGENT_DIR: process.env.CRAWCLAW_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".crawclaw", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.CRAWCLAW_STATE_DIR = path.join(home, ".crawclaw");
    process.env.CRAWCLAW_AGENT_DIR = path.join(home, ".crawclaw", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".crawclaw", "agent");
...
const replyRuntimeMockState = vi.hoisted(() => ({
  current: null as ReplyRuntimeMocks | null,
}));

function getInstalledReplyRuntimeMocks(): ReplyRuntimeMocks {
  const current = replyRuntimeMockState.current;
  if (!current) {
    throw new Error("reply runtime mocks have not been installed");
  }
  return current;
}
...
export function installReplyRuntimeMocks(mocks: ReplyRuntimeMocks) {
  replyRuntimeMockState.current = mocks;
}
