...
function makeFeedDetailUrl(feedId, xsecToken) {
  ...
  return FEED_DETAIL_URL_TEMPLATE.replace(
    "{feed_id}",
    encodeURIComponent(normalizedFeedId),
...
function extractRecommendKeywordsFromPayload(payload, keyword, maxSuggestions = 12) {
  ...
  const pushText = (value) => {
    ...
    if (normalized.length < 2 || normalized.length > 36) {
      return;
    }
    ...
    if (typeof node === "object") {
      for (const [key, value] of Object.entries(node)) {
        if (typeof value === "string") {
          ...
          if (
            ["word", "query", "keyword", "text", "title", "name", "suggest"].some((hint) =>
              keyLc.includes(hint),
            )
          ) {
            pushText(value);
...
async function captureSearchRecommendationsViaNetwork(page, keyword, timeoutMs = 8000) {
  return await new Promise(async (resolve) => {
    ...
    const timer = setTimeout(
      () => finish({ ok: false, reason: "recommend_request_timeout", suggestions: [] }),
      timeoutMs,
    );
    ...
    try {
      ...
      if (!typed || !typed.ok) {
        finish({
          ok: false,
          reason: typed && typed.reason ? typed.reason : "type_keyword_failed",
          suggestions: [],
        });
      }
    } catch (error) {
...
async function extractSearchFeeds(page) {
  ...
  if (typeof raw !== "string") {
    throw new XhsCliError("Search feed payload is not a JSON string.", {
  ...
  try {
    ...
    throw new XhsCliError(`Failed to parse search feed JSON: ${error.message}`, {
      code: "INVALID_SEARCH_PAYLOAD",
    });
  }
...
async function searchFeeds(page, { keyword, filters = null }) {
  if (filters && Object.values(filters).some(Boolean)) {
    throw new XhsCliError(
      "Node skeleton does not apply interactive search filters yet. Please use the Python driver for filtered search.",
      {
...
async function getFeedDetail(page, { feedId, xsecToken }) {
  ...
  if (!detail || typeof detail !== "object") {
    throw new XhsCliError(
      `Could not find feed detail for id '${normalizedFeedId}' in noteDetailMap.`,
      {
