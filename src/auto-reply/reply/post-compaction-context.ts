...
export async function readPostCompactionContext(
  ...
  try {
    ...
    // default injected context explicitly. When custom sections are configured,
    // keep the prose generic so it still matches deployments with different
    // section naming.
    const prose = isDefaultSections
      ? "Session was just compacted. The conversation summary above is a hint, not a substitute for your normal injected context. Re-check the sections below before responding to the user."
