...
export function resolveExtensionTestPlan(params = {}) {
  ...
  const config = "vitest.extensions.config.ts";
...
export function partitionExtensionTestFiles(params) {
  ...
  if (params.config === "vitest.extensions.config.ts") {
...
async function run() {
  ...
  const requireTests =
    ...
    process.env.CRAWCLAW_TEST_EXTENSION_REQUIRE_TESTS === "1";
  ...
  if (plan.isolatedTestFiles.length > 0) {
    const isolatedExitCode = await runVitestBatch({
      args: passthroughArgs,
      config: plan.config,
      env: { ...process.env, CRAWCLAW_TEST_ISOLATE: "1" },
