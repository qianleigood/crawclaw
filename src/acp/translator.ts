...
const ACP_PENDING_PROMPT_RECONCILE_INTERVAL_MS = 5_000;
...
function normalizeSessionKeyCandidate(sessionKey: string): string | undefined {
  const normalized = normalizeOptionalText(sessionKey);
  return normalized ? normalized.toLowerCase() : undefined;
}

function resolvePendingPromptSessionKeyAliases(sessionKey: string): string[] {
  const normalizedSessionKey = normalizeSessionKeyCandidate(sessionKey);
  if (!normalizedSessionKey) {
    return [];
  }
  const candidates = new Set<string>([normalizedSessionKey]);
  if (!isAcpSessionKey(sessionKey)) {
    return [...candidates];
  }

  const entry = readAcpSessionEntry({ sessionKey });
  if (!entry?.entry?.acp?.identity) {
    return [...candidates];
  }

  const requesterSessionKey =
    normalizeOptionalText(entry.entry.spawnedBy) ??
    normalizeOptionalText(entry.entry.parentSessionKey);
  ...
  for (const candidateId of [
    normalizeOptionalText(entry.entry.acp.identity.agentSessionId),
    normalizeOptionalText(entry.entry.acp.identity.acpxSessionId),
  ]) {
    if (!candidateId) {
      continue;
    }
    if (parseAgentSessionKey(candidateId)) {
      const direct = normalizeSessionKeyCandidate(candidateId);
      if (direct) {
        candidates.add(direct);
      }
      continue;
    }
    candidates.add(`agent:${requesterAgentId}:${candidateId}`.toLowerCase());
  }
  return [...candidates];
}
...
function buildSessionPresentation(params: {
  ...
  const configOptions: SessionConfigOption[] = [
    buildSelectConfigOption({
      ...
      description:
        "Controls how much deliberate reasoning CrawClaw requests from the Gateway model.",
    ...
    buildSelectConfigOption({
      ...
      name: "Tool verbosity",
      description:
        "Controls how much tool progress and output detail CrawClaw keeps enabled for the session.",
    ...
    buildSelectConfigOption({
      ...
      name: "Usage detail",
      description:
        "Controls how much usage information CrawClaw attaches to responses for the session.",
...
export class AcpGatewayAgent implements Agent {
  ...
  async prompt(params: PromptRequest): Promise<PromptResponse> {
    ...
    return new Promise<PromptResponse>((resolve, reject) => {
      this.pendingPrompts.set(params.sessionId, {
        sessionId: params.sessionId,
        sessionKey: session.sessionKey,
        sessionKeyAliases: resolvePendingPromptSessionKeyAliases(session.sessionKey),
        idempotencyKey: runId,
      ...
      const sendWithProvenanceFallback = async () => {
        try {
          ...
          if (pending) {
            ...
            this.armPendingSettleTimer(params.sessionId, pending);
          }
        } catch (err) {
          if (
      ...
      void sendWithProvenanceFallback().catch((err) => {
        ...
        if (pending) {
          this.clearPendingSettleTimer(pending);
        }
        if (this.pendingPrompts.size === 0) {
  ...
  private findPendingBySessionKey(sessionKey: string, runId?: string): PendingPrompt | undefined {
    const normalizedIncomingSessionKey = normalizeSessionKeyCandidate(sessionKey);
    if (!normalizedIncomingSessionKey) {
      return undefined;
    }
    for (const pending of this.pendingPrompts.values()) {
      const aliases =
        pending.sessionKeyAliases && pending.sessionKeyAliases.length > 0
          ? pending.sessionKeyAliases
          : resolvePendingPromptSessionKeyAliases(pending.sessionKey);
      if (!aliases.includes(normalizedIncomingSessionKey)) {
        const refreshedAliases =
          pending.sessionKey === sessionKey
            ? aliases
            : resolvePendingPromptSessionKeyAliases(pending.sessionKey);
        if (!refreshedAliases.includes(normalizedIncomingSessionKey)) {
          pending.sessionKeyAliases = refreshedAliases;
          continue;
        }
        pending.sessionKeyAliases = refreshedAliases;
      } else {
        pending.sessionKeyAliases = aliases;
      }
  ...
  private clearPendingSettleTimer(pending: PendingPrompt): void {
  ...
  private armPendingSettleTimer(sessionId: string, pending: PendingPrompt): void {
    this.clearPendingSettleTimer(pending);
    pending.settleTimer = setTimeout(() => {
      pending.settleTimer = undefined;
      void this.reconcileAcceptedPrompt(sessionId, pending);
    }, ACP_PENDING_PROMPT_RECONCILE_INTERVAL_MS);
    pending.settleTimer.unref?.();
  }

  private async reconcileAcceptedPrompt(sessionId: string, pending: PendingPrompt): Promise<void> {
    const currentPending = this.getPendingPrompt(sessionId, pending.idempotencyKey);
    if (!currentPending || currentPending !== pending || !currentPending.sendAccepted) {
      return;
    }

    try {
      const result = await this.gateway.request<AgentWaitResult>(
        "agent.wait",
        {
          runId: currentPending.idempotencyKey,
          timeoutMs: 0,
        },
        { timeoutMs: null },
      );
      const latestPending = this.getPendingPrompt(sessionId, currentPending.idempotencyKey);
      if (!latestPending || latestPending !== currentPending) {
        return;
      }
      if (result?.status === "ok" || result?.status === "error") {
        await this.finishPrompt(sessionId, latestPending, "end_turn");
        return;
      }
    } catch (err) {
      this.log(`agent.wait settle failed for ${pending.idempotencyKey}: ${String(err)}`);
    }

    const latestPending = this.getPendingPrompt(sessionId, pending.idempotencyKey);
    if (latestPending && latestPending === pending && latestPending.sendAccepted) {
      this.armPendingSettleTimer(sessionId, latestPending);
    }
  }
  ...
  private assertSupportedSessionSetup(mcpServers: ReadonlyArray<unknown>): void {
    ...
    throw new Error(
      "ACP bridge mode does not support per-session MCP servers. Configure MCP on the CrawClaw gateway or agent instead.",
    );
  }
