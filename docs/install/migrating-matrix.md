---
summary: "How CrawClaw upgrades the previous Matrix plugin in place, including encrypted-state recovery limits and manual recovery steps."
read_when:
...
- the plugin stays `@crawclaw/matrix`
- the channel stays `matrix`
- your config stays under `channels.matrix`
- cached credentials stay under `~/.crawclaw/credentials/matrix/`
- runtime state stays under `~/.crawclaw/matrix/`
...
- creating or reusing a pre-migration snapshot under `~/Backups/crawclaw-migrations/`
...
- CrawClaw writes a marker file at `~/.crawclaw/matrix/migration-snapshot.json` after a successful snapshot so later startup and repair passes can reuse the same archive.
- These automatic Matrix migration snapshots back up config + state only (`includeWorkspace: false`).
- If Matrix only has warning-only migration state, for example because `userId` or `accessToken` is still missing, CrawClaw does not create the snapshot yet because no Matrix mutation is actionable.
- If the snapshot step fails, CrawClaw skips Matrix migration for that run instead of mutating state without a recovery point.

About multi-account upgrades:

- the oldest flat Matrix store (`~/.crawclaw/matrix/bot-storage.json` and `~/.crawclaw/matrix/crypto/`) came from a single-store layout, so CrawClaw can only migrate it into one resolved Matrix account target
...
CrawClaw cannot automatically recover:
...
If the old store reports room keys that were never backed up, CrawClaw warns instead of pretending recovery succeeded.
...
- Meaning: CrawClaw created a recovery archive before mutating Matrix state.
- What to do: keep the printed archive path until you confirm migration succeeded.

`Matrix migration snapshot reused before applying Matrix upgrades.`

- Meaning: CrawClaw found an existing Matrix migration snapshot marker and reused that archive instead of creating a duplicate backup.
- What to do: keep the printed archive path until you confirm migration succeeded.
...
- Meaning: the new account-scoped location already has a sync or crypto store, so CrawClaw did not overwrite it automatically.
...
- Meaning: the Matrix client-side fallback found old flat storage, but the move failed. CrawClaw now aborts that fallback instead of silently starting with a fresh store.
...
- Meaning: CrawClaw detected a backup key conflict and refused to overwrite the current recovery-key file automatically.
