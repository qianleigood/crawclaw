...
import type { MsgContext } from "crawclaw/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "crawclaw/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "crawclaw/plugin-sdk/reply-runtime";
import type { MockFn } from "crawclaw/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("crawclaw/plugin-sdk/config-runtime").loadConfig;
type LoadSessionStoreFn = typeof import("crawclaw/plugin-sdk/config-runtime").loadSessionStore;
type ResolveStorePathFn = typeof import("crawclaw/plugin-sdk/config-runtime").resolveStorePath;
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("crawclaw/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/crawclaw-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("crawclaw/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("crawclaw/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("crawclaw/plugin-sdk/config-runtime", async (importOriginal) => {
...
vi.doMock("crawclaw/plugin-sdk/conversation-runtime", async (importOriginal) => {
...
vi.doMock("crawclaw/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("crawclaw/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
function createModelsProviderDataFromConfig(cfg: CrawClawConfig): {
...
vi.doMock("crawclaw/plugin-sdk/command-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("crawclaw/plugin-sdk/command-auth")>();
  return {
    ...actual,
...
vi.doMock("crawclaw/plugin-sdk/command-auth.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("crawclaw/plugin-sdk/command-auth")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("crawclaw/plugin-sdk/reply-runtime", async (importOriginal) => {
...
vi.doMock("crawclaw/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("crawclaw/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    getReplyFromConfig: replySpyHoisted.replySpy,
...
vi.doMock("crawclaw/plugin-sdk/channel-runtime", async (importOriginal) => {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: CrawClawConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: CrawClawConfig) => {
