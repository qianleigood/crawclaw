...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses CRAWCLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      CRAWCLAW_HOME: "/srv/crawclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/crawclaw-home"), ".crawclaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".crawclaw", "workspace-state.json"] as const;
...
function expectSubagentAllowedBootstrapNames(files: WorkspaceBootstrapFile[]) {
  expect(files.map((file) => file.name)).toEqual(["AGENTS.md"]);
}
...
describe("ensureAgentWorkspace", () => {
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("crawclaw-workspace-");
    await fs.mkdir(path.join(tempDir, ".crawclaw"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-workspace-hardlink-"));
    try {
...
describe("filterBootstrapFilesForSession", () => {
  ...
  it("returns all files for main session (no sessionKey)", () => {
    ...
    expect(result.map((file) => file.name)).toEqual(["AGENTS.md"]);
  });
