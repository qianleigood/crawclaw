...
describe("waitForAgentJob", () => {
  ...
  it("resolves from manual runtime completion markers", async () => {
    const runId = `run-manual-complete-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    registerAgentRuntimeRun({
      runId,
      status: "running",
      startedAt: 500,
  ...
  it("resolves from manual runtime failure markers", async () => {
    const runId = `run-manual-fail-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    registerAgentRuntimeRun({
      runId,
      status: "running",
      startedAt: 700,
    ...
    expect(snapshot?.startedAt).toBe(700);
    expect(snapshot?.endedAt).toBe(850);
    expect(snapshot?.error).toBe("manual failure");
  });
});
...
describe("logs.tail", () => {
  ...
  it("falls back to latest rolling log file when today is missing", async () => {
    const tempDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "crawclaw-logs-"));
    const older = path.join(tempDir, "crawclaw-2026-01-20.log");
    const newer = path.join(tempDir, "crawclaw-2026-01-21.log");
    ...
    setLoggerOverride({ file: path.join(tempDir, "crawclaw-2026-01-22.log") });
