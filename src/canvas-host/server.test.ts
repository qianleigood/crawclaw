...
describe("canvas host", () => {
  ...
  beforeAll(async () => {
    ...
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-canvas-fixtures-"));
  });
  ...
  it("injects live reload script", () => {
    ...
    expect(out).toContain("crawclawCanvasA2UIAction");
    expect(out).toContain("crawclawSendUserAction");
  });
  ...
  it("creates a default index.html when missing", async () => {
    ...
    try {
      ...
      expect(html).toContain("crawclawSendUserAction");
  ...
  it("serves A2UI scaffold and blocks traversal/symlink escapes", async () => {
    ...
    const bundlePath = path.join(a2uiRoot, "generated", "a2ui.bundle.js");
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.crawclawA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      ...
      const res = await realFetch(`http://127.0.0.1:${server.port}/__crawclaw__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("crawclaw-a2ui-host");
      expect(html).toContain("crawclawCanvasA2UIAction");

      const bundleRes = await realFetch(
        `http://127.0.0.1:${server.port}/__crawclaw__/a2ui/generated/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("crawclawA2UI");
