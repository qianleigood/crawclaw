...
async function resolveNodePairApproveScopes(
  ...
  const pairing = await listNodePairings<{
    pending?: Array<{ requestId?: string; commands?: unknown }>;
  }>(gatewayOpts, ["operator.pairing", "operator.write"]);
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      try {
        switch (action) {
          case "status":
            return jsonResult(await listGatewayNodes(gatewayOpts));
          case "describe": {
            const node = readStringParam(params, "node", { required: true });
            const nodeId = await resolveNodeId(gatewayOpts, node);
            return jsonResult(await describeGatewayNode(gatewayOpts, nodeId));
          }
          case "pending":
            return jsonResult(await listNodePairings(gatewayOpts));
          case "approve": {
            const requestId = readStringParam(params, "requestId", {
              required: true,
            });
            const scopes = await resolveNodePairApproveScopes(gatewayOpts, requestId);
            return jsonResult(await approveNodePairing(gatewayOpts, requestId, scopes));
          }
          case "reject": {
            const requestId = readStringParam(params, "requestId", {
              required: true,
            });
            return jsonResult(await rejectNodePairing(gatewayOpts, requestId));
          }
          case "notify": {
            const node = readStringParam(params, "node", { required: true });
