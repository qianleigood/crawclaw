...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "crawclaw-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "crawclaw-plugin");
  ...
  describe("resolveHookEntries", () => {
    ...
    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "crawclaw-managed");
      const workspace = makeHookEntry("shared", "crawclaw-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "crawclaw-workspace");
      const second = makeHookEntry("shared", "crawclaw-workspace");
      second.hook.handlerPath = "/tmp/crawclaw-workspace/shared/handler-2.js";
