...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.CRAWCLAW_TEST_FAST;
...
describe("crawclaw-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnAnnounceFlowOverride();
    resetSessionsSpawnHookRunnerOverride();
    resetSessionsSpawnConfigOverride();
  ...
  afterAll(() => {
    ...
    process.env.CRAWCLAW_TEST_FAST = fastModeEnv.previous;
  });
