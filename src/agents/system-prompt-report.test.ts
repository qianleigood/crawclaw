...
describe("buildSystemPromptReport", () => {
  ...
  it("records surfaced skills when the skills prompt was prefiltered", () => {
    const file = makeBootstrapFile({ path: "/tmp/workspace/policies/AGENTS.md" });
    const report = buildSystemPromptReport({
      source: "run",
      generatedAt: 0,
      ...
      injectedFiles: [],
      skillsPrompt:
        "<available_skills>\n<skill><name>deploy-runbook</name></skill>\n</available_skills>",
      surfacedSkills: ["deploy-runbook"],
      tools: [],
    });
  ...
  it("records discovered skills when discover expanded the visible set", () => {
    const file = makeBootstrapFile({ path: "/tmp/workspace/policies/AGENTS.md" });
    const report = buildSystemPromptReport({
      source: "run",
      generatedAt: 0,
...
<skill><name>deploy-runbook</name></skill>
<skill><name>feishu-create-doc</name></skill>
</available_skills>`,
      surfacedSkills: ["deploy-runbook", "feishu-create-doc"],
      discoveredSkills: ["feishu-create-doc"],
      tools: [],
    });

    expect(report.skills.surfacedSkills).toEqual(["deploy-runbook", "feishu-create-doc"]);
    expect(report.skills.discoveredSkills).toEqual(["feishu-create-doc"]);
  });

  it("keeps only filtered skill entries in the report", () => {
    const file = makeBootstrapFile({ path: "/tmp/workspace/policies/AGENTS.md" });
    const report = buildSystemPromptReport({
      source: "run",
      generatedAt: 0,
      ...
      skillsPrompt: `<available_skills>
<skill><name>deploy-runbook</name></skill>
<skill><name>incident-response</name></skill>
</available_skills>`,
      surfacedSkills: ["incident-response"],
      tools: [],
    });
