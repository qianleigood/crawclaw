...
import type { CrawClawPackageManifest } from "./manifest.js";
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const linkedManifest = path.join(rootDir, "crawclaw.plugin.json");
...
describe("loadPluginManifestRegistry", () => {
  ...
  it.each([
    {
      ...
      env: { CRAWCLAW_VERSION: "2026.3.21" } as NodeJS.ProcessEnv,
      expectedMessage: "plugin requires CrawClaw >=2026.3.22, but this host is 2026.3.21",
      expectWarn: false,
    },
    {
      name: "rejects invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedMessage: "plugin manifest invalid | crawclaw.install.minHostVersion must use",
      expectWarn: false,
    },
    {
      name: "warns distinctly when host version cannot be determined",
      minHostVersion: ">=2026.3.22",
      env: { CRAWCLAW_VERSION: "unknown" } as NodeJS.ProcessEnv,
  ...
  it.each([
    ...
    { name: "plugin-style", manifestId: "searxng", idHint: "searxng-plugin" },
  ...
  it("does not reuse cached load-path manifests across env home changes", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        ...
        CRAWCLAW_STATE_DIR: path.join(homeA, ".state"),
      }),
    });
    const second = loadPluginManifestRegistry({
      cache: true,
      ...
      env: hermeticEnv({
        HOME: homeB,
        CRAWCLAW_HOME: undefined,
        CRAWCLAW_STATE_DIR: path.join(homeB, ".state"),
      }),
    });
