...
import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "crawclaw/plugin-sdk/core";
...
/**
 * Legacy single-file token: `credentials/weixin/credentials.json` (pre per-account files).
 */
function loadLegacyToken(): string | undefined {
  const legacyPath = path.join(resolveStateDir(), "credentials", "weixin", "credentials.json");
  try {
 ...
 *   - credentials/weixin-{accountId}-allowFrom.json (authorized users)
 */
...
/**
 * Resolve the crawclaw.json config file path.
 * Checks CRAWCLAW_CONFIG_PATH env var, then state dir.
 */
function resolveConfigPath(): string {
  const envPath = process.env.CRAWCLAW_CONFIG_PATH?.trim();
  if (envPath) return envPath;
  return path.join(resolveStateDir(), "crawclaw.json");
}

/**
 * Read `routeTag` from crawclaw.json (for callers without an `CrawClawConfig` object).
 * Checks per-account `channels.<id>.accounts[accountId].routeTag` first, then section-level
 * `channels.<id>.routeTag`. Matches `feat_weixin_extension` behavior; channel key is `"weixin"`.
 *
...
function loadRouteTagSection(): Record<string, unknown> | null {
  ...
  try {
    ...
    const section = (channels?.["weixin"] as Record<string, unknown>) ?? null;
...
/**
 * Bump `channels.weixin.channelConfigUpdatedAt` in crawclaw.json on each successful login
...
export async function triggerWeixinChannelReload(): Promise<void> {
  try {
    const { loadConfig, writeConfigFile } = await import("crawclaw/plugin-sdk/config-runtime");
    const cfg = loadConfig();
    const channels = (cfg.channels ?? {}) as Record<string, unknown>;
    const existing = (channels["weixin"] as Record<string, unknown> | undefined) ?? {};
    const updated: CrawClawConfig = {
      ...cfg,
      channels: {
        ...channels,
        weixin: {
    ...
    logger.info("triggerWeixinChannelReload: wrote channel config to crawclaw.json");
  } catch (err) {
...
type WeixinAccountConfig = {
  ...
  enabled?: boolean;
  cdnBaseUrl?: string;
  /** Optional SKRouteTag source; read from crawclaw.json when `accountId` is passed to `loadConfigRouteTag`. */
...
export function listWeixinAccountIds(_cfg: CrawClawConfig): string[] {
  const indexed = listIndexedWeixinAccountIds();
  if (indexed.length > 0) {
    return indexed;
  }
  return loadLegacyToken() ? [DEFAULT_ACCOUNT_ID] : [];
}
...
export function resolveWeixinAccount(
  ...
  const section = cfg.channels?.["weixin"] as WeixinSectionConfig | undefined;
  const accountCfg: WeixinAccountConfig =
    section?.accounts?.[id] ?? (id === DEFAULT_ACCOUNT_ID ? (section ?? {}) : {});
