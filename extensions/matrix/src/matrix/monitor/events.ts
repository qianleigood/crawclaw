...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another CrawClaw runtime is using the same account. " +
    `Check 'crawclaw matrix verify status --verbose --account ${accountId}' and 'crawclaw matrix devices list --account ${accountId}'.`
  );
}
