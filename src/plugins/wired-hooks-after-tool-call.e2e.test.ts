...
describe("after_tool_call hook wiring", () => {
  ...
  it("records a loaded skill when read successfully opens a SKILL.md file", async () => {
    hookMocks.runner.hasHooks.mockReturnValue(true);

    const ctx = createToolHandlerCtx({
      runId: "skill-load-run",
      sessionKey: "skill-load-session",
      sessionId: "skill-load-ephemeral",
    });
    ...
    await handleToolExecutionStart(
      ctx as never,
      {
        ...
        args: { path: "/tmp/skills/deploy-runbook/SKILL.md" },
      } as never,
    );
