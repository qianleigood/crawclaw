...
- 已新增 `src/channels/workflow-controls.ts` 作为 channels 层的 workflow Feishu / QQBot 控制组件 seam。
- 已新增 `src/channels/index-model-picker.ts`，把 `/models` 的 Feishu provider/model picker 从 `auto-reply/reply` 收回 channels 层。
- 已新增 `src/channels/index-pagination.ts`，把 `/commands` 的 Feishu 分页 inline keyboard 从 `auto-reply/reply` 收回 channels 层。
...
- 已新增 `src/channels/typing-mode.ts`，把 typing mode / feishuer 语义从 `auto-reply/reply` 收回 channels 层。
- 已新增 `src/channels/typing-policy.ts`，把 typing policy 判定从 `auto-reply/reply` 收回 channels 层。
- 已新增 `src/channels/index-command-replies.ts`，把 `/commands` 与 `/models` 的 Feishu reply payload 成形从 `auto-reply/reply` 收回 channels 层。
- 已新增 `src/channels/index-context.ts` 与 `src/channels/index-context.ts`，把 Feishu / Feishu conversation helper 从 `auto-reply/reply` 收回 channels 层。
- 已新增 `src/channels/line-directives.ts` 与 `src/channels/index-directives.ts`，把 LINE / DingTalk reply directive transform 从 `auto-reply/reply` 收回 channels 层。
- `src/workflows/channel-forwarder.ts` 已开始改为复用 channels projector，而不是继续内联拼装 DingTalk / LINE / Feishu / QQBot payload。
...
- `src/auto-reply/reply/commands-workflow.ts` 的 workflow command reply channelData 也已开始复用 `src/channels/workflow-projection.ts` 的共享装配器，不再单独维护 Feishu / QQBot channelData 结构。
...
- `src/auto-reply/reply/commands-info.ts` 与 `src/auto-reply/reply/commands-models.ts` 已开始复用 `src/channels/index-command-replies.ts`，不再直接内联 Feishu command reply payload。
- `src/auto-reply/reply/commands-session.ts` 与 `src/auto-reply/reply/commands-subagents/*` 已开始复用 `src/channels/index-context.ts` / `src/channels/index-context.ts`，原来的 `src/auto-reply/reply/feishu-context.ts` 与 `src/auto-reply/reply/matrix-context.ts` 已删除。
- `src/auto-reply/reply/normalize-reply.ts` 已改为复用 `src/channels/line-directives.ts` / `src/channels/index-directives.ts`，原来的 `src/auto-reply/reply/line-directives.ts` 与 `src/auto-reply/reply/ddingtalk-directives.ts` 已删除。
- 已补 focused tests：
...
- 这条 seam 已先把“channel payload 如何投影”和“Feishu / QQBot 控件如何成形”从 workflow 逻辑里剥出来。
- workflow 控制命令和 resume callback 语义仍暂留在 `src/workflows`，但实际渠道展示结构已经开始收回 `src/channels`。
- inbound normalization、threading / binding / typing、Feishu / Feishu / LINE / DingTalk transform 与 workflow/projector 主链都已收进 `src/channels`。
