...
summary: "Architecture of CrawClaw's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in CrawClaw
...
Image injection is prompt-local: CrawClaw loads image refs from the current prompt and
...
CrawClaw wraps this with `guardSessionManager()` for tool result safety.
...
CrawClaw maintains an auth profile store with multiple API keys per provider:
...
CrawClaw loads custom pi extensions for specialized behavior:
...
| Tools           | Default coding tools    | Custom CrawClaw tool suite                                                                     |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `CRAWCLAW_LIVE_TEST=1`)
