...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    CRAWCLAW_STATE_DIR: "/tmp/crawclaw-daemon",
    CRAWCLAW_CONFIG_PATH: "/tmp/crawclaw-daemon/crawclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.CRAWCLAW_STATE_DIR ?? "/tmp/crawclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.CRAWCLAW_CONFIG_PATH ?? `${stateDir}/crawclaw.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/crawclaw-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.CRAWCLAW_STATE_DIR = "/tmp/crawclaw-cli";
    process.env.CRAWCLAW_CONFIG_PATH = "/tmp/crawclaw-cli/crawclaw.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/crawclaw-daemon/crawclaw.json",
      }),
    );
  });
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.CRAWCLAW_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.CRAWCLAW_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.CRAWCLAW_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "crawclaw-gateway" }],
        hints: [],
      },
      healthy: false,
