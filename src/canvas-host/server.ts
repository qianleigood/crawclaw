...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.crawclawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.crawclawCanvasA2UIAction &&
        typeof window.crawclawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.crawclawSendUserAction === "function";
  ...
  statusEl.appendChild(
    document.createTextNode(
      " · webkitBridge=" + (hasIOS() ? "yes" : "no") + " · androidBridge=" + (hasAndroid() ? "yes" : "no"),
    ),
  );
  ...
  window.addEventListener("crawclaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on a CrawClaw node canvas with the action bridge enabled.");
      return;
    }
    const sendUserAction =
      typeof window.crawclawSendUserAction === "function"
        ? window.crawclawSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.CRAWCLAW_SKIP_CANVAS_HOST)) {
    return true;
  }
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>CrawClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
