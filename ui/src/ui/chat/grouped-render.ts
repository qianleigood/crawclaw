        ...
        ${group.messages.map((item, index) =>
          renderGroupedMessage(
            item.message,
            {
              ...
              onInspectRun: opts.onInspectRun,
            },
            opts.onOpenSidebar,
          ),
        )}
...
export const SKIP_DELETE_CONFIRM_KEY = "crawclaw:skipDeleteConfirm";
...
function renderInspectButton(runId: string, onInspectRun: (runId: string) => void) {
  return html`
    <button
      class="btn btn--xs chat-expand-btn"
      type="button"
      ...
      @click=${() => onInspectRun(runId)}
    >
      <span class="chat-expand-btn__icon" aria-hidden="true">${icons.search}</span>
    </button>
  `;
}
...
function renderGroupedMessage(
  ...
  const runId = typeof m.runId === "string" && m.runId.trim() ? m.runId.trim() : null;
  const canInspect = Boolean(runId && opts.onInspectRun);
      ...
      <div class="${bubbleClasses}">
        ${canInspect
          ? html`<div class="chat-bubble-actions">
              ${renderInspectButton(runId!, opts.onInspectRun!)}
            </div>`
          : nothing}
  ...
  const hasActions = canCopyMarkdown || canExpand || canInspect;

  return html`
    <div class="${bubbleClasses}">
      ${hasActions
        ? html`<div class="chat-bubble-actions">
            ${canInspect ? renderInspectButton(runId!, opts.onInspectRun!) : nothing}
