...
function setWideSessionsVisibilityConfig() {
...
describe("sessions_list transcriptPath resolution", () => {
  beforeEach(() => {
    callGatewayMock.mockClear();
    setWideSessionsVisibilityConfig();
  });

  it.each([
    {
      name: "resolves cross-agent transcript paths from agent defaults when gateway store path is relative",
      stateDirName: "crawclaw-state-relative",
      response: {
        path: "agents/main/sessions/sessions.json",
        sessions: [{ key: "agent:worker:main", kind: "direct", sessionId: "sess-worker" }],
      },
    ...
    {
      name: "falls back to agent defaults when gateway path is non-string",
      stateDirName: "crawclaw-state-non-string-path",
      response: {
        path: { raw: "agents/main/sessions/sessions.json" },
        sessions: [{ key: "agent:worker:main", kind: "direct", sessionId: "sess-worker-shape" }],
      },
      expectedPath: (_stateDir: string) => path.join("agents", "worker", "sessions"),
      sessionId: "sess-worker-shape",
    },
    {
      name: "falls back to agent defaults when gateway path is '(multiple)'",
  ...
  ])("$name", async ({ stateDirName, response, expectedPath, sessionId }) => {
    await withStubbedStateDir(stateDirName, async (stateDir) => {
      callGatewayMock.mockResolvedValueOnce(response);
      const result = await executeMainSessionsList();
      expectWorkerTranscriptPath(result, {
        containsPath: expectedPath(stateDir),
        sessionId,
      });
    });
  });
  ...
  it("resolves absolute {agentId} template paths per session agent", async () => {
    const templateStorePath = "/tmp/crawclaw/agents/{agentId}/sessions/sessions.json";
