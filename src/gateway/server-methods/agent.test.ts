...
describe("gateway agent handler", () => {
  ...
  it("tracks async gateway agent runs in the shared task registry", async () => {
    await withTempDir({ prefix: "crawclaw-gateway-agent-task-" }, async (root) => {
  ...
  it("handles bare /new by resetting the same session and sending reset greeting prompt", async () => {
    ...
    expect(call?.message).toContain("Greet the user briefly");
    ...
    const respond = await invokeAgent(
      {
        message: "/new",
        sessionKey: "agent:main:main",
        idempotencyKey: "test-new-write-scope",
      },
      {
        reqId: "4c",
  ...
  it("returns a runtime inspection snapshot for agent.inspect", async () => {
    ...
    await agentHandlers["agent.inspect"]({
      ...
      respond: respond as never,
      context: makeContext(),
      req: { type: "req", id: "inspect-1", method: "agent.inspect" },
  ...
  it("rejects agent.inspect without a runId, taskId, or traceId", async () => {
    ...
    await agentHandlers["agent.inspect"]({
      ...
      respond: respond as never,
      context: makeContext(),
      req: { type: "req", id: "inspect-2", method: "agent.inspect" },
      client: { connect: { role: "operator", scopes: ["operator.read"] } } as never,
      isWebchatConnect: () => false,
    });
