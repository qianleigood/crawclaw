...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "crawclaw", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".crawclaw", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".crawclaw", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "crawclaw-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".crawclaw", "extensions"),
      },
    });
  });
});
