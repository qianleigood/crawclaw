...
Gateway 网关 WS 协议是 CrawClaw 的**单一控制平面 + 节点传输**。所有客户端（CLI、自动化、浏览器来源客户端和节点主机）都通过 WebSocket 连接，并在握手时声明其**角色** + **作用域**。
...
- 如果设置了 `CRAWCLAW_GATEWAY_TOKEN`（或 `--token`），`connect.params.auth.token`
  必须匹配，否则套接字将被关闭。
...
- 所有 WS 客户端在 `connect` 期间必须包含 `device` 身份（operator + node）。
  浏览器来源客户端**仅**在启用 `gateway.browserClients.allowInsecureAuth` 时可以省略它
  （或使用 `gateway.browserClients.dangerouslyDisableDeviceAuth` 用于紧急情况）。
