...
export function listAgentEntries(cfg: CrawClawConfig): AgentEntry[] {
...
export function resolveDefaultAgentId(cfg: CrawClawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: CrawClawConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: CrawClawConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: CrawClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
