...
export const CRAWCLAW_NODE_OPTIONS_READY = "CRAWCLAW_NODE_OPTIONS_READY";
export const CRAWCLAW_NODE_EXTRA_CA_CERTS_READY = "CRAWCLAW_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (
    ...
    isTruthyEnvValue(env.CRAWCLAW_NO_RESPAWN ?? env.CRAWCLAW_NO_RESPAWN)
  ) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[CRAWCLAW_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[CRAWCLAW_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !isTruthyEnvValue(env[CRAWCLAW_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[CRAWCLAW_NODE_OPTIONS_READY] = "1";
