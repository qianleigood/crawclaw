...
import type { CrawClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  crawclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  crawclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: CrawClawConfig,
  pathSegments: string[],
): { changed: boolean; next: CrawClawConfig } {
...
function coerceConfig(value: unknown): CrawClawConfig {
...
function stampConfigVersion(cfg: CrawClawConfig): CrawClawConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: CrawClawConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer CrawClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as CrawClawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): CrawClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(effectiveConfigRaw as CrawClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(
    ...
    let cfgToWrite = persistCandidate as CrawClawConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as CrawClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.CRAWCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.CRAWCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      argv: process.argv.slice(0, 8),
      execArgv: process.execArgv.slice(0, 8),
      watchMode: deps.env.CRAWCLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.CRAWCLAW_WATCH_SESSION === "string" &&
        deps.env.CRAWCLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.CRAWCLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.CRAWCLAW_WATCH_COMMAND === "string" &&
        deps.env.CRAWCLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.CRAWCLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `CRAWCLAW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: CrawClawConfig | null = null;
let runtimeConfigSourceSnapshot: CrawClawConfig | null = null;
...
export function getRuntimeConfigSourceSnapshot(): CrawClawConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: CrawClawConfig;
...
export function projectConfigOntoRuntimeSourceSnapshot(config: CrawClawConfig): CrawClawConfig {
...
export function loadConfig(): CrawClawConfig {
  ...
  // reparsing crawclaw.json on hot code paths.
  setRuntimeConfigSnapshot(config);
  return runtimeConfigSnapshot ?? config;
}

export function getRuntimeConfig(): CrawClawConfig {
  return loadConfig();
}
