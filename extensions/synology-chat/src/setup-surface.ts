...
function getChannelConfig(cfg: CrawClawConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: CrawClawConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: CrawClawConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: CrawClawConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL CrawClaw uses to send replies back to Chat.",
      ],
