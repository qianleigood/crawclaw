...
describe("sandbox fs bridge anchored ops", () => {
  ...
  it.each(pinnedReadCases)("$name", async (testCase) => {
    await withTempDir("crawclaw-fs-bridge-contract-read-", async (stateDir) => {
  ...
  it.each(pinnedCases)("$name", async (testCase) => {
    await withTempDir("crawclaw-fs-bridge-contract-write-", async (stateDir) => {
      ...
      const opCall = mockedExecDockerRaw.mock.calls.find(
        ([args]) =>
          ...
          args[5].includes("operation = sys.argv[1]") &&
  ...
  it("stat anchors parent + basename", async () => {
    await withTempDir("crawclaw-fs-bridge-contract-stat-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
