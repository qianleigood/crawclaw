...
import { type CrawClawConfig, readConfigFileSnapshot } from "../../config/config.js";
...
function formatFeishuCliStatusLine(status: FeishuCliStatusSnapshot): string {
  const bits: string[] = [];
  bits.push(status.status ?? "unknown");
  bits.push(status.identity ?? "user");
  if (status.version) {
    bits.push(`lark-cli ${status.version}`);
  }
  if (status.installed === false) {
    bits.push("not installed");
  }
  if (status.authOk === false) {
    bits.push("auth missing");
  }
  if (status.message && status.status !== "ready") {
    bits.push(status.message);
  }
  return `- Feishu user identity: ${bits.join(", ")}`;
}

export function formatGatewayChannelsStatusLines(
  payload: Record<string, unknown>,
  opts: { feishuCliStatus?: FeishuCliStatusSnapshot | null } = {},
): string[] {
  const lines: string[] = [];
  ...
  const accountLines = (provider: ChatChannel, accounts: Array<Record<string, unknown>>) =>
    accounts.map((account) => {
      ...
      const streaming = account.streaming as
        | {
            enabled?: boolean;
            surface?: string;
            reason?: string;
        ...
        | undefined;
      if (streaming && typeof streaming.enabled === "boolean") {
        const streamMode = streaming.enabled ? "streaming" : "fallback";
        const streamSurface =
          typeof streaming.surface === "string" && streaming.surface.length > 0
            ? streaming.surface
            : "unknown";
        const streamReason =
          typeof streaming.reason === "string" && streaming.reason.length > 0
            ? streaming.reason
            : "unknown";
        bits.push(`stream:${streamMode}:${streamSurface}:${streamReason}`);
      }
  ...
  if (opts.feishuCliStatus) {
    lines.push(formatFeishuCliStatusLine(opts.feishuCliStatus));
  }
  ...
  if (issues.length > 0) {
    ...
    lines.push(`- Run: ${formatCliCommand("crawclaw doctor")}`);
    lines.push("");
  }
  lines.push(
...
export async function formatConfigChannelsStatusLines(
  ...
  opts?: { sourceConfig?: CrawClawConfig },
): Promise<string[]> {
  const lines: string[] = [];
