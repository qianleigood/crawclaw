...
function resolveAttachmentLimits(config: CrawClawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".crawclaw", "attachments");
  const relDir = path.posix.join(".crawclaw", "attachments", attachmentId);
