...
describe("gateway server chat", () => {
  ...
  const withMainSessionStore = async <T>(run: (dir: string) => Promise<T>): Promise<T> => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-gw-"));
    try {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-gw-"));
      ...
      await withMainSessionStore(async () => {
        ...
        try {
          ...
          const sendRes = await rpcReq(scopedWs, "chat.send", {
            sessionKey: "main",
            message: "/new",
            idempotencyKey: "idem-write-scope-new-no-rotate",
          });
          expect(sendRes.ok).toBe(true);
