...
function filterSkillEntries(
  ...
  if (!surfacedSkills?.length) {
    return entries;
  }
  const surfacedNames = new Set(surfacedSkills);
  return entries.filter((entry) => surfacedNames.has(entry.name));
}
...
export function buildSystemPromptReport(params: {
  ...
  return {
    ...
    skills: {
      ...
      ...(surfacedSkills?.length
        ? {
            prefiltered: true,
            surfacedSkills,
            ...(params.discoveredSkills?.length
              ? {
