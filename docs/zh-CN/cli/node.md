...
summary: "`crawclaw node` 的 CLI 参考（无头节点主机）"
title: node
x-i18n:
...
当你希望智能体**在网络中的其他机器上运行命令**，而无需在那里安装完整的 macOS 节点主机时，请使用节点主机。
...
使用 `crawclaw node run` 运行前台节点主机（无服务）。
...
- `crawclaw approvals --node <id|name|ip>`（从 Gateway 网关编辑）
