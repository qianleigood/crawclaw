...
在 Gateway 网关拥有的配对中，**Gateway 网关**是允许哪些节点加入的唯一信息源。UI（节点主机、未来的客户端）只是审批或拒绝待处理请求的前端。
...
配对状态存储在 Gateway 网关状态目录下（默认 `~/.crawclaw`）：

- `~/.crawclaw/nodes/paired.json`
- `~/.crawclaw/nodes/pending.json`

如果你覆盖了 `CRAWCLAW_STATE_DIR`，`nodes/` 文件夹会随之移动。
