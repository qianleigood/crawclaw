...
read_when:
  - You want to connect CrawClaw to QQ
...
QQ Bot connects to CrawClaw via the official QQ Bot API (WebSocket gateway). The
...
## Bundled with CrawClaw

Current CrawClaw installs bundle QQ Bot. You do not need a separate
`crawclaw plugins install` step for normal setup.
...
```bash
crawclaw channels add --channel qqbot --token "AppID:AppSecret"
```
...
- `crawclaw channels add --channel qqbot --token-file ...` provides the
...
Run multiple QQ bots under a single CrawClaw instance:
...
```bash
crawclaw channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
