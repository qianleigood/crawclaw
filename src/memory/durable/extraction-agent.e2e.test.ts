...
import { __testing as durableAgentTesting, runDurableExtractionAgentOnce } from "./agent-runner.ts";
...
function createFakeExtractorRun(params: { capturedEmbeddedRuns: RunEmbeddedPiAgentParams[] }) {
  return vi.fn(async (embeddedParams: RunEmbeddedPiAgentParams): Promise<EmbeddedPiRunResult> => {
    ...
    expect(embeddedParams.specialAgentSpawnSource).toBe("memory-extraction");
    ...
    expect(embeddedParams.specialParentPromptEnvelope?.forkContextMessages).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ role: "user", content: "旧上下文：我只想要中文回答。" }),
        expect.objectContaining({ role: "user", content: "以后操作类回答先给步骤。" }),
      ]),
    );
...
describe("durable extraction agent e2e", () => {
  it("runs from stop lifecycle through the embedded memory extractor and scoped durable files", async () => {
    ...
    expect(capturedEmbeddedRuns[0]?.sessionKey).toMatch(/^embedded:memory_extractor:/);
