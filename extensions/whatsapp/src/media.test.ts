...
import { optimizeImageToPng } from "crawclaw/plugin-sdk/media-runtime";
...
import { mockPinnedHostnameResolution } from "crawclaw/plugin-sdk/testing";
...
const mediaFetchActual = await vi.importActual<typeof import("../../../src/media/fetch.js")>(
  "../../../src/media/fetch.js",
);
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredCrawClawTmpDir(), "crawclaw-media-test-"),
  );
...
beforeEach(() => {
  fetchRemoteMediaMock.mockImplementation(mediaFetchActual.fetchRemoteMedia);
});

describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub CRAWCLAW_STATE_DIR.
    // Also keep it outside the CrawClaw temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["CRAWCLAW_STATE_DIR"]);
    process.env.CRAWCLAW_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "crawclaw-media-state-test",
    );
  });
  ...
  it("includes URL + status in fetch errors", async () => {
    fetchRemoteMediaMock.mockRejectedValueOnce(
      new Error("Failed to fetch media from https://example.com/missing.jpg: HTTP 404 Not Found"),
    );
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredCrawClawTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("rejects default CrawClaw state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
