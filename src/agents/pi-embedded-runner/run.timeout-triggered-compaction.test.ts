...
describe("timeout-triggered compaction", () => {
  ...
  it("catches thrown errors from memoryRuntime.compact during timeout recovery", async () => {
    // Timeout with high prompt usage
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(
      makeAttemptResult({
        timedOut: true,
  ...
  it("fires compaction compatibility events during timeout recovery for ownsCompaction engines", async () => {
