...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("crawclaw-daemon-install-int-");
    configPath = path.join(tempHome, "crawclaw.json");
    process.env.HOME = tempHome;
    process.env.CRAWCLAW_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.CRAWCLAW_GATEWAY_TOKEN).toBeUndefined();
  });
});
