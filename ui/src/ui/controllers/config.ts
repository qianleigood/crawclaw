...
type ConfigSaveMethod = "config.patch" | "config.set";
...
/**
 * Serialize the form state for submission to `config.patch` / `config.set` / `config.apply`.
 *
...
function resolveConfigSaveRequest(state: ConfigState): {
  method: ConfigSaveMethod;
  params: ControlUiMethodParamsMap[ConfigSaveMethod];
} {
...
export async function saveConfig(state: ConfigState) {
  ...
  state.lastError = null;
  try {
    const { method, params } = resolveConfigSaveRequest(state);
...
export async function applyConfig(state: ConfigState) {
  ...
  try {
    ...
    const params: ControlUiMethodParamsMap["config.apply"] = {
      raw,
      baseHash,
      sessionKey: state.applySessionKey,
    };
    await state.client.request("config.apply", params);
    state.configFormDirty = false;
    await loadConfig(state);
  } catch (err) {
    state.lastError = String(err);
