...
- **初始化副作用：** `process-message.ts` 中将 `resolveSenderCommandAuthorizationWithRuntime` / `resolveDirectDmAuthorizationOutcome` 改为懒加载，避免模块初始化时触发宿主的 `ensureContextWindowCacheLoaded` 副作用，进而导致插件加载重入。
...
- **登录后配置刷新：** 每次微信登录成功后，在 `crawclaw.json` 中更新 `channels.weixin.channelConfigUpdatedAt`（ISO 8601），让网关从磁盘重新加载配置；不再写入空的 `accounts: {}` 占位。
- **扫码登录：** `get_bot_qrcode` 客户端超时由 5s 调整为 10s。
- **文档：** 卸载说明改为使用 `crawclaw plugins uninstall @tencent-weixin/weixin`，与插件 CLI 一致。
- **日志：** `debug-check` 日志不再输出 `stateDir` / `CRAWCLAW_STATE_DIR`。

### 移除

- **`weixin` 子命令**（删除 `src/weixin-cli.ts` 及 `index.ts` 中的注册）。请使用宿主自带的 `crawclaw plugins uninstall …` 卸载流程。

### 修复

- 解决在较新宿主版本上安装插件时出现的 **dangerous code pattern** 提示（宿主插件安装 / 静态检查）。
