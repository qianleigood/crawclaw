...
export function resolveSessionKeyForRequest(opts: {
  ...
  const hasRoutingTarget = Boolean(opts.to?.trim());
  const shouldPreferSessionIdKey =
    Boolean(explicitSessionId) && !opts.sessionKey?.trim() && !hasRoutingTarget;
  const explicitSessionKey =
    opts.sessionKey?.trim() ||
    (!shouldPreferSessionIdKey
      ? resolveExplicitAgentSessionKey({
          cfg: opts.cfg,
  ...
  if (opts.sessionId && !explicitSessionKey && !sessionKey) {
    const resumeTarget = resolveAgentTaskResumeTargetBySessionId(opts.sessionId);
    if (resumeTarget?.sessionKey) {
      const resumeStorePath = resumeTarget.storePath ?? storePath;
      return {
        sessionKey: resumeTarget.sessionKey,
        sessionStore: loadSessionStore(resumeStorePath),
        storePath: resumeStorePath,
      };
    }
  }

  // For local/CLI sessions that provide an explicit sessionId but have no
  // routable sender context, fall back to using the sessionId as the stable
  // store key so per-session metadata persists across separate process runs.
  if (!sessionKey && explicitSessionId) {
    sessionKey = explicitSessionId;
  }
