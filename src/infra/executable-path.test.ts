...
describe("executable path helpers", () => {
  it("detects executable files and rejects directories or non-executables", async () => {
    const base = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-exec-path-"));
  ...
  it("does not treat drive-less rooted windows paths as cwd-relative executables", () => {
    ...
    expect(
      resolveExecutablePath(String.raw`:\Users\demo\AI\system\crawclaw\git.exe`, {
        cwd: String.raw`C:\Users\demo\AI\system\crawclaw`,
      }),
    ).toBeUndefined();
    expect(
      resolveExecutablePath(String.raw`:/Users/demo/AI/system/crawclaw/git.exe`, {
