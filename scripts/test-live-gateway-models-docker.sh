...
elif [[ -n "${CRAWCLAW_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(crawclaw_live_collect_auth_dirs_from_csv "${CRAWCLAW_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(crawclaw_live_collect_auth_files_from_csv "${CRAWCLAW_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
docker run --rm -t \
  ...
  -e CRAWCLAW_LIVE_GATEWAY_MODELS="${CRAWCLAW_LIVE_GATEWAY_MODELS:-modern}" \
  -e CRAWCLAW_LIVE_GATEWAY_PROVIDERS="${CRAWCLAW_LIVE_GATEWAY_PROVIDERS:-}" \
  -e CRAWCLAW_LIVE_GATEWAY_SMOKE="${CRAWCLAW_LIVE_GATEWAY_SMOKE:-1}" \
  -e CRAWCLAW_LIVE_GATEWAY_MAX_MODELS="${CRAWCLAW_LIVE_GATEWAY_MAX_MODELS:-8}" \
  -e CRAWCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS="${CRAWCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS:-45000}" \
  -e CRAWCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${CRAWCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-90000}" \
  -v "$ROOT_DIR":/src:ro \
