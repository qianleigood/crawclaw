...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred CrawClaw tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideCrawClaw = path.join(os.tmpdir(), "outside-crawclaw", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
