...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when CRAWCLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".crawclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when CRAWCLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", CRAWCLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".crawclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers CRAWCLAW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        CRAWCLAW_PROFILE: "rescue",
        CRAWCLAW_STATE_DIR: "C:\\State\\crawclaw",
      },
      expected: path.join("C:\\State\\crawclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", CRAWCLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".crawclaw", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem CrawClaw Gateway",
          "cd /d C:\\Projects\\crawclaw",
          "set NODE_ENV=production",
          "set CRAWCLAW_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\crawclaw",
          environment: {
            NODE_ENV: "production",
            CRAWCLAW_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\crawclaw\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\CrawClaw Share\\node.exe" "\\\\fileserver\\CrawClaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\CrawClaw Share\\node.exe",
            "\\\\fileserver\\CrawClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from CRAWCLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ CRAWCLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),
