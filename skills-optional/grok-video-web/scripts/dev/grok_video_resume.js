...
function usage() {
  process.stderr.write(
    `Usage: grok_video_resume.js <command> [options]\n\n` +
      `Commands:\n` +
...
function buildResumePlan(context) {
  ...
  const current =
    context.checkpoints && context.checkpoints.current ? context.checkpoints.current : null;
  ...
  const blocked = Boolean(
    status.blocked || (status.status || "").startsWith("blocked_") || blockReason,
  );
...
function main() {
  ...
  if (args.command === "init") {
    ...
    process.stdout.write(
      `${JSON.stringify({ ok: true, stateDir, paths, status, checkpoint }, null, 2)}\n`,
    );
    return;
  }
  ...
  if (args.command === "checkpoint") {
    ...
    process.stdout.write(
      `${JSON.stringify({ ok: true, stateDir, status, checkpoint }, null, 2)}\n`,
    );
    return;
  }
  ...
  if (args.command === "block") {
    ...
    const blockReason = setBlockReason(stateDir, {
      ...
      recommendedAction: {
        ...
        noBruteRetry: true,
        summary:
          "Wait for the human to clear the blocker in the same browser profile/tab, then resume from the last checkpoint.",
      },
    });
    const status = updateStatus(stateDir, {
      status: statusValue,
    ...
    process.stdout.write(
      `${JSON.stringify({ ok: true, stateDir, status, blockReason, checkpoint }, null, 2)}\n`,
    );
    return;
  }
  ...
  if (args.command === "result-url") {
    ...
    process.stdout.write(
      `${JSON.stringify({ ok: true, stateDir, resultUrl: args.resultUrl, status, checkpoint }, null, 2)}\n`,
    );
    return;
  }
  ...
  if (args.command === "plan") {
    ...
    logger.info("resume.plan_generated", {
      ...
      message: plan.blocked
        ? "Generated blocked-job resume plan."
        : "Generated normal resume plan.",
    });
