...
const serveCrawClawChannelMcp = mocks.serveCrawClawChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveCrawClawChannelMcp: mocks.serveCrawClawChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("crawclaw-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("crawclaw-cli-mcp-home-", async () => {
      ...
      expect(serveCrawClawChannelMcp).toHaveBeenCalledWith({
