...
read_when:
  - Building or debugging historical node clients and macOS node mode
...
<Warning>
The TCP bridge has been **removed**. Current CrawClaw builds do not ship the bridge listener and `bridge.*` config keys are no longer in the schema. This page is kept for historical reference only. Use the [Gateway Protocol](/gateway/protocol) for all node/operator clients.
</Warning>
