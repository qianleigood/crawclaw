...
async function prepareAndActivate(params: { config: CrawClawConfig; env?: NodeJS.ProcessEnv }) {
...
describe("crawclaw tools runtime web metadata wiring", () => {
  ...
  it("falls back to direct fetch when no web fetch provider is active", async () => {
    const snapshot = await prepareAndActivate({
      config: asConfig({
        plugins: {
          entries: {
    ...
    expect(mockFetch.mock.calls[0]?.[0]).toBe("http://127.0.0.1/runtime-off");
  });
});
