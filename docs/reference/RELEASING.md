...
CrawClaw has three public release lanes:
...
- Every CrawClaw release ships the npm package as the canonical artifact
...
- Maintainer release automation now uses preflight-then-promote:
  ...
  - stable npm promotion from `beta` to `latest` is still available as an explicit manual mode on the trusted `CrawClaw NPM Release` workflow
...
`CrawClaw NPM Release` accepts these operator-controlled inputs:
...
1. Run `CrawClaw NPM Release` with `preflight_only=true`
2. Choose `npm_dist_tag=beta` for the normal beta-first flow, or `latest` only
   when you intentionally want a direct stable publish
3. Save the successful `preflight_run_id`
4. Run `CrawClaw NPM Release` again with `preflight_only=false`, the same
   `tag`, the same `npm_dist_tag`, and the saved `preflight_run_id`
5. If the release landed on `beta`, run `CrawClaw NPM Release` later with the
