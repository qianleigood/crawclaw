...
BASE_IMAGE="${BASE_IMAGE:-crawclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-crawclaw-sandbox-common:bookworm-slim}"
...
CRAWCLAW_DOCKER_BUILD_USE_BUILDX="${CRAWCLAW_DOCKER_BUILD_USE_BUILDX:-0}"
CRAWCLAW_DOCKER_BUILD_CACHE_FROM="${CRAWCLAW_DOCKER_BUILD_CACHE_FROM:-}"
CRAWCLAW_DOCKER_BUILD_CACHE_TO="${CRAWCLAW_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${CRAWCLAW_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${CRAWCLAW_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${CRAWCLAW_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${CRAWCLAW_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${CRAWCLAW_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=crawclaw.sandbox=1)
NOTE
