...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("crawclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "crawclaw-sandbox-browser-global" },
      agentBrowser: { network: "crawclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("crawclaw-sandbox-browser-agent");
  });
