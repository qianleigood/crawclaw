...
import { parseReleaseVersion } from "../crawclaw-npm-release-check.ts";
...
export function collectPublishablePluginPackageErrors(
  ...
  if (!packageName.startsWith("@crawclaw/")) {
    errors.push(
      `package name must start with "@crawclaw/"; found "${packageName || "<missing>"}".`,
    );
  }
...
export function collectPublishablePluginPackages(
  ...
  for (const dir of dirs) {
    ...
    if (packageJson.crawclaw?.release?.publishToNpm !== true) {
      continue;
    }
    ...
    publishable.push({
      ...
      installNpmSpec: packageJson.crawclaw?.install?.npmSpec?.trim() || undefined,
    });
  }
...
export function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "crawclaw-plugin-npm-view-"));
