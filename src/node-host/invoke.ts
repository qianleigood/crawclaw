...
const execHostEnforced =
  (process.env.CRAWCLAW_NODE_EXEC_HOST ?? process.env.CRAWCLAW_NODE_EXEC_HOST)
    ?.trim()
    .toLowerCase() === "app";
const execHostFallbackAllowed =
  (process.env.CRAWCLAW_NODE_EXEC_FALLBACK ?? process.env.CRAWCLAW_NODE_EXEC_FALLBACK)
    ?.trim()
    .toLowerCase() !== "0";
