...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set CRAWCLAW_GATEWAY_TOKEN/CRAWCLAW_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "crawclaw-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.CRAWCLAW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.CRAWCLAW_GATEWAY_PASSWORD);
