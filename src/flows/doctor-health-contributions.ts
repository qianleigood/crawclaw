...
export type DoctorHealthFlowContext = {
  ...
  cfg: CrawClawConfig;
  cfgForPersistence: CrawClawConfig;
  sourceConfigValid: boolean;
  configPath: string;
...
export function resolveDoctorMode(cfg: CrawClawConfig): DoctorFlowMode {
...
async function runGatewayConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  if (!ctx.cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("crawclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("crawclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(ctx.configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("crawclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveDoctorMode(ctx.cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("crawclaw config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("crawclaw config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
...
async function runMemoryHealthContribution(ctx: DoctorHealthFlowContext): Promise<void> {
  const summary =
    ctx.gatewayMemoryProbe?.checked && ctx.gatewayMemoryProbe.memoryHealth
      ? ctx.gatewayMemoryProbe.memoryHealth
      : await resolveDoctorMemoryHealth(ctx.cfg);
  await noteMemoryHealth(ctx.cfg, { summary });
}
...
async function runWriteConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if (!ctx.prompter.shouldRepair) {
    ctx.runtime.log(`Run "${formatCliCommand("crawclaw doctor --fix")}" to apply changes.`);
  }
}
...
export function resolveDoctorHealthContributions(): DoctorHealthContribution[] {
  return [
    ...
    createDoctorHealthContribution({
      id: "doctor:memory-health",
      label: "Memory health",
      run: runMemoryHealthContribution,
    }),
    createDoctorHealthContribution({
