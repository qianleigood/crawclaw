...
describe("doctor matrix provider helpers", () => {
  ...
  it("formats the legacy state preview", () => {
    ...
    expect(preview).toContain("Matrix legacy state was detected.");
    expect(preview).toContain("Legacy sync store: /tmp/legacy-sync.json");
    expect(preview).toContain("Current sync store: /tmp/new-sync.json");
    expect(preview).toContain("Picked the newest account.");
    expect(preview).toContain("Automatic Matrix state migration was removed");
  });
  ...
  it("formats encrypted-state migration previews", () => {
    ...
    expect(previews[1]).toContain("Automatic Matrix crypto migration was removed");
  });

  it("warns on stale custom Matrix plugin paths", async () => {
    const missingPath = path.join(tmpdir(), "crawclaw-matrix-missing-provider-test");
  ...
  it("returns warnings instead of mutating Matrix state during repair", async () => {
    ...
    try {
      ...
      expect(result.warnings).toEqual([
        expect.stringContaining("Matrix legacy state was detected."),
        "- matrix warning",
      ]);
    } finally {
  ...
  it("removes stale Matrix plugin config when install path is missing", async () => {
    const missingPath = path.join(tmpdir(), `crawclaw-matrix-stale-cleanup-test-${Date.now()}`);
    await fs.rm(missingPath, { recursive: true, force: true });
