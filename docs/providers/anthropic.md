---
summary: "Use Anthropic Claude via API keys, setup-token, or Claude CLI in CrawClaw"
read_when:
  - You want to use Anthropic models in CrawClaw
...
In CrawClaw you can authenticate with an API key or a **setup-token**.
...
- Anthropic Claude 4.6 models default to `adaptive` thinking in CrawClaw when no explicit thinking level is set.
...
CrawClaw's shared `/fast` toggle also supports direct public Anthropic traffic, including API-key and OAuth-authenticated requests sent to `api.anthropic.com`.
...
- CrawClaw only injects Anthropic service tiers for direct `api.anthropic.com` requests. If you route `anthropic/*` through a proxy or gateway, `/fast` leaves `service_tier` untouched.
...
CrawClaw supports Anthropic's prompt caching feature. This is **API-only**; subscription auth does not honor cache settings.
...
When using Anthropic API Key authentication, CrawClaw automatically applies `cacheRetention: "short"` (5-minute cache) for all Anthropic models. You can override this by explicitly setting `cacheRetention` in your config.
...
CrawClaw includes the `extended-cache-ttl-2025-04-11` beta flag for Anthropic API
requests; keep it if you override provider headers (see [/gateway/configuration](/gateway/configuration)).

## 1M context window (Anthropic beta)

Anthropic's 1M context window is beta-gated. In CrawClaw, enable it per model
...
CrawClaw maps this to `anthropic-beta: context-1m-2025-08-07` on Anthropic
requests.
...
with subscription auth, CrawClaw logs a warning and falls back to the standard
...
1. CrawClaw launches `claude -p --output-format json ...` on the **gateway
   host**.
2. The first turn sends `--session-id <uuid>`.
3. Follow-up turns reuse the stored Claude session via `--resume <sessionId>`.
4. Your chat messages still go through the normal CrawClaw message pipeline, but
...
- CrawClaw auto-loads the bundled Anthropic plugin at gateway startup when your
...
- Normal CrawClaw message/session routing
