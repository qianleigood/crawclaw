...
function getDiscordExecApprovalApproversForTests(params: { cfg: CrawClawConfig }): string[] {
...
const { buildCommandsPaginationKeyboard } = await import("../../channels/telegram-pagination.js");
...
function listConfiguredTelegramAccountIds(cfg: CrawClawConfig): string[] {
...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-commands-"));
...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-commands-config-"));
  const configPath = path.join(dir, "crawclaw.json");
  const previous = process.env.CRAWCLAW_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.CRAWCLAW_CONFIG_PATH = previous;
    }
    await fs.rm(dir, {
      recursive: true,
      force: true,
...
function buildParams(commandBody: string, cfg: CrawClawConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/crawclaw-session-store.json" },
    } as CrawClawConfig;
    const params = buildParams("/compact: focus on decisions", cfg, {
      From: "+15550001",
      To: "+15550002",
    });
    const agentDir = "/tmp/crawclaw-agent-compact";
      ...
      },
      {
        ...
        run: async () => {
          await withTempConfigPath({ messages: { ackReaction: ":)" } }, async (configPath) => {
            ...
            const written = await readJsonFile<CrawClawConfig>(configPath);
