...
CrawClaw 提供三个安装脚本，由 `crawclaw.ai` 提供。
...
<Note>
如果安装成功但在新终端中找不到 `crawclaw`，请参见 [Node.js 故障排除](/install/node#troubleshooting)。
</Note>
...
<Steps>
  ...
  <Step title="安装 CrawClaw">
    - `npm` 方法（默认）：全局 npm 安装
    - `git` 方法：克隆/更新仓库，使用 pnpm 安装依赖，构建，然后将包装器安装到 `~/.local/bin/crawclaw`
  </Step>
  <Step title="安装后任务">
    - 在升级和 git 安装时运行 `crawclaw doctor --non-interactive`（尽力而为）
...
如果在 CrawClaw 检出目录中运行（`package.json` + `pnpm-workspace.yaml`），脚本会提供：
...
| `--git-dir <path>`                    | 检出目录（默认：`~/crawclaw`）。别名：`--dir`     |
...
<Info>
适用于你希望所有内容都放在本地前缀（默认 `~/.crawclaw`）下，并且不依赖系统 Node 的环境。
</Info>
...
<Steps>
  ...
  <Step title="在前缀下安装 CrawClaw">
    使用 `--prefix <prefix>` 通过 npm 安装，然后将包装器写入 `<prefix>/bin/crawclaw`。
  </Step>
</Steps>
...
| 标志                   | 说明                                                                   |
| ---------------------- | ---------------------------------------------------------------------- |
| `--prefix <path>`      | 安装前缀（默认：`~/.crawclaw`）                                        |
| `--version <ver>`      | CrawClaw 版本或 dist-tag（默认：`latest`）                             |
| `--node-version <ver>` | Node 版本（默认：`22.22.0`）                                           |
| `--json`               | 输出 NDJSON 事件                                                       |
| `--onboard`            | 安装后运行 `crawclaw onboard`                                          |
...
<Steps>
  ...
  <Step title="安装 CrawClaw">
    ...
    - `git` 方法：克隆/更新仓库，使用 pnpm 安装/构建，并将包装器安装到 `%USERPROFILE%\.local\bin\crawclaw.cmd`
  </Step>
  <Step title="安装后任务">
    在可能情况下将所需 bin 目录添加到用户 PATH，然后在升级和 git 安装时运行 `crawclaw doctor --non-interactive`（尽力而为）。
  </Step>
</Steps>
...
| `-GitDir <path>`            | 检出目录（默认：`%USERPROFILE%\crawclaw`）   |
| `-NoOnboard`                | 跳过新手引导                                 |
...
<AccordionGroup>
  ...
  <Accordion title="安装后找不到 crawclaw">
