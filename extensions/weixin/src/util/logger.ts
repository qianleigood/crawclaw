...
/**
 * Plugin logger — writes JSON lines to the main crawclaw log file:
 *   <tmpDir>/crawclaw-YYYY-MM-DD.log
 * Same file and format used by all other channels.
 */

const MAIN_LOG_DIR = resolvePreferredCrawClawTmpDir();
const SUBSYSTEM = "gateway/channels/weixin";
const RUNTIME = "node";
const RUNTIME_VERSION = process.versions.node;
const HOSTNAME = os.hostname() || "unknown";
const PARENT_NAMES = ["crawclaw"];
...
function resolveMinLevel(): number {
  const env = process.env.CRAWCLAW_LOG_LEVEL?.toUpperCase();
...
function resolveMainLogPath(): string {
  ...
  return path.join(MAIN_LOG_DIR, `crawclaw-${dateKey}.log`);
}
