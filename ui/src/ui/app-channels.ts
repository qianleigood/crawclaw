...
export async function handleWhatsAppStart(host: CrawClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: CrawClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: CrawClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: CrawClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: CrawClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: CrawClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: CrawClawApp): string | null {
...
function buildGatewayHttpHeaders(host: CrawClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: CrawClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: CrawClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: CrawClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: CrawClawApp) {
  const state = host.nostrProfileFormState;
