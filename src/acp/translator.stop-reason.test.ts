...
const readAcpSessionEntryMock = vi.fn<() => AcpSessionStoreEntry | null>(() => null);
...
describe("acp translator stop reason mapping", () => {
  ...
  it("matches ACP backend chat final events through wrapper session aliases", async () => {
    const sessionId = "session-1";
    const wrapperSessionKey = "agent:crawclaw:acp:wrapper-1";
    const backendSessionKey = "agent:main:backend-session-1";
    ...
    readAcpSessionEntryMock.mockReturnValue({
      ...
      sessionKey: wrapperSessionKey,
      storeSessionKey: wrapperSessionKey,
      entry: {
        sessionId,
        updatedAt: Date.now(),
        ...
        acp: {
          backend: "acpx",
          agent: "crawclaw",
          runtimeSessionName: "acpx-wrapper-1",
          identity: {
            state: "pending",
            source: "ensure",
  ...
  it("reconciles an accepted prompt via periodic agent.wait when the final event is missed", async () => {
    vi.useFakeTimers();
    try {
      const sessionId = "session-1";
      const sessionKey = "agent:main:main";
