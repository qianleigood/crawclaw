...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/crawclaw-state", "devices")).toEqual({
      dir: path.join("/tmp/crawclaw-state", "devices"),
      pendingPath: path.join("/tmp/crawclaw-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/crawclaw-state", "devices", "paired.json"),
    });
  });
