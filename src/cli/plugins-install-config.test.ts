...
import type { ConfigFileSnapshot } from "../config/types.crawclaw.js";
...
vi.mock("../commands/doctor/providers/matrix.js", () => ({
  cleanStaleMatrixPluginConfig: (cfg: CrawClawConfig) => cleanStaleMatrixPluginConfigMock(cfg),
}));
...
function makeSnapshot(overrides: Partial<ConfigFileSnapshot> = {}): ConfigFileSnapshot {
  return {
    ...
    resolved: { plugins: {} } as CrawClawConfig,
    valid: false,
    runtimeConfig: { plugins: {} } as ConfigFileSnapshot["runtimeConfig"],
    config: { plugins: {} } as CrawClawConfig,
    hash: "abc",
...
describe("loadConfigForInstall", () => {
  const matrixNpmRequest = {
    rawSpec: "@crawclaw/matrix",
    normalizedSpec: "@crawclaw/matrix",
  };
  ...
  beforeEach(() => {
    ...
    cleanStaleMatrixPluginConfigMock.mockImplementation((cfg: CrawClawConfig) => ({
      config: cfg,
      changes: [],
    }));
  });
  ...
  it("returns the config directly when loadConfig succeeds", async () => {
    const cfg = { plugins: { entries: { matrix: { enabled: true } } } } as CrawClawConfig;
  ...
  it("does not run stale Matrix cleanup on the happy path", async () => {
    const cfg = { plugins: {} } as CrawClawConfig;
    loadConfigMock.mockReturnValue(cfg);
  ...
  it("allows explicit repo-checkout Matrix reinstall recovery", async () => {
    ...
    const snapshotCfg = { plugins: {} } as CrawClawConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        config: snapshotCfg,
  ...
  it("rejects non-Matrix install requests when config is invalid", async () => {
    ...
    ).rejects.toThrow("Config invalid; run `crawclaw doctor --fix` before installing plugins.");
    expect(readConfigFileSnapshotMock).not.toHaveBeenCalled();
  });
