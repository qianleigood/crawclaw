...
async function clickTab(page, tabSelector, tabText) {
  const clicked = await page.evaluate(
    ({ inputSelector, inputSelectorAlt, selector, text }) => {
      ...
      if (text.includes("图文")) {
        return (
          !!document.querySelector(inputSelector) || !!document.querySelector(inputSelectorAlt)
        );
      }
      return false;
    },
    {
      inputSelector: SELECTORS.uploadInput,
      inputSelectorAlt: SELECTORS.uploadInputAlt,
...
async function uploadVideo(page, videoPath) {
  ...
  if (!normalized) {
    throw new XhsCliError("A video file is required to publish video on Xiaohongshu.", {
      code: "EMPTY_VIDEO_PATH",
    });
  }
  ...
  if (!input) {
    throw new XhsCliError(
      "Could not find file input element for video upload. The page structure may have changed.",
      {
...
async function waitVideoProcessing(page) {
  ...
  while (Date.now() < deadline) {
    const readySelector = await waitForAnySelector(
      page,
      [SELECTORS.titleInput, SELECTORS.titleInputAlt],
      1200,
    );
    if (readySelector) {
      await delay(1000);
...
async function uploadImages(page, imagePaths) {
  if (!Array.isArray(imagePaths) || !imagePaths.length) {
    throw new XhsCliError("At least one image is required to publish on Xiaohongshu.", {
  ...
  if (!input) {
    throw new XhsCliError(
      "Could not find file input element. The page structure may have changed.",
      {
        code: "UPLOAD_INPUT_NOT_FOUND",
      },
    );
  ...
  if (!readySelector) {
    throw new XhsCliError("Image upload did not reveal the editor within the expected time.", {
      code: "UPLOAD_EDITOR_TIMEOUT",
    });
  }
  await delay(5000);
  return normalized;
...
async function fillContent(page, content, interactionMode = "normal") {
  const target = await findVisibleElement(page, [
    SELECTORS.contentEditor,
    SELECTORS.contentEditorAlt,
  ]);
  if (!target) {
    throw new XhsCliError("Could not find content editor element.", {
      code: "CONTENT_EDITOR_NOT_FOUND",
    });
  }
  ...
  const button = await page.evaluateHandle((buttonText) => {
    ...
    for (const span of spans) {
      if ((span.textContent || "").trim() === buttonText) {
        const el =
          span.closest('button, [role="button"], .d-button, [class*="btn"], [class*="button"]') ||
          span;
        if (el instanceof HTMLElement) {
          return el;
        }
  ...
  while (Date.now() < deadline) {
    const outcome = await page.evaluate(() => {
      ...
      if (noteIdMatch) {
        return {
          published: true,
          noteLink: `https://www.xiaohongshu.com/explore/${noteIdMatch[0]}`,
          detector: "body-note-id",
        };
      }
    ...
    if (outcome && outcome.published === false) {
      throw new XhsCliError(`Publish failed: ${outcome.failureReason || "unknown error"}`, {
        code: "PUBLISH_FAILED",
      });
    }
    await delay(800);
...
async function publishImageTextPost(
  page,
  { title, content, imagePaths, interactionMode = "normal" },
) {
  const fillPayload = await fillImageTextPost(page, {
    title,
    content,
    imagePaths,
    interactionMode,
