...
async function withOpenRouterStateDir(run: (stateDir: string) => Promise<void>) {
  const stateDir = mkdtempSync(join(tmpdir(), "crawclaw-model-capabilities-openrouter-"));
...
async function importOpenRouterModelCapabilities(scope: string) {
  return await importFreshModule<typeof import("./openrouter.js")>(
    import.meta.url,
    `./openrouter.js?scope=${scope}`,
  );
}

describe("model-capabilities openrouter", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
