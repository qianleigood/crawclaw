...
```bash
crawclaw plugins install @crawclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
crawclaw plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "CrawClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `crawclaw pairing list nextcloud-talk`
  - `crawclaw pairing approve nextcloud-talk <CODE>`
