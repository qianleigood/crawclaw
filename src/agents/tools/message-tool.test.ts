...
function setTestPlugins(...plugins: ChannelPlugin[]) {
  setActivePluginRegistry(
    createTestRegistry(plugins.map((plugin) => ({ pluginId: plugin.id, source: "test", plugin }))),
  );
}

function createTool(toolOptions: Partial<Parameters<typeof createMessageTool>[0]> = {}) {
...
function getToolSchema(tool: ReturnType<CreateMessageTool>) {
...
function createSchemaView(
  ...
  return {
    ...
    properties,
    actionEnum: getActionEnum(properties),
  };
}
...
describe("message tool schema scoping", () => {
  ...
  it.each([
    ...
    ({
      ...
      const { properties, actionEnum } = createSchemaView(
        [telegramPlugin, discordPlugin, slackPlugin],
  ...
  it("includes poll in the action enum when the current channel supports poll actions", () => {
    const { actionEnum } = createSchemaView([telegramPlugin], {
  ...
  it.each([
    ...
    ({ plugin }) => {
      const { schema } = createSchemaView([plugin], {
  ...
  it("keeps buttons schema optional so plain sends do not require buttons", () => {
    const { schema } = createSchemaView([telegramPlugin], {
      currentChannelProvider: "telegram",
    });
  ...
  it("hides telegram poll extras when telegram polls are disabled in scoped mode", () => {
    ...
    const { properties, actionEnum } = createSchemaView([telegramPluginWithConfig], {
      config: {
        channels: {
          telegram: {
            actions: {
  ...
  it("uses discovery account scope for capability-gated shared fields", () => {
    ...
    setTestPlugins(scopedInteractivePlugin);

    const scopedTool = createTool({
      currentChannelProvider: "telegram",
      agentAccountId: "ops",
    });
    const unscopedTool = createTool({
      currentChannelProvider: "telegram",
    });
  ...
  it("uses discovery account scope for other configured channel actions", () => {
    ...
    setTestPlugins(currentPlugin, scopedOtherPlugin);
...
describe("message tool description", () => {
  ...
  it("hides BlueBubbles group actions for DM targets", () => {
    setTestPlugins(bluebubblesPlugin);
  ...
  it("includes other configured channels when currentChannel is set", () => {
    ...
    setTestPlugins(signalPlugin, telegramPluginFull);
