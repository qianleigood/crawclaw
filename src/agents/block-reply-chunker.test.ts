...
import { BlockReplyChunker } from "./block-reply-chunker.js";

function createFlushOnParagraphChunker(params: { minChars: number; maxChars: number }) {
  return new BlockReplyChunker({
...
function drainChunks(chunker: BlockReplyChunker, force = false) {
  const chunks: string[] = [];
  chunker.drain({ force, emit: (chunk) => chunks.push(chunk) });
  return chunks;
}

describe("BlockReplyChunker", () => {
  it("breaks at paragraph boundary right after fence close", () => {
    const chunker = new BlockReplyChunker({
      minChars: 1,
      maxChars: 40,
      breakPreference: "paragraph",
    });
