---
summary: "Automated, hardened CrawClaw installation with Ansible, Tailscale VPN, and firewall isolation"
read_when:
...
<Steps>
  <Step title="Switch to the crawclaw user">
    ```bash
    sudo -i -u crawclaw
    ```
  </Step>
...
<Steps>
  ...
  <Step title="Run the playbook">
    ...
    # Then run: /tmp/crawclaw-setup.sh
    ```
...
The Ansible installer sets up CrawClaw for manual updates. See [Updating](/install/updating) for the standard update flow.
