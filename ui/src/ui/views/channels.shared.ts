...
export function formatNullableBoolean(value: boolean | null): string {
  ...
  return value ? uiLiteral("Yes") : uiLiteral("No");
}
...
export function renderSingleAccountChannelCard(params: {
  ...
  return html`
    <div class="card">
      <div class="card-title">${uiLiteral(params.title)}</div>
      <div class="card-sub">${uiLiteral(params.subtitle)}</div>
      ...
      <div class="status-list" style="margin-top: 16px;">
        ${params.statusRows.map(
          (row) => html`
            <div>
              <span class="label">${uiLiteral(row.label)}</span>
              <span>${typeof row.value === "string" ? uiLiteral(row.value) : row.value}</span>
            </div>
          `,
        )}
      </div>
...
export function renderChannelAccountCount(
  ...
  return html`<div class="account-count">${uiLiteral("Accounts")} (${count})</div>`;
}
