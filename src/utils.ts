...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.CRAWCLAW_HOME?.trim() || process.env.CRAWCLAW_HOME?.trim();
  if (explicitHome) {
    return {
      home,
      prefix: process.env.CRAWCLAW_HOME?.trim() ? "$CRAWCLAW_HOME" : "$CRAWCLAW_HOME",
    };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via CRAWCLAW_STATE_DIR (legacy: CRAWCLAW_STATE_DIR).
