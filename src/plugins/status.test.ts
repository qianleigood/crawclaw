...
vi.mock("./loader.js", () => ({
  loadCrawClawPlugins: (...args: unknown[]) => loadCrawClawPluginsMock(...args),
}));
...
function setPluginLoadResult(overrides: Partial<ReturnType<typeof createPluginLoadResult>>) {
  loadCrawClawPluginsMock.mockReturnValue(
    createPluginLoadResult({
      plugins: [],
      ...overrides,
    }),
...
describe("plugin status reports", () => {
  ...
  beforeEach(() => {
    ...
    loadCrawClawPluginsMock.mockReset();
