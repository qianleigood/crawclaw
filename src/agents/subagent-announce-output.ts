...
function buildBackendSessionCandidates(sessionKey: string): string[] {
  ...
  const candidateIds = [
    normalizeOptionalText(entry.acp.identity.agentSessionId),
    normalizeOptionalText(entry.acp.identity.acpxSessionId),
  ].filter((value): value is string => Boolean(value));

  const candidates: string[] = [];
  const seen = new Set<string>();
  for (const candidateId of candidateIds) {
    const directSessionKey = normalizeOptionalText(candidateId);
    if (directSessionKey) {
      if (parseAgentSessionKey(directSessionKey)) {
        const normalizedDirect = directSessionKey.toLowerCase();
        if (!seen.has(normalizedDirect)) {
          seen.add(normalizedDirect);
          candidates.push(normalizedDirect);
        }
      } else {
        const derived = `agent:${requesterAgentId}:${directSessionKey}`.toLowerCase();
        if (!seen.has(derived)) {
          seen.add(derived);
...
async function readSubagentOutputForSessionKey(
  sessionKey: string,
  outcome?: SubagentRunOutcome,
...
export async function readSubagentOutput(
  ...
  const direct = await readSubagentOutputForSessionKey(sessionKey, outcome);
  if (direct?.trim()) {
    return direct;
  }
  for (const candidateSessionKey of buildBackendSessionCandidates(sessionKey)) {
    const candidate = await readSubagentOutputForSessionKey(candidateSessionKey, outcome);
    if (candidate?.trim()) {
      return candidate;
    }
  }
