...
function normalizeDenyList(cfg: CrawClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: CrawClawPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: CrawClawPluginApi) {
    ...
    const timerService: CrawClawPluginService = {
