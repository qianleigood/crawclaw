...
const workspaceHarness = createCommandWorkspaceHarness("crawclaw-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".crawclaw", "extensions", params.pluginId);
...
function buildCfg(): CrawClawConfig {
  return {
    plugins: {
      enabled: true,
    },
