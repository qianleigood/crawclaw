...
const buildStatusMessageMock = vi.hoisted(() => vi.fn(() => "CrawClaw\n🧠 Model: GPT-5.4"));
...
async function loadFreshCrawClawToolsForSessionStatusTest() {
  vi.resetModules();
...
describe("session_status tool", () => {
  beforeEach(async () => {
    buildStatusMessageMock.mockClear();
    await loadFreshCrawClawToolsForSessionStatusTest();
  });
  ...
  it("returns a status card for the current session", async () => {
    ...
    expect(details.statusText).toContain("CrawClaw");
