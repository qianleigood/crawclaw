...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([CRAWCLAW_MODEL_ID, CRAWCLAW_DEFAULT_MODEL_ID]);
  ids.add(`crawclaw/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`crawclaw/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== CRAWCLAW_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
