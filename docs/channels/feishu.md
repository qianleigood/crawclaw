...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects CrawClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current CrawClaw releases, so no separate plugin install
is required.
...
```bash
crawclaw plugins install @crawclaw/feishu
```

---

## Feishu CLI user identity

CrawClaw now supports two Feishu execution surfaces that should coexist:

- `extensions/feishu` = bot/app identity for channel replies, cards, mentions, and streaming delivery
- `extensions/feishu-cli` = user identity through the official `lark-cli` for personal calendar, tasks, and message search

Do not treat them as replacements for each other:

- Use the bot plugin when CrawClaw needs to appear inside Feishu as a bot.
- Use Feishu CLI when CrawClaw needs to act as the authenticated user.

Enable the Feishu CLI user layer under:

- `plugins.entries.feishu-cli.enabled`
- `plugins.entries.feishu-cli.config.command`
- `plugins.entries.feishu-cli.config.profile`
- `plugins.entries.feishu-cli.config.timeoutMs`
...
Check the user-identity side with:

```bash
crawclaw feishu-cli status
```

If auth is missing, sign in with the official CLI first:

```bash
crawclaw feishu-cli auth login
crawclaw feishu-cli status --verify
```
...
If you just installed CrawClaw, run onboarding:
...
2. Configuring app credentials in CrawClaw
...
1. You already ran `crawclaw channels add` for Feishu
2. The gateway is running (`crawclaw gateway status`)
...
In addition to allowing the group itself, **all messages** in that group are gated by the sender open_id: only users listed in `groups.<chat_id>.allowFrom` have their messages processed; messages from other members are ignored (this is full sender-level gating, not only for control commands like /new).
...
2. Run `crawclaw logs --follow` and look for `chat_id`
...
2. Run `crawclaw logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `crawclaw gateway status`  | Show gateway status           |
| `crawclaw gateway install` | Install/start gateway service |
| `crawclaw gateway stop`    | Stop gateway service          |
| `crawclaw gateway restart` | Restart gateway service       |
| `crawclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `crawclaw logs --follow`
...
5. Ensure the gateway is running: `crawclaw gateway status`
6. Check logs: `crawclaw logs --follow`
...
Streaming is enabled by default for card-capable replies, but CrawClaw automatically falls back to non-streaming delivery when:

- `renderMode: "raw"` forces plain-text output
- the reply is already inside a Feishu topic/thread context, where direct reply metadata is more reliable than streaming cards
...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "support-agent",
        workspace: "/home/user/crawclaw-agents/support",
        agentDir: "/home/user/.crawclaw/agents/support-agent/agent",
      },
      {
        id: "ops-agent",
        workspace: "/home/user/crawclaw-agents/ops",
        agentDir: "/home/user/.crawclaw/agents/ops-agent/agent",
      },
    ],
  },
  bindings: [
