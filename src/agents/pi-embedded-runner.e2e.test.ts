...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("../memory/engine/memory-runtime.js", () => ({
...
beforeAll(async () => {
  ...
  e2eWorkspace = await createEmbeddedPiRunnerTestWorkspace("crawclaw-embedded-agent-");
...
describe("runEmbeddedPiAgent", () => {
  ...
  it("prefers the built-in memory runtime over the legacy context engine path", async () => {
    ...
    expect(runEmbeddedAttemptMock).toHaveBeenCalledWith(
      expect.objectContaining({
        memoryRuntime: builtInRuntime,
      }),
    );
  });
