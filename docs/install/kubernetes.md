---
summary: "Deploy CrawClaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run CrawClaw on a Kubernetes cluster
  - You want to test CrawClaw in a Kubernetes environment
title: "Kubernetes"
---

# CrawClaw on Kubernetes

A minimal starting point for running CrawClaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

CrawClaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
```
Namespace: crawclaw (configurable via CRAWCLAW_NAMESPACE)
├── Deployment/crawclaw        # Single pod, init container + gateway
├── Service/crawclaw           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/crawclaw-config  # crawclaw.json + AGENTS.md
└── Secret/crawclaw-secrets    # Gateway token + API keys
```
...
Edit `crawclaw.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret crawclaw-secrets -n crawclaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/crawclaw -n crawclaw
```

### Custom namespace

```bash
CRAWCLAW_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
└── manifests/
    ...
    ├── configmap.yaml          # crawclaw.json + AGENTS.md
