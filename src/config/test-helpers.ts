...
import { resetConfigRuntimeState, type CrawClawConfig } from "./config.js";

export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  resetConfigRuntimeState();
  try {
    return await withTempHomeBase(fn, { prefix: "crawclaw-config-" });
  } finally {
    resetConfigRuntimeState();
  }
}

export async function writeCrawClawConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.CRAWCLAW_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected CRAWCLAW_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeCrawClawConfig(home, config);
