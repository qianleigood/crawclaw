...
function resolveRequestedProviderOrThrow(
  ...
  throw new Error(
    `Unknown provider "${requested}". Loaded providers: ${availableText}. Verify plugins via \`${formatCliCommand("crawclaw plugins list --json")}\`.`,
  );
}
...
export async function modelsAuthSetupTokenCommand(
  ...
  if (tokenProviders.length === 0) {
    throw new Error(
      `No provider token-auth plugins found. Install one via \`${formatCliCommand("crawclaw plugins install")}\`.`,
    );
  }
...
export async function modelsAuthPasteTokenCommand(
  ...
  const expires =
    opts.expiresIn?.trim() && opts.expiresIn.trim().length > 0
      ? Date.now() + parseDurationMs((opts.expiresIn ?? "").trim(), { defaultUnit: "d" })
      : undefined;
  ...
  const expiresIn = wantsExpiry
    ? (
        await text({
          ...
          validate: (value) => {
            try {
              parseDurationMs((value ?? "").trim(), { defaultUnit: "d" });
              return undefined;
            } catch {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (authProviders.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("crawclaw plugins install")}\`.`,
    );
  }
