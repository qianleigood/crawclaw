...
type CreateCrawClawCodingToolsArgs = Parameters<PiToolsModule["createCrawClawCodingTools"]>[0];
type MockCrawClawToolsOptions = {
...
async function createCrawClawCodingToolsWithFreshModules(options?: CreateCrawClawCodingToolsArgs) {
  vi.resetModules();
  vi.doMock("../crawclaw-tools.js", () => ({
    createCrawClawTools: vi.fn((mockOptions?: MockCrawClawToolsOptions) => {
      const imageTool = createImageTool({
        config: mockOptions?.config,
        agentDir: mockOptions?.agentDir,
        workspaceDir: mockOptions?.workspaceDir,
        sandbox:
          mockOptions?.sandboxRoot && mockOptions?.sandboxFsBridge
            ? {
                root: mockOptions.sandboxRoot,
                bridge: mockOptions.sandboxFsBridge,
              }
            : undefined,
        fsPolicy: mockOptions?.fsPolicy,
        modelHasVision: mockOptions?.modelHasVision,
      });
      return imageTool ? [imageTool] : [];
    }),
  }));
  const { createCrawClawCodingTools } = await import("../pi-tools.js");
  try {
    return createCrawClawCodingTools(options);
  } finally {
    vi.doUnmock("../crawclaw-tools.js");
  }
}

async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-image-"));
  try {
...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(parentDir, "crawclaw-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): CrawClawConfig {
  return {
    agents: {
      defaults: {
...
async function withTempSandboxState(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-image-sandbox-"));
...
describe("image tool implicit imageModel config", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox image paths", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      ...
      await withTempAgentDir(async (agentDir) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-outside-"));
  ...
  it("allows non-workspace local image paths when workspaceOnly is disabled", async () => {
    ...
    await withTempAgentDir(async (agentDir) => {
      const cfg = createMinimaxImageConfig();
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-image-outside-"));
      const outsideImage = path.join(outsideDir, "secret.png");
      await fs.writeFile(outsideImage, Buffer.from(ONE_PIXEL_PNG_B64, "base64"));
      try {
        const tool = createRequiredImageTool({
  ...
  it("allows workspace images via createCrawClawCodingTools when workspace root is explicit", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      const fetch = stubMinimaxOkFetch();
      await withTempAgentDir(async (agentDir) => {
        const cfg = createMinimaxImageConfig();

        const tools = await createCrawClawCodingToolsWithFreshModules({
          config: cfg,
          agentDir,
          workspaceDir,
        });
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      ...
      const res = await tool.execute("t1", {
        prompt: "Describe the image.",
        image: "@/Users/steipete/.crawclaw/media/inbound/photo.png",
      });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  async function createMinimaxVlmFixture(baseResp: { status_code: number; status_msg: string }) {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "crawclaw-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg = createMinimaxImageConfig();
  ...
  it("accepts image for single-image requests and calls /v1/coding_plan/vlm", async () => {
    ...
    expect(String(url)).toBe("https://api.minimaxi.com/v1/coding_plan/vlm");
