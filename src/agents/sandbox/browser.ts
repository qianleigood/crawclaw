...
import {
  ...
  DEFAULT_CRAWCLAW_BROWSER_COLOR,
...
import {
  ...
  DEFAULT_SANDBOX_BROWSER_PINCHTAB_PORT,
...
const CDP_SOURCE_RANGE_ENV_KEY = "CRAWCLAW_BROWSER_CDP_SOURCE_RANGE";
const PINCHTAB_START_ENV_KEY = "CRAWCLAW_BROWSER_ENABLE_PINCHTAB";
const PINCHTAB_PORT_ENV_KEY = "CRAWCLAW_BROWSER_PINCHTAB_PORT";
const PINCHTAB_TOKEN_ENV_KEY = "CRAWCLAW_BROWSER_PINCHTAB_TOKEN";
...
async function waitForSandboxHttpHealth(params: {
  port: number;
  timeoutMs: number;
}): Promise<boolean> {
  const deadline = Date.now() + Math.max(0, params.timeoutMs);
  const url = `http://127.0.0.1:${params.port}/health`;
  while (Date.now() < deadline) {
    try {
      const ctrl = new AbortController();
      const t = setTimeout(ctrl.abort.bind(ctrl), 1000);
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_CRAWCLAW_BROWSER_COLOR,
...
export async function ensureSandboxBrowser(params: {
  ...
  pinchTab?: { enabled?: boolean; token?: string };
  ...
  const pinchTabEnabled = params.pinchTab?.enabled === true;
  const pinchTabPort = params.cfg.browser.pinchTabPort ?? DEFAULT_SANDBOX_BROWSER_PINCHTAB_PORT;
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "crawclaw.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `crawclaw sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `crawclaw sandbox recreate --browser --agent ${agentId}`;
          }
          return "crawclaw sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: {
        ...
        "crawclaw.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
      },
    ...
    if (pinchTabEnabled) {
      args.push("-p", `127.0.0.1::${pinchTabPort}`);
    }
    if (noVncEnabled) {
      args.push("-p", `127.0.0.1::${params.cfg.browser.noVncPort}`);
    }
    args.push("-e", `CRAWCLAW_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `CRAWCLAW_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `CRAWCLAW_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `${PINCHTAB_START_ENV_KEY}=${pinchTabEnabled ? "1" : "0"}`);
    args.push("-e", `${PINCHTAB_PORT_ENV_KEY}=${pinchTabPort}`);
    if (pinchTabEnabled && params.pinchTab?.token?.trim()) {
      args.push("-e", `${PINCHTAB_TOKEN_ENV_KEY}=${params.pinchTab.token.trim()}`);
    }
    if (cdpSourceRange) {
      args.push("-e", `${CDP_SOURCE_RANGE_ENV_KEY}=${cdpSourceRange}`);
    }
    args.push("-e", `CRAWCLAW_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `CRAWCLAW_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
    ...
    args.push("-e", "CRAWCLAW_BROWSER_NO_SANDBOX=1");
  ...
  const mappedPinchTab = pinchTabEnabled ? await readDockerPort(containerName, pinchTabPort) : null;
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_CRAWCLAW_BROWSER_PROFILE_NAME)
    : null;
  ...
  const noVncUrl =
    mappedNoVnc && noVncEnabled
      ...
      : undefined;
  const pinchTabUrl =
    pinchTabEnabled && mappedPinchTab
      ? (await waitForSandboxHttpHealth({
          port: mappedPinchTab,
          timeoutMs: Math.min(params.cfg.browser.autoStartTimeoutMs, 5000),
        }))
        ? `http://127.0.0.1:${mappedPinchTab}`
        : undefined
      : undefined;
